...
| Convert | `activeTab === 'convert'` (~2464–2669) | ~205 + 4 effects | inline | 7 states (`convert*` cluster) | low (reads both caches; no default selection since v2.2879) | low | **Extract 2nd** → `ProspectsConvertTab` |
...
- **Cross-tab/shared state:** reads `prospectListProspects` + `followUpProspects` (dropdown options are the union; `convertProspect` is looked up in both) — **no** default selection: v2.2879 deleted the effect that set `convertProspectId` to the Follow Up queue head on tab switch (J34-F6 — it hid the tab's own `searchConvertProspects` / `suggestRecentlyAnswered` UI, which renders only when nothing is selected). Writes `prospectListProspects` / `followUpProspects` patches after a successful convert.
- **Derived values:** `convertProspect` (lookup, computed in the component body — not memoized).
- **Handlers/effects:** `handleConvertSubmit(payload: NewCustomerFormPayload)` — inserts `customers`, then loops `customer_contact_persons` (skips blank names), then loops `bids` (skips blank project/service-type; hardcodes `materials_model: 'rough'`), then `markProspectConverted(convertProspectId, customerId, payload.name, authUser.id)` from [`lib/prospects/prospectConversion.ts`](../src/lib/prospects/prospectConversion.ts) (the shared mark tail — status flip + `converted` comment naming `/customers/<id>`; v2.2879) + `recordProspectConverted(…, 'convert-tab')`, then `navigate(\`/customers/${customerId}\`)`; effects: earliest-`prospect_comments` date → `convertFirstInteractionDate` (cancellation-guarded), `service_types` load on tab activation (**`as any`** — table missing from generated types), pre-fill first contact person from the prospect.
...
- **Extraction status + risk + approach:** Inline. **Low risk — extract second** → `ProspectsConvertTab`. Props: `prospects` (the union list, or both caches), `authUserId`, `onConverted`/`navigate` (no `defaultProspectId` — the tab starts on search since v2.2879). All seven states and three effects move verbatim. Stage A: none needed (the submit is IO, not calc).
...
- Lib kernels: [`teamProspectRanking.ts`](../src/lib/teamProspectRanking.ts) (`UNSORTED_ROLE_KEY`, `groupTeamProspects`, `reorderActiveTeamProspects`, `moveTeamProspectAcrossRoles`, `nextTeamProspectRank`, `roleKeyOf` — 21+ tests) and [`teamProspectSourceSummary.ts`](../src/lib/teamProspectSourceSummary.ts) (`normalizeTeamProspectSource` — URL → registrable host → `SOURCE_ALIASES` board label, free text → alias or first-seen spelling (v2.2887); `summarizeTeamProspectSources` with per-row `variants`, `describeSourceVariants`, `distinctTeamProspectSources` — 46 tests).
...
12. **Convert marks the prospect only through `markProspectConverted`** (v2.2879 — status → `converted` + a `prospect_comments` row `interaction_type: 'converted'` whose text carries `/customers/<id>`; the same kernel Add customer and Follow Up use — never re-inline it); no delete; the submit button lives outside `NewCustomerForm` via `form="convert-customer-form"`; new bids hardcode `materials_model: 'rough'`.
...
3. **`convert` → `ProspectsConvertTab`** — 7 owned states + 3 effects; props: prospect lists + `authUserId`.
