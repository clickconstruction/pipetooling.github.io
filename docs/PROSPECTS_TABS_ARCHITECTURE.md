...
2. **Calling locks are steal-on-view**: the lock upsert uses `onConflict: 'prospect_id'`, so viewing a prospect takes the lock even if someone else holds it; `loadFollowUpProspects` only hides rows locked by *others* at load time. Cleanup deletes only rows matching own `user_id`.
