...
| `plan_cost_batch(batch)` · `apply_cost_batch(batch, plan_hash)` · `revert_cost_batch(batch_id, reason)` | **Move job cost** as one audited, revertible batch ([`COST_BATCHES.md`](../COST_BATCHES.md)): `plan_` runs `cost_batch_apply` as a dry run — every write performed and unwound — and replies with the summary by job, each op's before/after image and a `plan_hash`; `apply_` runs the dry run **again**, and writes only when the hash still matches (the payload and the rows it touches are what the hash covers — never the dry run's own timestamps) **and the plan has not been applied before** (one apply per plan, from the call log; a batch since reverted does not count); `revert_` needs a reason in words. The `cost_batches` row names you. |
