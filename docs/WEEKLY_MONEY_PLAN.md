...
- **Week = Mon–Sun Central**, matching Weekly movement (`stagesWeeklyMovement.ts`). Note: pay surfaces use Sun–Sat company weeks (`companyWeekStartSundayContaining`) — this report is a *job* lens, not a payroll lens, so it pairs with its sibling report instead. The doc'd divergence is deliberate — with **one exception (v2.2858)**: Moneyfill queue 3d "Sessions pending approval" is a payroll lens, so it reviews the Sun–Sat pay week that ends inside the close week (`payWeekForCloseWeek` in [`payWeekAnchor.ts`](../src/lib/payWeekAnchor.ts); close week Aug 24–30 → pay week Aug 23–29), the same window Draft Payroll opens to. Every other queue stays Mon–Sun. **Both families have one anchor file (v2.2898):** [`payWeekAnchor.ts`](../src/lib/payWeekAnchor.ts) (Sun–Sat) and [`closeWeekAnchor.ts`](../src/lib/closeWeekAnchor.ts) (Mon–Sun; default = previous complete week — the week you close Monday morning). Moneyfill's picker AND the report open on that same default week; `moneyfillWeekClose.previousCompleteWeekMonday` delegates to the anchor.
...
- **Report entry points**: Jobs → Pipeline stage-strip Section tools menu (Pipeline group, beside Weekly movement) + `?stagesMoney=1` deep link (handle-gated + cold-load e2e, per the v2.832 rule) + Moneyfill's "See the week's report" button (v2.2890; `weeklyMoneyReportHref` adds `&stagesMoneyWeek=<monday>` so the report opens on the close week the picker was showing — `JobsWeeklyMoneyModal.initialMondayYmd`). Opened without a pinned week (menu, bare `?stagesMoney=1`) the modal lands on the **previous complete close week** via `resolveWeeklyMoneyReportWeek` (v2.2898 — it used to open on the current week, seven days off Moneyfill); `/moneyfill?week=<monday>` pins the picker the other way, which is how the Quickfill "Close week: $N open" station chips (`closeWeekChip.ts`, same `fetchWeekCloseCounts`, no second mark system) land there.
...
| 3d | Closed sessions pending approval (labor not yet booked) — **Sun–Sat pay week** ending inside the close week, not Mon–Sun (v2.2858) | pending-sessions queries; link to People → Hours | new |
