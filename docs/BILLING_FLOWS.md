...
3. The "unallocated" kernel exists 5× (ensure RPC SQL, `dashboardJobBillingUnallocCents`, `jobsStagesBoard.jobBillingUnallocatedDollars`, `wouldEnsureNothingLeftToBillForJob`, `JobFormModal.unallocatedBillableDollars`). Bases diverge on the primary bundle: the ensure RPC (v2.1134) and `unallocatedBillableDollars` (v2.2446) exclude it; `jobsStagesBoard.jobBillingUnallocatedDollars` still counts it (its "same basis as ensure RPC" comment is stale) — it feeds the Pipeline partial-invoice modal (same $0-clamp wedge on RTB jobs with an auto draft) but also board row-building and `promoteJobToBilledIfFullyInvoiced`, so aligning it needs its own scoped pass.
