...
| **Open Bill Customer** (job) | **none** (v2.2876) — the remainder is computed client-side; the ensure RPC (INSERT/resize of the primary RTB row) runs only inside Send / Save / Send invoice | none | Yes — Cancel leaves the job exactly as found |
