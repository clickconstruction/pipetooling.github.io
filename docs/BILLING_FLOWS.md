...
- `supabase/functions/_shared/paidJobBillGuard.ts` (v2.2846) — `shouldBlockBillOnPaidJob`: a Bill Customer send on a **paid** job is refused everywhere (modal gate on all three channels; server-side 409 `job_already_paid` in `create-stripe-invoice` and `send-physical-invoice-email`) unless the caller sends `allow_rebill: true` — the modal's "Bill this job again anyway" checkbox. HouseCall Pro is a direct row update with no edge function, so the modal is its only stop. Refusals log `job_activity_events.rtb_paid_job_blocked`.
...
- **Dashboard Billing Pipeline** (`src/components/dashboard/BillingPipelineCard.tsx` → `DashboardBillingPipelineSection.tsx`, engine `src/hooks/useDashboardBillingInvoices.ts`): Stage 1 = `DashboardFieldCollectPaymentQueue`; Stage 2 "Ready to Bill" (Bill Customer, Delete draft bill, Send Back); Stage 3 "Billed Waiting for Payment" (Mark Paid, Send back). Units built by `src/lib/buildReadyToBillDashboardUnits.ts` / `src/lib/dashboardBillingInvoiceUnits.ts`. **Membership (v2.2846):** Stage 2 = RTB-status jobs ∪ `ready_to_bill` invoice rows, Stage 3 = billed-status jobs ∪ `billed` invoice rows — in both, **invoice rows whose parent job is `paid` are dropped** (`isDashboardBillOnPaidJob`; the join now selects `jobs_ledger.status`), matching the Pipeline board's job-status rule below. A billed invoice on a working job stays: an open bill is defined by invoice status (decision 2026-09-05), a bill on a paid job is the one exception. The stale drafts this retires are listed for deletion at Settings → Data & recovery → "Draft bills on paid jobs (dev)".
...
`ensure_single_ready_to_bill_invoice_for_job(p_job_id)` (baseline): unallocated = `GREATEST(0, revenue − payments_made − Σ RTB+billed lines excluding the never-sent primary bundle itself, each line counted for what is still unpaid on it)` (v2.1134 — the old sum included the primary, which broke the resync branch; v2.3775 / `20260923233000` — the old sum took each line's face amount, so a partly paid billed line was subtracted twice, once through `payments_made` and once as a bill, and job 978 read "Nothing left to bill" with $577 unbilled; the client twin is `src/lib/billing/openLineAllocation.ts`). Requires job status `ready_to_bill`, roles dev/master_technician/assistant/primary + job access. Creates or re-syncs the single `is_primary_rtb_bundle` row to the unallocated remainder (skipping amount rewrites on Stripe-finalized rows); partial rows are never touched. Since v2.2446 (`20260828170937`) a fully-allocated remainder with a never-sent primary **DELETEs the primary and returns `{ok, fully_allocated: true}`** — before, it parked a $0.00 draft and returned an error the resync callers threw on after their invoice insert had already succeeded. Still errors on multiple primaries, and on nothing-left-to-bill when no primary exists; resync callers classify envelopes via `ensureRemainderResyncOutcome` (`src/lib/jobs/ensureRtbRemainderResult.ts`). **Opening Bill Customer for a job writes nothing (v2.2885, decision 17).** The modal computes what this RPC *would* do from the job row + invoices through `planPrimaryRtbForBillCustomer` (`src/lib/billing/proposedPrimaryRtbAmount.ts` — every branch mirrored, RPC error strings verbatim; the fully-allocated DELETE branch renders "Nothing left to bill" and deletes nothing) and runs the RPC **once, inside Send / Save / Send invoice, immediately before that channel's write** (`ensurePrimaryRowForCommit`). A remainder that moved since open is surfaced ("The amount left to bill changed to $X…") rather than billed. v2.2464's delete-on-cancel guard (`mintedPrimaryDraftCleanup.ts`) is gone — nothing is minted on open. Older client mirror of one branch: `src/lib/wouldEnsureNothingLeftToBillForJob.ts`.
...
Which fixtures a bill lists (`scopeFixturesToInvoice` edge / `fixturesForInvoiceBill` client mirror): rows LINKED to the invoice when any exist (v2.1133, segment invoices); else, for the **primary remainder bundle only**, the still-unlinked billable rows at their real prices when their cents sum EXACTLY to the target amount (v2.2469 — any payment, unlinked dollar carve, rider, or extra breaks the equality); else the still-unlinked rows, prorated (v2.2589 — rows linked to OTHER invoices are on those bills and never re-list; before this the fallback prorated the whole job, re-listing already-billed change orders. Every row linked elsewhere → empty set → the single-line fallback). **v2.2913 (journey B6 / J3-6):** for the primary remainder, when its amount is *smaller* than the unlinked billable rows, the gap is the payments/carve pool — poured into the rows in order exactly like `dollarCoverageForSegments`; rows the pool swallows whole never re-list (`dropPaymentsCoveredRows`), the row it runs out inside and every row after it stay for proration. A positive remainder always keeps at least the last row; riders/extras that inflate the remainder above the row sum leave the pool empty. Mirrored in the edge helper — redeploy `create-stripe-invoice` / `send-physical-invoice-email` after merging for Stripe parity.
...
- Two bases everywhere: invoice-line remaining = `amount − Σ payments(invoice_id)`; job-shell remaining = `revenue − payments_made`; the billing-unallocated variant additionally subtracts the RTB+billed lines' **remaining** (never their face amounts — v2.3775, `lib/billing/openLineAllocation.allocatedOpenCents`, the one sum behind Bill Customer's plan, the Pipeline's remaining and partial clamp, Edit Job's Make Invoice / bar / ② strip, and the Dashboard's Prepare-bill gate).
...
- **Post-create panel / "View bill"** (`src/components/jobs/HostedStripeBillPanel.tsx`): edge `get-stripe-invoice-details` (reads + silent memo/footer DB backfill, and since v2.3590 the current hosted link too — the panel prefers the link Stripe just returned); "Customer pay page" opens `hosted_invoice_url` (what the customer sees — Stripe expires that link 30 days after the due date, so the nightly `refresh-stripe-invoice-links` sweep (v2.3589) re-fetches every open Stripe bill's link and stores the current one; every reader of the column hands out a link no more than a day old; the portal and View bill re-fetch a bill past 25 days on the spot — v2.3590); `StripeInvoiceSharePanel` copy-link/SMS-draft/email-draft buttons **send nothing** (`SmsBillDraftModal`/`EmailBillDraftModal` are clipboard + `mailto:` only; templates in `src/lib/stripeInvoiceShareCopy.ts`).
- **"Send Email invoice from Stripe"** (`src/components/jobs/StripeInvoiceSendFromStripeButton.tsx`) → edge `send-stripe-invoice`: the **only path where the Stripe bill is emailed to the customer** — `stripe.invoices.sendInvoice()`, from Stripe, to the email on the Stripe invoice/customer object. Logs to `jobs_ledger_invoice_stripe_email_sends`, stamps `sent_to_customer_at`. Test mode: Stripe accepts the send but delivers no real email. **Copies (v2.3359)**: the row's `copy_emails` then go out one Resend email each with the same Pay link (`copies_sent` / `copies_failed` in the response; the confirm dialog lists them beforehand).
...
`confirmOutsideBill()` ("Save"): **record-only**. The form says so on its face since v2.2913 (journey B6 / J3-5): a muted caption above the Date field — *"Records the bill as sent through HouseCall Pro — ClickTooling emails nobody. The job moves to Billed Awaiting Payment; the customer sees nothing from here."* Direct client UPDATE → `{status:'billed', external_send_channel:'housecallpro', external_send_note, sent_to_customer_at}`. **There is no HouseCall Pro API integration anywhere in the repo** — `hcp` in code always means the imported job number (`jobs_ledger.hcp_number`). Customer sees nothing from PipeTooling. Reversible while unpaid.
...
| `send-stripe-invoice` | `invoices.sendInvoice` — **emails customer** | `sent_to_customer_at`, send-log row | row-mode authoritative; sub/helpers/superintendent (v2.2637) allowed via collect-flow gate |
...
**Which bill a payment pays** (v2.3592, the owner's rule 2026-09-18 — **oldest bill first**; kernel [`supabase/functions/_shared/paymentAttribution.ts`](../supabase/functions/_shared/paymentAttribution.ts), client re-export `src/lib/jobs/paymentAttribution.ts`): a payment with an `invoice_id` is that bill's in full; a job-level payment (`invoice_id` NULL — 191 rows, $2.32M on 2026-09-16) is applied to the job's *sent* bills (billed or paid, never a ready-to-bill draft) oldest first by `sequence_order` then `billed_at`, each taking no more than it still needs after its own linked money, one payment splitting across bills when it must; what no sent bill needs is the job's **surplus**, shown as a job-level line and never on a bill. Four readers share it and cannot drift: the bill's own paper (`filterPaymentsForPhysicalInvoiceHistory` — a partial share prints *· part of $X*), the demand letter and lien claim (`paymentsAppliedToInvoice`), the Bill tab's rows (`JobFormInvoiceList`; the sum line's *+ $X on no bill* is the surplus plus payments on bills not listed) and the customer portal (`buildPortalBills` / `buildPortalSharedBills`; `customer-portal` reads payments by job so unlinked ones reach the kernel). Before it, the paper printed unlinked money on every bill of a multi-bill job (v2.3498's other half) and the letter counted none of it (v2.3515 took the single-bill case).
...
| **Open Bill Customer** (job) | **none** (v2.2885) — the remainder is computed client-side; the ensure RPC (INSERT/resize of the primary RTB row) runs only inside Send / Save / Send invoice | none | Yes — Cancel leaves the job exactly as found |
