...
- **Post-create panel / "View bill"** (`src/components/jobs/HostedStripeBillPanel.tsx`): edge `get-stripe-invoice-details` (reads + silent memo/footer DB backfill); "Customer pay page" opens `hosted_invoice_url` (what the customer sees — Stripe expires that link 30 days after the due date, so the nightly `refresh-stripe-invoice-links` sweep (v2.3589) re-fetches every open Stripe bill's link and stores the current one; every reader of the column hands out a link no more than a day old); `StripeInvoiceSharePanel` copy-link/SMS-draft/email-draft buttons **send nothing** (`SmsBillDraftModal`/`EmailBillDraftModal` are clipboard + `mailto:` only; templates in `src/lib/stripeInvoiceShareCopy.ts`).
