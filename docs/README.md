...
| [`TAKEOFFS_REFRESH_PLAN.md`](./TAKEOFFS_REFRESH_PLAN.md) | Build plan for the Bids → Takeoffs refresh: Old / One at a time (New 1 at build time) / Sheet (New 2, the cost rail) behind pills, shared substrate (fixture key, coverage, book that learns, fixture-history RPC), Fill-from-book under Combined, retirement criteria. |
