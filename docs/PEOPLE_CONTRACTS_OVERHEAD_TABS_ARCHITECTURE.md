...
Stage A is **largely already done** for this tab — the calc core lives in tested libs: [`overheadDailyLabor.ts`](../src/lib/overheadDailyLabor.ts) (`buildOverheadDailyLabor`, `buildOtherJobsLaborByDay`, `mergeOverheadDayTableRows`, `overheadFactorTotalOverOtherJobs`, `filterOverheadDetailLines`, `aggregateOverheadDetailByPerson[TotalScope]`, `aggregateOtherJobsLaborByPerson`, `buildOverheadWageLookup`), [`fetchOverheadOfficePartsByDay.ts`](../src/lib/fetchOverheadOfficePartsByDay.ts) (`fetchOverheadOfficePartsByDay`, `fetchOtherJobsPartsByDay`), [`overheadPartsAccountingBuckets.ts`](../src/lib/overheadPartsAccountingBuckets.ts) (`bucketOverheadPartsLinesByAccountingLabel`, `sumMaterialsTotalUsdExcludingInternalTransfer`, `overheadPartsAccountingBucketFromDefaultKey`), [`overheadOfficeJobSettings.ts`](../src/lib/overheadOfficeJobSettings.ts) (app_settings key `overhead_office_job_ledger_id_v1`), [`overheadTableViewStorage.ts`](../src/lib/overheadTableViewStorage.ts) (localStorage `people_overhead_table_simple_view_v1`). What remains inline is orchestration + render.
...
- **Effects, all gated `canAccessOverheadTab && authUser?.id` with `cancelled` cleanup:**
  ...
  3. **Office parts** → `overheadOfficePartsUsdByDay`/`overheadOfficePartsDetailByDay` via `fetchOverheadOfficePartsByDay` (cleared to empty Maps when no office job).
  4. **Other-jobs parts** → `overheadOtherJobsPartsUsdByDay`/`overheadOtherJobsPartsDetailByDay` via `fetchOtherJobsPartsByDay`.
  5. **Accounting buckets** → `overheadOtherJobsAccountingBucketByTxId`: collects Mercury tx ids from the other-jobs detail map, two-step query — `mercury_transaction_drag_sort_assignments` `.in(txIds)` then `mercury_drag_sort_labels` `.in(labelIds)` → `overheadPartsAccountingBucketFromDefaultKey`. Recomputed per detail-map change (not per modal open) so day flips inside the modal are instant.
- **`useMercuryLedgerNicknames({ enabled: true })`** → `overheadMercuryNicknameByDebitCard` (role-gated inside the hook) for "· on <card>" labels in the office-materials list.
- **Memos:** `overheadLabor` (= `buildOverheadDailyLabor` over sessions + `buildOverheadWageLookup(payConfig)`), `overheadOtherJobsLabor` (= `buildOtherJobsLaborByDay`), `overheadMergedByDay` (= `mergeOverheadDayTableRows` of the five per-day inputs), `overheadTableTotals` (column sums for the `<tfoot>`), `overheadTableColCount` (4 simple / 7 advanced).
- **Supabase:** `clock_sessions`, plus (inside the libs) mercury allocations / supply invoices / tally parts tables.

### KPI strip — averages (~769–842 render, effect ~438–547)

- **State:** `overheadAvgDailyCost` (`avg7/30/90`, `per100_7/30/90`, `loading`).
- **Effect:** independent 90-day fetch — `clock_sessions` for `[today-89, today]` (same office-or-bid `.or()` shape), `fetchOverheadOfficePartsByDay` over the same range, rebuilds labor via the same lib fns, merges with `mergeOverheadDayTableRows(labor.byDay, partsByDay, empty, empty, empty)` (other-jobs slots deliberately empty), then `jobs_ledger_invoices` SELECT (`amount`, `sent_to_customer_at` in `[start, today+1)` ISO bounds) bucketed by `calendarYmdInAppTzFromIso` → inner `sumWindow(n)` (cost/n calendar average incl. zero days; per-$100 = cost/revenue×100, null when revenue ≤ 0).
- **Extraction:** low-medium. Stage A: lift `sumWindow` + the revenue bucketing into `lib/` (see Stage-A table). The render block is trivially presentational once fed the state.
...
| 90-day `sumWindow` + invoice `revenueByDay` bucketing | inline in the avg-cost effect | `lib/overheadAvgDailyCost.ts` — `computeOverheadTrailingAverages({ totalsByDay, revenueByDay, todayYmd })` + tests (zero-day inclusion, revenue-0 → null) |
...
Already extracted with tests (do not re-do): `contractBodyFormat`, `contractSigningContent`, `contractSendEmailPreview`, `overheadDailyLabor`, `overheadPartsAccountingBuckets`, `fetchOverheadOfficePartsByDay`, `overheadOfficeJobSettings`, `overheadTableViewStorage`.
