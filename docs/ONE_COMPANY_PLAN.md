...
- **Phase 4** — next, after a quiet week: backfill `master_user_id` to the company owner account; stop writing / rename `master_assistants` and `master_shares` (retire `sync_company_access_grants()` with them).
