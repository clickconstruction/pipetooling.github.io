...
- **`src/pages/Prospects.tsx`** - Lead management; top-level **Customers** tab groups sub-tabs Follow Up / Prospect List / Convert / **Activity** (formerly "Team"; dev/assistant) (**v2.708**); top-level **Team** tab = prospective-hires **board** (`?tab=team`): one drag-ranked column per role (**`team_prospect_roles`** + **`team_prospects.role_id`**, role deletable only when empty via FK RESTRICT; kernel [`teamProspectRanking.ts`](../src/lib/teamProspectRanking.ts), UI [`TeamProspectsTab.tsx`](../src/components/prospects/TeamProspectsTab.tsx) — **v2.709** / **v2.712**)
