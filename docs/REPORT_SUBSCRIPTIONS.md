...
| `report_email` | event | `report_email_subscriptions` + `_authors` (named authors, v2.746) + `_team_leads` (a lead's whole team via `team_leader_assignments`, resolved at send time, v2.3480) + `report_email_dispatch_log` | `send-report-email` (fired by the report modals at filing; manual "Send now" for the last 14 days) | — | ✅ "Field reports" row (v2.3472; matched by user id or by an outside address equal to the account email) |
...
| `gc_statement` | scheduled report | `gc_statement_email_requests` (v2.1426; free-text `sent_to`; scheduling UI v2.1427; **office-wide SELECT since v2.2888**, Cancel requester-or-dev) | `gc-statement-email-dispatch` (send-time dedupe v2.2888) | ✅ requester-scoped (v2.1428) | — (scheduled report, not an event stream) |
...
**One shared view, no double sends (v2.2888, journey-map #45).** The pending-requests read (`listPendingStatementRequests`, was `listMyPending…`) is now the GC Review cohort's — migration `20260905190000` widened the SELECT policy to the INSERT cohort (dev / assistant-like / master / primary) — so GC Review's "Scheduled statement sends" box shows every chain to whoever runs the round, with **Cancel only for the requester or a dev** (`canCancelStatementRequest`; the DELETE policy is unchanged). Both app-send lanes run one send-time dedupe kernel, [`_shared/gcStatementSendDedupe.ts`](../supabase/functions/_shared/gcStatementSendDedupe.ts): same entity + same address inside the window, by any lane → skipped with a logged reason (Draft Message: 10 min, answered as `{ success: false, skipped: 'duplicate' }`; the dispatcher: 12 h, `error = 'skipped: duplicate — …'`, chain advances). Both stamp `email_send_log.email_type` (`gc_statement_manual` / `gc_statement_scheduled`), and the same migration lets the office read those two types, so the per-GC **"What went out"** list (`GcStatementSendHistoryModal`, kernel `src/lib/jobs/gcStatementSendHistory.ts`) shows marks and app sends in one newest-first list with lane, who, recipient, total and delivery status. `get_my_email_schedule()`'s requester-or-recipient scoping is unchanged — the Settings page is still "my" schedule; GC Review is the office's.

**One statement, three lanes (v2.2874, journey-map #46).** The dispatcher's `render.ts` and the client's `gcStatementEmail.ts` are hand-mirrored; `src/lib/jobsDocuments/gcStatementEmailParity.test.ts` pins them byte-for-byte (per-GC + Share-all, HTML + text). The dispatcher's nothing-outstanding skip now has a client twin: `gcStatementSendGuard` disables Draft Message's **Send statement** on a $0 group ("Nothing owed — no statement goes out."); **Schedule…** stays allowed because the dispatcher rebuilds fresh and skips empties itself. The `gc_statement_scheduled` template (subject + intro) is read by both app-sent lanes. Every lane's portal card says how to pay — "Pay online any time at <portal> — this statement stays current there." — with `?src=gc-statement` on the link for attribution.
