...
| `crew_day` | scheduled report | `crew_day_email_requests` (v2.2603; internal `recipient_user_id`; **per-recipient payload** — `get_crew_day_payload_for_user` scopes to the recipient's role, superintendents included as senders AND recipients; emailed day = the send's Chicago day) | `crew-day-email-dispatch` | ✅ recipient-scoped | — (scheduled report) |
