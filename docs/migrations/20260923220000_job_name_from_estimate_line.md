...
**Idempotent**: `CREATE OR REPLACE` both; the apply is re-runnable (a renamed job no longer matches). No table. **Order**: after `20260923120000` (it rebuilds that function's body). Client and migration are independent. **Superseded in part by `20260924010000`** — this version's plan built a temp table and could not run read-only; see that doc for the shape that is live.
