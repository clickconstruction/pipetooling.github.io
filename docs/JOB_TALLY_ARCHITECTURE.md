...
- **Gate-split defects (found while mapping at a05cef4c4; each fix changes behavior — ship it as its own PR, never inside a move):**
  - **R1 — flag merge is still dev-only (348).** For a non-dev payroll-access user (controller, pay-approved master) `payrollFlaggedIds` stays empty and `is_payroll` never merges. After *Mark payroll* the row stays under "Show unlinked" with its button, the `Payroll ✓` banner and payroll chip never render, and the page's `N unlinked` runs above the Dashboard card (server counters exclude payroll since `20260709160000`). Likely fix: gate the merge on `canMarkPayroll` (RLS already allows the SELECT).
  - **R2 — "Create rule…" is dead for non-devs.** The confirm modal's `onCreateRule` (2392–2398) sets `payrollRulesModalOpen`, but `TallyPayrollRulesModal` mounts only under `isDevTally` (2348), so the button closes the confirm and opens nothing. Likely fix: hide the button unless dev.
...
| Payroll sub-cluster | `tallyPayrollRules.test.ts`, `tallyPayrollRuleSeed.test.ts`, `payWeekLinks.test.ts`, `orgDefaults.test.ts` | **chip `Σ|amount|`** (money), gate split R1/R2, both modals |
...
3. **Payroll gate fix (R1, R2)** — a small behavior PR, not an extraction; land it before step 5 so the Transactions move stays behavior-preserving and the split isn't carried into the new file.
