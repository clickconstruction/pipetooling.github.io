...
[`src/pages/Settings.tsx`](../src/pages/Settings.tsx) is a ~5,132-line "God component" (as of v2.735: ~282 `useState` declarations, 32 `useEffect`, 9 `useMemo`, 1 `useCallback`). It follows the process in [`PAGE_DECOMPOSITION_PLAYBOOK.md`](./PAGE_DECOMPOSITION_PLAYBOOK.md) and the format of [`BIDS_TABS_ARCHITECTURE.md`](./BIDS_TABS_ARCHITECTURE.md) / [`DASHBOARD_SECTIONS_ARCHITECTURE.md`](./DASHBOARD_SECTIONS_ARCHITECTURE.md).
...
So the remaining ~5.1k lines are **not un-extracted tabs** — they are the *engine rooms* of tabs whose JSX already moved out. The remaining work is mostly **Stage-A/seam work**: push each tab's state + handlers into a hook or into the tab component, collapsing the prop lists. One large JSX section is still fully inline (Sharing & Adoption, ~280 lines).
...
2. **There is NO URL sync — `?tab=` and `#anchor` deep links into Settings are inert.** `DashboardBulkDeleteAlertBanner` links to `/settings?tab=settings-data`, `DashboardClaimDevAttemptsBanner` to `/settings?tab=settings-people`, and `Calendar.tsx` links (×6) to `/settings#settings-time-off` / `/settings#settings-salary-workday` — Settings reads neither `location.search` nor `location.hash`, so all of them land on the default first tab ("Recent push"). The hash targets exist (ids inside `SettingsAccountSchedulingTab`) but that component only mounts when the Account tab is active. This broke when collapsible sections became tabs (PR #35). Preserve-or-fix decision belongs to a feature PR, not the decomposition.
...
| 4 | People & accounts (`settings-people`) | `SettingsPeopleTab` (dev) + **INLINE Sharing & Adoption** + `TeamFeedbackMasterAggregates` | partly extracted | parent (adoptions/shares/groups/people/merge ~45 state); `ActiveAccountsPanel` self-contained | ~50 | low–med | **Order #1:** extract inline Sharing & Adoption (self-contained). Order #4: people-directory residue |
...
- **Render:** `SettingsGroup id="settings-people"` (~4301). Three blocks:
  ...
  2. **INLINE "Sharing and Adoption"** (~4354–4636, dev|master): collapsible (`roleSharingSectionOpen`) with four sub-blocks — Adopt Assistants (`master_assistants`; **assistants list includes controllers**: `.in('role', ['assistant', 'controller' as 'assistant'])`), Adopt Primaries (`master_primaries`), Adopt Superintendents (`master_superintendents`), Share with other Master (`master_shares`). Dev extra: `selectedMasterIdForAdoptions` picker (three identical copies of the same `<select>`) to manage **another master's** adoptions via `adoptionMasterId`; sharing always acts as self. Handlers ~1505–1841 (`load*AndAdoptions` ×3, `loadMastersAndShares`, `toggle*` ×4, `handleAdoptionMasterChange`); ~17 state vars.
...
- **Approach:** **Order #1: extract Sharing & Adoption** as a self-contained `SettingsSharingAdoptionSection` + `useMasterAdoptions(adoptionMasterId)` hook (single-surface; follows the ActiveAccountsPanel/DeletedRecordsSection precedent; ~620 lines out of the parent; no pure-calc Stage A needed, though the three adoption blocks are one parameterizable component). **Order #4:** move the merge modal + handlers into SettingsPeopleTab, and pull groups/pay-approved/people handlers into `useSettingsPeopleDirectory`.
...
2. **Inert deep links:** `/settings?tab=…` (2 dashboard banners) and `/settings#settings-time-off` / `#settings-salary-workday` (Calendar ×6) do nothing — no `?tab=` router exists and the hash targets are hidden/unmounted until the Account tab is selected. Known breakage since the sections→tabs conversion (PR #35); fixing it is a feature PR (add a `?tab=` reader), not part of a behavior-preserving move.
...
1. **Sharing & Adoption → self-contained section** (`SettingsSharingAdoptionSection` + `useMasterAdoptions`). The only large inline JSX (~280 lines) plus ~340 handler lines and ~17 state vars; single-surface; proven precedent (ActiveAccountsPanel). Lowest risk, immediate ~600-line win.
