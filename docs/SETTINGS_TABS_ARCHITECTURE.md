...
2. **~~There is NO URL sync — `?tab=` and `#anchor` deep links into Settings are inert.~~ FIXED in v2.737.** `DashboardBulkDeleteAlertBanner` links to `/settings?tab=settings-data`, `DashboardClaimDevAttemptsBanner` to `/settings?tab=settings-people`, and `Calendar.tsx` links (×6) to `/settings#settings-time-off` / `/settings#settings-salary-workday`. Settings now reads `location.search`/`location.hash` via [`resolveSettingsDeepLink`](../src/lib/settingsDeepLink.ts): `?tab=` activates the group; a `#settings-*` section anchor activates its owning tab and scrolls the anchor into view (the apply effect retries when the role-filtered groups load and polls for the async-mounted anchor). Broke originally when collapsible sections became tabs (PR #35). v2.2890 added two anchors to `SETTINGS_HASH_ANCHOR_TO_TAB`: `#settings-page-pins` (Your dashboard → Dashboard Page Pins; Settings opens the collapsed `financialPinsSectionOpen` group for it, from a deep link or a search pick — the "Page pins" search entry used to point at Your account) and `#settings-claim-code` (Advanced → the claim-code form, which opens its collapsed group when that hash is present; the Needs You "Someone tried to become a dev" card lands here instead of on People). v2.2902 added `guideSlug` to `SettingsSearchEntry` (`settingsSearch.ts`): a guide-backed pick ("Search for jobs, bids, customers, and estimates" → `global-search`) navigates to `?tab=settings-guides&g=<slug>` via `settingsSearchGuideQuery`, so the deep-link effect and `GuideBrowser` (shared `HELP_GUIDE_URL_PARAM`, `src/lib/helpGuideUrlParam.ts`) open the article together.
...
| 4 | People & accounts (`settings-people`) | `SettingsPeopleTab` (dev); `TeamFeedbackMasterAggregates` removed v2.2824 | extracted | **`useSettingsPeopleDirectory` hook, parent-instantiated (v2.857)**; `ActiveAccountsPanel` self-contained | ~50 | low–med | ~~Order #1~~ done (v2.853; section since removed, v2.922). ~~Order #4~~ **done (v2.857)** |
| 5 | Data & recovery (`settings-data`) | `SettingsDataTab` | extracted | `useSettingsBackupExports` (parent hook) + self-contained `StaleDraftBillsOnPaidJobsSection` (v2.2846 — lists `ready_to_bill` drafts on `paid` jobs, retires via `delete_ready_to_bill_invoice`) / `DeletedRecordsSection` / `BulkDeleteAlertSettingsBlock` | ~25 (all from one hook) | — | Effectively done; optionally move the hook call into the tab |
| 5b | Usage (`settings-usage`, v2.2342, dev-only) | `SettingsUsageTab` | extracted (born external) | **self-contained** — four `usage_*` SECURITY DEFINER RPCs, shaping in [`usageDashboard.ts`](../src/lib/usageDashboard.ts) kernel | 0 | — | Done |
| 5c | What customers see (`settings-what-customers-see`, v2.2758, dev-only) | `SettingsWhatCustomersSeeTab` | extracted (born external) | **self-contained** — one `app_settings` fetch (`CUSTOMER_SAMPLE_SETTING_KEYS`), journeys from the [`customerJourneys.ts`](../src/lib/customerJourneys.ts) kernel, emails from [`customerSampleEmails.ts`](../src/lib/customerSampleEmails.ts), pages as same-origin iframes on the sample token | 0 | — | Done (v2.2758 + v2.2760) |
| 6 | Jobs & dispatch (`settings-jobs`) | `SettingsJobsTab` + `TripChargeAmountsSettingsBlock` | extracted | **`useSettingsJobsAdmin` hook, parent-instantiated (v2.856)**; dispatch/estimator group toggles still parent | ~24 (from the hook) | — | ~~done (v2.856)~~ except group toggles (order #4b) |
...
| ↳ Daily goals / Job Book / Report notifications (Team Hours Sharing manager moved to People → Users → **Team leads**; one-line pointer remains) | ✓ | ✓ | ✓ | — | — | — | — |
...
- **Toast/error conventions.** Success → `showToast(…, 'success')` (10 call sites). Failures → per-section error state (`serviceTypeError`, …) rendered next to the form. The page-wide `error` state renders **only** in the pre-role early return and inside SettingsPeopleTab's Additional People section — see quirk #4.
- **Proven seam-hook precedents to copy:** `useActiveAccountsManagement` (state+handlers hook, component self-contains), `useSettingsBackupExports` (parent hook, values threaded), `useDeletedRecordsArchive` (fully self-contained single-surface section). The named candidates all landed during the v2.853–v2.859 campaign: `useSettingsTemplatesEngine`, `useSettingsCatalogs`, `useSettingsPeopleDirectory`, `useSettingsFinancialPins`, `useSettingsMyReports`, `useSettingsTeamLeaderAssignments` (since renamed `useTeamLeaderAssignments` and rehomed to the People → Users Team leads modal), `useSettingsAccount` (all in `src/hooks/`). `useMasterAdoptions` also shipped (v2.853) but was deleted with its section in v2.922.
...
- **Status: JSX extracted; engine in parent.**
  ...
  - [`SettingsAccountSchedulingTab`](../src/components/settings/SettingsAccountSchedulingTab.tsx) (323 lines): renders `SalaryWorkScheduleSettings` (self, gated dev-or-self-salaried) + since v2.3705 a one-line pointer to People → Users → Pay in place of the dev-only "All salaried" picker (the picker's state, `devPayConfigForSalaried` and `buildSalariedWorkdayPickerRows` left `useSettingsAccount` with it; before that: `devPayConfigForSalaried` open-triggered load from `people_pay_config`, `buildSalariedWorkdayPickerRows` memo — pure lib, already tested) + (until v2.1544) ungated `TimeOffSettings` — **moved** to the Dashboard My Time section's "Personal Time Off…" modal ([`PersonalTimeOffModal`](../src/components/PersonalTimeOffModal.tsx)); the `#settings-time-off` deep-link mapping is retained for legacy bookmarks (lands on the account tab, no anchor). Holds the `#settings-salary-workday` anchor id (quirk #2).
...
- **Approach (~~order #5~~ done v2.858):** parent residue split into `useSettingsFinancialPins` (dev cluster: state + 4 loaders), `useSettingsMyReports` (loader + realtime + edit-window; modal wiring stays in the parent, which hands it the reload fn), `useSettingsTeamLeaderAssignments` (rows + memos + picker deriveds; since renamed `useTeamLeaderAssignments` and rehomed with the manager to the People → Users Team leads modal, 2026-08-02) — all in `src/hooks/`, parent-instantiated. Remaining follow-on: sub-decompose the child per section (which would also collapse the ~95-prop membrane).

### 4. People & accounts

- **Render:** `SettingsGroup id="settings-people"` (~4301). Three blocks:
  1. **`SettingsPeopleTab`** (dev-only, ~4303): since v2.3705 (People spine PR 6) the accounts card is a one-line **pointer** to People → Users → Account (the v2.2835 pattern) with *Manage accounts…* (the app-level modal) and *Find duplicates…* beside it; the sample-accounts heading moved to Settings → System → Digital twins & samples (`SampleAccountsCard`). Before that: [`ActiveAccountsPanel`](../src/components/settings/ActiveAccountsPanel.tsx) `variant="card"` (**self-contained** via [`useActiveAccountsManagement`](../src/hooks/useActiveAccountsManagement.ts): users table with role select, **`read_only` training-mode checkbox**, **`team_prospects_access` + `estimator_prospects_access` grants**, service-type pills, invite (`invite-user`) / manual add (`create-user`) / sign-in email (`send-sign-in-email`) / archive + archive-reassign (`archive-user`, v2.710 cascade) / restore (`restore-user`) / set-password (`set-user-password`) / merge (`merge-users`) — also mounted app-level via `ActiveAccountsModalContext`; syncs back through `onActiveAccountsDataChanged={() => void loadData()}` and `onOpenFindDuplicates`); `Role visibility` static table; `Task Dispatch group` + `Estimator Inbox group` toggles (parent handlers, `dispatch_group_members` / `estimator_group_members`); `Pay Approved Masters` (parent, `pay_approved_masters`); `TeamFeedbackDevSettingsBlock` (self-contained); `Additional People` (parent: `myPeople`/`nonUserPeople` from `loadData`/`loadPeopleForDev`, person edit modal + `cascadePersonNameInPayTables`, delete with 90-day-restore copy, **the only render sites of the shared `error` state** — quirk #4).
  2. **Sharing & Adoption — REMOVED in v2.922** (commit `e0b8ca07`): the section (extracted v2.853 as `SettingsSharingAdoptionSection` + `useMasterAdoptions`) was deleted entirely; adoption grants are now auto-synced via the `sync_company_access_grants` DB function. Neither file exists anymore.
  3. ~~`TeamFeedbackMasterAggregates`~~ — **removed v2.2824** (crew ratings are anonymous to everyone but dev; nothing attributed reaches masters). Since **v2.2835** the dev `TeamFeedbackDevSettingsBlock` here is a one-line pointer to People → Feedback (the switch, table, feed, and settings live there).
...
- **Role gate:** tab listed for dev|master; since v2.2824 a master sees no block here (block 3 removed).
- **Approach:** ~~Order #4~~ **done v2.857** — merge modal + handlers moved into SettingsPeopleTab; groups/pay-approved/people handlers live in [`useSettingsPeopleDirectory`](../src/hooks/useSettingsPeopleDirectory.ts) (parent-instantiated).
...
- **Status: JSX extracted; engine in parent.** Sections: job-creation owner overrides (`app_settings` dynamic keys `job_owner_override_<userId>`, creators = dev|master|assistant|**controller**), bulk job re-assign (`jobs_ledger.master_user_id` update + optimistic `jobCountByUserId` fix-up; counts from RPC `list_job_counts_by_master_for_dev_settings` in `loadData`), default labor rate (`app_settings.default_labor_rate`), plus self-contained [`TripChargeAmountsSettingsBlock`](../src/components/settings/TripChargeAmountsSettingsBlock.tsx), [`JobAddressCityListSettingsBlock`](../src/components/settings/JobAddressCityListSettingsBlock.tsx), and [`DevelopmentsSettingsBlock`](../src/components/settings/DevelopmentsSettingsBlock.tsx) (v2.1218 — manage developments: rename / default GC / archive / delete, own loads on expand).
...
- **Render:** `SettingsGroup id="settings-release-notes"` wrapping `<SettingsReleaseNotesSection role={myRole} />`, between Advanced and How it works. All roles. **Done — born extracted, self-contained** (one optional `role` prop since v2.2908; renders the static `RELEASE_NOTES` array from [`src/content/releaseNotes.ts`](../src/content/releaseNotes.ts)). v2.2908 (B16): the tail is **paged** — 15 notes per "Show earlier updates" click via `releaseNotesPage` / `describeEarlierUpdatesButton` in `lib/releaseNotes.ts` (the old button mounted ~1,800 cards in one commit) — and notes may declare `roles?: UserRole[]`; `releaseNotesForRole` hides them from other roles (devs and a not-yet-loaded role see all) and the header says how many were hidden.
...
3. **`controller` is missing from the local `UserRole` union** (~line 76) yet fully live at runtime: assistant pickers fetch controllers via `'controller' as 'assistant'` casts (see `usePeopleRoster` / `useFirstAssistantDispatchPhone`), `saveJobOwnerOverrides` (now in `useSettingsJobsAdmin`) includes `'controller'` in creators, `isAssistantLike` matches it. `getSettingsJumpGroups` intentionally(?) gives controllers only the base tabs. Preserve the casts; unifying on the app-wide `UserRole` (from `useAuth`) is a separate cleanup.
...
1. ~~Sharing & Adoption → self-contained section~~ **DONE v2.853** (−607 parent lines, 5,171 → 4,564) — then the entire section was **removed in v2.922** (commit `e0b8ca07`); adoption grants are now auto-synced via `sync_company_access_grants`, and `SettingsSharingAdoptionSection` / `useMasterAdoptions` no longer exist.
2. ~~Templates engine → `useSettingsTemplatesEngine`~~ **DONE v2.854**: hook consumed inside the tab; prop membrane ~85 → 3. −738 parent lines (4,564 → 3,826). (The optional shared edge-fn-invoke util was skipped to keep the diff a pure move — the ~25-line refreshSession/FunctionsHttpError boilerplate ×3 now lives in the hook and remains a future cleanup.)
3. ~~Catalogs engine → `useSettingsCatalogs`~~ **DONE v2.855**: five engines + orphan prices in the hook (parent-instantiated; `serviceTypes` stays exposed; ledger-prefix reload via context inside the hook); orphan modal moved into the tab; Stage-A kernels in `lib/settingsCatalogs.ts` (+ tests). −1,149 parent lines (3,826 → 2,677). The prospects-block savers were NOT moved (deferred to order #4 to keep this diff reviewable).
4. **People/Jobs residue** — split into two halves:
   - ~~4a (Advanced + Jobs admin + prospects savers)~~ **DONE v2.856**: `SettingsAdvancedTab` self-contained; `useSettingsJobsAdmin` + `useSettingsProspectsCatalog` (both parent-instantiated; prospects hook stays in the parent because its tab is conditional-mount). −242 lines (2,677 → 2,435).
   - ~~4b~~ **DONE v2.857**: `useSettingsPeopleDirectory` (parent-instantiated: people directory, group toggles, pay-approved, merge flow); merge-duplicates modal JSX moved into SettingsPeopleTab. −314 lines (2,435 → 2,121). `loadData`'s dev branch now loads only the shared `users` list + `loadServiceTypes` + dashboard data.
5. ~~Dashboard residue → three hooks~~ **DONE v2.858**: `useSettingsFinancialPins` (pin writes stay in the tab, quirk #8), `useSettingsMyReports` (realtime channel + `loadMyReportsRef` handed to the page-level modals), `useSettingsTeamLeaderAssignments` (rows + memos + own load; since renamed `useTeamLeaderAssignments` when the manager moved to People → Users, 2026-08-02). −152 lines (2,121 → 1,969). The prop membrane into `SettingsDashboardTab` still exists (props now come from the hooks; **~95 props** since the Team Hours Sharing manager moved out, 2026-08-02) — collapsing it means sub-decomposing the ~1,560-line child per section, which remains the follow-on campaign.
6. ~~Account residue → `useSettingsAccount`~~ **DONE v2.859**: profile/password/push/location/self-salaried + the dev "All salaried" picker in one parent-instantiated hook; `loadData` hydrates profile fields via `applyProfileRow` (no duplicate query); the shell header still opens the password modal via the destructured `openPasswordChange`. −266 lines (1,969 → 1,703).
