...
| 5c | What customers see (`settings-what-customers-see`, v2.2758, dev-only) | `SettingsWhatCustomersSeeTab` | extracted (born external) | **self-contained** — one `app_settings` fetch (`CUSTOMER_SAMPLE_SETTING_KEYS`), journeys from the [`customerJourneys.ts`](../src/lib/customerJourneys.ts) kernel, emails from [`customerSampleEmails.ts`](../src/lib/customerSampleEmails.ts), pages as same-origin iframes on the sample token | 0 | — | Done (PR 1; portals + contract in PR 2) |
...
- **Role gate:** tab listed for dev|master; master sees only block 3 (`TeamFeedbackMasterAggregates`).
