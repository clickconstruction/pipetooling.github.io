...
| 3 | Dashboard & alerts (`settings-dashboard`) | `SettingsDashboardTab` (1,985 lines) | extracted | **split**: parent-instantiated hooks `useSettingsFinancialPins` / `useSettingsMyReports` / `useSettingsTeamLeaderAssignments` (v2.858); child does its own writes | **~120** | high | ~~Order #5~~ **done (v2.858)**; follow-on: sub-decompose the child per section |
...
- **Approach (~~order #5~~ done v2.858):** parent residue split into `useSettingsFinancialPins` (dev cluster: state + 4 loaders), `useSettingsMyReports` (loader + realtime + edit-window; modal wiring stays in the parent, which hands it the reload fn), `useSettingsTeamLeaderAssignments` (rows + memos + picker deriveds) — all in `src/hooks/`, parent-instantiated. Remaining follow-on: sub-decompose the 1,985-line child per section (which would also collapse the ~120-prop membrane).
