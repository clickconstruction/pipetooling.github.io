...
**The plainest way — no Terminal, and it works on claude.ai, Claude Desktop and your phone.** In PipeTooling issue the pricer's key (**Settings → System → Digital twins → Fleet → Twin Pricer 1 → Issue key**, labelled with your name) and the card that shows it walks the three clicks: in Claude open **Customize → Connectors → Add custom connector**, name it after the pricer, paste the address `{{CONNECTOR_URL}}`, choose **No sign-in**, and under **Request headers** add `authorization` with the card's **Copy header value** (`Bearer` and the key, pasted once, never into a chat). Then start a **new incognito chat** and paste this kickoff. If the *Add custom connector* screen has no **Request headers** section, your account is not on that beta yet — use one of the two Terminal ways below instead; they exist for exactly that case.
