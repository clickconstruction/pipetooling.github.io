...
| 2026-08-30 | M6-v8 | twin-estimator-1 (Claude, harness = same session) | **Keyed-note census — the hexes ARE the drops.** Both sheets' KEYED NOTES tables decoded from the same margin region (raw300 (300,75,900,750)): P200 P1=2"W dn+2"V up / P2=3"W dn+2"V up / P26=2"V up / P42=3" P-trap + MIFAB MI-HAIR hair trap (closes the P1/P2 RFI); P201 adds **W32=3/4"CW dn — a key the v0.5 substrate MISSED because its table crop was half-height**. Every hex counted by 4-tile census as a sized-drop counter on a new **Drops** layer: P1 x15, P2 x8, P26 x28, P42 x3, W1 x12, W2 x2, W4 x2, W23 x4, W32 x2, G1 x1 = **77 sized drops**; project now 173 counters + 59 runs on 7 layers. Gas reconciled by name: 23.8 ft on-plan + G1 roof riser + offsite continuation symbol = printed 140', WH-1 load 199 CFH; scope-boundary notes dropped at the break symbols ("4" waste continued at tenant", "gas continues offsite"). Census cross-check pairs waste and water keys at the same fixtures across sheets, and surfaced a possible second L-6 (staff row) for recount. Schedule report honestly ✗ on the 10 keyed counters — keyed notes are not scheduled fixtures; explained in the import note. | ~1 harness turn (2 table crops + 8 census tiles + 1 gas crop) | Doctrine (new PLACEMENT.md chapters): sub-resolution stubs are counted as keyed sized-drop counters, never traced; decode the FULL keyed table before counting (half-height crops silently lose rows); printed totals reconcile as on-plan + NAMED allowances, each an on-sheet note. |
| 2026-08-30 | M5 | twin-estimator-1 (Claude, harness = same session) | **FIRST RUN of the pricing middle — PASS-partial (drafted, nothing sent), and it did its job: it found the fence gap.** Operator-authorized from a `ready` (not yet reviewed) takeoff. Stage 1: 36 count rows imported via Copy to /Tooling paste (173 ea, 900.9 ft, 2 plan pages) — mirror-exact vs CT. Stage 2: takeoff book matched **0/36** (2 entries, no schedule-tag aliases); 9 catalog assemblies attached manually; no floor-drain/laundry-tub/wall-box/hose-reel assemblies exist. Stage 3: **BLOCKED — the twin write fence denies `cost_estimate_labor_rows` inserts** (no `bid_id` column → the bid-child detection misses it) and the Labor tab swallows the 403s into an empty table; question parked (5170bb88), kept moving per mission rules. Stage 4: price book matched 0 by name too; 18/36 rows assigned book entries by hand; Workbench **REVENUE $79,697 · PROFIT $79,337 · MARGIN 100% · MULTIPLE 221.4×** (cost = $360 estimator time; labor blocked). Keyed drops + fittings left unpriced deliberately (they ride per-fixture prices). Stage 5: letter drafted unsent — $79,697.12 headline, all 36 inclusions, 12 assumption/exclusion lines carrying all 3 bid RFIs + open takeoff RFIs + scope flags. | ~1 harness turn (browser UI throughout; 5 ledger stamps) | Stumbles → pipeline work: (1) fence applier must admit cost_estimate child tables (and the Labor tab must surface denied writes); (2) takeoff/price books need schedule-tag aliases or a tag-normalizer — 0/36 auto-matches twice; (3) per-bid letter text lives only in page state until sent — not resumable for an agent; (4) an unscoped assign helper mis-priced a dozen rows before a per-row audit caught it — UI-driving agents need row-scoped actions + verify-after-write as doctrine; (5) no canonical labor rate exists in-app. |
| 2026-08-30 | M5b | twin-estimator-1 (Claude, harness = same session) | **Stage-3 redo — labor COMPLETE via the robot labor book (v2.2513), first robot-book learning loop closed.** The twin seeded its 🤖 Robot Default labor book with 6 entries, every hour mirrored from a named in-app source (Default labor book: Toilet 1/1/1, Sink 1/1/1, Hose bib 1/0.5/1, Water Fountain 2/2/3, Gas drop 3/0/1; fixture_labor_defaults: Lavatory 0.5/0.5/0.5) and aliased to the LIVSTE schedule tags; minted the missing `Lavatory` fixture type (additive allowance). The app then built the hours table itself: 36 rows, 12 houred. App totals: **17.50 RI + 13.00 TO + 16.50 TS = 47.00 hrs × $285.00/hr (owner-set rate) = $13,395.00 labor**; Workbench moved from the bookless 100%-margin draft to **REVENUE $79,697 · PROFIT $65,942 · MARGIN 83% · MULTIPLE 5.8×** (12/12 coverage). Un-houred honestly: FD-2, WB-1/WB-2, keyed drops, fittings, per-foot rows — no grounded mirror exists yet; flagged for the human book. | ~half a harness turn | Stumbles → doctrine: (1) the labor-row sync derives hours only at row CREATION — switching books later is a no-op; the redo protocol is delete rows → ONE sync with the right book already selected (overlapping syncs race and split the hours); (2) robot-book grounding discipline held — every hour names its source, and rows with no defensible mirror stayed at zero rather than being invented. |
