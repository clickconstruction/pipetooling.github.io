...
- **Handlers/loaders:** `load` (useCallback: `estimates` SELECT `*, customers(name, address, contact_info), jobs_ledger(hcp_number)`, optional `.eq('customer_id', …)` from the `?customer=` URL param, order `updated_at` desc, **limit 200** — deliberately **not** joined to `estimate_customer_events`; that is the separate chunked effect above), `createDraft(projectId?, docKind, trigger)` (resolves master via `resolveMasterUserId`, INSERTs a draft with `line_items_snapshot: [defaultDraftFirstLine()]`, records `estimate_draft_created` telemetry, navigates to `/estimates/:number` with `state: { freshEstimateDraft: true }` — v2.2885: the detail editor deletes that row again on leave if nothing was ever committed to it, see quirk 22), `toggleEstimateThreadExpanded`. Effects: load-on-mount/param-change; 320ms-debounced `refreshEstimateThreadStatsForEstimateIds` over visible row ids (gated `listTab === 'followup'`); collapse expanded thread when leaving Pipeline.
...
- **Step rail line count (v2.2907, journey-map B18 / J17-F5):** `railData` feeds `computeEstimateDraftSteps` with `countMeaningfulEstimateLines(lines)` from [`estimatePipelineRefresh.ts`](../src/lib/estimatePipelineRefresh.ts) — the same rule the list's `estimateDraftMeaningfulLineCount` applies to the JSON snapshot — so the seeded `$0` "Custom Service Visit" stub never ticks **Line items** on the rail while the Pipeline chip still reads "1 left: cost lines".
...
- **Estimate options (v2.2457 → v2.3556):** the option-card strip above Line items (`estimateOptions` / `viewedOptionKey` state, `syncedEstimateOptions` memo; handlers `addEstimateOption`, `switchViewedOption`, `removeViewedOption`, `patchViewedOption`, `setViewedOptionKind`), the name + pitch fields, the **Offered as** switch (choice · add-on — `setEstimateOptionKind` re-seats the star), ★ Recommended (disabled on an add-on while a choice exists) and Remove option. Save mirrors the ★ option into the legacy fields via `estimateOptionsDraftPersistFields`. The Page preview's selection is a set (`previewSelectedOptionKeys`, toggled with `toggleEstimateOptionSelection`). The accepted view's record reads `describeAcceptedEstimateRecord` over `acceptedEstimateOptionKeys(row)` (the key list first, the single key for older acceptances). Kernels: [`lib/estimates/estimateOptions.ts`](../src/lib/estimates/estimateOptions.ts), [`estimateAcceptSelection.ts`](../src/lib/estimates/estimateAcceptSelection.ts), [`estimateAcceptedRecord.ts`](../src/lib/estimates/estimateAcceptedRecord.ts).
...
22. **Fresh drafts self-delete on leave (v2.2885, decision 17).** The editor is row-backed (27 `row.id` effects, `saveDraft` UPDATEs, autosave diffs the hydrated row), so New estimate / New change order / the Projects `?newEstimate` deep link still INSERT — but `createDraft` navigates with `state.freshEstimateDraft`, `load()` stamps `freshDraftRowIdRef` for the draft it hydrates under that marker, every write (`saveDraft`, `deleteDraft`) stamps `committedDraftRowIdRef`, and a leave hook **keyed on `routeSegment`** (its cleanup runs at a route change while state is still the OLD row's, and on unmount) calls `shouldDiscardFreshEstimateDraftOnLeave` (`src/lib/estimateFreshDraftDiscard.ts` = fresh ∧ never saved ∧ `isEmptyEstimateDraft(live form)`) and, when true, cancels the pending autosave and DELETEs the row. Since v2.2911 (J16-N1) the emptiness rule also counts a CO's `response_requested_by`, `for_address`, and `internal_notes` as content — the snapshot passes `forAddress` / `internalNotes` — so a draft carrying only a due date, an address, or bridged notes is neither discarded on leave nor offered by "Clean up empty drafts". Do not key it on `row.id` — by then the state is the new row's. Hard reload / tab close cannot run it; `cleanUpEmptyDrafts` remains for those.
