...
- **Render location:** `{!isDraft && (...)}` block — locked notify-recipients card, then per-status: `customer_accepted` renders `EstimateCustomerDocument` + acceptance record (name / at / IP / signature) + Customer activity + Job section; `sent` renders Customer activity (default open) + waiting note + `EstimateCustomerAcceptLinkButtons`. Above it, a small read-only header block (`For:` line, acceptance-page logo, `EstimateLineItemsTable`) renders for `sent`/`declined`/`superseded` (gate `!isDraft && row.status !== 'customer_accepted'`).
...
- **Handlers:** `openCreateJobModal`, `openUnlinkJobConfirm`/`closeUnlinkJobConfirm`/`confirmUnlinkLinkedJob` (UPDATE `estimates.job_ledger_id = null` **gated `.eq('status','customer_accepted')`**), `copyCustomerAcceptUrl`, `openCustomerAcceptUrl`.
...
22. **Fresh drafts self-delete on leave (v2.2876, decision 17).** The editor is row-backed (27 `row.id` effects, `saveDraft` UPDATEs, autosave diffs the hydrated row), so New estimate / New change order / the Projects `?newEstimate` deep link still INSERT — but `createDraft` navigates with `state.freshEstimateDraft`, `load()` stamps `freshDraftRowIdRef` for the draft it hydrates under that marker, every write (`saveDraft`, `deleteDraft`) stamps `committedDraftRowIdRef`, and a leave hook **keyed on `routeSegment`** (its cleanup runs at a route change while state is still the OLD row's, and on unmount) calls `shouldDiscardFreshEstimateDraftOnLeave` (`src/lib/estimateFreshDraftDiscard.ts` = fresh ∧ never saved ∧ `isEmptyEstimateDraft(live form)`) and, when true, cancels the pending autosave and DELETEs the row. Do not key it on `row.id` — by then the state is the new row's. Hard reload / tab close cannot run it; `cleanUpEmptyDrafts` remains for those.
