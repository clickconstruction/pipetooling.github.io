...
- **Effects (all keyed on `open` + reset-on-close):**
  ...
  2. **Ensure-RTB** (kind:'job' only): RPC `ensure_single_ready_to_bill_invoice_for_job(p_job_id)` via `withSupabaseRetry` → `ensuredInvoice` + `billAmountStr`, then `onAfterEnsureSuccessRef.current?.()` (opener refreshes its row). Guarded by `ensuredInvoice?.jobId === job.id`. **Cancel-leak guard (v2.2464):** `mintedPrimaryDraftRef` records the RPC's `created: true`; on close/unmount/job-swap the modal deletes the row it minted via `delete_ready_to_bill_invoice` IF `isMintedPrimaryDraftStillUntouched` (`src/lib/jobs/mintedPrimaryDraftCleanup.ts`) passes on a re-fetched row + zero payments; `openJobIdRef` blocks a late cleanup during a fast reopen of the same job. Moving the engine into `useBillCustomerBillingTarget` must carry the guard (the ref pair + cleanup callback + its three triggers) along with it.
