...
| `applyScheduleProportionsToClockSession.ts` | colocated | 6 | 6 |
...
2. **`fenceOverridden` forces the `leader_*` RPCs even for self-edits** (both in `persistDirtyChangesAsync` and `resolveAssignSessionForSegment`; **not** in `applyScheduleProportionsToCluster`, whose kernel picks own/leader by `editingSelf` alone — preserve as-is, flag separately) — the `own_*` RPCs stay week-fenced server-side; pay-access users pass `can_edit_clock_sessions_for_user` for themselves.
