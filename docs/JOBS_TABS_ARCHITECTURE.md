...
[`src/pages/Jobs.tsx`](../src/pages/Jobs.tsx) is a ~9,852-line "God component" (as of v2.821: 162 `useState` declarations, 56 `useEffect`, 24 `useMemo`, 26 `useCallback`, ~19 refs). It already shrank from ~15k through ad-hoc extractions **without** a Step-0 map, and the mapped extraction is now underway (v2.820 print builders, v2.821 Billing tab) — seven tabs are thin(ish) wrappers today, but the remaining inline mass is concentrated in one place: the **Stages** tab (~3,560 lines of inline JSX) plus the **Sub Labor CRUD engine + its inline modal** (~1,900 lines combined). This map follows [`PAGE_DECOMPOSITION_PLAYBOOK.md`](./PAGE_DECOMPOSITION_PLAYBOOK.md) and the format of [`DASHBOARD_SECTIONS_ARCHITECTURE.md`](./DASHBOARD_SECTIONS_ARCHITECTURE.md) / [`BIDS_TABS_ARCHITECTURE.md`](./BIDS_TABS_ARCHITECTURE.md).
