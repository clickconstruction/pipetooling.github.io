...
[`src/pages/Jobs.tsx`](../src/pages/Jobs.tsx) is a ~7,718-line "God component" (as of v2.823: 116 `useState` declarations, 52 `useEffect`, 24 `useMemo`, 26 `useCallback`, ~21 refs). It already shrank from ~15k through ad-hoc extractions **without** a Step-0 map, and the mapped extraction is now underway (v2.820 print builders, v2.821 Billing tab, v2.822 `useSubLaborLedger`, v2.823 `JobsSubLaborFormModal`) — seven tabs are thin(ish) wrappers today, and with the Sub Labor form modal extracted the remaining inline mass is concentrated in one place: the **Stages** tab (~3,560 lines of inline JSX) plus the Sub Labor payment/settings modal remainder (step 4b). This map follows [`PAGE_DECOMPOSITION_PLAYBOOK.md`](./PAGE_DECOMPOSITION_PLAYBOOK.md) and the format of [`DASHBOARD_SECTIONS_ARCHITECTURE.md`](./DASHBOARD_SECTIONS_ARCHITECTURE.md) / [`BIDS_TABS_ARCHITECTURE.md`](./BIDS_TABS_ARCHITECTURE.md).
...
| `sub_sheet_ledger` | Sub Labor | thin wrapper `<JobsSubLaborTab/>` + always-mounted `<JobsSubLaborFormModal/>` (v2.823) | **partial** | ~15 state vars (payments modals, roster, drive settings) | med (roster + drive settings shared with other tabs; `fillLaborFromBilling*` from Billing via the modal handle) | low-med | Step 4b: fold the payment/backcharge/edit-payment + settings modals into `JobsSubLaborTab` |
...
- **Render location:** list = `<JobsSubLaborTab/>` ([447 lines](../src/components/jobs/JobsSubLaborTab.tsx), extracted); form = **always-mounted** [`JobsSubLaborFormModal`](../src/components/jobs/JobsSubLaborFormModal.tsx) (extracted v2.823 — the New/Edit Sub Labor modal + Add Subcontractor + labor-book version/entry form modals + all form state, so form state survives open/close as before; the parent drives it via an imperative handle `open`/`openNew`/`openEdit`/`openNewWithJobNumber`/`openWithBillingPrefill`). Still inline in the parent's modal tail: Make Payment / Backcharge / Edit Payment / Default Labor Rate / Drive Settings.
- **Owned local state (parent, ~15 vars):** ledger cluster (`editingLaborJob` — stays parent-side for the `?editLabor=` deep link and `useSubLaborLedger`'s `onLaborJobsReloaded` sync), payments cluster (`makePayment*`, `backcharge*`, `editingPayment`/`editPayment*`), roster (`people`, `users` partly), settings (`driveSettings*`, `defaultLaborRate*`). The labor-form, labor-book, crew-search, and Add Subcontractor clusters (42 `useState`s) now live inside `JobsSubLaborFormModal`.
...
- **Extraction status + risk + approach:** **Partial — the form modal is out.** Stage A **done (v2.820)**: the two print builders live in `lib/jobsDocuments/subLaborSheet.ts` (one parameterized builder + tests); labor-line math already in `lib/peopleLaborJobItemLineCost`/`lib/jobs/subLaborCost`. Stage B: (1) `useSubLaborLedger` hook **done (v2.822)**; (2) [`JobsSubLaborFormModal`](../src/components/jobs/JobsSubLaborFormModal.tsx) **done (v2.823)** — the modal + form state + labor-book pickers + Add Subcontractor, always mounted, imperative handle; `Person`/`UserRow` types stay exported from Jobs.tsx (parent still owns the roster state). Remaining (step 4b): `JobsSubLaborTab` absorbs the payment/backcharge/edit-payment modals (single-opener); Drive/default-rate settings modals can move with the tab (openers only there). What stays: `loadTeamLaborData` consumers' wiring; the Billing→Sub Labor prefill callback (now a thin `openWithBillingPrefill` handle call).
...
| Sub Labor modal cluster (New/Edit + Add Subcontractor + labor-book version/entry forms → [`JobsSubLaborFormModal`](../src/components/jobs/JobsSubLaborFormModal.tsx) v2.823; Make Payment, Backcharge, Edit Payment, Drive Settings, Default Labor Rate still inline) | component + inline | Sub Labor (+ Billing prefill opener, + `?editLabor=`/`?newJob=` — all via the imperative handle) | form modal **extracted** (parent keeps the prefill + deep-link glue); payment/settings modals move in step 4b |
...
4. **`sub_sheet_ledger` Stage B ×2:** ~~`JobsSubLaborFormModal` (the inline modal + form state + Add Subcontractor)~~ **done (v2.823** — always-mounted component with an imperative handle; Jobs.tsx 9,722 → 7,718 lines**)**; then (4b) fold the payment/backcharge/edit-payment + settings modals into `JobsSubLaborTab`.
