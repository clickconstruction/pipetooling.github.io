...
- **Cross-tab coupling:** the mercury allocation modal + `mercuryCardChargesByJobId` + `bankingAttributionUsersOptions` are **shared with Job Summary** (drilldown reassign flow via the hook-internal `jobSummaryMercuryEditFlowJobIdRef`); Job Summary's lazy cache/drilldown are bridged into the hook via the temporary `onJobSummaryMercuryTouched`/`onJobSummaryDrilldownClose` callbacks; `jobs` list gates which jobs appear; `?editParts=` deep link.
...
- **Extraction status + risk + approach:** **Partial — seam done (v2.825).** [`useJobsMercuryAllocations`](../src/hooks/useJobsMercuryAllocations.ts) owns the per-job allocation cache, card totals, the alloc-modal open/save routing, and the two flow refs — consumed by both Parts and Job Summary (the page's closest analog to Dashboard's shared-engine rule). Job Summary's lazy mercury cache stays parent-side, bridged via `onJobSummaryMercuryTouched`/`onJobSummaryDrilldownClose` until the step-6 `useJobSummaryData` seam. Still open: the three mercury modals + remaining parts state move into `JobsPartsTab`. Risk med: the save-refresh routing (`onPartsAllocSaved`) touches both tabs' caches.
...
- **Extraction status + risk + approach:** **Partial.** Stage A: `printJobSummaryCostBreakdown` → `lib/jobsDocuments/jobSummaryCostBreakdown.ts` **done (v2.820** — the page thunk still resolves the invoice/mercury/clock caches and fetch fallbacks, then calls the pure builder**)**; `jobSummaryData` math → `lib/jobSummaryPnl.ts` + tests still open. Stage B: `useJobSummaryData` hook (ledger loader + lazy caches + the memo) — when it lands it should also absorb the temporary `onJobSummaryMercuryTouched`/`onJobSummaryDrilldownClose` bridge callbacks the page currently passes to `useJobsMercuryAllocations` (v2.825). The drilldown-modal ReactNode state (`jobSummaryCostDrilldown`) is an anti-pattern to preserve as-is during the move (quirk #11).
...
| [`useJobsMercuryAllocations`](../src/hooks/useJobsMercuryAllocations.ts) (v2.825, 379 lines) | mercury card totals + parts-tab allocation cache + unattributed/alloc-modal states + save-refresh routing (flow refs) + banking users options; Job Summary lazy cache bridged in via `onJobSummaryMercuryTouched`/`onJobSummaryDrilldownClose` | Parts, Job Summary drilldowns |
