...
**Named verbs (v2.3646 / 0.2.0)**: `find_job` / `find_bid` (the app's `search_jobs_ledger` / `search_bids_for_clock`, labelled with the service type's ledger prefixes), `find_customer`, `find_person` (active, human, not a sample); `get_job`, `get_customer`, `get_bid` — composites in `_shared/devMcpComposites.ts` over an injected `Reader` whose two methods apply the same catalog, denied-table, query and redaction rules as the generic door. Money is never restated: `get_job` feeds the Job window's rows to `jobMaterialsCostLines` + `jobSubLaborInputs` + `buildJobProfitSummary` and `profileJobRowMoney`; `get_customer` to `customerMoneyStats` / `customerDaysToPay` / `customerEstimateOutcomes`; `get_bid.totals` says in words that the priced total lives in a client hook. A part that fails is `{ error }` in place. **`view_as(role | user, verb, args?)`** runs any read verb under another person's session — a role's sample account (`users.is_sample`) or a user id; a `dev` target is refused (Imitate's rule), an archived one too; the call logs `as_user_id` and the reply is headed `// read as …`. `callTool` is now `resolveDev` → `runVerb(identity, lazy session, verb, args)`, so the catalog verbs mint no session.

**Health verbs (v2.3649 / 0.3.0; batches v2.3654 / 0.3.1)**: `check_sampler(hours?)`, `check_connections`, `check_locks`, `check_migration_ledger(n?)` — each one GET of a dev-gated `SECURITY DEFINER` RPC (`dev_health_sampler_gaps`, `dev_health_connections`, `dev_health_locks`, `dev_migration_ledger_tail`; migration `20260920232141`), because `monitoring.*`, `pg_stat_activity` and `supabase_migrations.schema_migrations` are outside the API's schema. They read as the dev like every other verb — no service role — and `view_as` a non-dev is refused by the RPC's own `is_dev()` gate. The verb names are fixed in `_shared/devMcpHealth.ts` (`HEALTH_RPCS`), so they answer before a `gen-types` run lists the RPCs in the catalog; the same kernel writes the one-line `reading` that opens each reply, on the thresholds of `DB_FREEZE_RUNBOOK.md`. `check_edge_boot` OPTIONS-probes every function in `EDGE_FUNCTIONS` (generated into `catalog.ts` from `supabase/functions/*/index.ts` — adding, renaming or removing a function now means re-running `node scripts/build-dev-mcp-catalog.mjs`), 16 at a time, 5 s each, no key sent, and names any `503 BOOT_ERROR` — **one batch of 50 per call** since v2.3654 / 0.3.1 (`edgeBootBatch`; the reply's `next_after` is the cursor for `after`): the platform refuses a function's calls to other functions past about 60 a minute, and a refused probe is reported as `rate_limited`, never as a dead function.

**Write verbs (v2.3722 / 0.4.0)**: `plan_cost_batch(batch)` / `apply_cost_batch(batch, plan_hash)` / `revert_cost_batch(batch_id, reason)` and `plan_hr_entry(entry)` / `apply_hr_entry(entry, plan_hash)` — a **second path** (`restPost`, reached only from these verbs) that POSTs one of three RPC names fixed in `_shared/devMcpWrites.ts` (`WRITE_VERBS`): `cost_batch_apply`, `cost_batch_revert` (`20260909161532`) and `dev_hr_entry_write` (`20260922102017`, the dry-run wrapper over the live `hr_agent_write` that names the dev as author). Each is a dev-gated `SECURITY DEFINER` function, called as the dev, so the database refuses a non-dev and training mode blocks the write inside. `plan_` POSTs with `p_dry_run = true` and replies with the RPC's dry-run summary under a `plan_hash` — sha256 over the canonical payload plus what the plan committed to (`planCommitment`: for a cost batch the op count, the money by job and each op's seq, kind and **before** image, never the after images, which carry the dry run's own ids and timestamps); `apply_` POSTs the dry run **again**, hashes it the same way, and writes only when it equals the quoted hash — otherwise *Nothing written* with both prefixes and the plan verb to call. No clock, no window: the hash is the payload and the world. **One apply per plan (v2.3730)**: a payload with no before-state (a thread note, an HR entry) hashes the same after it lands, so before the real POST the function asks `dev_mcp_calls` for an ok `apply_*` with that `plan_hash` (`args->>plan_hash`, any dev's key) and refuses with the prior batch id and time — a cost batch since reverted does not count. `revert_` needs a uuid and a non-empty reason. `view_as` refuses every write verb before resolving its target. Logged like every call, `target` = the batch label / id or the person id.

**Endpoint**: `POST https://mcp.clicktooling.com/dev` (the `mcp-router` Worker, which refuses a twin key, `ptt_…`, with a 401 before the fetch) → `POST /functions/v1/dev-mcp` · **Auth**: in-function, the dev key on every `tools/call` (`initialize` / `tools/list` are open metadata). `verify_jwt = false` in `config.toml` — preserve on redeploys.

**Required secrets**: `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY` (all platform-provided). **Tables**: `dev_mcp_credentials`, `dev_mcp_calls` (migration `20260920063130`), `users`; the health RPCs read `monitoring.health_checks`, `monitoring.connection_samples`, `pg_stat_activity` and the migration ledger.

---

### twin-login

**Purpose**: Sign-in mint for **digital twin** accounts (docs/DIGITAL_TWINS_PLAN.md Phase E1) so a cloud-hosted agent harness can establish a session on the **deployed** app: POST → magic-link `action_link` → the harness navigates a headless browser to it. dev-login's sibling with four hard guards — a leaked credential can only ever produce a session as a flagged, estimator-role twin, never a real person:

1. One of two credentials: `X-Twin-Login-Secret` matching `TWIN_LOGIN_SECRET` (owner/ops — its **own** secret, rotatable independently; may mint any twin, and rotating it is the fleet-wide kill switch), or a per-twin `X-Twin-Token` sha256-matched against `twin_credentials` with `revoked_at` null (partners and twin-mcp — the token IS the identity, it mints only its own twin, and revoking the row cuts off one holder).
...
**Endpoint**: `POST /functions/v1/twin-login` · **Auth**: one of the two headers above (`verify_jwt = false`). **Request**: `{ email?, redirectTo?, run? }` — `email` is optional with a token (a mismatch is 403); `run` is a mission label logged with the mint. **Response**: `{ success, action_link }`. Rate limit: 6 mints per twin per minute, counted in `twin_runs` (429 + `retry_after_seconds`). Every mint logs to the function log and inserts a `twin_runs` row.
...
- *Work-state reads*: `get_assignments`, `get_plan_brief`, `get_work_state`, `get_plan_pages`, `get_answers`, `get_robot_book`, `get_reference_rows`.
- *Fenced pipeline writes*: `file_plans` (pass-through to [drive-intake](#drive-intake), the token re-authenticates there), `stage_plan_pdf`, `put_substrate`, `ct_finish_takeoff`, `tt_finish_costing`, `paste_counts`, `extend_robot_book`, `add_bid_note`, `seed_audit_questions`, `ask_question`, `heartbeat`.
- *Confidence runs*: `open_backtest`, `next_backtest`, `score_backtest`; `get_shadow_queue`, `next_shadow`, `open_shadow`, `lock_shadow`, `score_shadows`, `void_shadow`.
- *Pricing twin* (a `twin_kind = 'pricer'` key only): `get_pricing_guide`, `get_component_rules`, `get_component_corrections`, `next_price_matrix`, `get_quote_documents`, `put_quote`, `finish_price_matrix`, `extend_component_rules`.
- *Submittal robot*: `get_submittal_guide`, `next_submittal_task`, `put_submittal_result`, `finish_submittal_task`.
...
**Endpoint**: `POST https://mcp.clicktooling.com/twin` — the address every copy button and `.mcp.json` hand out since v2.3634, served by the `mcp-router` Worker (`scripts/cloudflare/mcp-router.worker.js` + its `.wrangler.toml`, a secret-less pass-through, live 2026-09-20; since v2.3648 it refuses a dev key, `ptd_…`, with a 401 before the fetch) in front of `POST /functions/v1/twin-mcp`, which keeps answering for machines set up earlier · **Auth**: per-twin token on every `tools/call` (`X-Twin-Token` or `Authorization: Bearer`; `ptt_` + 64 hex since v2.3648, bare hex before — lookup hashes the whole presented string, so both answer; `initialize`/`tools/list` are open metadata). `verify_jwt = false`.
...
**Discipline fence + void door (v2.3032 / v1.3.10)**: twin-estimator-1 is a PLUMBING estimator, and on 2026-09-07 `next_shadow` claimed b378 — an Electrical-division bid — so the robot locked a $907k plumbing number against an electrical reference and mirrored 34 plumbing entries into the empty electrical robot book. Now every claim door checks the reference's `service_type_id` against the Plumbing service type (`plumbingServiceTypeId`, looked up by name and cached): `get_shadow_queue` (both queries + the coverage block), `next_shadow`, `open_shadow`, and `openBacktestShell` (so `open_backtest` and `next_backtest`) all refuse or skip non-plumbing references with a plain-words error. **`void_shadow(bid, reason)`** takes a shadow out of the scoring loop — own shells only, a scored run stays scored — sets `twin_shadow_runs.status = 'void'` (which `score_shadows` never selects) and stamps `[shadow VOID] <reason>`.

**Teacher attribution + readable-plans dispatch (v2.3080 / v1.3.11)**: `score_shadows` now records WHOSE number each run scored against (`resolveTeacher`: the reference's `estimator_id`, else `bid_date_sent_attested_by`, else `created_by`) into `twin_shadow_runs.teacher_user_id / teacher_name`, names the teacher in the `[shadow SCORECARD]` line with their standing ("calibration standard" vs "practice teacher — not a gate run"), and its per-axis scoreboard takes only `users.calibration_standard` teachers for `gate_b_met` / `last5_in_8pct` (`standard_runs` and `practice_runs` reported side by side). `get_shadow_queue` and `next_shadow` first call `plan-fetch ?probe=all&limit=15` with the caller's own token (`probePlansSweep`: best-effort, 12 s cap) and then exclude bids whose `plans_robot_readable = false` — the queue lists them under `unreadable` with the probe's reason and reports `coverage.unreadable_by_robots`; the dispatcher reports `skipped_unreadable` when it runs dry. A bid the service account cannot read is never claimed.
...
**Teacher on backtests (v2.3099 / v1.3.12)**: `score_backtest` resolves the reference's teacher with the same `resolveTeacher` and stamps `twin_run_scores.teacher_user_id / teacher_name` (migration `20260907180000`, backfilled), naming the teacher and standing in the `[STG-6 SCORECARD]` note. Standing is not stored — the Scoreboard decides practice vs standard at read time against `users.calibration_standard`.

**Shadow opt-out (v2.3142 / v1.3.13)**: `bids.robot_opt_out` ("Don't let robots shadow this bid" on the bid form) is honoured at every door — `get_shadow_queue` (both queue queries and the coverage count), `next_shadow` (both claim queries) filter it out, and `open_shadow` refuses an opted-out reference in plain words.

**Question audiences (v2.3186 / v1.3.14)**: `ask_question` gains `audience` — `'estimator'` (a judgment about the job; shows on Bids → Audits → Standing rulings) or `'operator'` (the machine is in the robot's way — sandbox, sign-in, the write fence, a table it can't write, a file the service account can't read; shows on Bids → 🤖 Robots → Console since v2.3224, Settings → Digital twins before). Omitted, the text decides through the shared kernel `_shared/twinQuestionAudience.ts` (machine vocabulary — sandbox, fence, service account, a snake_case table or tool name, `JSON`, `403`, a leading "ANSWER PARKED" — routes to the operator), and the reply names the lane and the signals. Written to `twin_questions.audience` (migration `20260909045818`, which also backfilled the open rows); the insert retries without the column if the function is deployed ahead of the push. The tool description carries the estimator-lane writing rules (one decision, two sentences, project + sheet, no run codes / table or tool names, a `topic` for doctrine) — the same rules as `docs/twins/estimator.md` §9.

**Tap-answerable questions (v2.3210 / v1.3.15)**: `ask_question` gains `choices` (2–4 short labels) and `recommended` (the robot's pick, one of them), written to `twin_questions.choices / recommended` (migration `20260909233000`; the insert retries without them when the columns are absent). An **estimator-audience** question is now REFUSED by `checkEstimatorQuestionShape` (`_shared/twinQuestionShape.ts`, shared with the client) unless it is one decision under 320 characters, one question mark, no "(1) … (2)" enumeration, with 2–4 distinct choices ≤40 chars and a `recommended` that is among them; the refusal lists each problem plus one fixed hint (split per decision; detail goes to `add_bid_note`). The operator lane is unconstrained. `get_answers` returns both columns; the Standing rulings card and the operator console render the choices as buttons, the robot's pick first, and save the tapped label as `answer`.

**Plans asks routed to the bid (v2.3212 / v1.3.16)**: `ask_question` gains `kind` — `'decision'` (a ruling) or `'plans'` (the robot needs a different or additional plan set on this bid), written to `twin_questions.kind` (migration `20260910003115`; insert retries without it). Omitted, the shared classifier `_shared/twinQuestionKind.ts` reads the text ("attach the plumbing sheets", "the file on the bid is the electrical set", "no plumbing sheets", "can't open the plans"). A plans ask with no `choices` gets the standard three taps (`Attached — rerun` / `Use what is on the bid` / `Skip this bid`, recommended first) so it still passes the shape gate. The success line names the lane and warns when a plans ask carries no `bid`. `get_answers` returns `kind`. Client side: the Standing rulings kernel keeps plans asks out of the rulings and the count and exposes them as `plansAsks`; the panel shows one pointer line linking each HUMAN bid (`/bids?tab=bid-board&bidId=…&robot=needs`), the Bids page keys a shell's plans ask to its source bid so the amber icon lands on the estimator's row, and the robot needs sheet renders it with the blocking gaps — Edit bid + Copy intake address beside it, its taps underneath.

**Dispatcher resumes answered plans asks (v2.3229 / v1.3.17)**: `next_shadow` first reads this twin's `twin_questions` rows with `status = 'answered'` and `acted_at IS NULL` (migration `20260910123000`). A **plans** ask (column, else text) whose answer says rerun (`answerRequestsRerun`) is resolved to the twin's shell — the ask's `about_bid_id` when that is the shell, else the twin's shell for that human bid — and, if the shell's `twin_shadow_runs` row is still `open`, handed back as the run: `{ resumed: true, shadow_bid, reference, plans_link, asked, answered, next }`, with a `[pipeline STG-0 resume]` ledger note on the shell. Every answered ask it looks at is stamped `acted_at` (a non-rerun answer, a locked/scored/void shell, or no shell = consumed, nothing to resume), so a shell is handed back once per answer. Best-effort: any error (including the column not yet applied) falls through to the normal claim. This is what makes the needs sheet's *Attached — rerun* tap (v2.3223) sufficient on its own.

**Plan pages for a shell-less harness (v2.3230 / v1.3.18)**: `get_plan_pages(bid, pages?, embed?)` — for Claude Desktop, which cannot call plan-fetch itself. Fetches the bid's own set through plan-fetch with the caller's twin token (folders merged; an `X-Plan-Parts` folder has its remaining parts joined with pdf-lib, up to 8 files / 60 MB), splits the requested pages (`'1-4,9'`, default `1-6`, at most 8 per call) into single-page PDFs uploaded to `twin-plans-tmp/plan-pages/<bid>/<setKey>/pNNN.pdf` (upsert; the bucket is public, as `stage_plan_pdf` relies on), and returns `{ page_count, pages: [{ page, url, bytes }] }`; `embed: true` also attaches up to 3 of them (≤ 3 MB) as `application/pdf` resource content blocks in the same reply. Own/assigned fence (or the bid of a held `read_schedule` submittal task — see Submittal robot below); a 403/404 from plan-fetch is explained as the intake-share problem. Stamps `[pipeline STG-2] get_plan_pages …` on the bid ledger. Whether a given client renders the embedded resources or must open the URLs is the client's business — both are returned.

**The pricing twin (v2.3270 / v1.4.0 — Price Matrix PR 3, `docs/PRICE_MATRIX_PLAN.md`)**: `resolveTwin` reads `users.twin_kind` (`estimator` | `pricer`, migration `20260911023538`; the legacy shape reads as estimator) and `callTool` gates on it before the switch — a **pricer key is refused every bid verb** (`next_shadow`, `open_backtest`, `paste_counts`, `lock_shadow`, …) and a bid robot is refused the pricer's; the shared verbs (`heartbeat`, `add_bid_note` — which now also admits a pricer on a bid whose request it holds — `ask_question`, `get_answers`, `submit_report`, the doc verbs) and the submittal robot's four serve both. Seven verbs: **`get_pricing_guide`** (docs/twins/pricer.md, bundled as `PRICING_GUIDE`); **`get_component_rules`** (active `fixture_component_rules`); **`next_price_matrix`** — the dispatcher: a request this twin already holds `working` comes back first (`resumed: true`), else the OLDEST `queued` `bid_price_matrix_requests` row is claimed by a conditional `UPDATE … WHERE status = 'queued'` (the update IS the claim — two pricers never share a request), returning the bid, the fixture-row snapshot (the only rows it prices) and the 1-based sources; stamps `[pricer STG-0]`; **`get_quote_documents(request, source?, pages?, embed?)`** — with no `source` lists every source's files through plan-fetch's `?request=&source=&list=1` lane; with one, pulls the link through plan-fetch (folders merged, parts joined, 60 MB cap), splits the named pages with pdf-lib into `twin-plans-tmp/quote-pages/<request>/s<n>/…` and returns URLs (+ up to 3 embedded); **`put_quote(request, house, lines, …)`** — validates every line (fixture must be a snapshot row name verbatim unless `component_role: 'loose'`; roles from `COMPONENT_ROLES`; at most one `option_chosen` per group), writes `bid_quotes` (`source = 'robot'`, `robot_request_id`, `source_doc_url`) + `bid_quote_lines` (`price_basis 'each'`, `match_confidence 'manual'`, the PR-1 columns), upserts the price memory for kit subtotals / plain lines / chosen options, refuses a second quote for the same (request, house) unless `replace: true`, stamps `[pricer STG-2]`; **`finish_price_matrix(request, picks, asks, summary, blocked?)`** — clears earlier robot picks, sets `picked / pick_reason / pick_source = 'robot'` on the winning house's lines per row, files each ask as an estimator-lane `twin_questions` row (shape-checked, `kind 'decision'`, `topic price-matrix-<row>`), computes `result` (`houses_read`, `rows_priced`, `rows_asked`, `total_cents_at_counts` = Σ picked $/each × snapshot count, `expired_houses`), flips the request `ready` (or `blocked` with the summary), stamps `[pricer STG-3]`; **`extend_component_rules(rules, mirror_note)`** — robot-authored rules with provenance. Fence: `loadPricerRequest` requires `claimed_by = caller` and status `working` (or `ready` for a re-finish). The pricer never writes counts, costs, or anything a vendor sees. Kickoff: `docs/twins/kickoffs/pricing-operator.md` (Console → Run the robots → Pricing robot); the seat is minted from Settings → Digital twins → **Mint pricing twin** (`twin-pricer-<n>@twins.pipetooling.local`, role estimator, `twin_kind = 'pricer'`, no CT/TT seats).
...
- `{ action: 'redeem', code, machine? }` — **no session; the code is the auth** (a Terminal command on a fresh Mac calls it). Normalizes the code (dashes dropped, upper-cased, O→0, I/L→1), claims the row with a conditional `UPDATE … WHERE code_hash = ? AND redeemed_at IS NULL AND expires_at > now()` (two Macs racing get exactly one winner), re-checks the twin is still eligible, mints a `twin_credentials` token server-side (`ptt_` + 64 hex since v2.3648, `formatTwinMcpKey` in `_shared/mcpKeyPrefixes.ts` — the shape the Digital twins card mints; label = the code's label, `created_by` = the minter), stamps `credential_id` on the code, logs a `twin_runs` row (`mission = 'setup'`), and returns `{ token, twin_email, twin_kind, label, connector_url, kickoff, check_call }` — the kickoff is the matching Desktop kickoff doc with the connector URL filled in (from the generated `_shared/twinKickoffs.ts`), so the command can put it on the clipboard. A miss (unknown, used, expired, malformed) waits 400 ms and returns 404 with one generic sentence.

**Endpoint**: `POST /functions/v1/twin-setup` · **Auth**: in-function (mint: staff JWT via `auth.getUser`; redeem: the code). `verify_jwt = false` in `config.toml` — preserve on redeploys. **Env**: `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`; `TWIN_MCP_PUBLIC_URL` (set to `https://mcp.clicktooling.com/twin` since v2.3634) when the connector is reached at another address — a bare origin gets `/functions/v1/twin-mcp` appended, an address with a path (`https://mcp.clicktooling.com/twin`) is taken verbatim (`_shared/twinConnectorUrl.ts`, shared with the client's copy buttons; v2.3633).
...
**Shape**: one name-contains query across everything the service account can see (paged, ≤ 20 000 files), the child folders of each root as the job-folder map — the roots are `DRIVE_JOBS_FOLDER_ID` plus **`DRIVE_CONTRACT_ROOTS`** (comma-separated folder ids the owner shared with the SA as Viewer; set 2026-09-14 to the folder the signed contracts live in) — a file sitting directly in a root matched by its own name, and one parent lookup per unknown parent (≤ 400) so a file one level down (a "Contracts" subfolder) is attributed to its job folder. Deeper files come back unattributed and are counted, not listed.

**Endpoint**: `POST /functions/v1/drive-contract-scan` · **Auth**: staff JWT validated in-body — office roles only (dev / master / assistant / controller); `verify_jwt = false`. **Body**: `{}` or `{ force: true }` → `{ ok, files: [{ id, name, mimeType, modifiedTime, webViewLink, size, folderId, folderName }], job_folders, scanned, unattributed, folder_lookups, took_ms, cached, scanned_at }`. **One scan for the office (v2.3709)**: the answer is kept in `drive_contract_scans` (one row, service role only — `docs/migrations/20260922040000_drive_contract_scans.md`) and served from there with `cached: true` while it is younger than an hour; `force: true` reads Drive again now. The folder walk attributes eight files at a time and never looks a folder up twice. Drive secrets unset → **503** with a setup pointer (the same secrets as drive-intake; the SA must be shared into the Jobs folder — `docs/DRIVE_INTAKE_SETUP.md`).
...
**Doors**: the Contract sweep calls it **by itself when it opens** (office set; v2.3669) — one scan shared between callers and kept 15 minutes in the tab (`src/lib/jobs/driveContractScanCache.ts`) on top of the hour the function keeps for everyone (v2.3709; a fresh read took 66 s for 123 files and 273 job folders on 2026-09-21, before the walk ran eight wide), its finds marking rows, filling the *In Drive* tab and pre-filling the filing sheet for **confident** matches only (`contractSweepDrive.ts`). For filing a whole batch: Jobs → Pipeline → Contract sweep → ⋯ → **Look in Drive for signed contracts…** (the office set since v2.3587) → the *Found in Drive* review list ([`DriveContractsFoundModal`](../src/components/jobs/DriveContractsFoundModal.tsx)), which shows the scan the sweep already holds — *Drive read 12m ago · read it again* — instead of running its own.
...
**Endpoint**: `GET /functions/v1/plan-fetch?bid=b403` (or `POST {"bid":"b403"}`) · **Auth**: `X-Twin-Token` (per-twin credential; the bid must be the twin's own/assigned — assignment-is-the-grant — or the bid of a working `read_schedule` submittal task the twin holds, `twinMayReadPlans` in `_shared/twinSeatGate.ts`) **or** staff JWT (dev/master/assistant/controller/estimator). `verify_jwt = false` (in-function auth, drive-intake's model). Responds with raw PDF bytes (streamed, Content-Type/Length passed through).

**Link probe for the bid form (v2.3142)**: `?probe_url=<link>` (or POST `{probe_url}`) runs the same verdict on an arbitrary Drive file/folder link with no bid row and records nothing — the New Bid / Edit Bid readiness line asks it while the link is being typed. **Staff JWT only**; a twin token is refused (403).

**"Plans readable by robots" probe (v2.3080)**: `?bid=b403&probe=1` answers whether the service account can read the file — a Drive *metadata* GET, no bytes — and records it on the bid (`plans_robot_readable`, `plans_robot_probed_at`, `plans_robot_probe_note`). Folder links, non-Drive links, non-PDF files and Drive 403/404 all come back `readable: false` with a plain-words reason; the file name/mime/size are returned to staff only. No assignment fence on probes (presence-level information). `?probe=all&limit=N` (default 25, cap 100) sweeps live, unsent, non-ZZ bids with a plans link that were never probed or were probed more than 24 h ago (`force=1` re-probes all), returning `{probed, readable, unreadable:[{bid, project, note}]}`; twin-mcp's shadow queue and dispatcher call it before listing/claiming. A real byte fetch also records its outcome (failure → unreadable with the Drive status), so a pipeline pull that fails marks the bid for the board.

**Drive folder links (v2.3117)**: `plans_link` may be a Drive *folder* (`/drive/folders/<id>` or `/drive/u/N/folders/<id>`) — what estimators actually file. The folder's PDFs (Drive `files.list`, `mimeType='application/pdf'`, trashed excluded, shared drives included; Google-native docs and other files ignored), in natural name order (`A-1.2` before `A-1.10`), are the plan set. **Probe**: readable when the service account can list the folder and it holds ≥ 1 PDF — note `folder · N PDFs, merged on fetch: a.pdf, b.pdf, …` (names truncated); zero PDFs → unreadable, `folder holds no PDF — upload the plan set`; 403/404 on the folder → the same "not shared with the intake service account" wording as a file. **Fetch**: one PDF streams exactly as a file link does; several are merged in name order with pdf-lib into one PDF (`Content-Disposition: <project> - plans (merged N).pdf`, `X-Plan-Parts: N`). **Cap**: when the PDFs' summed `size` exceeds **60 MB** nothing is merged — the largest PDF streams alone with `X-Plan-Parts: N` and `X-Plan-Note: merged set exceeds 60 MB; streamed largest part only — stage_plan_pdf per part` (a part pdf-lib cannot open falls back the same way with `X-Plan-Note: merge failed (…)`). **`?part=<n>`** (1-based, name order; also `POST {part}`) streams one specific PDF from the folder; out of range → 400 listing the parts. Every folder fetch records the probe verdict on the bid first, and the twin assignment fence applies as before. File-link behaviour is unchanged byte for byte.

**Required secrets**: `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_ANON_KEY`, **`GOOGLE_SERVICE_ACCOUNT_JSON`**.

**Quote sources for the pricing twin (v2.3270 — Price Matrix PR 3)**: `?request=<uuid>&source=<n>[&part=<m>][&list=1]` reads the n-th link on a `bid_price_matrix_requests` row's `sources` (a Drive file or folder the estimator pasted on the bid's Price-requests table) with the same Drive plumbing as a plan set — folders list PDFs in name order, `?part` streams one, several merge under the 60 MB cap, `X-Plan-Parts` / `X-Plan-Note` as for plans. `list=1` answers JSON (`files: [{name, size}]`) instead of bytes. Fence: a twin may read only a request it claimed (`claimed_by = caller`, status `working` or `ready`); staff may read any. A non-PDF link answers 415; nothing is recorded on the bid — quotes are not plans.
...
> **v2.3375 — share this bill** (supersedes the v2.3346 listing): the job select adds `show_bills_to_other_party`, the invoice select `shown_to_party`. `buildPortalBills` is **owed-only** (`billedTo` is gone); `buildPortalSharedBills` returns the bills stamped for this viewer (`statementRoleFor` in `_shared/billVisibility.ts` = owed · shared · hidden) with `jobId` (v2.3378), the payer's name, open / billed / received and the viewer's role on the job, as the new `sharedBills` field. Every other bill on the viewer's jobs is filtered out **before** the payload. Telemetry adds `shared_bill_count`. **Redeploy required.**
...
**Bank transfer details (v2.3308)**: the payload gains `bankTransfer` — the one `company_bank_transfer_details` row (`payee_name, bank_name, bank_note, routing_number, account_number, account_kind, beneficiary_address, check_mailing_address, show_on_portal`), read with the service role and returned only when `show_on_portal` is on; `null` otherwise. The client gates it again through `bankTransferDetailsForPortal` (at least the transfer half or the check address must be filled) and renders the collapsed "Prefer to pay by bank transfer?" card under the ledger. The row is entered at Settings → Company and never lives in the repo. Sample mode returns invented numbers.
...
**Payment promise** (v2.3283, Their Word PR 2, migration `20260911052931`): `kind: 'payment_promise'` (`date` YYYY-MM-DD, `note?`) — the customer's own pay-by date from the statement's *Can't pay today? Tell us when to expect it.* strip. Validated with `_shared/portalPromise.ts` `promiseDateProblem` (today … +60 days); rate-limited at `PROMISE_MAX_PER_HOUR` (5) customer-sourced rows per customer per hour on `job_payment_promises` (no inbox row is written, so the dispatch counter doesn't apply); the job scope is exactly `customer-portal`'s (`openBillJobIds` over the link's audience) and an empty scope is a 400 ("Nothing is open on your account"). Writes through the service-role-only RPC `add_customer_payment_promise(customer, job_ids, date, note)` — one `source='customer'` / `channel='portal'` event per open-bill job the customer pays for plus the latest `job_promised_pay_dates` row (`marked_by NULL`), so the office's Billed chip reads *✓ Promised … · customer*. Logs one `portal_payment_promise` line. Response `{ ok, promisedYmd, jobs }`. **Redeploy required.**

**Endpoint**: `POST /functions/v1/submit-portal-request` — `{ token, kind: 'visit'|'bid'|'stage_window'|'payment_promise'|'share_bill_ask', jobId?, description, availability?, phone?, plansLink?, website }` (`website` is the honeypot).

> **v2.3378 — share_bill_ask** (Share this bill PR 4): `{ token, kind: 'share_bill_ask', ask: 'bill_me' | 'remind_owner', jobId, amount?, note? }`. The job must name the link's customer as its GC and someone else as its customer, and at least one of its billed invoices (or its billed shell remainder) must be **shared** with the viewer (`statementRoleFor` in `_shared/billVisibility.ts`), else 403 *That bill is not on your statement*. The function builds the sentence itself (*Done Right Foundation asks to be billed for J1017 · 4410 Cedar Hollow ($4,420.00) instead of Maria Delgado.* + *Note: …*) and inserts a `dispatch_requests` row (priority high, `job_ledger_id` = the job) with `pending_payload { source: 'portal', kind: 'share_bill_ask', ask, jobNumber, amount, ownerName, customerName, description, phone, phoneSource }`; the generic per-link rate limit applies. Never emails the owner. **Redeploy required.**
...
**Stages** (v2.2767): each sheet carries `stage` (`working` | `walkthrough` | `customer_pay`), `stageChangedOn` (YMD of the last move) and `stageSource` (`office` | `portal` | `auto`) — read from `people_labor_jobs.stage*`; unknown values normalize to `working`. The old `status` field is gone (paid sheets never appear: open = $0 leaves the list). The stage keys are unchanged by the v2.2922 rename — the page words them Work · Pre-inspection · Post-inspection: Trigger draw · You're paid.

**Plans online** (v2.2922): each sheet carries `plansUrl` — the Pipeline job's `jobs_ledger.job_plans_link` (the job is the sheet's `job_ledger_id` since v2.3080; it used to be matched by `hcp_number` = the sheet's `job_number`), else its bid's `bids.count_tooling_plans_link`, else null. The same read gives **Your days** the job's real number (HCP, else click) for office-set sheet days. The card wears a 📐 Plans pill when set. **Viewer grants (2026-09-06)**: when the resolved link is a CountTooling view link (`counttooling.com/…?t=<token>`), the function appends `&g=<grant>` — `base64url(claims).base64url(HMAC-SHA256)` over `COUNTTOOLING_VIEW_GRANT_SECRET`, claims `{ t, name, email?, person, via: 'pipetooling-sub-portal', iat, exp }`, bound to that token, good for 24 h, minted fresh on every portal load (`_shared/viewGrant.ts`, tested from `src/lib/viewGrant.test.ts` against the same fixtures as CountTooling's verify side). CountTooling's `get-view-project` verifies it with its `PT_VIEW_GRANT_SECRET` (the same value), skips its email domain gate, and logs the visit as the sub. No secret set = the bare link (the sub meets CountTooling's email gate). **Required secret**: `COUNTTOOLING_VIEW_GRANT_SECRET`.
...
**Change requests** (v2.2934): `pick_dates` skips the day-before rule while `step_commitments.change_requested_at` is set and clears `change_requested_at / _note` on a successful pick. `sub-portal` exposes it as `dates.changeRequested`.

**Watchers** (v2.2932): after `progress`, `mark_work_done`, an `accept_offer` with a pick, and `pick_dates`, the function calls `notifyJobWatchers` (`_shared/jobWatchers.ts`): explicit `job_watchers` rows ∪ the job's assigned superintendents (`jobWatchersCore.resolveWatchers`), filtered by kind, folded to one email per user × job × kind per hour via `job_watcher_notices`, sent through Resend with `buildWatchEmail`. Secrets: `RESEND_API_KEY`, `APP_ORIGIN`.
...
**Behavior**: Reads `job_contracts` as the caller (403 when RLS hides it); refuses voided / signed rows and rows without terms. Mints the durable plaintext `public_token` on the first send, reuses it after; refreshes `public_token_expires_at` (+90 days); sets `status = 'sent'`, `sent_at` (first), `last_sent_at`, `send_count + 1`, `sent_channel = 'link'` (v2.3723 — a row first emailed as a PDF by `share-job-contract` and sent again from here reads as a link send), the recipient block, `next_reminder_at` (+3 days when reminders are on); logs a `sent` event (`metadata.channel` email|link). `mode: 'email'` sends the Resend email (Review & sign button, reply-to = the sender, CC list) and returns `{ ok, emailed, sign_url }`; `mode: 'link'` returns the URL without emailing. The `job_contracts_to_activity()` trigger writes `contract_sent` on every send.
...
**Endpoint**: `POST /functions/v1/share-job-contract` — `{ contract_id } | { estimate_id, job_id? }`, `mode: 'email' | 'pdf_url' | 'draft_pdf' | 'send_to_sign'`, `to?: string[]` (≤10; first To, rest CC), `note?`, `public_origin?`; staff user JWT in `Authorization`. **`draft_pdf`** (v2.3527, Signing it on paper PR 1): the UNSIGNED agreement as `{ ok, filename, pdf_base64 }` — from `contract_id` (any status) or from `job_id` + `draft: { fields, body_html, body_format, template_name, recipient_name, revision? }` (the pane's own fields, so a download writes nothing); blank Sign / Date rules, no SIGNED ELECTRONICALLY tag, never a link or a token. **`send_to_sign`** (v2.3631, PR 3): emails that same unsigned PDF to the customer to sign by hand — `contract_id` (a live draft or sent row, read through the caller's RLS; refused when voided, signed, without terms, or without a valid `recipient_email`), optional `recipient_name`, `message`, `public_origin`. **The email goes first**; only then is the row stamped `sent` / `sent_channel = 'pdf_email'` with the durable signing token minted (the email offers the link as the second door, and `remind-job-contracts` needs it) and `next_reminder_at` set as a link send would; a `sent` event is logged with `channel: 'pdf_email'`. An email failure is a 502 and nothing is stamped; a stamp failure after the email returns `{ ok, emailed: true, warning }`. Wording: `buildJobContractPaperEmail` in `_shared/jobContractEmail.ts`. Imports `_shared/jobContractPdf.ts` — redeploy `sign-job-contract` too when that file changes.
...
**Staff notice** (v2.2743): after the acceptance is saved, `notifySignedAgreement` (`_shared/signedAgreementNotify.ts`) runs — optional auto-create via `auto_create_job_from_signed_estimate` when `signed_agreements_auto_create_job_estimates` is on, then the "<who> signed $4,250 · <work>" letter (subject since v2.3793, dash dropped v2.3794; the work only when the title is not the app default; the amount is the row's `total_cents`, or the chosen option's frozen total) to `signed_agreement_notify_recipients` ∪ the estimate's own picks. Replaces the old per-estimate + org-wide notify. **Auto-create guard** (v2.2838): before the RPC, the pure kernel `_shared/autoCreateJobGuard.ts` decides against a bounded candidate set (jobs on the bid + the customer's jobs from the last 90 days) — skip when the estimate is already linked, is a `change_order` (never auto-created; the letter's Create the job opens Apply-to-job), a job carries the bid (linked, not duplicated), or a same-customer/name/value twin exists (`duplicate_by_name_value`, no write); otherwise create. One structured log line per decision: `signed_agreement_auto_create_decision {outcome, skipped_reason, matched_job_id, via}`. The SQL function enforces the CO and twin rules too (migration `20260905110000`).

**Expiry** (v2.2703): same end-of-Central-day rule as `get-estimate-for-customer`.

**Body**: `{ "token": string, "printedName": string, "agreedTerms": true, "optionKeys"?: string[], "optionKey"?: string }` — with 2+ options a selection is **required**: `optionKeys` (v2.3554 — the chosen choice plus any add-ons, or one or more add-ons when the estimate has no choice group) wins; a client from before add-ons sends the single `optionKey`, the one-element case of the same rule. `isValidSharedEstimateSelection` answers 400 `option_required` (no choice picked / nothing ticked) or `option_unknown`; the same `sent → customer_accepted` update freezes **every** accepted option's lines in offered order into `line_items_snapshot` / `total_cents` and stamps `accepted_option_keys` (+ `accepted_option_key` = the choice, null when there was none). The staff notice reads *"Replace 50-gal" + 2 add-ons · $5,740.00*.
...
> **v2.3733 — the office person's line** (Day book PR 6b): after the crew payload, `fetchOutcomeLines` calls **`get_day_book_payload_for_user(p_user_id, p_day)`** (service_role only; the Day book's read as the recipient — a recipient the Day book refuses gets `forbidden` and no lines) and `_shared/dayBookOneLiner.ts` turns that day's raw events into one line per person (*billed 3 · 4 deposits · 2 contracts sent*); `payload.outcomes` → `PersonLine.outcome`, printed under the person in the HTML row and after the hours in the text version. Degrades to no line on any error. `src/lib/people/dayBookOneLinerShared.test.ts` pins the port to the Dashboard's wording.
...
**Subs on site** (v2.2929): after the RPC payload, the function attaches `subs` — signed `step_commitments` whose `picked_start ≤ day ≤ picked_end`, with the job label and the stage's line-item name (via `stage_window_id` → `job_stage_windows` → `jobs_ledger_fixtures`) — and `render.ts` ends the email with a "Subs on site today" section (HTML + text). Office-wide like the rest of the email; `[]` on any error.
...
**Purpose**: **`invoices.retrieve`** (with **`expand: ['customer']`**) + line items for a billed **`jobs_ledger_invoices`** row with **`stripe_invoice_id`**. Used by **Hosted bill** UI and **Collect Payment** step 3 (Stripe-resolved customer email). Since **v2.3590** the response carries **`hosted_invoice_url`** — the current link Stripe handed back on this retrieve — and the silent backfill writes it to the row when it differs from the stored one (Stripe expires links 30 days after the due date); `HostedStripeBillPanel` prefers the response's link over the row's for Customer pay page and the share cluster. Response **`customer_email`** matches **`send-stripe-invoice`** resolution (**expanded Customer `email` first**, then **`invoice.customer_email`**) per **[`customerEmailFromStripeInvoice`](../supabase/functions/_shared/stripeInvoiceCustomerEmail.ts)**.
...
- **Server-side auto-approve** (v2.2889, journey-map Tier-2 #27) — when the org switch `app_settings.accounting_label_auto_approve_rule_matches` is `'true'` (ships `'false'`; flipped at Banking → Accounting by dev/master), a freshly inserted suggestion approves itself: `autoApproveIfSwitchedOn` reads the setting (one read, early return while off), evaluates the shared kernel [`supabase/functions/_shared/accountingLabelAutoApprove.ts`](../supabase/functions/_shared/accountingLabelAutoApprove.ts) (`shouldAutoApproveSuggestion` — twin of `src/lib/accountingLabelAutoApprove.ts`, parity-tested) with the suggested label's `default_key` and whether the tx has job splits, then calls RPC **`auto_approve_pending_accounting_label_suggestions([txId])`** (service_role only) which re-checks every gate in SQL and writes exactly what the client's `bulk_approve_accounting_label_suggestions` writes, with `resolved_by = NULL` (= by the rule). Internal Transfers on a split tx stays pending. Logs `{ event: 'label_suggestion_approved', by: 'rule', count, tx, rule }` or `label_suggestion_left_pending` + reason. Best-effort — never fails the webhook. Migration `20260905180000_accounting_label_auto_approve_server_side.sql`.
...
- **The office's "check returned" notice** (v2.3804, punch list #40 PR 2) — after the upsert (which now also selects `status, posted_at`), `notifyBankReturn` runs the shared rule [`supabase/functions/_shared/bankReturnedDeposits.ts`](../supabase/functions/_shared/bankReturnedDeposits.ts) (`mercuryBankReturn`: a deposit that **posted and then went `failed`**, money in — the same rule the Dashboard card and Accounts Receivable read since v2.3795; a failed row that never posted is a processing failure and raises nothing). When a `jobs_ledger_payments` row still carries the deposit, it **inserts first** into `mercury_bank_return_notices` (keyed on the deposit; unique violation = already told, so Mercury's at-least-once retries and later updates never send twice), then sends one email (Resend, `email_type = 'bank_return'`, hardcoded design built by `buildBankReturnNoticeEmail`) and one push (`buildBankReturnNoticePush`, tag `bank-return-<tx id>`) per recipient, writes `notification_history`, and stamps the counts on the ledger row. Recipients: the *Payment made* list (`app_settings.payment_made_email_recipients_v1`), or when it is empty every non-sample, unarchived dev / master / assistant / controller. The words — *"Southern Post's check for $13,680 on J878 Take 5 – Seguin came back: Insufficient funds."* — and the deep link `/jobs?tab=stages&edit=<job>&editFocus=payments` land Edit Job on ③ Payments received, where **Unlink and remove** is the only write; the function takes nothing off a job. A deposit **nobody matched** raises no message (AR's chip covers it). Best-effort — never fails the delivery; the ledger row is written before the send, so a Resend outage means no retry (the Dashboard card is the backstop). Migration `20260924090000_mercury_bank_return_notices.sql`; secrets `RESEND_API_KEY`, `VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` (push skipped when unset), `APP_ORIGIN` (default `https://clicktooling.com`).
- **Migrations**: `20270605120000_mercury_webhook_events_dedupe.sql`, `20270605130000_insert_accounting_label_suggestion_service_rpc.sql`, `20260924090000_mercury_bank_return_notices.sql`.
...
2. **Secrets** (Dashboard → Edge Functions → Secrets, or `supabase secrets set`): `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `MERCURY_API_KEY`, `MERCURY_WEBHOOK_SECRET` (must match Mercury’s webhook signing secret); for the "check returned" notice also `RESEND_API_KEY`, `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `APP_ORIGIN` (shared with the other senders).
