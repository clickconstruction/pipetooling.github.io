...
**Sample** (v2.2760, What customers see): `token=sample` skips link and slug resolution — office JWT required (`_shared/sampleViewer.ts`); returns Sam's Plumbing's fixture statement from `_shared/customerSampleFixtures.ts` with `payRun` from the live `sub_pay_run_day` / `sub_pay_explainer` settings. No person, no rows.
...
**Sample** (v2.2760, What customers see): `token=sample` / `sample-done` skips the row lookup — office JWT required (`_shared/sampleViewer.ts`); returns the sample agreement (`SAMPLE_CONTRACT`, HTML body) or the 409 `already_signed` shape. No `signer_last_viewed_at` stamp.
