...
**Endpoint**: `POST https://mcp.clicktooling.com/twin` — the address every copy button and `.mcp.json` hand out since v2.3634, served by the `mcp-router` Worker (`scripts/cloudflare/mcp-router.worker.js` + its `.wrangler.toml`, a secret-less pass-through, live 2026-09-20) in front of `POST /functions/v1/twin-mcp`, which keeps answering for machines set up earlier · **Auth**: per-twin token on every `tools/call` (`X-Twin-Token` or `Authorization: Bearer`; `initialize`/`tools/list` are open metadata). `verify_jwt = false`.
