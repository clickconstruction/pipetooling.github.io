...
> **v2.3438 — the offered columns are gone**: the `stage_window` ask reads the window **by id only** (`.eq('id', stageId)`) and selects no `bundle_id` / `offered_to_gc` — migration `20260914230000` drops both plus `offered_to_gc_at`. **Redeploy before the push**: the old bundle selects those columns and would 400 once they are gone.
