...
**Job link (v2.3071)**: `mark_work_done` and `progress` find the sheet's job through `people_labor_jobs.job_ledger_id` (watcher notifications, the `sub_progress` activity event); the case-sensitive `hcp_number = job_number` lookups are gone — a sheet with no link notifies nobody and writes no event.
