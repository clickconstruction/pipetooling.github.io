...
**Endpoint**: `POST /functions/v1/submit-portal-request` — `{ token, kind: 'visit'|'bid'|'stage_window'|'payment_promise', jobId?, description, availability?, phone?, plansLink?, website }` (`website` is the honeypot).
