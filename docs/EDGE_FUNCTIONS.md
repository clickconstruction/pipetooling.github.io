...
**Asks** (v2.2934): each stage entry carries `asked` (`{ start, end, note, answer: open | accepted | proposed, answerNote }`) and `rescheduling` (the fulfilling order's `change_requested_at` is set).

**Stages** (v2.2933): for `gc` / `all` links, jobs whose `gc_shares_stage_dates` is on and whose `gc_customer_id` is the viewer contribute `stages[]` — offered `job_stage_windows` (bundles as one entry via `bundle_id`) with the line-item name, the window, the fulfilling `step_commitments` order's first name and picked dates, and the sheet's `progress_pct` / stage → `state` (`_shared/gcStages.ts`, tested from `src/lib/subs/gcStages.test.ts`). Never money, paperwork, or a phone.
...
**Stage asks** (v2.2934): `kind: 'stage_window'` (`stageId` = a `job_stage_windows` id or a `bundle_id`, `start`, `end`, `note`) — GC / merged links only; the window must be offered on a job whose `gc_customer_id` is the viewer and whose `gc_shares_stage_dates` is on; validated with `_shared/stageAsk.askProblem`; writes `asked_*` on every window of the bundle (clearing any earlier answer); inserts `dispatch_requests` (`pending_action: 'gc_stage_ask'`, `pending_payload` carries the window ids, dates, note and GC name) and calls `notify-dispatch-request`.
