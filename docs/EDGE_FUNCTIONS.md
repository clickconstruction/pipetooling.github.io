...
**Endpoint**: `POST /functions/v1/twin-setup` · **Auth**: in-function (mint: staff JWT via `auth.getUser`; redeem: the code). `verify_jwt = false` in `config.toml` — preserve on redeploys. **Env**: `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`; optional `TWIN_MCP_PUBLIC_URL` when the connector is reached at another address — a bare origin gets `/functions/v1/twin-mcp` appended, an address with a path (`https://mcp.clicktooling.com/twin`) is taken verbatim (`_shared/twinConnectorUrl.ts`, shared with the client's copy buttons; v2.3633).
