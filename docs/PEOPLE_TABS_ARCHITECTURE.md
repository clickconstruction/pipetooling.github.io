...
| `whos_where` | thin wrapper (`{activeTab === 'whos_where' && canSeeDayBook && <PeopleWhosWhereTab … />}`) | ~430 (component) | extracted-from-birth (`PeopleWhosWhereTab`, v2.3607; kernel `lib/people/whosWhere.ts`, loader `fetchWhosWhereWeek.ts`; no RPC — reads `clock_sessions` + `job_schedule_blocks` directly) | 0 in parent | `authRole` | low | Done (`to-dos/whos-where/`) |
