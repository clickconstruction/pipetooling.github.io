...
| `hours` | inline (sub-sections extracting) | ~1,900 | partial | ~41 (`hours*`, `costMatrix*`, clock sessions) | **owns** `payConfig`/`teams`/`crewJobsByDatePerson` | very high | Phase 3 — decomposing sub-section by sub-section; Sharing → `PeopleHoursSharing`, Teams → `PeopleHoursTeams` done |
