...
The strip shows **six group tabs** and a **second row** of the active group's views; every view keeps its own render gate and `?tab=<view key>` URL. The map is `PEOPLE_TAB_GROUPS` in the kernel above (pure, tested in `peopleTabGroups.test.ts`):
...
`People.tsx` builds `visiblePeopleTabs` from the gates, `visibleTabGroups()` drops a group with no visible view, `groupOfTab(activeTab)` picks the active group, and the row renders only when the group offers more than one view. A group click lands on `landingViewForGroup(group, memory)` — the device-local per-group memory (`people.tabGroupMemory.v1`, `parseGroupMemory`/`rememberTab`) — else the first visible view. Sub-tab pills use `pageSubTabStyle` ([`pageTabStyle.ts`](../src/lib/pageTabStyle.ts)). The URL-gate effect is unchanged except that the long `tab === '…' || …` chain is `isPeopleTab(tab)`.
...
- **Assistant view (journey-map J32-F1, verified in code v2.2915):** `PersonDeskBody` derives `viewer.canAccessPay` from `usePeopleAccess` (assistant → false); `PersonDeskPaySection` skips all of its loads and returns `null` when it is false, so an assistant's Desk fires no wage / stub / offsets query and shows no Pay section. Every section reads its own person's rows only (`usePersonDesk` resolves one `users` / `people` row by id); the rail loads the roster once.
...
| `users` | `PeopleUsersTab` | ~3 (`{activeTab === 'users' && <PeopleUsersTab .../>}`) | extracted (`PeopleUsersTab` + `useUsersTabTags`/`PeopleUserTagsPanel`; shared consts/`byKind` in `peopleUsersTabShared`; hosted the **Team leads** modal 2026-08-02 → retired v2.3616) | — | reads `people`/`users`, `contractSigningStatusByPersonName`, push/location | done | Person-edit form stays in `usePeopleRoster`; the edit-user-note modal stays in the parent  **v2.2762 refresh**: one [`UsersTabRow`](../src/components/people/UsersTabRow.tsx) for account and roster-only rows (imitate unchanged · dot · `PersonNameDoor` · login chip · signal chips from [`loadRailFacts`](../src/lib/people/loadRailFacts.ts) via `useUsersTabSignals` · ⋯ menu); groups ordered/filtered/folded by [`usersTabRows.ts`](../src/lib/people/usersTabRows.ts); the External Subcontractors / External Helpers groups are retired; sticky toolbar + filter chips; the Add form is "Add to roster" with invite / open-desk follow-ons (`usePeopleRoster.handleSave` returns the saved `Person`). **v2.3185 phone branch** (`narrowViewport`): a non-sticky search + Add + ⋯ toolbar, one-row scrolling filter chips with counts, one card per kind with a sticky header, and [`UsersTabPhoneRow`](../src/components/people/UsersTabPhoneRow.tsx) — tap = desk, swipe left = Desk · Imitate · More — over the pure [`usersTabPhone.ts`](../src/lib/people/usersTabPhone.ts) |
...
| `whos_where` | thin wrapper (`{activeTab === 'whos_where' && canSeeDayBook && <PeopleWhosWhereTab … />}`) | ~500 + `WhosWhereWeek` ~200 | extracted-from-birth (`PeopleWhosWhereTab`, v2.3607 the day, v2.3609 the week; kernel `lib/people/whosWhere.ts`, loader `fetchWhosWhereWeek.ts`; no RPC — reads `clock_sessions` + `job_schedule_blocks` directly) | 0 in parent | `authRole` | low | Done (v2.3607, v2.3609) |
...
>
> **Team leads modal (2026-08-02 → retired v2.3616):** the `team_leader_assignments` manager lived here as `TeamLeadsModal` from 2026-08-02 until Supervision replaced the list with the switch on each helper's and sub's account; the button, modal, manager and hook are deleted. The modal is now pure chrome (backdrop/panel/title/close/scroll-lock) around the shared leader-centric [`TeamLeadsManager`](../src/components/people/TeamLeadsManager.tsx), which was briefly also rendered by the **Teams** tab until that tab was removed in v2.1292 (`?tab=teams` now redirects to Users) — the modal is the single surface. The manager is self-contained: gets `user`/`role` from `useAuth` (gate: dev / master_technician / assistant / controller — the same set as `canAccessTeamsTab`), loads its own roster (labels include archived users; pickers stay non-archived), drives rows through the slimmed [`useTeamLeaderAssignments`](../src/hooks/useTeamLeaderAssignments.ts) hook, and groups/searches via the pure kernel [`teamLeadsGrouping`](../src/lib/people/teamLeadsGrouping.ts). One collapsible card per leader (search matches leader OR member), per-member **Full/Strip** segmented toggle for `dashboard_hours_visibility` (still dev-only editable), × remove with confirm (**Remove stale link** for archived members), per-card **+ Add member**, and a **+ New leader** flow. Settings keeps a one-line pointer.

- **Render:** ~513 pre-extraction lines; person create/edit form modal + invite-confirm modal render at page level. **Lenses (v2.3702, People spine PR 5):** `?lens=contact|account|pay` ([`lib/people/usersTabLens.ts`](../src/lib/people/usersTabLens.ts)) — a column set over the same grouped rows: Account (role, sign-in, training, supervision, a Desk door) and Pay (the `usePayConfig` inputs + *Workday…*) from [`UsersTabLensCells`](../src/components/people/UsersTabLensCells.tsx); `UsersTabRow` takes `cells`; the pay lens keeps the no-login fold open; the Payroll tab's *People pay config* is a link here and `PeoplePayConfigModal` is gone; the Users toolbar's *Accounts · dev* button and its phone-menu item went in v2.3705 (the desk's *Manage account…* and the Settings pointer open the modal). **+ Hire (v2.3701, People spine PR 3):** the toolbar's primary door is [`HirePersonModal`](../src/components/people/HirePersonModal.tsx) (`onOpenHire`, mounted by the page) — one form run as ordered writes through [`lib/people/hireWrites.ts`](../src/lib/people/hireWrites.ts) (`hirePlan` pure: invite → born-linked roster row, pay row, salaried workday, packet; Retry per step); *+ Add to roster* is the secondary roster-row-only door. `invite-user` / `create-user` write the linked `people` row themselves since v2.3701 (`_shared/rosterRow.ts`).
...
- **Add document (v2.2857, journey-map Tier-2 #33):** the expander's `+ Add document` mounts [`SubDocumentAddForm`](../src/components/people/SubDocumentAddForm.tsx) — type · expires (required for COI) · optional https file link · name → one INSERT with `doc_type` set from birth, `status='signed'` (on file), fresh lineage; no write until Save. Kernel [`subDocumentDraft.ts`](../src/lib/people/subDocumentDraft.ts) (`subDocumentDraftValid`, `buildSubDocumentInsert`). Gate `CAN_ADD_DOCUMENT_ROLES` = dev / master_technician / assistant / controller (mirrors the table's INSERT policy; `is_assistant()` includes controller since v2.714) and not training mode. The same form mounts in Person Desk → Paperwork. Telemetry `ui_nav_clicks` `control='sub_document_added'`, `target='#<type>'`. **N2 hint:** `doc_type` is `NOT NULL DEFAULT 'agreement'` — there is no untyped state — so `suggestedRetype` flags rows still typed `agreement` whose *name* reads W-9 / COI / license with an amber "Looks like a W-9 — set type" pill that runs `updateDoc`. The old `?? 'agreement'` client fallbacks are gone.
...
>
> **v2.2168 — Ledger view (dev-only; labels renamed v2.3317 to `Pay run | Balances`):** a `Pay run | Balances` switch above the tab (`?view=ledger&person=<name>`) swaps `PeoplePayStubsTab` for [`PeoplePayLedgerView`](../src/components/people/PeoplePayLedgerView.tsx) — roster of per-person balances + (v2.3689) the selected person's open reports first — oldest week up, Paid to date / Balance / Payment state / Pay to here, the off-report charges and credits, the unreported weeks, one settle-up line — with the dated journal folded underneath as the statement (grouped by report or date order); kernels [`src/lib/people/personLedger.ts`](../src/lib/people/personLedger.ts) (wraps `buildPartnerJournal`; every offset books at its date) and [`src/lib/people/openReports.ts`](../src/lib/people/openReports.ts) (open rows, settle-up, oldest-first allocation, overpayment move, roster caption). The parent passes `loadUnreportedWeeks` (the catch-up scan for one person) and `onGenerateReport`. The view loads `person_offsets` itself and calls the parent's `loadPayStubs()` on mount; the stub/payment/deduction/additional maps stay parent-owned.
...
- **Grid roster + header hints (v2.2915, journey-map B9):** the row list is derived once by [`lib/people/hoursGridRoster.ts`](../src/lib/people/hoursGridRoster.ts) (`buildHoursGridRoster` = pay-config rows − archived account names − what the roster view says is not a person: `is_pay_roster` from `public.roster_people` via [`lib/people/rosterPeople.ts`](../src/lib/people/rosterPeople.ts), v2.3698 — a twin, a sample, an archived roster row; a row matching no roster row is shown; a null index is "no verdict", so the archived-name rule holds alone until the view is pushed — in `people_hours_display_order`); `showPeopleForMatrixBase` reuses it, as do `useWeeklyTeamLaborTotal`, Quickfill `HoursSection` and the Review tab. The view loads for `canAccessHours || canAccessPay` in the hours-tab load cycle and runs with owner rights, so an assistant's grid is the owner's grid (J7-6 — the archived-name RPC once loaded `canAccessPay`-only and carried archived helpers as zero-hour rows). [`lib/people/hoursGridLiveByCell.ts`](../src/lib/people/hoursGridLiveByCell.ts) folds `activeClockSessions` into one `{ people, elapsedHours }` per day column (`liveByWorkDate` prop) so the header can say **"+N on the clock"** under a closed-sessions-only total (J7-4). The cell's amber pending chip takes its tooltip / aria-label from [`lib/people/hoursGridPendingChipCopy.ts`](../src/lib/people/hoursGridPendingChipCopy.ts) — "click to review", because the Approve lives in `PeopleHoursPendingCellPopover` (J7-N1).
...
- **Render:** thin wrapper `TeamFeedbackDevSettingsBlock layout="standalone"`. **v2.2835 "one table, one feed":** `FeedbackStatusStrip` (switch · sentence · due/rated/words · Try the deck → `CrewReviewDeck preview` · gear) + `FeedbackPeopleTable` (rows from [`feedbackTabRows.ts`](../src/lib/people/feedbackTabRows.ts): `deckStateFor`, `buildFeedbackRows`, `filterFeedbackRows`, `feedbackStats`; cards under 640px) → `FeedbackPersonDrawer` (lanes, notes, words, dev-only raters, Reset) + `OpenWordsFeed` (Unread via `people.feedbackWordsReadAt.v1`) + `FeedbackSettingsDrawer` (`CrewFeedbackSettingsForm` + `RetiredQuestionsPanel`). `layout="settings"` (Settings → People & accounts) renders only a pointer link. Data: `team_member_reviews` (source `crew`/`office`), `team_feedback_submissions` (open words), `team_feedback_settings`, `team_feedback_user_state`, RPC `list_team_member_recent_jobs` (who clocks out). Crew kernel: [`crewReview.ts`](../src/lib/people/crewReview.ts).
- **Owned state:** none in the parent (`isDev` gate only).
- **v2.2915 (journey-map J32-F5):** `feedbackStats` gains `skippedLately` — people whose newest deal was a skip (null when Off) — rendered as the strip's fourth stat **"skipped last deal"**; a skip re-arms the full cadence, so this is the number that explains an On-but-quiet deck.
...
| [`usePeopleRoster`](../src/hooks/usePeopleRoster.ts) | **extracted (PR #26)** | `people`/`users` + refs, `loadPeople`, `loadArchivedPeople`, person create/edit form + `handleSave` (via a 6-field deps ref). `handleMergeDuplicate` stays in the parent (pay/hours-entangled). **v2.2893:** `loadPeople` reads accounts through the shared active-people query — [`activeUsersQuery` / `fetchActiveUsers`](../src/lib/people/fetchActiveUsers.ts) (`archived_at IS NULL AND is_digital_twin = false`; dev rows only for dev viewers), whose pure predicate is [`isActiveRosterPerson`](../src/lib/people/activeRoster.ts). The same loader feeds the Jobs / job-form crew pickers, the Person rail, the `/` quick sheet, the checklist assignee pickers, the Review deck and the "Team reviews due" nag — any new roster/picker should go through it rather than a raw `users` select. | nearly all tabs |
