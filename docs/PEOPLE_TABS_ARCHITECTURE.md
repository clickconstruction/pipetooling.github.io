...
- **Grid roster + header hints (v2.2915, journey-map B9):** the row list is derived once by [`lib/people/hoursGridRoster.ts`](../src/lib/people/hoursGridRoster.ts) (`buildHoursGridRoster` = pay-config keys − `archivedUserNames`, in `people_hours_display_order`); `showPeopleForMatrixBase` reuses it. The archived-name RPC now loads for `canAccessHours || canAccessPay` in the hours-tab load cycle — it was `canAccessPay`-only, which is why an assistant's grid carried archived helpers as zero-hour rows (J7-6). [`lib/people/hoursGridLiveByCell.ts`](../src/lib/people/hoursGridLiveByCell.ts) folds `activeClockSessions` into one `{ people, elapsedHours }` per day column (`liveByWorkDate` prop) so the header can say **"+N on the clock"** under a closed-sessions-only total (J7-4). The cell's amber pending chip takes its tooltip / aria-label from [`lib/people/hoursGridPendingChipCopy.ts`](../src/lib/people/hoursGridPendingChipCopy.ts) — "click to review", because the Approve lives in `PeopleHoursPendingCellPopover` (J7-N1).
