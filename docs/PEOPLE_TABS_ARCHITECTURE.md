...
| `hours` | inline (sub-sections extracting) | ~1,280 | partial | ~39 (`hours*`, `costMatrix*`, clock sessions) | **owns** `payConfig`/`teams`/`crewJobsByDatePerson` | very high | Phase 3 — decomposing sub-section by sub-section; Sharing → `PeopleHoursSharing`, Teams → `PeopleHoursTeams`, Due-Summaries → `PeopleHoursDueSummaries`, Sessions → `PeopleHoursSessions`, Week → `PeopleHoursWeekRange`, Cost matrix → `PeopleCostMatrix` done |
