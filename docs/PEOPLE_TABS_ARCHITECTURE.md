...
- **Grid roster + header hints (v2.2915, journey-map B9):** the row list is derived once by [`lib/people/hoursGridRoster.ts`](../src/lib/people/hoursGridRoster.ts) (`buildHoursGridRoster` = pay-config rows the roster view calls people — `is_pay_roster` from `public.roster_people` via [`lib/people/rosterPeople.ts`](../src/lib/people/rosterPeople.ts), v2.3698: not a twin, not a sample, neither half archived; a row matching no roster row is shown; a null index is "no verdict" — in `people_hours_display_order`); `showPeopleForMatrixBase` reuses it, as do `useWeeklyTeamLaborTotal`, Quickfill `HoursSection` and the Review tab. The view loads for `canAccessHours || canAccessPay` in the hours-tab load cycle and runs with owner rights, so an assistant's grid is the owner's grid (J7-6 — before v2.3698 the exclusion was the archived-name RPC, which once loaded `canAccessPay`-only and carried archived helpers as zero-hour rows). [`lib/people/hoursGridLiveByCell.ts`](../src/lib/people/hoursGridLiveByCell.ts) folds `activeClockSessions` into one `{ people, elapsedHours }` per day column (`liveByWorkDate` prop) so the header can say **"+N on the clock"** under a closed-sessions-only total (J7-4). The cell's amber pending chip takes its tooltip / aria-label from [`lib/people/hoursGridPendingChipCopy.ts`](../src/lib/people/hoursGridPendingChipCopy.ts) — "click to review", because the Approve lives in `PeopleHoursPendingCellPopover` (J7-N1).
