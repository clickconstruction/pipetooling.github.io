...
- **Phase 3 (hub tabs) — IN PROGRESS.** ~~`overhead`~~ (`PeopleOverheadTab`), ~~`review`~~ (`PeopleReviewTab`), and ~~`pay_stubs`~~ (`PeoplePayStubsTab`, the **Ledger** half only — see the dossier) are extracted. `users` is in progress: its **tag subsystem** moved to `useUsersTabTags` hook + `PeopleUserTagsPanel` (Stage 1 of 2); the **roster UI** (incl. the roster search vars) is the remaining seam. Remaining: `users` roster → `hours` (hours last), consuming the Phase-2 hooks.
...
|---|---|---|---|---|---|---|---|
| `users` | roster render + person form (inline) | ~530 (roster) | partial — tag subsystem extracted (`useUsersTabTags` + `PeopleUserTagsPanel`); roster still inline | ~13 (roster + notes + roster search) | reads `people`/`users`, `contractSigningStatusByPersonName`, push/location | high | Roster UI is the remaining seam (Stage 2) — move roster search vars with it |
...
### `users` — Roster
> **Tag subsystem extracted (Stage 1 of 2).** The dev-only tag/label feature moved out of `People.tsx` (~580 lines: 7,712 → 7,132) into two artifacts: the [`useUsersTabTags`](../src/hooks/useUsersTabTags.ts) hook (all ~16 `usersTab*` tag state vars, both loader effects — tag-org overrides + label-usage counts — the catalog memos, and the mutation callbacks `tagOrgMasterLabel`/`applyUserTagOrgChange`/`deleteLabelFromCatalog`, plus the `showTags`/`showTagOrgSignals` toggles with localStorage persistence), and the presentational [`PeopleUserTagsPanel`](../src/components/people/PeopleUserTagsPanel.tsx) component (the per-row tag panel, invoked once per roster row with `{ anchor, people, tags, showToast }`). The parent now calls `useUsersTabTags({ isDev, activeTab, people, users, authUserId, showToast })` and renders `<PeopleUserTagsPanel>` at the two roster call sites; the label-catalog table + dev toggles stay inline in the roster render (they move with the roster in Stage 2), rewired to the hook bundle.
>
> **Deliberately left in the parent (it's the roster, not tags):** the roster search vars — `usersTabSearch`/`usersTabSearchQ`/`usersTabRowMatchesSearch`/`usersTabSectionHasVisibleRows`/`usersTabSearchShowsNoSections` — are misleadingly `usersTab*`-named but filter the whole roster; they move with the **roster UI (Stage 2)**. `resolvePersonIdForUsersRow`/`resolveUsersTabTagAnchor` (anchor builders) also stay (they read `people`). The dossier below is the pre-extraction inventory.
...
11. **`users`** — two-stage. ~~**Stage 1: tag subsystem**~~ — **DONE** (`useUsersTabTags` hook + `PeopleUserTagsPanel` component; hook-first to avoid prop-drilling the per-row panel). `People.tsx` 7,712 → 7,132. **Stage 2: roster UI** — remaining; move the roster render + person-form wiring + the roster search vars (`usersTabSearch*`); the person-edit form already lives in `usePeopleRoster`.
