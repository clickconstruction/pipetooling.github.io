...
>
> **Team leads modal (2026-08-02):** the `team_leader_assignments` manager that lived on Settings → Dashboard & alerts ("Team Hours Sharing") moved here as [`TeamLeadsModal`](../src/components/people/TeamLeadsModal.tsx), opened from a **Team leads** button beside the roster search (next to the dev-only Manage accounts button). Self-contained: gets `user`/`role` from `useAuth` (gate: dev / master_technician / assistant / controller — the same set as the Settings manager and `canAccessTeamsTab`), loads its own picker roster (non-archived `users`), and drives rows through the renamed [`useTeamLeaderAssignments`](../src/hooks/useTeamLeaderAssignments.ts) hook (ex-`useSettingsTeamLeaderAssignments`). Per-row **Leader dashboard** visibility (full vs strip_only) stays dev-only editable. Settings keeps a one-line pointer. The **Teams** tab (`PeopleTeamsTab`) still manages the same table independently — intentionally untouched pending a separate decision.
