...
| `teams` | thin wrapper | ~3 | extracted (`PeopleTeamsTab` → shared `TeamLeadsManager`, 2026-08-02) | 0 in parent | `authUser`/`authRole` (accepted for call-site stability; manager is self-contained) | low | Done |
...
### `teams` — Teams
- **Render:** ~3 lines. Thin wrapper around `PeopleTeamsTab` ([`src/components/people/PeopleTeamsTab.tsx`](../src/components/people/PeopleTeamsTab.tsx)), which since 2026-08-02 is itself a ~17-line shell around the shared [`TeamLeadsManager`](../src/components/people/TeamLeadsManager.tsx) (same component the Users tab's **Team leads** modal renders — see the note above the `users` dossier). **Done.** Note: the parent still owns `teams` + `loadTeams` + team CRUD for the **Hours** tab's Teams / Due-by-Team sections, not for this tab.
