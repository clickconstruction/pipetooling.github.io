...
| Assembly authoring modal cluster (Add Assembly / Add Parts / Edit Template) | modal-open flags | ~65 (cluster render) | — | — | **extracted (v2.1304)** → [`TakeoffAssemblyAuthoringModals`](../src/components/bids/TakeoffAssemblyAuthoringModals.tsx) | Done — open pointers, PartFormModal-routed picker states, and the Save-as-Assembly bridge stay parent-owned; preview modal stays with the exact region for T8 |
