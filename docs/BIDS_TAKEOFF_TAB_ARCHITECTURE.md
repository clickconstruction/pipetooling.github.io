...
- **Extraction status + risk + approach:** Inline, **med-high risk** — the biggest single chunk (~1,270 lines) and the hub the two model bodies call into. Extract as one cluster (e.g. `TakeoffAssemblyEditorModals.tsx` exposing `openAddAssembly`/`openEditTemplate`/`openAddParts` via a small hook, or three sibling components sharing a `useTakeoffAssemblyEditor` hook). The Save-as-Assembly handoff (`saveAsAssemblyCountRowId` + bundle override) must remain reachable from the rough region — keep those two states in the parent and pass them in. Stage A: the part-merge logic duplicated in `saveTakeoffNewTemplate` + `addTakeoffNewTemplateItem` → pure `mergeTemplateItemDrafts` in `lib/bids/` + tests.
