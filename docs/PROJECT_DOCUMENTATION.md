...
- **Key Fields**:
  ...
  - `kind` (text, `fixture` | `task` | `sub`, default `fixture`; v2.3289) - a sub line carries none of our field hours
  - `unit` (text, `each` | `per_100ft`, default `each`; v2.3289) - per 100 ft reads count ÷ 100 × hours
  - `source` (text, nullable: `book` | `alias` | `typed` | `robot`; v2.3289) - where the hours came from; NULL = written before v2.3289
  - `source_note` (text, nullable; v2.3289) - the entry and book, or what was learned on which bid
  - `sequence_order` (integer)
  - `created_at` (timestamptz)
...
- **Key Fields**:
  ...
  - `unit` (text, `each` | `per_100ft`, default `each`; v2.3289) - footage entries carry hours per 100 ft
  - `kind` (text, `fixture` | `task`, default `fixture`; v2.3289) - a task is fixed hours for the line
  - `sequence_order` (integer)
  - `created_at` (timestamptz)
  - **UNIQUE** `(version_id, fixture_type_id)`
- **RLS**: dev, master_technician, assistant, estimator (full CRUD)
- **Entry Creation**: Input field with autocomplete; auto-creates fixture types if they don't exist; *Reads as* and *Hours are per* selects (v2.3289)
