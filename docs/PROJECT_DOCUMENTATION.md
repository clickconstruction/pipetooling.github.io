...
### Database Migrations
The authoritative rules live in [AGENTS.md](../AGENTS.md) (hard constraints) and [MIGRATIONS.md](./MIGRATIONS.md) (workflow + full ledger history) — read those, not this section, before touching DDL. The two rules people get wrong:
- Migrations are repo files under `supabase/migrations/`, applied **only** with `supabase db push` after the file is on `main` (or in the PR merging right now). Never apply DDL via MCP `apply_migration`, MCP `execute_sql`, or the dashboard SQL editor — all three caused the drift behind the 2026-07-04 ledger rewrite.
- Every new migration starts with `SET lock_timeout = '3s';`, and migrations that CREATE TABLE end with both read-only-block calls (see [AGENTS.md](../AGENTS.md); CI enforces the preamble via `scripts/check-migrations.sh`).

There is no staging — `db push` runs against prod. Check ledger alignment with `npm run check:migration-drift`.
...
- After any schema change, regenerate with `npm run gen-types:local` (local stack) or `npm run gen-types:linked` (linked prod project)
...
### Technical Debt
1. ✅ **Type Generation**: Resolved — `src/types/database.ts` is generated via `npm run gen-types:local` / `npm run gen-types:linked`
