...
- The **Database Schema** section documents roughly **55 of the ~355 tables** — the core project/workflow/people/materials/bids tables — plus a summary of the `jobs_ledger` family. For any other table, grep [RECENT_FEATURES.md](./RECENT_FEATURES.md) for the version that introduced it.
...
- Prefer the specialist docs when they exist: [ACCESS_CONTROL.md](./ACCESS_CONTROL.md) for role permissions, [BIDS_SYSTEM.md](./BIDS_SYSTEM.md) for bids, [EDGE_FUNCTIONS.md](./EDGE_FUNCTIONS.md) for the ~104 Edge Functions, [MIGRATIONS.md](./MIGRATIONS.md) for schema history, and [RECENT_FEATURES.md](./RECENT_FEATURES.md) for the per-version changelog.
...
#### `public.team_leader_assignments` — frozen (v2.3616)
- **Retired**: no UI writes rows any more (the Team leads modal, the desk's team section and the lifecycle's leader items are gone; My Team's roster is the crew the viewer supervised this week via `get_supervised_days_payload`). Existing rows still feed the `is_team_lead_for_member` RLS / RPC branches and `notify-team-lead-clock` until the table is dropped — see `to-dos/team-leads-table-retirement.md`.
- **Purpose (historical)**: Directed **leader → member** links so a leader can view and approve/reject/revoke that member’s clock sessions from Dashboard **My Team** without full People Hours pay access. Any `users.role` may be leader or member; multiple leaders can share the same member (first approver wins; others see `approved_by`).
...
- **Usage**: Used in the Materials **Price coverage** modal statistics section (titled "Supply Houses" until v2.2903)
- **Logic**:
...
#### Child tables
- `jobs_ledger_fixtures` — Specific Work lines: `name`, `count`, optional `line_unit_price`, optional `line_description` (scope text for that row's Stripe invoice line), `sequence_order`. Stage Plan (v2.3083): `stage_kind` (`order` / `any` / NULL = plain; default `any`) and `shared_with_gc` (the eye) — the line item is the stage; see GLOSSARY → Stage kind / Draw / The eye.
...
- `day_book_queue_snapshots` (v2.3736) — what a dev or controller's Dashboard counted as still waiting on a day (`kind` ∈ deposits · contracts · billing, `n`, `taken_at`, `taken_by`; PK `(day, kind)`, the last look of the day wins), written by `record_day_book_queue(p_day, p_counts)` (dev · controller) from the Needs You card's own figures and, since v2.3801, the Ready to Bill stage's count (`billing`), read by the Day book's queue beside the approvals it reconstructs. RLS select: dev, controller; no client insert.
- `schedule_block_events` (v2.3726) — the schedule's ledger: one row per `job_schedule_blocks` add / move / reassign / remove (`change`, `old` / `new` window + assignee, `block_id`, `job_id`, `bid_id`, `assignee_user_id`, `work_date`), stamped `actor_user_id = auth.uid()` by SECURITY DEFINER triggers (`job_schedule_blocks_to_ledger`); NULL actor = a service-role or backfill write. RLS select: dev, payroll access, or your own rows; no client insert. Read by the Day book's *Updated the schedule* line.
...
### 5. People Roster
`People.tsx` — six group tabs with a second row of views (v2.2811; map in `src/lib/people/peopleTabGroups.ts`): **People** (Users · Subs · Person · Day book · Who's where), **Pay** (Hours · Payroll `?tab=pay_stubs` · Offsets · Employment · Overhead), **Paperwork** (Contracts · Licenses · Writeups · HR), **Fleet & Housing** (Vehicles · Housing), **Review** (Review · Scoreboard · Activity), **Feedback**. `?tab=<view key>` is unchanged; the group is derived from the view. Tables: `people` (+`account_user_id` login link), `people_pay_config`, `people_hours`, `people_crew_jobs` / `people_crew_bids`, `clock_sessions`, the `pay_stubs` family, `hours_reviewed`, `people_teams`. Identity is `person_id`-keyed; name text is legacy ([PERSON_IDENTITY_PLAN.md](./PERSON_IDENTITY_PLAN.md)). Depth: [PEOPLE_TABS_ARCHITECTURE.md](./PEOPLE_TABS_ARCHITECTURE.md) (page map), [PEOPLE_REVIEW_TAB_ARCHITECTURE.md](./PEOPLE_REVIEW_TAB_ARCHITECTURE.md), [PEOPLE_CONTRACTS_OVERHEAD_TABS_ARCHITECTURE.md](./PEOPLE_CONTRACTS_OVERHEAD_TABS_ARCHITECTURE.md), [SALARY_CLOCK_SESSIONS.md](./SALARY_CLOCK_SESSIONS.md) (salary semantics, 8/0 rule), [CREW_PNL_DATA_FLOW.md](./CREW_PNL_DATA_FLOW.md), [TIME_AND_ZONES.md](./TIME_AND_ZONES.md).

### 6. Jobs Page
`Jobs.tsx` — the jobs pipeline. Tabs: Reports | **Pipeline** (the Stages board, default) | Billing | **Team** (v2.2974: the crew board — jobs × days, clocked hours against the dispatch plan, Board/Ledger; `JobsTeamTab` over `lib/teamBoard.ts`; was Team Labor) | **Subs** (v2.2927: Work Orders + Sub Labor folded into one tab with a Work / Pay switch — `JobsSubsTab` over `JobsSubsWorkView` (was `JobsWorkOrdersTab`) and `JobsSubLaborTab`; stages = line items with a window, `job_stage_windows`; **Stage Plan** v2.3083–v2.3134: the line item's `stage_kind` (In order / Any time since v2.3696; null = a discount row) and `shared_with_gc` (the eye) set on the Bill tab, the Edit tab's Stages read-out + customer drawer, draws follow stages, the GC portal's *Where the job is* card; v2.2819: sub work orders board + assembler, `WorkOrderAssemblerModal`; v2.2865–v2.2867: every row is a sub sheet — `buildWorkOrderBoard` + the seven-dot `SheetRail`, groups by rail position, handshake tiles, Link-to-a-job door; v2.2829: the job window's **Sub work order** fact row + View bill strip via `JobWorkOrderStrip`, and the Needs You `work-orders-unpriced` item) | Crew P&L (dev) | Parts | Job Summary | Inspections. Jobs move waiting → working → ready_to_bill → billed → paid; billing runs through the Bill Customer modal (Stripe hosted invoice / HouseCall Pro record-only / Physical PDF + email). **Contracts** (Contract Desk, v2.2679–v2.2681): every row carries a contract-coverage chip (`job_contracts` + e-signed estimates + signed bid-room proposals, kernel `lib/jobs/jobContractCoverage.ts`); the `JobContractModal` sends the agreement (email / link / text / in person) and the customer signs at `/contract/sign?t=…`. Tables: the `jobs_ledger` family (see [Jobs Ledger Family](#jobs-ledger-family-summary)), `people_labor_jobs` (+items) for Sub Labor, `inspections`. Depth: [JOBS_TABS_ARCHITECTURE.md](./JOBS_TABS_ARCHITECTURE.md) (page map), [JOBS_STAGES_TAB_ARCHITECTURE.md](./JOBS_STAGES_TAB_ARCHITECTURE.md), [JOBS_JOB_SUMMARY_TAB_ARCHITECTURE.md](./JOBS_JOB_SUMMARY_TAB_ARCHITECTURE.md), [JOBS_MODALS_ARCHITECTURE.md](./JOBS_MODALS_ARCHITECTURE.md) (Sub Labor form + Job Detail), [JOB_FORM_MODAL_ARCHITECTURE.md](./JOB_FORM_MODAL_ARCHITECTURE.md) (New/Edit Job), [BILLING_FLOWS.md](./BILLING_FLOWS.md) + [SEND_RECORD_INVOICE_MODAL_ARCHITECTURE.md](./SEND_RECORD_INVOICE_MODAL_ARCHITECTURE.md) (billing), [SCHEDULE_DISPATCH_ARCHITECTURE.md](./SCHEDULE_DISPATCH_ARCHITECTURE.md) (dispatch + schedule blocks).
...
### 6c. Moneyfill
`Moneyfill.tsx` at `/moneyfill` (dev + controller only) — financial queues worked to zero (bank-transfer attribution), kept off the assistants' daily loop. Queues link to the existing fix surface, never a second editor: **Card charges not split to jobs** → per-row **Sort in Banking → User Sort** opens `/banking?tab=sorting&q=<counterparty>` (v2.2849). History: RECENT_FEATURES.md v2.1378.

### 7. Calendar View
`Calendar.tsx` — month grid of the signed-in user's assigned stages, bids and prospect callbacks, planned dispatch blocks, recorded clock time, and salaried-workday / time-off layers, plus the My Day single-day card. All times Central. Tables read: `project_workflow_steps` (+ `project_workflows`, `projects` for names), `bids`, `prospect_callbacks`, `job_schedule_blocks`, `clock_sessions`, `attendance_incidents`, `users`, `people_pay_config`, `user_time_off`, `salary_work_schedule_templates`, `salary_work_schedule_day_overrides`. Depth: [CLOCK_SURFACES_ARCHITECTURE.md](./CLOCK_SURFACES_ARCHITECTURE.md), [SALARY_CLOCK_SESSIONS.md](./SALARY_CLOCK_SESSIONS.md); history: grep RECENT_FEATURES.md (v2.558 wave).

### 8. Dashboard
`Dashboard.tsx` — the role-adaptive home: Clock In/Out with unified job/bid search and dispatch quick-picks, the team clock strip (approve / assign / My-Time day editor), **My crew** (v2.3613 — for anyone who supervised a job-day: reports owed with the report door, the crew's hours read-only; RPC `get_supervised_days_payload`; v2.3614 Rate my crew), **My Team** (v2.3616 — its roster is the crew the viewer supervised this week; approve controls for dev / pay-approved masters / the office only), Ready to Bill / Billed lists for staff, assigned- and superintendent-job cards, My Bids, inbox cards, alert banners (stale tally, unallocated deposits, lost-bid reasons), and the Job Mode / Dispatch Mode phone shells. Tables: `clock_sessions`, `jobs_ledger` (via RPCs), `user_dashboard_goals`, checklist tables. Depth: [DASHBOARD_SECTIONS_ARCHITECTURE.md](./DASHBOARD_SECTIONS_ARCHITECTURE.md) (section map), [MY_TIME_DAY_EDITOR_MODAL_ARCHITECTURE.md](./MY_TIME_DAY_EDITOR_MODAL_ARCHITECTURE.md), [CLOCK_SURFACES_ARCHITECTURE.md](./CLOCK_SURFACES_ARCHITECTURE.md).

### 9. Settings
`Settings.tsx` — grouped tabs in four zones, role-filtered (`getZonedSettingsGroups`, `src/lib/settingsGroups.ts`, v2.2088): **You** (Your account, Your dashboard), **Company** (Jobs & billing, Bids & materials, People & teams, Emails & reports, What customers see, Company), **System** (Usage, Data & recovery, Email templates & testing, Digital twins & samples, Advanced, Activity logs), **Help** (Guides, Release notes): profile, adoption/sharing, user management (dev), email templates, org toggles in `app_settings`, and My email schedule ([REPORT_SUBSCRIPTIONS.md](./REPORT_SUBSCRIPTIONS.md)). Depth: [SETTINGS_TABS_ARCHITECTURE.md](./SETTINGS_TABS_ARCHITECTURE.md). Data & recovery also holds the dev-only **Draft bills on paid jobs** sweep (v2.2846): never-sent draft bills sitting on Paid-in-Full jobs, retired as audited deletes into Recently deleted.
...
`Materials.tsx` — tabs: Supply houses (AP invoices) | Held for suppliers | PO Generator | Parts Book | Assembly Book | PO Builder | Purchase Orders. Tables: `material_parts`, `material_part_prices` (+ history via the `track_price_history()` trigger), `material_templates` (+items, nestable), `purchase_orders` (+items), `supply_houses`, `supply_house_invoices`, `material_po_generator_entries` — schema details above under Materials Management Tables. Depth: [MATERIALS_TABS_ARCHITECTURE.md](./MATERIALS_TABS_ARCHITECTURE.md). The **Price coverage** button/modal (named "Supply Houses" until v2.2903) reports how many parts carry a price at each supply house.
...
### 13. Bids Management
`Bids.tsx` — the estimating pipeline: Bid Board (+ the 🤖 group tab, v2.2527 — the Robot Board, a mirror of the human board with a robot column since v2.3222, and Audits, the robot-feedback review lane (v2.2517: quick links, twin questions, sectioned notes, receipts; tables `bid_audits`/`bid_audit_notes`), as lenses; the robot's envelope opens at send, v2.3222), Followup, Unsent/Working Kanban, Counts → Takeoff → Cost Estimate → Pricing (price-book versions, margins) → Cover Letter → Submission, plus RFI / Change Order / Lien Release. **[BIDS_SYSTEM.md](./BIDS_SYSTEM.md) is the authoritative doc** for all tabs, workflows, and integrations; component maps: [BIDS_TABS_ARCHITECTURE.md](./BIDS_TABS_ARCHITECTURE.md), [BIDS_TAKEOFF_TAB_ARCHITECTURE.md](./BIDS_TAKEOFF_TAB_ARCHITECTURE.md), [BIDS_PRICING_LABOR_TABS_ARCHITECTURE.md](./BIDS_PRICING_LABOR_TABS_ARCHITECTURE.md), [BID_SUBMISSION_FOLLOWUP_TAB_ARCHITECTURE.md](./BID_SUBMISSION_FOLLOWUP_TAB_ARCHITECTURE.md). Core tables: `bids`, `bids_count_rows`, `bids_submission_entries`, `cost_estimates` (+labor rows), `price_book_versions`/`price_book_entries`, `bid_pricing_assignments` — schema details above under Bids Management Tables.
...
- **Page**: [`Prospects.tsx`](../src/pages/Prospects.tsx) with two top-level tabs:
  ...
  - **Team** — the **prospective-hires board** ([`TeamProspectsTab`](../src/components/prospects/TeamProspectsTab.tsx)): candidate cards in `team_prospects` with roles in `team_prospect_roles` (`20260717190000`, `20260717230000`), including per-source success tracking (v2.715). Access is per-user via `users.team_prospects_access` (`20260717250000_team_prospects_access_flag.sql`, helper `user_has_team_prospects_access()`, v2.714) — prospects-staff role alone is not enough. The **Try-out** stage (v2.3627) holds trial helpers; each card carries the tally and the nudge (v2.3715 — `team_prospect_trial_tally()` read into `src/lib/hiring/trialTally.ts`; *Keep trying* stamps `trial_deferred_at / _by`). A column can be **shared** with an account that has prospects staff access but not the switch (v2.3798 `team_prospect_role_shares` + `user_hiring_shared_role_ids()` in the policies; v2.3802 the header's ⋯ → *Share with…*, `ShareColumnDialog`, and the *Hiring columns shared* line in Settings → Active accounts; v2.3805 the share holder's tab — the Hiring pill opens for a share (`useMyHiringShares`) and the board renders `shared`, trimmed by `src/lib/hiring/sharedHiringTab.ts`).
...
│   │   ├── Jobs.tsx           # Jobs (Reports, Stages, Billing, Team, Subs [Work / Pay], Crew P&L, Parts, Job Summary, Inspections)
