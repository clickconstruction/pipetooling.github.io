...
### 8. Dashboard
`Dashboard.tsx` — the role-adaptive home: Clock In/Out with unified job/bid search and dispatch quick-picks, the team clock strip (approve / assign / My-Time day editor), **My crew** (v2.3613 — for anyone who supervised a job-day: reports owed with the report door, the crew's hours read-only; RPC `get_supervised_days_payload`), Ready to Bill / Billed lists for staff, assigned- and superintendent-job cards, My Bids, inbox cards, alert banners (stale tally, unallocated deposits, lost-bid reasons), and the Job Mode / Dispatch Mode phone shells. Tables: `clock_sessions`, `jobs_ledger` (via RPCs), `user_dashboard_goals`, checklist tables. Depth: [DASHBOARD_SECTIONS_ARCHITECTURE.md](./DASHBOARD_SECTIONS_ARCHITECTURE.md) (section map), [MY_TIME_DAY_EDITOR_MODAL_ARCHITECTURE.md](./MY_TIME_DAY_EDITOR_MODAL_ARCHITECTURE.md), [CLOCK_SURFACES_ARCHITECTURE.md](./CLOCK_SURFACES_ARCHITECTURE.md).
