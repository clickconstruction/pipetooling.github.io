...
- `day_book_queue_snapshots` (v2.3736) — what a dev or controller's Dashboard counted as still waiting on a day (`kind` ∈ deposits · contracts · billing, `n`, `taken_at`, `taken_by`; PK `(day, kind)`, the last look of the day wins), written by `record_day_book_queue(p_day, p_counts)` (dev · controller) from the Needs You card's own figures, read by the Day book's queue beside the approvals it reconstructs. RLS select: dev, controller; no client insert.
