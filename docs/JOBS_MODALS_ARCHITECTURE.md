...
21. **Limited body's "Last bill date" is always `—`** (derived billing needs invoices/payments, which the limited fetch omits); the full body's "Last bill date" is *derived activity* (`deriveRecordedBillingActivityDetail`) while "Last manual bill date" is the raw `last_bill_date` column — three deliberately different date semantics.
