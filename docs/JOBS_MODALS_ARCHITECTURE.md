...
> **v2.2909 — New Job papercuts (journey-map B5).** In `JobFormModal`: `clickNumberSuggesting` brackets the `next_job_number_suggestion` RPC (the C# box in `JobFormIdentityFields` shows "finding…" + a status caption; the late suggestion fills only an empty box). `JobFormHeaderRow` renders Import for every new-job render via `newJobImportButtonState` (`lib/jobs/jobFormRows.ts`) — greyed with `NEW_JOB_IMPORT_BLOCKED_HINT` as title once `newJobFormHasBlockingContent` is true, tap → `onImportBlockedClick(hint)` → toast; never the `disabled` attribute. `applyPrefillFromBid` no longer back-fills `bids.agreed_value` silently: it carries `sentValue` (forced/picked packet → auto-resolved winner → lone packet) and asks "Start the job at $X?" before filling; Yes = one "Bid price" fixture row + the agreed-value write (only when null), No = no write anywhere. The delete confirm (`JobFormDeleteMigrateModals`) formats hours with `approxHoursLabel` and names the tab **Data & recovery**.
...
- **Owned local state:** `laborModalOpen`, `laborAssignedTo` (written by S2/S7, read by save/print), `laborAddress`, `laborDistance` (string, default `'0'`), `laborJobNumber` (HCP / click number, display text — no length cap since v2.3435; the picked job's id travels as `laborPickedJobId`), `laborDate` (default `new Date().toLocaleDateString('en-CA')`), `laborSaving`, plus `serviceTypes`/`selectedServiceTypeId` (loaded on open; feeds S6's fixture-type scope).
...
- **Move to job… on customer payments (v2.3576):** `JobFormPaymentsTable` takes `requestMovePaymentRow`; the form holds `paymentMoveRow` and mounts [`JobPaymentMoveModal`](../src/components/jobs/JobPaymentMoveModal.tsx) (job search via `search_jobs_ledger`, the What-changes plan from `jobPaymentMove.ts`, one RPC `move_jobs_ledger_payment`), then re-reads the job. The table draws the job's trace lines from `jobs_ledger_payment_events` through `useJobPaymentTrace`.
- **Move / Remove (v2.3562):** two more optional openers, `onOpenMovePayment` / `onOpenRemovePayment`, route to the sibling [`SubLaborPaymentMoveRemoveModals`](../src/components/jobs/SubLaborPaymentMoveRemoveModals.tsx) (its own handle: `openMove` / `openRemove` / `clear`; the Edit dialog's Remove also hands off through `SubLaborPaymentModals`' `onRequestRemove`). The Payments table draws the sheet's trace lines from `payment_events` via `subPaymentTraceLines` and offers `restoreLaborJobPayment` (Undo) on a removal; on a phone (`useIsMobile`) the row shows Edit and a ⋯ menu. The hook `useSubLaborLedger` loads `people_labor_job_payment_events` fail-soft and exposes the three RPC mutations.
