...
| `job-accounts` | Held for suppliers | always-mounted `<MaterialsJobAccountsTab active={…}>` | ~15 | **born extracted (v2.2652)** ([`MaterialsJobAccountsTab`](../src/components/materials/MaterialsJobAccountsTab.tsx); kernel [`lib/materials/jobAccountsFlow.ts`](../src/lib/materials/jobAccountsFlow.ts)) | 0 in parent | low (`onOpenSupplyHouse` writes `supplyHouseToAutoOpen` + switches to supply-houses) | — | Done |
...
- **Extraction status:** **Done.** The Parts Book toolbar's Price coverage modal is stats-only since v2.3168 (quirk 16 closed); the one house editor is `useSupplyHouseEditor`.
- **Job accounts roster (v2.3423):** the expanded house renders [`SupplyHouseJobAccountsRoster`](../src/components/materials/SupplyHouseJobAccountsRoster.tsx) above Invoices — `job_supply_house_accounts` rows for the house beside the evidence rows (jobs with invoices allocated here and no account), kernel [`jobSupplyHouseAccounts.ts`](../src/lib/materials/jobSupplyHouseAccounts.ts); Mark opened / Not needed open the shared [`MarkJobAccountOpenedModal`](../src/components/materials/MarkJobAccountOpenedModal.tsx). The house form carries the `job_accounts` policy chips; the contacts block carries `role` + `phone`. The Accounts-payable "Job accounts" section is retitled *Job account packets sent* (it is the v2.1606 send log, not the accounts).
...
16. **Closed (v2.3168).** The Parts Book toolbar's **Price coverage** modal used to carry a second copy of the supply-house CRUD (add button, inline form, name / contact / phone / email list with Edit) — the only door estimators had after v2.2903 renamed the button. It is now the coverage table only, with a link to the Supply houses tab for roles that have it (`canOpenMaterialsTab`); house editing is `useSupplyHouseEditor` behind the Directory pane (v2.3166) and the estimator's tab (v2.3167).
