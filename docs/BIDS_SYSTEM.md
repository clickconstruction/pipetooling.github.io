...
- **Robot Board is a mirror of this board** (v2.3222): kernel [`robotMirror.ts`](../src/lib/bids/robotMirror.ts), lens [`BidsRobotMirrorTab`](../src/components/bids/BidsRobotMirrorTab.tsx). One row per HUMAN bid with a robot run, in the same five sections (`getSubmissionSectionKey`), a four-stat strip (bids with a run · live plumbing bids shadowed · audits waiting · axes past Gate B), and a robot column: `queued` / `working` / `sealed` before send (status only — `list_shadow_runs` NULLs the money and the kernel never reads a live shell's draft), the robot's number, ours and Δ after (`scored` from `twin_shadow_runs` / `twin_run_scores`; `audited` = audit filed and priced but no ledger row, draft total on a sent bid only). Newest run leads, earlier rounds fold. Doors: **Review now** (envelope, audit waiting), **Open audit** (`focusAuditId` on the Audits lens), **Compare**, **Robot bid b###** (the shell's Counts tab). The ZZ shells list nowhere; the row's footer names shells that pair to no bid.
