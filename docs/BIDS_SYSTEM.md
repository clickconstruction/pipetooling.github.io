...
- One tab between Bid Board and Followup, three lenses: **Robot Board** (`?tab=robot-board`), **Audits** (`?tab=audits`), **Scoreboard** (`?tab=robot-scoreboard`, dev). The dev **Queue** keeps `?tab=robot-queue` and opens from Settings → Digital twins; `?tab=robot-shadows` (the v2.2544 Shadows lens) redirects to the Robot Board. Landing: Audits when work is pending, else the Robot Board.
