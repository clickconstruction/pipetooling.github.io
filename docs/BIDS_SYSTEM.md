...
- One tab between Bid Board and Followup, three lenses: **Robot Board** (`?tab=robot-board`), **Audits** (`?tab=audits`), **Scoreboard** (`?tab=robot-scoreboard`; every audit role since v2.3221 — `canWorkRobotAudits` — rewritten in plain words by [`robotScoreboard.ts`](../src/lib/bids/robotScoreboard.ts): job types instead of axes, *N of 5 in a row*, a *Your part* strip from `robotRowState`, live-bid runs with the sealed-envelope stepper, practice runs; devs keep the operator notes behind a toggle). The dev **Queue** keeps `?tab=robot-queue` and opens from Settings → Digital twins; `?tab=robot-shadows` (the v2.2544 Shadows lens) redirects to the Robot Board. Landing: Audits when work is pending, else the Robot Board.
