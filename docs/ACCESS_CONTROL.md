...
- **People → Teams** (`?tab=teams`): manage **`team_leader_assignments`** (leader→member links for Dashboard **My Team**); renders the **same shared `TeamLeadsManager`** as the People → Users **Team leads** modal (leader-centric collapsible cards, leader-or-member search, **+ New leader** / per-card **+ Add member**); per-link **Leader dashboard** visibility (**Full** vs **Strip** segmented toggle) — **dev-only**
...
- **People → Teams** (`?tab=teams`): manage **`team_leader_assignments`** (same shared `TeamLeadsManager` as the People → Users **Team leads** modal); per-link **Leader dashboard** (**Full**/**Strip**) — **dev-only**
...
| **Teams** tab (`?tab=teams`): manage **`team_leader_assignments`** (add/remove leader→member links; leader-centric collapsible cards; leader-or-member search) — the **same shared `TeamLeadsManager`** as the Users tab's **Team leads** modal; **Leader dashboard** visibility (**Full**/**Strip** toggle) **dev-only** | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ |
