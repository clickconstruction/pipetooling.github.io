...
- **What**: A `BEFORE UPDATE OF role, read_only, archived_at, is_sample, needs_supervision` trigger (`users_guard_privileged_columns()`) on **`public.users`** blocks self-privilege-escalation. Only a **`dev`** may change **`role`** or **`is_sample`**; a dev, controller or pay-approved master may change **`read_only`** (never their own row, except a dev — v2.2713); a dev, master or assistant may change **`needs_supervision`** (never their own row, except a dev — v2.3611); **`archived_at`** is changeable only by the archive/restore edge flow.
