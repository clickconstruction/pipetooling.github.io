...
- **What**: A `BEFORE DELETE` trigger (`archive_deleted_record()`, `SECURITY DEFINER`) snapshots the full deleted row (`to_jsonb(OLD)`) of high-value tables into **`public.deleted_records_archive`** — `jobs_ledger` + its ON DELETE CASCADE closure, `jobs_ledger_invoices`, `reports`, and `bids` + its cascade closure (**83 tables** as of v2.702 — the jobs and bids trees plus customers→projects→workflows, estimates, payroll, time, supply houses/AP, sub labor, purchase orders, the material catalog, licences and writeups) — grouped by the top-level job/bid, with `deleted_by`/`deleted_at`. Fires for all delete paths (direct `.delete()`, delete RPCs, cascade); guarded so it never blocks a delete. Rows auto-purge after 90 days (`pg_cron`).
