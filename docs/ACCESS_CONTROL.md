...
- **People → Users → Team leads modal**: manage **`team_leader_assignments`** (leader→member links for Dashboard **My Team**) via the shared `TeamLeadsManager` (leader-centric collapsible cards, leader-or-member search, **+ New leader** / per-card **+ Add member**); per-link **Leader dashboard** visibility (**Full** vs **Strip** segmented toggle) — **dev-only**. (The former **Teams tab** was removed in v2.1292; `?tab=teams` redirects to Users.)
...
- **People → Users → Team leads modal**: manage **`team_leader_assignments`** via the shared `TeamLeadsManager`; per-link **Leader dashboard** (**Full**/**Strip**) — **dev-only**
