...
- **People → Feedback** (`?tab=feedback`, dev-only; v2.2824): team feedback admin (**`TeamFeedbackDevSettingsBlock`**) — **Enabled** persists to **`team_feedback_settings`**; pills **Ratings** (crew lane vs office lane per person, crew notes, dev-only **Who rated**), **Open words** (writer name dev-only), **Who's due**, **Settings**, **Retired questions**; **Try the deck** preview. Same surface as **Settings → People & accounts → Team feedback**. **Crew ratings are anonymous to everyone but dev**: `team_member_reviews` rows with `source='crew'` are SELECT-able only by dev and their writer; the office reaches them only through `crew_review_aggregates()` (averages + rater count, months with 2+ raters). Any signed-in account may write crew rows about someone else (the clock-out deck).
