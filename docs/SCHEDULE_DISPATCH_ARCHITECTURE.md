...
- **Cross-tab/shared state (props):** `rows: ScheduleDispatchHubMergedRow[]` (page's `hubMergedRows`), `bidRows: ScheduleDispatchHubBidMatrixRow[]` (page's `hubBidMatrixRows` — bids with a block this week, v2.2901; kept out of `hubMergedRows` because that array also feeds the assign picker and `?placeJob=`), `loading`, `jobsError`, `summariesError`, `visibleDayKeys`, `hideWeekend`+`onHideWeekendChange` (shared with People's View menu — the same page state `hideWeekend`), `onOpenJob`, `scheduleTodayYmd`, `columnFocusDayYmd`, `columnScrollKey`.
- **Derived memos:** `filteredRows` (search over `hcp_number`/`job_name`/`displayTitle`, then `totalBlocks > 0` filter); `filteredBidRows` (search over `displayTitle` only — bid rows exist only because they have blocks, so the checkbox is moot for them).
- **Handlers:** none of its own beyond the two filter inputs; row/Open buttons call `onOpenJob(jobId)` (navigates to the JobWeek grid via `?jobId=`; bid rows pass their `bid:<uuid>` anchor through the same callback, which `scheduleBlockTarget` routes to Edit Bid — see the anchors note atop this doc).
...
- **Hidden blocks (v2.2892):** the panel also receives `hiddenBlockCounts` (rows from `schedule_hidden_block_counts`, superintendent only — `[]` for office roles). Memos `expectedManpowerHiddenSelection` / `expectedManpowerHiddenWeekHours` (`scheduleHiddenManpowerForDayKeys`) feed `formatManpowerWithHidden` so the stats line reads `**83** person-hours · 38 on your projects · 4 jobs · 6 people · 3 busy elsewhere`, the empty-selection copy becomes "No blocks on your projects for … · N person-hours busy elsewhere", and the week footer gains "· N on your projects". With nothing hidden every string is unchanged. Kernel: [`lib/scheduleHiddenBlocks.ts`](../src/lib/scheduleHiddenBlocks.ts) (tested).
...
- **Role gate (v2.2892):** all three entry points (`onMarkNotComingInForCell`, `onRequestUndoNotComingIn`, the picker's `notComingIn` option) are passed only when `canTimeOff = canEdit && canWriteTimeOff(role)` — [`lib/scheduleDispatchEditRoles.ts`](../src/lib/scheduleDispatchEditRoles.ts) mirrors the RPC gate (dev / master_technician / assistant / controller). `canEdit` alone armed 63 dead `off` buttons for a superintendent (J18-N2). The same file now separates `CAN_VIEW_SCHEDULE_DISPATCH_ROLES` (page gate) from `CAN_USE_SCHEDULE_DISPATCH_EDIT_ROLES` (block edits).
...
- **Share modal internals (v2.2919, J18-F8/F9):** `ScheduleShareModal` registers with the Tier-2 #42 modal stack (`useModalStackEntry(open)`) and closes on Escape only when topmost and not sending, ignoring keys the recipient `SearchableSelect` already consumed (`ev.defaultPrevented`). Its Send now tab renders [`ScheduleEmailPreviewPanel`](../src/components/schedule/ScheduleEmailPreviewPanel.tsx) fed by `supabase.rpc('list_schedule_blocks_for_share', { p_viewer: me, p_start, p_end })` — the same call the edge function's instant path makes from the sharer's visibility — shaped by [`lib/scheduleSharePreview.ts`](../src/lib/scheduleSharePreview.ts) (mirrors `_shared/scheduleShareCore.ts`: Include-set filter, `— · Job` / `(Unassigned)` fallbacks, per-person grouping). `ScheduleDayEmailModal` (Dashboard clock strip) shares the panel + Escape handling and previews `list_job_schedule_blocks_for_schedule_email(p_recipient, p_work_date)` flat in time order.
- **Supabase:** `job_schedule_blocks` (delete/update via lib); read-only preview RPCs `list_schedule_blocks_for_share` / `list_job_schedule_blocks_for_schedule_email` (SECURITY DEFINER, granted to `authenticated` in the baseline).
