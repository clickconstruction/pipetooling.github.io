...
- **Main-row cells:** `effectiveJobLedgerNumber(job.hcp_number, job.click_number)`; Team Labor and Revenue-before-Overhead render `—` unless `showTeamLaborAndProfit` (v2.660 pay lockdown — parent passes `authRole === 'dev' || 'master_technician' || 'controller'`); negative profit paints literal `#b91c1c`; `%` column = `formatJobSummaryPercentComplete(resolveJobSummaryPercentComplete(jobSummaryReportPctByJobId.get(job.id) ?? null, job.pct_complete, { invoicesAllPaidWithAmount: jobInvoicesAllPaidWithAmount(job.invoices) }))`.
