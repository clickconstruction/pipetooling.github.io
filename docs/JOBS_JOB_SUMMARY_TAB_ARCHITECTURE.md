...
- **Main-row cells:** `effectiveJobLedgerNumber(job.hcp_number, job.click_number)`; Team Labor and Revenue-before-Overhead render `—` unless `showTeamLaborAndProfit` (v2.660 pay lockdown — parent passes `authRole === 'dev' || 'master_technician' || 'controller'`); negative profit paints literal `#b91c1c`; `%` column = `formatJobSummaryPercentComplete(enriched.pct)`, where [`jobSummaryLedgerView.ts`](../src/lib/jobs/jobSummaryLedgerView.ts) `enrichJobSummaryRows` resolves `pct`/`pctSource` via `resolveJobSummaryPercentCompleteWithSource(reportPct, job.pct_complete, jobSummaryPaidInvoiceOpts(job.invoices, contractUsd))`. The paid-invoices → 100 branch fires **only when Σ invoices covers the contract** (`paidInvoicesCoverContract`, tolerance max($1, 0.5 %)); otherwise the chain falls through to the latest report % → `pct_complete` → null. `finished = pct === 100` and the contract-vs-earned Revenue switch inherit the gated %, and so does every kernel built on the enriched rows (Months, Scatter, Cycle, Ahead, Cut-by, Compare, the `jobs-stale-open` Dashboard card). Same resolver + opts in the Quickfill "Complete, no Total Bill" list (`quickfillCompleteNoBill.ts`) and the Cost Timeline's fallback value point (`resolveJobCurrentPercentFallback`). **Provenance badge (v2.2852):** the % cell renders [`PercentProvenanceChip`](../src/components/jobs/PercentProvenanceChip.tsx) under the number from `enriched.pctSource` (`percentProvenanceLabel` in [`lib/jobPercentProvenance.ts`](../src/lib/jobPercentProvenance.ts) — "crew report Aug 27" · "set by office" · "fully collected"); the date comes from the RPC's `reported_at` (`currentReportDateByJobId` → `jobSummaryReportDateByJobId`, v2.3441) so it shows on the collapsed row, with the lazy `jobSummaryReportsByJobId` cache via `latestReportPercent` as the fallback once the row has been expanded. The Gross / True profit headers take label + tooltip from [`lib/jobs/profitLabels.ts`](../src/lib/jobs/profitLabels.ts) (`PROFIT_FIGURE_LABELS`), and the chart receives `teamLaborIncluded={showTeamLaborAndProfit}` so its green line reads "cash position" ("before team labor" when wages are stripped). `useJobSummaryView` records one `job-summary-view` row in `ui_nav_clicks` per open (`status`, `revenue` mode).
...
- **Render location:** after the table: centered box "Hide older imported jobs with HCP # at or below [n]", the footer sentence from `jobSummaryFloorFooter` (`src/lib/jobSummaryFooterCopy.ts`, v2.2914 — "417 shown · 398 older imported jobs (HCP # 500 and below) hidden by the default HCP # 500 floor" + a **show all** button that sets the floor to −1; "All N jobs shown · no HCP # floor" at −1), and the help copy ("Jobs with no HCP # (or a non-numeric one) always show. Set the floor to −1 to show every job."). The hidden count is `jobSummaryHiddenByMinHcp` from `useJobSummaryData` = the fetch's `hiddenByMinHcp` (rows the server-side floor dropped before enrichment) + any client-side raise of the floor; before v2.2914 the footer compared two already-floored lists and read "417 of 417" by construction (journey map J6-7).
