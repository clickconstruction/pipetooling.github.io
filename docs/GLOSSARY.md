...
### Bid Version ("Bids in this package") / Price scenario
The two variant levels inside a bid, re-anchored to purpose in v2.2110: a **version** (`bid_versions`) is a **separate bid you can send** — its own takeoff mappings (`bids_takeoff_template_mappings.bid_version_id`) and rough-in lines, its own prices, optionally its own GC (`bid_versions.customer_id`, v2.1159), over **its own counts** (`bids_count_rows.bid_version_id`, v2.2132 — before that, counts were shared by every version of a bid) — and versions bundle into the Cover Letter as separately-priced sections via `include_in_submission` + `sort_order`. A **price scenario** (bid-scoped `price_book_versions` row) is **the same bid at a different price point** — same counts, different sell prices; the ★ starred scenario (`bids.selected_price_book_version_id`) is what the customer sees (Cover Letter, Share, bid value). UI: version picker "Bids in this package — each goes to the customer on its own" ("One bid" when unsplit, v2.2122) on the Takeoff/Pricing/Cover Letter tabs; Workbench solo bids collapse to one line with "＋ New price or version…" (v2.2104/v2.2110); door options "Another price point — for you to compare" / "Another bid to send — the customer sees it" (v2.2122). Word rule since v2.2122: *version* = sendable bid, *scenario* = price point, *book* = catalog (takeoff/labor/price book editions are "books", never "versions", in the UI). **Adopt** (v2.2137) = the reverse of split: an existing board bid becomes a version of another bid (`adopt_bid_as_version`; source row keeps `adopted_into_bid_id`). **Cover Letter New (v2.2117)** bundles *versions* at their ★ (`bid_versions.starred_price_book_version_id`), each **Base** or **Alternate** (`bid_versions.is_alternate`); the letter total = sum of base bids; the picker badge ("in letter ✓ · base/alternate") and the bundle read the one flag `bid_versions.include_in_submission`. Old still bundles checked price scenarios. Rule of thumb: **another price point → new scenario; another bid to send → new version.**
