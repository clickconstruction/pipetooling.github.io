...
### Lifetime value (LCV)
Everything ever billed to a customer — Σ billed/paid invoice amounts per job, with a job-shell (`revenue`) fallback for billed/paid jobs that predate invoice rows. The HouseCall Pro "customer lifetime value" equivalent. Shown on the Customer Hub money strip (collected beneath), the Invoices tab lifetime footer, and the Customers list rows ("$ Top customers" sort). Kernels: `customerProfileStats.lifetimeBilled`, `customersListLcv.ts` (keep-in-sync pair).
