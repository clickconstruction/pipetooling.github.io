...
**Quick add** (v2.3655): 5–30 minutes, in fives, that an hourly office person adds *after* an off-hours call or email instead of clocking in for it — an ordinary finished session on the Office job, pending approval like any other, marked `quick_add_minutes` so the approver can tell it from a punch. Written only by `add_quick_time()`, which holds the rules (today only, never while clocked in, never over existing hours, a daily ceiling in `app_settings.quick_add_daily_ceiling_minutes`). Plan and mock-up: `to-dos/quick-time-add`.
