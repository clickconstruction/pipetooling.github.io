...
### twin-mcp
The MCP server an agent holds a twin seat through, at `https://mcp.clicktooling.com/twin` (the **mcp-router** Cloudflare Worker, a secret-less pass-through in front of the edge function; `/dev` is reserved for the devs' server — [`to-dos/mcp-servers.md`](../to-dos/mcp-servers.md)). Per-twin revocable tokens; every verb by family in [`EDGE_FUNCTIONS.md`](./EDGE_FUNCTIONS.md). The pipeline's steps are fenced, ledger-stamped verbs and `mint_session` opens a signed-in browser for the rest; composite reads (`get_work_state`) + the bid-note ledger are a stateless agent's memory. Sending, outcomes and costs have no verb — they stay human acts.
