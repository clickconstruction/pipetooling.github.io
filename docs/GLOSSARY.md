...
**Key Fields**: name, trade, source, status, role_id, rank_order (per-column), last_contact, master_user_id, trial_user_id, trial_started_at

**Trial verdict** (v2.3650): a leader's one-tap *take them again?* — yes / no / not sure + an optional word — on a **trial helper** (a helper on the Try-out stage), for one day. Asked of whoever could run the helper's job that day (the Supervision rule, read off the schedule and the clock), when the helper clocks out: a push, a card on top of the Dashboard, and the leader's own clock-out prompt. **By name**, unlike the anonymous crew deck; one row per card, leader and day.
