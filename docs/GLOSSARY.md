...
**v2.3055.** A sub labor sheet's job is a real foreign key, `job_ledger_id` → `jobs_ledger.id`; `job_number` stays as the display text. Set by the Sub Labor form when a job is picked, by **Subs → Work → Link this sheet**, and by `create_sheet_for_work_order` from the work order's job; a BEFORE trigger (`people_labor_jobs_link_job_ledger`) resolves the link from the number for writers that only set `job_number` (via `resolve_job_ledger_id_by_number`: trimmed, case-insensitive, HCP before click number, newest job wins) and fills the number from the job when only the link is set. Readers should use the link first and the number match only as a fallback — the Team board, the stage → activity trigger and the superintendent access helper do; the rest are listed in [`to-dos/sub-sheet-job-link-followups.md`](../to-dos/sub-sheet-job-link-followups.md). To unlink a sheet, clear both columns.
