...
- ~150 lines of **module-level pure helpers** (`formatDatetime`, `formatDateShort`, `daysOpen`, `daysBetween`, `formatAmount`, `formatLineItemDate`, `ymdFromDateLike`, `formatScheduledDateShort`, `ymdAddDays`, `ymdDaysBetween`, `getStepStatusStyle`) and the small `PersonDisplayWithContact` module component.
...
- A second module-level component, **`StepFormModal`** (~line 4278–4782, ~505 lines), with its own ~17 `useState`, 1 `useEffect`, its own supabase loads, and a nested Add Person sub-modal.
...
4. **Nothing is extracted yet.** There is no `src/components/workflow/` directory. The only already-external pieces are `JobThreadNotesPanel` + `useJobThreadNotes` (shared with Jobs), `EditProjectModalContext`, and the lib helpers `parseWorkflowLineItemPaste`, `parsePercentCompleteInput`, `formatProjectNumberLabel`, `toDatetimeLocal`/`fromDatetimeLocal`, `isAssistantLike`/`isSubcontractorLikeRole`.
...
| `StepFormModal` (+ Add Person) | module component after `Workflow()` | ~505 | in-file module component | ~17 (self-contained) | props-only (`steps`, `onSave`, `onCopy`) | **low** | **Extract first** → `src/components/workflow/StepFormModal.tsx` (verbatim file move) |
...
| Contact modal + `PersonDisplayWithContact` | `personContactModal` block; module component | ~120 | inline / in-file component | 1 (`personContactModal`) + `personContacts`, `userNames` | rendered inside every stage card row | low | move component to own file; modal stays page-level |
...
- **Extraction status + risk + approach:** Inline. **Low risk — the Stage-A opener.** Move formatters to `src/lib/workflow/workflowFormat.ts` + tests; promote the ymd helpers into `src/utils/dateUtils.ts` (or `src/lib/workflow/ymd.ts`) and point the two Forecast libs at them in a later pass. `PersonDisplayWithContact` → `src/components/workflow/PersonDisplayWithContact.tsx` verbatim.

### `StepFormModal` — Add/Edit step modal (in-file module component)

- **Render location:** `{stepForm.open && <StepFormModal …/>}` in the parent; component defined after `Workflow()` (~line 4278–4782).
...
- **Extraction status + risk + approach:** In-file module component. **Lowest risk on the page — extract first.** Verbatim move to `src/components/workflow/StepFormModal.tsx`; parent wiring (`stepForm` state, `saveStep`, `copyStep`, `closeStepForm`) unchanged. No Stage-A prerequisite (its logic is IO + trivial filtering).
...
- **Sub-components:** `PersonDisplayWithContact` (in-file); everything else inline JSX (two inline SVG icon paths for edit/delete/clipboard/link).
...
2. **`StepFormModal` → `src/components/workflow/StepFormModal.tsx`** — verbatim move of an already-module-level, props-clean component (~505 lines out in one low-risk commit). Move `PersonDisplayWithContact` to its own file in the same wave.
