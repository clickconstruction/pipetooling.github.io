...
- **Close:** `closeForm()` is a **guarded async close** (v2.1077): it cancels the billing-autosave debounce and, if the money slice is dirty or a write is in flight, runs `flushDirtySliceForClose` (kernel [`jobFormCloseFlush.ts`](../src/lib/jobs/jobFormCloseFlush.ts)) under a 15s `withOperationTimeout` before resetting nested-overlay state and calling `onClose()`. On failure it keeps the modal open with Retry / Keep editing / Close-without-saving (`closeFlushState`). Callers that navigate after closing (trade pill → Stages, Job Detail bridge) await the returned boolean. Delete and migrate+delete use `closeFormWithoutSaving()` — the job row is gone, flushing would reinsert children. A `visibilitychange → hidden` listener flushes the pending debounce. Non-money fields still have **no unsaved-changes guard** — those edits are silently discarded on close (quirk #15).
