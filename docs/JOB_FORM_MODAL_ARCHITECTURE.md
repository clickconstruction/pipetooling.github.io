...
> **v2.2909 — New Job papercuts (journey-map Tier-3 B5).** `clickNumberSuggesting` brackets the `next_job_number_suggestion` RPC (C# box shows "finding…" + a `role="status"` caption in `JobFormIdentityFields`; the late suggestion fills only an empty box). `JobFormHeaderRow` renders Import for every new-job render via `newJobImportButtonState` (`lib/jobs/jobFormRows.ts`) — greyed with `NEW_JOB_IMPORT_BLOCKED_HINT` once `newJobFormHasBlockingContent` is true, tap → `onImportBlockedClick(hint)` → toast; never the `disabled` attribute. `applyPrefillFromBid` no longer back-fills `bids.agreed_value` silently: it tracks `sentValue` (forced/picked packet → auto-resolved winner → lone packet) and asks "Start the job at $X?" before filling — Yes = one "Bid price" fixture row + the agreed-value write (only when null), No = no write anywhere; `winningGcPick.agreedValue` is gone. The delete confirm (`JobFormDeleteMigrateModals`) formats hours with `approxHoursLabel` and names the tab **Data & recovery**.
...
- **Discount rows (v2.3252+):** the shell's `setFixtures` is a wrapper that runs `syncDiscountRows` on every write (derived negative price, cap, count 1, never a stage), so every reader sees invariant-true rows; hydration goes through `fixtureRowsFromDb` (positions → basis ids; a legacy negative work row becomes a dollar discount). The section renders a discount row via [`JobFormDiscountRow.tsx`](../src/components/jobs/JobFormDiscountRow.tsx) (smart field, twin label, readout, basis checklist, cap, lock) and takes `addDiscountRow`. Segments, the stage plan and the save engine read net dollars through `discountLine.ts`.
...
> **Status:** the two lists merged into one unified **Invoices** list → [`JobFormInvoiceList.tsx`](../src/components/jobs/JobFormInvoiceList.tsx). Since v2.3478 it is a row list (not a table): every bill — draft, open, paid — as chip · amount · actions / who / money, worded by the pure kernel [`invoiceLedgerRow.ts`](../src/lib/jobs/invoiceLedgerRow.ts); the action follows the state (Send bill… / the labelled Text · Copy link · Email cluster / View) and the rare ones sit under a per-row ⋯ menu (discount, Make Stripe bill, Pipeline, who else sees it, memo & footer, Delete draft → `delete_ready_to_bill_invoice` with the `onInvoiceDeleted` fixture-link clear, Send back); a sum line reconciles with the billing-bar tiles; layout is a container query (`.jobInvoiceLedger`, index.css); the expected-pay model comes from `useJobBilledExpectedPay`. It self-sources the router/toast/bill-customer context hooks; every modal action still opens shell-owned state (`billViewInvoice`, `agreedWriteDownInvoice`).
