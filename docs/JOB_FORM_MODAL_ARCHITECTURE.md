...
### Billing-area compaction + segment net billing (v2.1219)
...
- **§7 fixtures compaction** (`JobFormFixturesSection`): count/unit-price are in-border `[× n]`/`[$ amount]` input groups; the per-row secondary line is gone — scope pencil + Stripe-preview eye live inside the name field's border on every row (the eye is the preview's ONLY trigger; the pencil opens/closes-empty/focuses-nonempty), and the Stripe char counter renders only within 100 of the limit.
