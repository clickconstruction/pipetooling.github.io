...
- **Render location:** ~1077–1299 inside the selected-bid card (card itself ~961; gated by `costEstimateCountRows.length === 0 ? 'Add fixtures in the Counts tab first.'`). Above it sits the Materials **By Stage / Combined** toggle calling `openMaterialsModelSwitch('exact'|'rough', 'labor')` (engine-owned confirm modal renders in the parent).
