...
1. **The scenario pricing adapter, four hand-rolled copies in the Pricing tab** — custom-price map + assignment projection → `computeBidPricingRows` → revenue: `packageRowsFromInputs` 1801–1822 (Share/print/CSV of the ★ — where v2.3685 found every row pricing at \$0.00 against the wrong count rows), the `wbScenarioRevenue` effect 1943–1959 (card revenues), the `altVersionData` effect 2120–2136 (alternate revenues), `copyPricesIntoViewedScenario` 2365–2383. Same shape again in `useBidPricingRows` (×2), `BidPackageMapModal`, `BidsCoverLetterTab`, `approvalPdf.ts` (×2), `pricingPage.ts` (×2).
