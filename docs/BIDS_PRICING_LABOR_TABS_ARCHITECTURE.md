...
> **This dossier was rewritten after measuring the file.** The collapsible "This version's prices / Template library" panel the map described is gone: since the Workbench passes it is a right-hand **drawer**, and none of `priceBookSectionOpen`, `panelVersions` or `panelEntries` exist any more (`panelVersionId` does — derived, not state: `templatesMode ? editingTemplateId : selectedPricingVersionId`, and it gates the P5 entry form; pass it down). Anyone extracting here should trust the names below, not the old ones.
...
- **Behavior (v2.3292):** [`lib/bids/costEstimateAutosavePayload.ts`](../src/lib/bids/costEstimateAutosavePayload.ts) `buildCostEstimateAutosavePayload` parses/validates the ten boxes (defaults: driving `0.70`, hours/trip `2`, estimator `$10`/count; travel people ≥1, nights ≥0) and returns the `cost_estimates` values or `{ ok: false, reason }`; on a bad box the effect sets `costEstimateAutosaveStatus = 'invalid'` + `costEstimateAutosaveReason` and the footer reads "Not saved — …" (quirk #4 — the status stuck on `'saving'` — is closed). `laborRowAutosaveUpdate` / `stageAmountRowAutosaveUpdate` are the row payloads (also used by `saveLaborRows`). On success it UPDATEs `cost_estimates` (12 columns incl. the three `purchase_order_id_*` passthroughs), then sequentially UPDATEs every row of `cost_estimate_labor_rows`, `cost_estimate_equipment_rows`, `cost_estimate_permit_rows`, `cost_estimate_subcontractor_rows`, `cost_estimate_waste_rows`, `cost_estimate_other_rows` (N+1 by design), then flashes `'saved'` for 2 s.
...
- **The crew rate and the bottom line (v2.3294, PR 4a):** [`lib/bids/crewRate.ts`](../src/lib/bids/crewRate.ts) (`crewRateFromLedgerDays`, `effectiveLaborRate`) + [`hooks/useBidCrewRate.ts`](../src/hooks/useBidCrewRate.ts) (the Burn section's cached job day ledger, last 90 days × the burden setting; enabled only while New is showing). The view's Crew rate card, Overhead / field hour tile and "bid labor recorded" (from `teamLaborDataForBids`, a new tab prop) are facts; the effective rate feeds the Labor $ tile, completeness and the bottom line (`computeBidCostBreakdown` over the tab's live direct-cost tables). "Save it on the bid" / "use company rate" write through `setLaborRateInput` + `markCell('rate:labor')`, so the L2 autosave persists them like a typed rate.
- **Calibration (v2.3307, PR 5):** [`lib/bids/laborBookCalibration.ts`](../src/lib/bids/laborBookCalibration.ts) (`bookMultiplier`, `entryEvidence`, `calibratedEntryHours`) + [`hooks/useLaborBookCalibration.ts`](../src/hooks/useLaborBookCalibration.ts) (bids on the book → linked jobs → estimate rows, `get_man_hours_by_job`, clock-session dates; enabled only while New shows). The view's Book vs jobs tile, the evidence chip per filled row (`data-testid="labor-evidence-chip"`) and the drawer row under the grid (`labor-evidence-drawer`); **Set** writes `labor_book_entries` hours directly (with `set_note = calibrated ×… · N jobs` since v2.3597) and reloads the applied book.
- **One book (v2.3597, PR 6b):** no `<select>` — the bid's book is its trade's robot book (`laborBookForTrade`, the Bids page's auto-pick; `loadLaborBookVersions` reads `archived_at IS NULL`, robot first). The line under the tiles names it with `bookSummaryWords`. The tile offers *Set ×m on N entries* (`rights.calibrate === 'set'`: `calibrationSetPlan` → per-entry UPDATEs, then the version's proposal cleared) or *Propose ×m* (`'propose'`: UPDATE `labor_book_versions.proposed_*`), and names a standing proposal (`labor-book-proposal`, a leader's *clear*). `viewer` (id + role), `tradeName`, `bookProposal`, `onBookChanged` are the new props; the queue learns into the book only when `rights.edit`, and a learned entry carries `set_note = learned from <bid>`.

### Region L4 — Cost-parameter boxes (Vehicle Travel / Lodging & Meals / Bid labor recorded)

> v2.3294: the **Estimators Time** box (flat / per-count inputs) is retired — replaced by a read-only *Bid labor recorded* line (hours clocked on the bid, from `teamLaborDataForBids`). `estimatorCost` is still computed by the kernel and written by the autosave, but no total, print or PDF adds it; `estimatorTimeCollapsed` state is gone.
...
### Region L6 — Labor Book panel + entry modal + add-missing-fixture modal

- **One book (v2.3597, PR 6b):** the book chips, *Add book* / *Edit book*, the version form and its handlers (`openNewLaborVersion` … `deleteLaborVersion`) are gone. The panel shows `panelBookId = selectedLaborBookVersionId ?? laborBookForTrade(laborBookVersions)?.id`; an effect keeps `laborBookEntriesVersionId` on it (the browse/applied split of quirk 15 is over). New columns: **Hours from** (`entryProvenance` chip, `labor-hours-from`) with **Reset to robot** (`canResetToRobot`; `resetEntryToRobot` writes `robot_*_hrs` back, the trigger clears the stamp). `Add entry` and the pencil need `rights.edit`; `nameOf` resolves `set_by` / `proposed_by` from a `users` read the tab owns. The dossier below is the pre-6b shape.
...
- **Extraction — done v2.3550:** [`BidsLaborBookPanel.tsx`](../src/components/bids/BidsLaborBookPanel.tsx) takes grouped props (`book` / `versionForm` / `entryForm`); every write stays in `BidsLaborTab`. **The add-missing-fixture modal stayed in the tab** — it belongs to the apply-hours flow, which re-runs `applyLaborBookHoursToEstimate` on save. The panel reads `browsedVersionId` only, so the two book selections stay separate (quirk 15). Four render tests. `BidsLaborTab.tsx` 2,155 → 1,940.
...
11. **Delete-pricing-version recovery path:** type-the-exact-name confirm; if the deleted version was active, re-activate via `pickActivePricing({ savedVersionId: null, ... })`, clear entries when none remain, persist + `loadBids`. Recently-deleted restore (90 days) copy references Settings → Data & recovery.
