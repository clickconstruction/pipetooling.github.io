...
1. **The scenario pricing adapter, four hand-rolled copies in the Pricing tab** — custom-price map + assignment projection → `computeBidPricingRows` → revenue: `packageRowsFromInputs` 1801–1822 (Share/print/CSV of the ★ — where v2.3685 found every row pricing at \$0.00 against the wrong count rows), the `wbScenarioRevenue` effect (card revenues — since v2.3841 through the tested `scenarioCardRevenues` in `lib/bids/scenarioCardRevenues.ts`, which prices each scenario on its own bid version's count rows; before, another version's scenario priced against the on-screen rows read \$0 in the legacy all-scenarios fallback), the `altVersionData` effect 2120–2136 (alternate revenues), `copyPricesIntoViewedScenario` 2365–2383. Same shape again in `useBidPricingRows` (×2), `BidPackageMapModal`, `BidsCoverLetterTab`, `approvalPdf.ts` (×2), `pricingPage.ts` (×2).
...
|---|---|---|
| **Scenario pricing adapter** (custom-price map + assignment projection + `submissionHiddenIdsForVersion` → `computeBidPricingRows` → rows / revenue) | hand-rolled at 1802–1817, 1943–1958, 2120–2135, 2365–2383 (and in `useBidPricingRows`) | `lib/bids/scenarioPricingRows.ts` + tests: an alternate ★ on its own count rows (the v2.3685 bug), custom price vs assignment override, hidden rows, fixed price; the four sites and `useBidPricingRows` converge |
...
|---|---|---|---|---|---|
| 1 | **Stage A — `scenarioPricingRows` kernel** (+ `loadScenarioInputs` to `lib/bids`) | ~60 lines, 0 state | none | low | untested money path hand-rolled four times, with a shipped \$0 bug; unblocks 4 and 9 |
