...
3. **`cant-reach` is the only count-gated section** (documented exception in `sectionWouldRenderOnPage`); everything else is role/feature-gated only so page height is stable from first paint.
