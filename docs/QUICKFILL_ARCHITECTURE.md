...
- `noncard-attribution` (v2.1318): "Bank transfers needing attribution" — the unattributed non-card (ACH/wire/check) money-out queue on the v2.1308 substrate RPCs. Mark-less like `my-inbox` (queue empties by resolution; neutral jump chip) but keeps `showOutstandingInHeader` (count reported page-level from `useQuickfillNoncardAttribution`). Eligibility is **capability-probed, not role-derived**: the count RPC is attempted on mount; 42501 → `eligible=false` → section renders nothing (dev + `banking_attributors` holders only). Body `QuickfillNoncardAttributionSection` gets rows/loading/refetch as props; pure logic in `lib/banking/noncardAttributionQueue.ts` (+tests).
...
3. **`cant-reach` is the only count-gated section** (documented exception in `sectionWouldRenderOnPage`); everything else is role/feature-gated only so page height is stable from first paint. Second narrow exception (v2.1318): `noncard-attribution` gates on the count-RPC **permission probe** (42501 → hidden) — capability-based, not count-based; capability-holders see it even at count 0.
