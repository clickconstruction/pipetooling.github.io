...
| 12 | Team Ready to Bill (assigned RTB jobs) | h2 "Ready to Bill (`assignedReadyToBillJobs.length`)" (~7490) | render inline; data in `useDashboardAssignedJobs` (v2.725) | 2 (`Expanded`, `collectPaymentJob`) | med (job-row family; shared modals) | med | Extract with the job-row family (Stage A row helpers first) |
...
- **Render location:** gate `isDashboardTeamReadyToBillRole(role)` (in `lib/dashboardTeamAssignedJobRow.ts` as of v2.725: subcontractor/helpers/primary/superintendent/estimator), h2 "Ready to Bill (`assignedReadyToBillJobs.length`)" (~7490), block ~7481–7775. **This is a third, distinct "Ready to Bill"** — team-assigned jobs via RPC, not the billing-invoice pipeline.
- **Owned local state:** `assignedReadyToBillExpanded` (parent render state), `collectPaymentJob` (~1187, modal in tail); the list + loading live in `useDashboardAssignedJobs` (v2.725).
- **Cross-section/shared state:** writes `viewReportsJob`, `leaveReportJob`, `subcontractorJobActivityModalJob`; `openJobDetailFromDashboardJobRow` (~2842) + `detailModalAssignedJobsRows`; `leaveReportReminderForJobRow` (needs `subScheduleRows`); `submitLinkJobPicturesDispatchRequest`; `refreshAssignedReadyToBill` (hook-owned as of v2.725, also called by `CollectPaymentModal.onFlowChanged`); rows feed My Schedule labels.
- **Handlers/loaders:** loader effect + `refreshAssignedReadyToBill` (RPC `list_ready_to_bill_assigned_jobs_for_dashboard`) — in [`useDashboardAssignedJobs`](../src/hooks/useDashboardAssignedJobs.ts) as of v2.725.
...
- **Sub-components:** `DashboardJobPicturesLinkRow`, `DashboardLeaveReportButton` (own files in `src/components/dashboard/` as of v2.724, shared with My Schedule); `CollectPaymentModal` (tail, subcontractor-like only opener); pure helpers `subcontractorLastActivityBlock`/`TypeLine`/`subcontractorAssignedJobStageDisplay` (module-level) + tested libs `subcontractorJobActivityCopy`/`subcontractorLastActivityCompact`.
