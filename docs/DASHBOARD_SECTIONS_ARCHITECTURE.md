...
- **Cross-section/shared state:** `userNames` + `userName` + `todayChecklist`/`checklistLoading` + `userError`/`userLoading` come from **[`useDashboardBoot`](../src/hooks/useDashboardBoot.ts)** (extracted v2.720; `fetchDashboardPhase1` + `readDashboardBootCache`/`writeDashboardBootCache`) which also loads subscribed/assigned steps for the Projects card — the hook returns `setTodayChecklist`/`setUserError` so the parent's toggle/forward handlers keep mutating them; `getCurrentUserName` (~3925) is also used by the workflow-step action engine; `isDev` gate for Recently Completed.
...
- **Extraction status + risk + approach:** Inline; **boot seam done (v2.720)** — `useDashboardBoot` now owns the boot effect and returns userName/userNames/todayChecklist (+setter) + subs/assigned outputs. Stage A: `getDaysUntilDue`/`formatTDays` are already module-level pure (move to `lib/dashboard/` with tests when touched). Remaining: move the card + fwd/mute modal wiring into `DashboardMyInboxCard`. The `myInboxCard`-rendered-three-times pattern means extraction is naturally single-sourced already.
...
- **Owned local state:** `rejectStep`/`skipStep`/`setStartStep`, `assignedStagesExpanded`/`assignedStagesCompleteExpanded`/`subscribedStagesExpanded` + `assignedStagesExpandedDefaultAppliedRef` (one-time expand heuristic effect). `assignedSteps`/`assignedLoading`, `subscribedSteps`/`subscribedLoading`, and `userNames` (used by `personDisplay` here) moved into **`useDashboardBoot`** (v2.720) and are destructured from it.
- **Cross-section/shared state:** **initial load comes from [`useDashboardBoot`](../src/hooks/useDashboardBoot.ts)** (extracted v2.720; `fetchDashboardPhase1` → `loadSubscribed`/`loadAssigned`, which the hook owns along with the `loadAssignedSteps` reload); `getCurrentUserName` shared with My Inbox notifications; `editProjectModal` context; gates `showAssigned`/`showSubscribed`/`projectsCardVisible` (~4346–4368).
- **Derived memos:** `activeAssignedSteps`/`completedAssignedSteps` (~4348–4355).
- **Handlers (the workflow-step action engine, ~3536–4180):** `recordAction`, `findPreviousStep`/`findNextStep`, `markStarted`/`submitSetStart`, `markCompleted` (reopens rejected next step), `markApproved`, `submitReject` (reopens/notices previous step), `submitSkip` — these stay in the parent and call the hook-returned `loadAssignedSteps` (moved into `useDashboardBoot`, v2.720).
...
- **Extraction status + risk + approach:** Partial; **boot seam done (v2.720)** — `useDashboardBoot` exposes `assignedSteps`/`subscribedSteps` (+ setters and `loadAssignedSteps`). Remaining: `DashboardProjectsCard` takes the step lists + action callbacks, and the three modals move with it (opened only from here). `userNames` stays parent-destructured from the hook (also used by the `personDisplay` prop pattern already proven by `AssignedStageCard`).
