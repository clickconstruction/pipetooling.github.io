...
10. **Render-body ref assignments:** `refreshInvoicesRef.current = refreshInvoices` (~3086) and `resyncDashboardAfterUpdateJobStatusFailureRef.current = …` (~3071) are assigned during render, not in effects. Preserve the pattern when the engine moves into a hook. (**Seam as of v2.725:** the resync ref is *declared* in `useDashboardAssignedJobs` — it belongs with the lists its target reloads — but its `.current` is still assigned in the parent's render body, because the closure calls the parent-scope billing engine's `refreshInvoices` before reloading the three lists; the assignment moves only when the `useDashboardBillingInvoices` seam (item 12) exists.)
