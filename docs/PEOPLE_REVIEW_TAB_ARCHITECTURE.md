...
- **Person panel rollups (v2.2682):** Jobs Worked renders one header row per job from [`lib/people/reviewJobsRollup.ts`](../src/lib/people/reviewJobsRollup.ts) (`reviewJobsRollup` memo maps region E's `ReviewLaborJob[]` / `ReviewCrewJob[]` into neutral inputs; `reviewJobGroupsOpen` holds the open jobs) and lists the original day rows underneath — the two day-row `.map` callbacks are now `renderLaborRow` / `renderCrewRow` inside the tbody IIFE, bodies unchanged, so region F's per-day detail grid still keys on `reviewJobExpandedKey`. Tasks outstanding renders [`lib/people/reviewTasksRollup.ts`](../src/lib/people/reviewTasksRollup.ts) lines (`ReviewTask.checklist_item_id` now carried through from the query).
- **Next:** the Review-only math fixes from the 2026-09-03 audit (sub-sheet costing, office-rate wages, per-hour denominators, paid-in-full hour basis, company-calendar period bounds), then the crew-trigger migration.
