...
- **Data flow:** `dataPromise` = cache hit (`teamSummaryDataCacheRef` key match, popup only) or `loadTeamSummaryData()`. Enriches rows to `profitAfterOverhead = profit − totalHours × overheadRate` (all-hours overhead — must match `enrichTeamSummaryRowsForInline` and `drilldowns.tsx`; three surfaces reconcile).
...
5. **Overhead charged on ALL hours** in profit-after-overhead (`totalHours × rate`), while the split model's parts burden uses field hours only — three surfaces (inline enrich, popup, drilldowns.tsx) must keep reconciling.
