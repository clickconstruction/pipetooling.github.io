...
## Key facts the plan builds on (verified 2026-08-01)
...
- `people_labor_job_assignees(labor_job_id, person_id)` exists, trigger-synced from the `' | '`-delimited `assigned_to_name` (migration `20260722270000`), backfilled — **no app code reads it yet**. Junction coverage 45/56 sheets; unresolved: "Behar Kraja (Rough In)", "Edgar", "MIke Rodriguez", plus the "Kyle" ×2 ambiguity (owner decisions — Combine people / rename).
- `project_workflow_steps.assigned_to_name` is a name string with NO person_id column (workflow tables are absent from the identity plan's inventory — this plan adds them).
- The step lifecycle is implemented twice: `src/pages/Workflow.tsx` (markStarted L1777, markCompleted L1900, markApproved L1929, submitReject L2198, submitSkip L2241, markReopened L1981, recordAction L1223, sendWorkflowNotifications L1308) and `src/components/dashboard/DashboardProjectsCard.tsx` (L84–328). Divergences: the Dashboard copy sends **no notifications**, and its reject-cascade reopens the previous step to `pending` where Workflow.tsx sets `in_progress`.
- **Live bug**: `project_workflow_step_actions.action_type` CHECK (baseline L23193) allows only `started|completed|approved|rejected|reopened`. Both `submitSkip` implementations insert `'skipped'`; the insert fails the CHECK and both `recordAction`s swallow the error — every Skip flips the step but writes no ledger row.
