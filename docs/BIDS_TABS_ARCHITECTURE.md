...
- **Owned local state:** the `PursuitFilter` (window, outcome chips, show-empty, show-robots, estimator, GC, search), the lens (`pursuit` | `cost-to-win` | `bid-vs-actual`, v2.3341 / v2.3342) and the Cost-to-win group (`estimator` | `gc`). The Bid vs actual lens loads its own data through `useBidVsActual` only while open. Shared labor data loaded for it: `teamLaborDataForBids` (also used by Pricing).
