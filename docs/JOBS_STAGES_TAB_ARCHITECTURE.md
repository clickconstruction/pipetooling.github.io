...
- **Extraction — half done v2.3532:** the ⋯ trigger + menu is [`JobsStagesToolsMenu.tsx`](../src/components/jobs/JobsStagesToolsMenu.tsx) (props grouped as `filters` / `toggles` / `gates` / doors; `open` controlled because the whole-board load condition reads it; the *Follow cards I move* inline setter became `toggleStagesFollowMoves` in the tab; the row style the per-section ⋯ menus share lives in `stagesToolsMenuStyles.ts`); six render tests. **Left for the next PR:** the command bar — New Job, Follow-ups (+ `JobsFollowupModal`), Forecast, the search box with its busy hints, the Session notes chip, `StagesJobNumberJumpChip`, and the applied-filter chips (sort / contract / GC / development / Account Man / hidden groups) → `JobsStagesCommandBar.tsx`. Pure JSX move; no supabase, no effects.
...
| Toolbar + ⋯ tools menu | `JobsStagesToolsMenu.tsx` (v2.3532) · command bar still in `JobsStagesTab` | ~370 moved out; ~330 remain | low | low | **⋯ menu extracted**; command bar → `JobsStagesCommandBar` next |
...
2. **Toolbar** — ~~the ⋯ tools menu → `JobsStagesToolsMenu`~~ (done v2.3532); the command bar → `JobsStagesCommandBar` (next).
