...
- **Start every new migration with `SET lock_timeout = '3s';`** (v2.1083, CI-enforced for versions after 2026-07-29 via `check-migrations.sh`). There is no staging, so DDL runs against prod mid-workday: an `ALTER TABLE` that can't get its ACCESS EXCLUSIVE lock *queues*, and every query behind it queues too — the whole app appears frozen until the blocking query finishes. With the timeout, the `db push` errors out in ~3s with nobody affected; retry in a quieter moment. (Grandfathered: files ≤ 2026-07-29 predate the convention.)
- **Never copy a role gate from the baseline file.** Its function bodies still name `is_assistant_of_pay_approved_master()`, dropped by `20260714200000`; a copied body compiles and then raises "function does not exist" on first call (v2.1400, v2.2672, v2.3812). Gate on `is_dev() OR is_pay_approved_master() OR is_assistant()`; `check-migrations.sh` fails any later migration that names the dropped helper outside a comment.
...
**`20260820010000_checklist_instance_events.sql`** _(apply via `supabase db push` after the v2.1842 merge — additive; existing checklist UI keeps working unchanged and starts feeding history automatically via the trigger)_
- **Purpose**: Checklist card history + comments + lead review, Phase 1 (data spine). New append-only `checklist_instance_events` (`completed` / `reopened` / `accepted` trigger-written transitions + client-written `comment` rows; index on `(instance_id, created_at)`); `checklist_instances` gains `reviewed_at` / `reviewed_by` (the review queue's "dismiss" stamp); BEFORE UPDATE trigger `checklist_instance_transitions_trigger` logs completion transitions from ANY write path, clears review stamps on reopen, and guards every event INSERT so history can never break a completion toggle. Events RLS delegates to the parent instance's SELECT policies via nested EXISTS; clients may INSERT only `comment` rows as themselves. Backfills a `completed` event per already-completed instance and a `comment` event per legacy non-empty `notes`.
- **Category**: Checklist / activity
...
- **Purpose**: Person-identity Phase D — `self_salary_clock_state()` (SECURITY DEFINER, `authenticated` only): one round trip returning the caller's `is_salary` / `record_hours_but_salary` / `has_template`, resolved `people.account_user_id` → `people_pay_config.person_id` first with the legacy `btrim(users.name)` match as fallback. Replaces the C1-5 "deliberately skipped" name-keyed self-probes (2 queries in ClockInOutButton, 3 across two Dashboard effects) and sidesteps the name-keyed self-read RLS policy on `people_pay_config`.
- **Category**: People / new RPC
...
**`20260815154623_vehicle_trim.sql`** _(apply via `supabase db push` with the v2.1672 merge — additive nullable column, old clients unaffected)_
- **Purpose**: `vehicles.trim` (v2.1672) — free-text trim level ("XLT Crew Cab Long Bed") edited on the Add/Edit vehicle dialog above the VIN; shown muted beside the VIN on the open vehicle.
...
- **Security**: SECURITY DEFINER, EXECUTE revoked from PUBLIC/anon/authenticated, granted to service_role only.
...
- **Security**: gate is equivalent to the pre-v2.1399 live behavior (the sweep had already collapsed the assistant terms). SECURITY DEFINER + `SET search_path = public` unchanged.
- **Category**: Database Improvements
...
- **Security**: Both new tables RLS-enabled (dev manages; attributors/self read). All new functions SECURITY DEFINER with `SET search_path`, internal dev-or-attributor gates, EXECUTE revoked from `anon`. Mutual exclusions enforced in the RPCs: job splits ⟂ payroll flag ⟂ resolution. Ends with both read-only (training mode) block applications per the CREATE TABLE rule.
- **Category**: Feature schema
...
- **Purpose**: RUN_SUBS_PLAN Phase 4, PR 4.3 (v2.1219). `respond_to_work_order` returns the answer-notification context (creator contact with project-master fallback, step/project names, window) — sub-role RLS can't read those rows, and the function already sits behind a hard own-person gate.
...
- **Ordering**: migration first, then the client PRs (reverse of the usual client-first order).
- **Category**: Feature schema
...
- **Purpose**: freeze protection (v2.1136; [`DB_FREEZE_RUNBOOK.md`](./DB_FREEZE_RUNBOOK.md)) — `ALTER DATABASE postgres SET idle_in_transaction_session_timeout = '60s'`. The 2026-07-30 freezes were lock pileups behind idle-in-transaction sessions, not crashes/capacity; this kills such holders after 60s. New sessions only; rollback via `RESET`.
- **Security**: session GUC only; no schema, RLS, or grant changes.
...
- **Impact**: `BankingUserCardLinkModal.tsx` (since folded into [`BankingDebitCardsModal.tsx`](../src/components/banking/BankingDebitCardsModal.tsx)); see **`RECENT_FEATURES.md`** v2.401, **`PROJECT_DOCUMENTATION.md`** §15 Banking
