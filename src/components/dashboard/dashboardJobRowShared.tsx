 ...
 */
export const JobRowMissingPhoneButton = ({ onClick }: { onClick: MouseEventHandler<HTMLButtonElement> }) => (
  <button
    type="button"
    onClick={onClick}
    aria-label="No phone on file — tap to ask Dispatch to add one"
    title="No phone on file — tap to ask Dispatch to add one"
    style={{ flexShrink: 0, background: 'transparent', border: 'none', padding: '0.2rem', cursor: 'pointer', color: 'var(--text-red-600)', display: 'inline-flex', alignItems: 'center' }}
  >
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" width="2.5em" height="2.5em" aria-hidden focusable={false} style={{ display: 'block' }}>
      <path
        fill="currentColor"
