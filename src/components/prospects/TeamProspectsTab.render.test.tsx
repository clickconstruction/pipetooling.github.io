...
describe('TeamProspectsTab — a column-share holder (v2.3805)', () => {
  it('shows three stages, the shared intro, no Add role, no column menu, no Passed, and a Try-out card with no office buttons', async () => {
    renderWithProviders(<TeamProspectsTab authUserId="maria" isDev={false} resolveMasterId={async () => 'm1'} shared />)
    await waitFor(() => expect(screen.getByText('Austin Helper')).toBeTruthy())
    expect(screen.getAllByRole('tab').map((t) => t.textContent)).toEqual(expect.arrayContaining([expect.stringContaining('Screen'), expect.stringContaining('Interview'), expect.stringContaining('Try-out')]))
    expect(screen.getAllByRole('tab')).toHaveLength(3)
    expect(screen.getByText(/Two columns were shared with you/)).toBeTruthy()
    expect(screen.queryByRole('button', { name: '+ Add role' })).toBeNull()
    expect(screen.queryByRole('button', { name: /Column menu/ })).toBeNull()
    expect(screen.queryByRole('button', { name: 'Passed' })).toBeNull()
