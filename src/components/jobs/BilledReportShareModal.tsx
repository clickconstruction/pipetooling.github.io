...
import {
  cancelBilledReportSend,
  fetchBilledReportPreview,
  isBilledReportRecipientRole,
  listMyPendingBilledReportSends,
  openHtmlInNewTab,
  scheduleBilledReportSend,
  sendBilledReportNow,
  sendBilledReportTest,
  type ScheduledBilledReportSend,
} from '../../lib/billedReportEmailClient'

/**
 * "Share / Print" on the Billed Awaiting Payment header (v2.1316): email the
 * billed report to an office teammate — now or at a scheduled time — with the
 * print path kept as an option. Openable by dev / master_technician /
 * assistant-like (the gate lives at the button); recipients are office-capable
 * roles only (server-enforced; the picker filters to match).
 *
 * Scheduled sends are table rows the pg_cron dispatcher (every 5 min) processes at
 * send_at — the email is rebuilt from live data at send time, so a Monday
 * 7 AM send shows Monday numbers.
 */

type RecipientUser = { id: string; name: string; role: string | null; email: string | null }

export default function BilledReportShareModal({
  onClose,
  onPrint,
  printDisabled,
}: {
  onClose: () => void
  /** The existing print path (window.open + print) — offered as "Print instead". */
  onPrint: () => void
  ...
  const loadPending = useCallback(async () => {
    try {
      setPending(await listMyPendingBilledReportSends())
    } catch {
        ...
        const rows = await withSupabaseRetry(
          () =>
            supabase
              ...
              .is('archived_at', null)
              .order('name'),
          'billed report recipients',
        )
        if (cancelled) return
        setUsers(
          ((rows ?? []) as Array<RecipientUser & { archived_at: string | null }>)
            .filter((u) => isBilledReportRecipientRole(u.role))
  ...
  async function handleSend() {
    ...
    try {
      if (when === 'now') {
        await sendBilledReportNow(selected.id)
        showToast(`Report emailed to ${selected.name}.`, 'success')
      } else {
  ...
  async function handlePreview() {
    ...
    setBusy('preview')
    try {
      openHtmlInNewTab(await fetchBilledReportPreview())
    } catch (e) {
