...
import {
  cancelMoneyWaitingSend,
  fetchMoneyWaitingPreview,
  isMoneyWaitingRecipientRole,
  listMyPendingMoneyWaitingSends,
  openHtmlInNewTab,
  scheduleMoneyWaitingSend,
  sendMoneyWaitingNow,
  sendMoneyWaitingTest,
  type ScheduledMoneyWaitingSend,
} from '../../lib/moneyWaitingEmailClient'

/**
 * "Email…" on the Money waiting report modal header (v2.2226): email the report
...
export default function MoneyWaitingShareModal({ onClose }: { onClose: () => void }) {
  ...
  const [pending, setPending] = useState<ScheduledMoneyWaitingSend[]>([])

  const loadPending = useCallback(async () => {
    try {
      setPending(await listMyPendingMoneyWaitingSends())
    } catch {
        ...
        const rows = await withSupabaseRetry(
          () =>
            supabase
              ...
              .is('archived_at', null)
              .order('name'),
          'money waiting recipients',
        )
        if (cancelled) return
        setUsers(
          ((rows ?? []) as Array<RecipientUser & { archived_at: string | null }>)
            .filter((u) => isMoneyWaitingRecipientRole(u.role))
  ...
  async function handleSend() {
    ...
    try {
      if (when === 'now') {
        await sendMoneyWaitingNow(selected.id)
        ...
        await scheduleMoneyWaitingSend({ requestedBy: authUser.id, recipientUserId: selected.id, sendAtIso: iso, repeatWeekly })
        showToast(
          repeatWeekly
  ...
  async function handlePreview() {
    ...
    setBusy('preview')
    try {
      openHtmlInNewTab(await fetchMoneyWaitingPreview())
    } catch (e) {
  ...
  async function handleCancelPending(id: string) {
    try {
      await cancelMoneyWaitingSend(id)
      await loadPending()
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label="Email money waiting report"
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', marginBottom: '0.5rem' }}>
          <h2 style={{ margin: 0, fontSize: '1.125rem' }}>Email money waiting report</h2>
          <button
            type="button"
            onClick={onClose}
            aria-label="Close"
        ...
        <p style={{ margin: '0 0 1rem', fontSize: '0.8125rem', color: 'var(--text-muted)' }}>
          Emails the report — expected-payment buckets with every open bill, past-expected follow-ups first, linked
        ...
        <input
          ...
          aria-label="Search people to send the report to"
