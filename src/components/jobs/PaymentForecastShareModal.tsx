...
import {
  cancelPaymentForecastSend,
  fetchPaymentForecastPreview,
  isPaymentForecastRecipientRole,
  listMyPendingPaymentForecastSends,
  openHtmlInNewTab,
  schedulePaymentForecastSend,
  sendPaymentForecastNow,
  sendPaymentForecastTest,
  type ScheduledPaymentForecastSend,
} from '../../lib/paymentForecastEmailClient'

/**
 * "Email…" on the Payment forecast modal header (v2.2226): email the forecast
 * to an office teammate — now or at a scheduled time, optionally weekly.
 * A verbatim sibling of BilledReportShareModal (v2.1316) minus the print
 * path; the same five-piece Report Subscriptions stream shape
 * (docs/REPORT_SUBSCRIPTIONS.md).
 *
 * Scheduled sends are table rows the pg_cron dispatcher (every 5 min)
 * processes at send_at — the email is rebuilt from live data at send time, so
 * a Monday 7 AM send shows Monday numbers.
 */

type RecipientUser = { id: string; name: string; role: string | null; email: string | null }

export default function PaymentForecastShareModal({ onClose }: { onClose: () => void }) {
  ...
  const [pending, setPending] = useState<ScheduledPaymentForecastSend[]>([])

  const loadPending = useCallback(async () => {
    try {
      setPending(await listMyPendingPaymentForecastSends())
    } catch {
  ...
  useEffect(() => {
        ...
        const rows = await withSupabaseRetry(
          () =>
            supabase
              ...
              .is('archived_at', null)
              .order('name'),
          'payment forecast recipients',
        )
        if (cancelled) return
        setUsers(
          ((rows ?? []) as Array<RecipientUser & { archived_at: string | null }>)
            .filter((u) => isPaymentForecastRecipientRole(u.role))
    ...
    void loadPending()
    // Default schedule date: tomorrow (company calendar).
  ...
  async function handleSend() {
    ...
    try {
      if (when === 'now') {
        await sendPaymentForecastNow(selected.id)
        showToast(`Forecast emailed to ${selected.name}.`, 'success')
      } else {
        ...
        await schedulePaymentForecastSend({ requestedBy: authUser.id, recipientUserId: selected.id, sendAtIso: iso, repeatWeekly })
        showToast(
          repeatWeekly
  ...
  async function handlePreview() {
    ...
    setBusy('preview')
    try {
      openHtmlInNewTab(await fetchPaymentForecastPreview())
    } catch (e) {
  ...
  async function handleCancelPending(id: string) {
    try {
      await cancelPaymentForecastSend(id)
      await loadPending()
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', marginBottom: '0.5rem' }}>
          <h2 style={{ margin: 0, fontSize: '1.125rem' }}>Email payment forecast</h2>
          <button
            type="button"
            onClick={onClose}
            aria-label="Close"
        ...
        <p style={{ margin: '0 0 1rem', fontSize: '0.8125rem', color: 'var(--text-muted)' }}>
          Emails the forecast — expected-payment buckets with every open bill, past-expected follow-ups first, linked
          back to the board. Numbers are built fresh at send time.
        </p>
        ...
        <input
          ...
          aria-label="Search people to send the forecast to"
