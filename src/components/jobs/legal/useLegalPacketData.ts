...
// the packet loader reads through the untyped client and casts each result to the kernel's shapes.
        ...
        await Promise.all([
          src<LegalCustomerLike>('customer record', async () => {
            ...
            const row = await withSupabaseRetry<LegalCustomerLike>(
              () => db.from('customers').select('id, name, address, contact_info, customer_type, payment_terms, payment_terms_note').eq('id', customerId).maybeSingle(),
          ...
          src<LegalContactLike[]>('customer contacts', async () => {
            ...
            return (await withSupabaseRetry<LegalContactLike[]>(() => db.from('customer_contact_persons').select('name, email, phone, note').eq('customer_id', customerId).order('created_at'), 'load legal packet contacts')) ?? []
          }, []),
          ...
          src<CustomerAddressRow[]>('property record', async () => {
            if (!customerId) return []
            const rows = (await withSupabaseRetry<unknown[]>(() => db.from('customer_addresses').select(ADDRESS_COLUMNS).eq('customer_id', customerId).order('sequence_order'), 'load legal packet property record')) ?? []
          ...
          src<SignedEstimateLike[]>('accepted estimates', async () =>
            (await withSupabaseRetry<SignedEstimateLike[]>(
              () => db.from('estimates').select('id, job_ledger_id, bid_id, doc_kind, status, acceptor_consented_at, acceptor_printed_name, estimate_number, total_cents').in('job_ledger_id', jobIds).eq('status', 'customer_accepted').not('acceptor_consented_at', 'is', null),
          ...
          src('promise record', async () => {
            const { data, error } = await db.rpc('list_payment_promise_records' as never)
            if (error) throw error
          ...
          src('collection calls', async () => {
            const { data, error } = await db.rpc('list_payment_chase_touches' as never)
            if (error) throw error
          ...
          src<LegalReportLike[]>('field reports', async () => {
            const lists = await Promise.all(
              jobIds.map(async (jobId) => {
                const { data, error } = await db.rpc('list_reports_for_job_ledger', { p_job_id: jobId })
                if (error) throw error
          ...
          src<LegalClockSessionLike[]>('clock sessions', async () => {
            const rows = (await withSupabaseRetry<ClockSessionRow[]>(
              () => db.from('clock_sessions').select('job_ledger_id, work_date, clocked_in_at, clocked_out_at, clock_in_lat, approved_at, rejected_at, revoked_at').in('job_ledger_id', jobIds).order('work_date').limit(ROW_CAP),
