...
/** Same typography as amount-due headline; line-item panel uses smaller base `fontSize`. */
...
export default function BilledBillViewModal({
  ...
  onClose: () => void
  /** Use a higher value when opening above Edit Job (`JobFormModal`) or other stacked overlays. */
  ...
  const applied = inv && job ? sumPaymentsForInvoice(job.payments, inv.id) : 0
  const invoiceRemaining = inv && job ? Math.max(0, Number(inv.amount ?? 0) - applied) : 0
  ...
  const isStripeHosted = Boolean(inv && stripeId && hostedUrl)

  useEffect(() => {
    if (!inv || !isStripeHosted) {
          ...
          <>
            ...
            {stripeDetail ? (
              ...
              >
                ...
                <div style={{ ...stripeHeroAmountText, marginBottom: '0.25rem' }}>
                  {formatStripeCents(stripeDetail.amount_due, stripeDetail.currency)}
                </div>
            ...
            {applied > 0 && stripeDetail && !stripeError ? (
              <div style={{ marginBottom: '0.5rem', fontSize: '0.75rem', color: '#6b7280' }}>
                Applied in PipeTooling: ${formatMoney(applied)} · Open on invoice (app): ${formatMoney(invoiceRemaining)}
              </div>
            ) : null}
                ...
                >
                  ...
                  <div style={{ display: 'grid', gap: '0.35rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                      ...
                      <span style={{ fontWeight: 600 }}>{formatStripeCents(stripeDetail.amount_paid, stripeDetail.currency)}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                      <span style={{ color: '#374151' }}>Amount remaining</span>
                      <span style={{ fontWeight: 600 }}>{formatStripeCents(stripeDetail.amount_due, stripeDetail.currency)}</span>
                    </div>
                  </div>
                </div>
              </div>
