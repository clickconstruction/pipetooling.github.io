import { useState } from 'react'
import { useChecklistCostEstimates } from '../../hooks/useChecklistCostEstimates'
import { actualDollars, estimateDollars, formatMultiplier, formatWholeDollars } from '../../lib/checklistCostEstimate'
import ChecklistCostModal from './ChecklistCostModal'

type Props = {
  costKey: string
  taskTitle: string
  /** Pre-selected person in the modal (e.g. the row's assignee). */
  defaultUserId?: string | null
}

/**
 * Dev-only cost affordance for a checklist row: a muted calculator icon when
 * the task has no estimate, a gold whole-dollar chip once it does. Both open
 * the cost estimator modal. Callers gate on `isDev`.
 */
export function ChecklistCostButton({ costKey, taskTitle, defaultUserId }: Props) {
  const estimates = useChecklistCostEstimates(true)
  const [modalOpen, setModalOpen] = useState(false)
  const estimate = estimates[costKey]
  const actual = estimate ? actualDollars(estimate) : null
  const estTotal = estimate ? estimateDollars(estimate) : 0

  return (
    <>
      {estimate ? (
        <button
          type="button"
          onClick={(e) => {
            e.stopPropagation()
            setModalOpen(true)
          }}
          title={`${estimate.personName}: ${estimate.hours}h × $${estimate.rate}/hr — click to edit`}
          aria-label={`Cost estimate ${formatWholeDollars(estimateDollars(estimate))} — click to edit`}
          style={{
            flexShrink: 0,
            fontSize: '0.72rem',
            fontWeight: 700,
            padding: '0.1rem 0.45rem',
            borderRadius: 7,
            whiteSpace: 'nowrap',
            background: '#fbbf24',
            border: '1px solid #d97706',
            color: '#451a03',
            cursor: 'pointer',
          }}
        >
          {formatWholeDollars(estimateDollars(estimate))}
        </button>
      ) : null}
      {estimate && actual != null ? (
        // The truth tag: red over / green under, ×m when meaningfully over.
        <button
          type="button"
          onClick={(e) => {
            e.stopPropagation()
            setModalOpen(true)
          }}
          title={`Actually took ${estimate.actualHours}h — click to correct`}
          style={{
            flexShrink: 0,
            fontSize: '0.68rem',
            fontWeight: 700,
            padding: '0.1rem 0.4rem',
            borderRadius: 7,
            whiteSpace: 'nowrap',
            cursor: 'pointer',
            background: actual > estTotal ? 'var(--bg-red-100)' : 'var(--bg-green-100)',
            border: `1px solid ${actual > estTotal ? '#dc2626' : '#16a34a'}`,
            color: actual > estTotal ? 'var(--text-red-700)' : 'var(--text-green-700)',
          }}
        >
          was {formatWholeDollars(actual)}
          {estimate.hours > 0 && estimate.actualHours != null && estimate.actualHours / estimate.hours >= 1.45
            ? ` · ${formatMultiplier(estimate.actualHours / estimate.hours)}`
            : ''}
        </button>
      ) : null}
      {!estimate ? (
        <button
          type="button"
          onClick={(e) => {
            e.stopPropagation()
            setModalOpen(true)
          }}
          title="Cost this task"
          aria-label="Cost this task"
          style={{
            flexShrink: 0,
            padding: '0.25rem',
            background: 'none',
            border: 'none',
            cursor: 'pointer',
            color: 'var(--text-muted)',
            display: 'inline-flex',
            alignItems: 'center',
            justifyContent: 'center',
          }}
        >
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" width="16" height="16" fill="currentColor" aria-hidden="true">
            <path d="M192 64C156.7 64 128 92.7 128 128L128 512C128 547.3 156.7 576 192 576L448 576C483.3 576 512 547.3 512 512L512 128C512 92.7 483.3 64 448 64L192 64zM224 128L416 128C433.7 128 448 142.3 448 160L448 192C448 209.7 433.7 224 416 224L224 224C206.3 224 192 209.7 192 192L192 160C192 142.3 206.3 128 224 128zM240 296C240 309.3 229.3 320 216 320C202.7 320 192 309.3 192 296C192 282.7 202.7 272 216 272C229.3 272 240 282.7 240 296zM320 320C306.7 320 296 309.3 296 296C296 282.7 306.7 272 320 272C333.3 272 344 282.7 344 296C344 309.3 333.3 320 320 320zM448 296C448 309.3 437.3 320 424 320C410.7 320 400 309.3 400 296C400 282.7 410.7 272 424 272C437.3 272 448 282.7 448 296zM216 416C202.7 416 192 405.3 192 392C192 378.7 202.7 368 216 368C229.3 368 240 378.7 240 392C240 405.3 229.3 416 216 416zM344 392C344 405.3 333.3 416 320 416C306.7 416 296 405.3 296 392C296 378.7 306.7 368 320 368C333.3 368 344 378.7 344 392zM424 416C410.7 416 400 405.3 400 392C400 378.7 410.7 368 424 368C437.3 368 448 378.7 448 392C448 405.3 437.3 416 424 416zM192 488C192 474.7 202.7 464 216 464L328 464C341.3 464 352 474.7 352 488C352 501.3 341.3 512 328 512L216 512C202.7 512 192 501.3 192 488zM424 464C437.3 464 448 474.7 448 488C448 501.3 437.3 512 424 512C410.7 512 400 501.3 400 488C400 474.7 410.7 464 424 464z" />
          </svg>
        </button>
      ) : null}
      <ChecklistCostModal
        open={modalOpen}
        costKey={costKey}
        taskTitle={taskTitle}
        defaultUserId={defaultUserId}
        onClose={() => setModalOpen(false)}
      />
    </>
  )
}
