...
/** Open ring outer diameter 512 vs square/check outer frame 448 in the 640 viewBox — match checkbox extent. */
const STRIP_ICON_RING_ALIGN_SCALE = 448 / 512

/** ViewBox X nudge (positive = right); tune vs pending square if strip still looks off. */
const STRIP_ICON_RING_OPTICAL_NUDGE_X = 78
...
const pathD = {
  ...
  /** Open session — circle ring (Font Awesome Free v7.2.0 circle-regular style path). */
  openRing:
...
function StripIcon({
  ...
  if (status === 'open') {
    return (
      <svg {...FA_ATTR} width={16} height={16} aria-hidden style={iconWrap}>
        <g
          transform={`translate(${STRIP_ICON_RING_OPTICAL_NUDGE_X} 0) translate(320 320) scale(${STRIP_ICON_RING_ALIGN_SCALE}) translate(-320 -320)`}
        >
          <path fill="#6b7280" d={pathD.openRing} />
        </g>
      </svg>
    )
  }
