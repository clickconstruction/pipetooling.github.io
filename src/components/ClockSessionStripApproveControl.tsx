...
const pathD = {
  ...
  /** Solid rounded square (FA square outer path) — read-only open / view-only pending */
  openNonInteractive:
...
function StripIcon({
  ...
  /** When pending, ring square for clickable approve; solid square for view-only. */
  interactive: boolean
}) {
  if (status === 'approved') {
    return (
  ...
  if (status === 'open') {
    return (
      <svg {...FA_ATTR} width={16} height={16} aria-hidden style={iconWrap}>
        <path fill="#9ca3af" d={pathD.openNonInteractive} />
      </svg>
    )
  }
  const pendingPath = interactive ? pathD.square : pathD.openNonInteractive
  return (
    <svg {...FA_ATTR} width={16} height={16} aria-hidden style={iconWrap}>
      <path fill="#6b7280" d={pendingPath} />
    </svg>
  )
}
...
export function ClockSessionStripApproveControl({
  ...
  if (status === 'open' || (!interactive && !(actionsEligible && status === 'approved'))) {
    return (
      ...
      >
        <StripIcon status={status} interactive={interactive} />
      </span>
    )
  }
