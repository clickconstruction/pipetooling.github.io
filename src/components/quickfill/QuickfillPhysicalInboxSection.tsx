...
export function QuickfillPhysicalInboxSection({ markButtonPalette, onConfirmMark }: QuickfillPhysicalInboxSectionProps) {
  ...
  return (
    ...
    >
      <div id={promptsId}>
        <p style={{ ...hintStyle, marginBottom: '0.5rem' }}>Is my physical inbox clear?</p>
        <p style={hintStyle}>
          For items that can&apos;t be quickly cleared, have you added a task? Use the buttons below, same as the header.
        </p>
      </div>
      {(showDispatchEstimator || showTask) && (
        <div
          style={{
            display: 'flex',
            flexWrap: 'wrap',
