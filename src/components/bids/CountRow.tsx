...
export function SortableCountRow({ row, highlight, onUpdate, onDelete }: {
  ...
  const dragHandle = (
    <span
      {...attributes}
      {...listeners}
      style={{ cursor: 'grab', display: 'inline-flex', padding: '0.25rem', color: 'var(--text-faint)' }}
...
export function CountRow({ row, highlight, onUpdate, onDelete, dragHandle, trRef, trStyle }: {
  ...
  const rowStyle = highlight ? { borderBottom: '1px solid var(--border)', background: 'var(--bg-green-100)' } : { borderBottom: '1px solid var(--border)' }
    ...
    return (
      <tr ref={trRef} style={mergedStyle}>
        ...
        <td style={{ padding: '0.75rem' }}>
          <input type="text" value={groupTag} onChange={(e) => setGroupTag(e.target.value)} style={{ width: '100%', boxSizing: 'border-box', padding: '0.5rem', border: '1px solid var(--border-strong)', borderRadius: 4 }} />
        </td>
        <td style={{ padding: '0.75rem' }}>
          <input type="text" value={page} onChange={(e) => setPage(e.target.value)} style={{ width: '100%', boxSizing: 'border-box', padding: '0.5rem', border: '1px solid var(--border-strong)', borderRadius: 4 }} />
        </td>
        <td style={{ padding: '0.75rem' }}>
          ...
          <button type="button" onClick={() => setEditing(false)} style={{ padding: '0.25rem 0.5rem', background: 'var(--bg-muted)', border: '1px solid var(--border-strong)', borderRadius: 4, cursor: 'pointer' }}>Cancel</button>
        </td>
      </tr>
    )
  }
  return (
    <tr ref={trRef} style={mergedStyle}>
      ...
      <td style={{ padding: '0.75rem' }}>
        <button type="button" onClick={() => setEditing(true)} title="Edit" aria-label="Edit" style={{ marginRight: '0.25rem', padding: '0.25rem', cursor: 'pointer', background: 'none', border: 'none', color: 'var(--text-muted)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <button type="button" onClick={remove} title="Delete" aria-label="Delete" style={{ padding: '0.25rem', cursor: 'pointer', background: 'none', border: 'none', color: 'var(--text-red-800)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" width={16} height={16} fill="currentColor" aria-hidden="true">
