...
describe('PortalBankTransferCard', () => {
  it('is collapsed until tapped, then shows the numbers, memo, checks and guard lines', () => {
    ...
    expect(screen.getAllByText(/All checks must be mailed to 12925 FM 20, Kingsbury, TX 78638/).length).toBeGreaterThan(0)
  ...
  it('a checks-only record asks "Paying by check?" and shows no transfer grid', () => {
    ...
    expect(screen.getAllByText(/All checks must be mailed to/).length).toBeGreaterThan(0)
