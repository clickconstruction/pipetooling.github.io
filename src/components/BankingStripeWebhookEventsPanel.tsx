  ...
  return (
    <div>
      ...
      {loading && rows.length === 0 ? (
        ...
        <p style={{ margin: 0, color: '#6b7280', fontSize: '0.9375rem' }}>No webhook events yet (or none in range).</p>
      ) : (
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.stripe_event_id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                  ...
                  <td style={{ padding: '0.5rem 0.75rem', fontFamily: 'ui-monospace, monospace', wordBreak: 'break-all', color: '#4b5563' }}>
                    {r.stripe_event_id}
                  </td>
                </tr>
              ))}
