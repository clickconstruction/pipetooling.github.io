...
export type OpenEditJobOptions = {
  ...
  /** Land on ③ Payments received, flashed — the Dashboard's bank-returned deposits card (v2.3791). */
    ...
    <JobFormModalContext.Provider value={value}>
      ...
      {openState.kind === 'edit' ? (
        ...
        <JobFormModal
          ...
          onClose={closeJobForm}
          onSaved={openState.onSaved}
          onCreatedJobId={(jobId) => {
            openState.onCreatedJobId?.(jobId)
            setContractPromptJobId(jobId)
          }}
        />
      ) : null}
