...
export function useSubmittalsNudge(enabled: boolean): { nudge: SubmittalNudge | null; reload: () => void } {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const bids = ((bidRows ?? []) as Array<{ id: string; bid_number: string | null; project_name: string | null; outcome: string | null; outcome_at: string | null; updated_at: string | null; submittals_not_needed_at?: string | null }>).map<NudgeBid>((b) => ({
        bidId: b.id,
        ...
        notNeeded: Boolean(b.submittals_not_needed_at),
      }))
