...
**Keeping the baseline.** When a job bills, what it took is kept on its own — hours, wages, people, materials and price, as hours per $1k, and by fixture when the bid had a count sheet. The strip then reads {{chip:green|Baseline kept Sep 12 on billing · per $1k}} (fold open **By fixture** to see the split). Mid-job, {{button:blue|Keep as baseline}} keeps it now and {{button:gray|Keep again ↻}} refreshes it. Bids → Labor reads every kept baseline as the **Jobs baseline** tile.
