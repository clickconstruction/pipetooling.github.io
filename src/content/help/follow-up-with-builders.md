...
order: 69
---
The **Followup** tab (Bids → Followup) is where bid follow-up lives. The Bid Board deliberately goes colour-quiet once a bid is sent — the red and amber there are about getting letters out — so *this* tab is where the chase happens. Opening it lands on the **Call queue** — the lens the tab defaults to; the four original lenses are one click away behind the **Old:** divider, and if you flip to one of them, re-clicking Followup keeps you where you are.
...
The header line counts **bids** and names the trade you're looking at — *· Plumbing · 96 bids · 99 GC packets to chase · 59 losses need a reason …*. Bids come first; when a bid went to more than one GC, the GC-packet figure follows as the second number, because the queue lists one row per GC (three GCs, three calls) while a bid is still one bid. Bold numbers are work owed; the Done column says what's already collected ("16 of 24 fresh", "8 of 9 recorded"); a — means nothing owed. **Click any row and it drops open in place** — the pending bids with their one-tap chips, the lost bids with the six reason chips, the gettable tabs with the capture panel — so you can collect one thing between meetings without leaving the queue. The one-tap chips on a pending bid are the same ones as Waiting to hear — **Left message · Still pending · Bid tab received · Rebid / RFQ · Won · Lost…** — and every tap writes the bid note and stamps Last Contact for you.
...
::: Filter chips (**To chase** · **Need a reason** · **Tab gettable**) narrow the list to one kind of gap, and {{button:blue|📞 Start call}} jumps to the builder card for a full call session.
...
There's a second reason the tap matters, and the app now says it wherever you mark a bid lost without a reason: *Add why: an uncategorized loss can't teach the robots.* The robot estimators train on our decided bids, and a loss with no category can't be told apart from a loss that never really competed (GC lost the project, project died) — so the whole record gets left out of their training math. One tap on a reason chip keeps it usable. The amber line appears on Edit Bid, the quick lost panel, and the per-GC Sent panel — always with the six chips right there, never blocking the save.

The header reads the same way as Waiting to hear: the red count of **lost bids** that need a reason, then one line with **how many lost** (bids first, then the GC-packet figure when any went to several GCs — *of 114 lost · 115 GC packets · Plumbing*), the dollars unexplained and both loss rates, a search box (bid #, project name, GC/Builder, or address — the red count keeps counting the whole queue while you search), and a plain sentence explaining how the queue works. The trade name sits right on the line: these are this trade pill's bids, while the Dashboard card counts *all trades*.
...
- When you mark one GC **won** (board, Followup, or a chase tap), the other GCs you sent to are marked **lost · GC lost the project** for you — a confirm names them before anything is written, and warns when the bid was marked Lost by hand and is about to flip to Won. They land tagged {{chip:gray|auto}} in Why we lost, and you can tap a different reason any time. Nothing to triage. Setting the winner back to **waiting** undoes the whole thing — siblings back to waiting, the bid back where it was.
...
- In **By status**, the bid shows one row per GC, each in the bucket that GC's answer puts it; the GC column names the row's GC with a quiet *also to …* line, and the Lost row's reason is that GC's. The section headers count bids first — *Not yet won or lost (101 bids · 107 GC packets)* — and the Unsent header carries the trade name.
