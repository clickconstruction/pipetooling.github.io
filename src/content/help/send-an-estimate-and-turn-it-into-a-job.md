...
On the accepted record's **Job** block — or straight from the email's {{button:amber|Create the job}} — press {{button:blue|Create job from estimate}}. New Job opens with the customer, address and the accepted lines already in as Specific Work, and the **Job name** filled in for you: the customer's name when the estimate still carries its stock heading ("Estimate for Kimberly Coe" — that heading is for the customer's paper, not the Pipeline), or the heading itself when you wrote one for the work. Change it if you like, then press Create and the job is linked to the estimate (and to the bid, if the estimate came from one — the Bid Board shows a {{chip:green|J1234}} chip). Already typed the job by hand? Use **Link existing job** in the same window instead of making a twin.
