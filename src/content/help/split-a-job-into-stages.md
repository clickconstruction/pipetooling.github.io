...
- The draft tagged **auto** in the Invoices table is the job's remainder keeping itself up to date: it shrinks as you break stages off, it *is* the bill for whatever you never split out, and it disappears on its own once every stage is on its own invoice. You never need to create it, resize it, or delete it. When you send it, the customer's bill names the stages it covers at their real prices (as long as those stages add up to it exactly — a payment or hand-typed dollar bill in the mix falls back to a proportional breakdown).
