...
- **Schedule** — a month calendar with dots on days that have work, and the selected day's agenda below: every person's blocks with time, duration, job, customer, address, and who's assigned. Tap a visit to open the job. A **person chip row** above the agenda isolates whoever you pick — tap one or more names, or **Everyone** to clear. The {{icon:help|bell}} **bell** in the top-left sends a note to dispatch (it opens the Task Dispatch flow), and **Today** jumps back to today.
=======
- **Dashboard** — your clock in/out control up top, then **Job Stages**: every open job grouped under Waiting, Working, Ready to bill, Billed awaiting payment, and Collections — each card shows the customer, address, and the assigned crew (on the right on wider screens). Tap a job to open its Job Detail.
