...
- **Parts** now leaves out Internal Transfers (money moving between the company's own accounts) and counts a card charge only once when it's linked to a supply-house invoice. When a job's parts include fuel, the cell carries a small **fuel $X** line — card charges labelled {{chip:gray|Fuel / Gas}} in Banking, or filed as fuel by the bank until someone labels them — so pipe and fill-ups read apart. Parts and profit don't change; the line is a slice of what was already counted.
