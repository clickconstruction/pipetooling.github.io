...
70 % done · $41,550 bid
{{chip:green|Paid $16,620}} {{chip:yellow|Done, not billed $12,465}} {{chip:gray|Not done $12,465}} → **Left on Job $24,930**
:::
...
**A job split into stages shows the stages instead.** When the job's line items are numbered **Order** stages (see *split a job into stages*), or when they already read like a plan — **Rough In**, **Top Out**, **Trim Set** in order, the way the Multiple Segment Generator's preset writes them; the app recognizes the plumbing stages by name and nobody has to set Order — the cell swaps the money bar for a chip strip and one stage bar. The chips read {{chip:green|✓ Rough}} → {{chip:blue|② Top Out 60%}} → {{chip:gray|③ Trim}} — long names shorten by rule (Rough In → Rough, Trim Set → Trim) and collapse to just their numbers when the cell is narrow; hover a chip for the full name. Under them, the bar's segments are the stages, sized by their share of the job. A segment's **fill** is how far the crew is on that stage (from stage-weighted reports, or the job's % done spread across the stages); its thin **bottom edge** is what happened to that stage's draw — **green** paid, **blue** billed, **amber** passed but not yet billed, **gray** nothing yet. So a full segment with an amber edge means "bill this draw", and a half-filled one with a blue edge means the draw went out ahead of the work. The caption spells it out: `Stage 2 of 3 · Top Out 60% · draw 1 paid`. The money rows shrink to **Paid**, **Billed** and **Left on Job** — the bar already says which stage the money belongs to.
