...
**Under the bar, one line of words says where the job is.** Who was on site and when ("Behar & Malachi on site Sat", "3 on site today", "Texas Rooter on the sheet, no clock-ins", "nobody clocked in"), the percent and where it came from ("80% typed Sep 3", "12% reported Sep 11", "40% set Aug 7" for a number the job already carried when percent history began, "no % yet"). It leaves the money out because the rows right beneath it already spell it out — hover the bar to read the whole sentence, money included. The line turns **amber** when money is owed for work done or a bill is out unpaid, **green** when the job is paid in full, and **red** when a crew clocked in on a job that has no line items at all — nothing to bill against.
