...
**Under the bar, one line of words says where the job is.** When the crew was last on site ("on site Sat", "on site today", "on the sheet, no clock-ins", "nobody clocked in"), the percent and where it came from ("80% typed Sep 3", "12% reported Sep 11", "40% set Aug 7" for a number the job already carried when percent history began, "no % yet"). It leaves out what the row already shows — the names are in **Crew & Dates**, the stage is the lit chip above, the money is in the rows right beneath — hover the bar to read the whole sentence with all of it. The line turns **amber** when money is owed for work done or a bill is out unpaid, **green** when the job is paid in full, and **red** when a crew clocked in on a job that has no line items at all — nothing to bill against.
...
*Texas Rooter on the sheet, no clock-ins · 80% typed Sep 3*
:::
