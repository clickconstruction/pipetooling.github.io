...
:::
- **People Hours** and **Unassigned field time** — time approval. People Hours is where you approve yesterday's and today's sessions: the clock strip's approve pill asks once (*"Approve … ? This adds the hours to payroll."*) and then the hours are in payroll — assistants approve here, same as on People → Hours. **Unassigned field time** is a count card: how many person-days in the window (3 / 7 / 14 / 30 days) had paid field time with no job on it, one line per week — *Aug 30 – Sep 5 · 5 person-days · 9.2 h · 3 people* — each with {{button:outline|Open on Team board →}}, which lands on that week of Jobs → Team with **Only exceptions** ticked so the fix is one tap away (see [read the Team board](?g=read-the-team-board)). When the window is clear the card reads **All on a job**. When clock sessions from the last 7 days have no job or bid, a **Match sessions to jobs** block still appears under the card (the same flow People → Hours opens from its {{button:outline|Match sessions}} button): sessions spread out in columns grouped by person, each with one-tap suggestions, a job search, and {{button:red|Reject}} for time that was never a job (a test punch, an errand — it asks first, and rejected time never reaches payroll) — plus **Apply all** for sessions with exactly one Dispatch match.
