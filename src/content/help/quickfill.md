...
:::
- **People Hours** and **Unassigned field time** — time approval. People Hours is where you approve yesterday's and today's sessions: the clock strip's approve pill asks once (*"Approve … ? This adds the hours to payroll."*) and then the hours are in payroll — assistants approve here, same as on People → Hours. When clock sessions from the last 7 days have no job or bid, a **Match sessions to jobs** block appears right on the Unassigned field time section (the same flow People → Hours opens from its {{button:outline|Match sessions}} button): sessions spread out in columns grouped by person, each with one-tap suggestions, a job search, and Skip — plus **Apply all** for sessions with exactly one Dispatch match. When the window is clear, the block disappears. In the unassigned list, a day whose only assignment is the Office job reads **"Office only — doesn't cover field time"**: Office time is overhead, so the day stays listed until a field job is assigned. The day audit's **Job / bid assignments** panel marks Office rows the same way; its {{button:outline|↺ Re-sync from clock}} button rebuilds the day's assignments from its approved clock sessions, and its {{button:outline|+ Link 1 session (0.72 h)}} puts a job on the sessions that have none — the split is never typed by hand (see [put a person's clocked hours on the right job](?g=put-clocked-hours-on-the-right-job)).
- **Crew Jobs / Bids** — the per-person day splits behind team labor costs, always computed from approved clock sessions. A person with approved time and no job on it shows {{button:outline|Link 1 session (0.72 h)}}: pick the job or bid and the sessions are linked, the split recomputes, and the row locks with the **⏱ from clock** badge. **Split day…** opens the day editor when one person worked two jobs. A day with no approved session reads **No clock session** — there is no split without a clock session (see the [put a person's clocked hours on the right job](?g=put-clocked-hours-on-the-right-job) guide).
