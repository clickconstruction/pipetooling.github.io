...
- **Complete, no Total Bill** — jobs marked **100% complete** (latest field report %, or the Edit Job **% complete** field when no report has one) whose **Total Bill** is empty or $0, listed right in the section. Each job shows when work **started**, its **clock sessions** and hours (hover for every work date); {{button:red|Edit job}} opens Edit Job to set the Job Total, and {{button:outline|Activity ▾}} expands the same activity history you see in Job Detail. Uses the same 100% rule as the Job Summary **%** column and the same minimum-HCP cutoff as Jobs Billing.
