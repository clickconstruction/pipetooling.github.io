...
Every row wears a **Status** chip: {{chip:gray|waiting}}, {{chip:yellow|late 7d}} once the needed-by date has passed with nothing in, or {{chip:green|quote in}}; the count above the table says how many are late. Beside the date, {{chip:blue|by app}} or {{chip:yellow|sent outside}} says how it went, and the **Quote** column holds only the quote. A hand-sent row gets **Call** when the house's default rep has a phone on the Directory; an app-sent row keeps **Nudge**.
