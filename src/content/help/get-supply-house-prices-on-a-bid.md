...
- **Say why, while you remember.** Tap the status on a row you have picked and the popover asks three things, none required: what this pick is against the schedule ({{chip:yellow|As derived · Alternate}}, or your call — {{chip:blue|Superseded}}, {{chip:blue|Equal}}, {{chip:red|Design change}} — when the model numbers alone cannot tell), why ({{chip:gray|Long lead time}} {{chip:gray|Discontinued}} {{chip:gray|In stock}} {{chip:gray|Or-equal clause}} {{chip:gray|Cost}} {{chip:gray|Other}}, plus a note for what the GC will ask), and when it lands ({{chip:gray|In stock}} {{chip:gray|1 wk}} {{chip:gray|2 wk}} {{chip:gray|4+ wk}}, or type *3 wk* / *10 days*). {{button:green|Save}} writes it onto the pick — a kit's lines together. Picking a price that differs from the schedule opens the popover on its own; a row still owing a reason reads **why?**, the counts line says *3 without a reason*, and the footer repeats it. Nothing is blocked — {{button:outline|Mark reviewed}} still works — but the answer is on the row the day the GC asks, not rebuilt from memory.
...
Sent them by email or phone instead? {{button:blue|+ Add a request}} records them, and it takes as many houses as you asked. Pick the first house (or add a new one right there) and it becomes a card of its own; {{button:outline|+ Add another supply house}} adds the next. Each card carries **its own day** and **its own quote link**, because a batch you mailed on Friday comes back one quote at a time. Leave the link empty when the quote is not back yet — the row shows a **Paste the quote link…** box, so the day it lands you paste it right there and press Enter (or use **Edit**).

Every row wears a **Status** chip: {{chip:gray|waiting}}, {{chip:yellow|late 7d}} once the needed-by date has passed with nothing in, or {{chip:green|quote in}}; the count above the table says how many are late. Beside the date, {{chip:blue|by app}} or {{chip:yellow|sent outside}} says how it went, and the **Quote** column holds only the quote. A hand-sent row gets **Call** when the house's default rep has a phone on the Directory; an app-sent row keeps **Nudge**. Once at least one quote is in, the panel header offers {{button:blue|Price with robot · 1 quote in}} — it opens the robot's price sheet on Pricing, the same one Pricing's own button opens. And a request that passes its needed-by date with nothing in shows on your Dashboard's **Needs You** card (*past the date you asked for, with nothing in*); **Open Price requests** takes you to that bid.
