...
- **Skip this month and give up the lien right…** — the red link under the buttons — gives the lien right on those months up, on purpose, with a reason kept on the record. It is a sentence and not a button beside *Save draft* because it is the one decision here that cannot be undone. A window that closes with nothing recorded is never silent: the Dashboard's *Needs you* names it in red the day it happens — *The lien window for June 2026 closed on 258 · Dudley Mason with no notice* — and the pane carries a red strip above the months saying the same, with {{button:outline|Note it as missed}}. Noting it is not a skip: it writes down that a person saw the loss, with a name and a date, and the Dashboard stops naming it; the money is still owed and rides on the notice for the months that are open. A skipped month does not vanish: it stays under the month cards as a dot in **Earlier months** — {{chip:green|Sent}}, {{chip:yellow|Skipped}} or {{chip:red|Missed}} — and clicking the dot opens the record: when it was skipped, by whom, the reason that was typed, and what it meant for the lien right.
