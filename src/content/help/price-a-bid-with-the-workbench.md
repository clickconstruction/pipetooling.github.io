...
One sentence covers it: a **version** is a separate bid you can send — its own counts, its own takeoff, its own prices, its own GC — and a **price scenario** is the same bid at a different price point. Versions bundle into the cover letter (New) as their own priced sections — each at its ★, marked **Base** or **Alternate** — optionally addressed to their own GC. The rule underneath: **versions are what the customer sees; scenarios are what you compare.**

Most bids have one version and one price, so the Workbench shows just one quiet line — your price, the star, and {{button:outline|＋ New price or version…}}. That button asks what you want: **Another price point** (for you to compare — the customer only ever sees the ★), **Another bid to send** (the customer sees it — its own section or letter; it starts with a copy of this bid's counts, takeoff and prices, and they are its own from there), or **Adopt an existing bid** — pull a bid that's already on the board into this package as one of its bids: its counts, takeoff, prices and sent history come along, and its old board row retires (nothing is deleted; the number still looks up). Labor and cost are shared by the whole package: switching bids changes revenue, not cost. Once a second price or version exists, the full structure bar and scenario cards appear, and the picker at the top shows **Bids in this package** with each one's cover-letter status and GC.

Clicking a scenario card just **views** it. The {{chip:green|★ base}} scenario is the price on that GC's letter — Cover Letter, Share, Print, CSV, and the bid value use it — and it only changes when you press {{button:outline|☆ Make base}} and confirm. Any other priced scenario can be **offered** to the same GC as an alternate with {{button:outline|Offer as alternate}}: same counts, no new version, and it appears on their cover letter as an alternate section. **Another price point** asks for a name and can offer the new price right away. Viewing a different scenario and pressing **Share**, **Print**, or **CSV**? One question first — the customer's ★ price (default) or the one you're viewing (for a teammate to check). The view never switches. A scenario showing {{chip:yellow|No prices yet}} can't be starred yet — it offers a one-click **copy prices from** your priced scenario, or price it with the solver. When a new bid starts its prices from a scenario, the copy keeps that scenario's name (WENDI stays WENDI). The **Counts** tab shows the same version picker — each bid in the package has its own count list.
