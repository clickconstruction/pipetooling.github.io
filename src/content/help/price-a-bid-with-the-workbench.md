...
5. The **margin brush** lives left of Solve › — press **🖌 Margin ›** and a purple ring unfolds with the margin box and your recent margins (the loaded one lit solid purple). Your cursor becomes the brush: **press and sweep across rows**, and every row the brush crosses is priced at that margin the instant it's touched — one motion, many rows, totals moving as you go. Rows you've held with 📌, fixed-price rows, and rows with no Takeoffs cost are never painted. Let go and the sweep saves in one batch (**↩ Undo sweep** takes the whole stroke back); **‹** or Esc puts the brush down. The margin math is the same as everywhere else — cost ÷ (1 − margin), whole dollars — and the margin you sweep with becomes your top recent, shared with the Old grid's Margin-mode chips.
