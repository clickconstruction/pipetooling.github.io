...
3. Questions without a shared issue list individually below, each with its own {{button:blue|Answer}} box. The bid number inside a question is a link — open it in a new tab for context before you answer.
4. **Only questions about the job land here.** When a robot's problem is its own machine — a sandbox, a sign-in, a file it can't open, a table it can't write — that question goes to the person who runs the robots instead, on their console. If one slips through and reads like it's talking to someone else, hit {{button:gray|Not mine}} and it moves over; {{button:gray|Dismiss}} closes a question you don't intend to answer, and the robots stop asking.
