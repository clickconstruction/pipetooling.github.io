...
- **Projects** — full names with the current step and who's on it ("step 4/9 · Top Out · Malachi"), plus a flag when the step is stuck (waiting, unassigned, no schedule). Click to open the workflow.
- **Bids** — a summary line first ("44 total · 9 won · 11 lost · 24 undecided"), then two-line rows sorted chase-first: the bid number (opens Bid Preview), project, and **bid value** on top; the **address** and a **clock** below — {{chip:yellow|due in 3d}} for live bids, "sent 22d ago · undecided" for submitted ones, "won · Jun 12" once decided. The rest sit behind **show all**.
- **Estimates** — number, title, status, and the **dollar amount**. Click to open the estimate.
