...
- **Work rails** — their jobs, projects (with the current step), bids, and estimates as clickable pills with status dots. Each pill opens the thing itself: jobs open Job Detail, projects open the workflow, bids open Bid Preview, estimates open the estimate.
- **Footer** — {{button:outline|✎ Edit customer}} opens the normal customer editor; **Their projects** filters the Projects page to them.
