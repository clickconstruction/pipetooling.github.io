...
You do not have to know it before you press Generate. The card that shows the new code asks **What did they say they need?** — read them the code, then ask, and {{button:blue|Write it down}} while they are still on the line. A ledger row with nothing written down reads *add what it was for…*; click it any time later (when the tech texts you the list, say), and *change* fixes a claim that was typed wrong.
