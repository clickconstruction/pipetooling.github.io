...
- {{chip:blue|Job}} — the read view: photo and address, customer and contacts, the **Job accounts** line (Ferguson ✓ · Reece none yet — tap a chip for what to say at the counter or to ask the office; see *open a job account before buying parts*), the numbered activity feed, work/bill dates, and a compact **Costs** card (team labor, sub labor, parts, margin — one line each; tap it to open the Costs tab). This is where "open job detail" lands.
...
- {{chip:blue|Bill}} — money **in**: the **Line Items** (the job's scope and Job Total) right at the top, then the billing summary bar, the segment bar with the break-off slider, **New Invoice**, and the Invoices and Payments received tables. A saved payment that landed on the wrong job has {{button:outline|Move to job…}}: pick the right job, read both jobs' paid and open before and after, say why, and it moves with its date, amount and bank link — a payment a sent bill already counted asks you to unlink it from that bill first, and both jobs keep a grey trace line under the table. When a supply house **job account** is on file for the job, a teal note sits just above Invoices — who holds it, when the packet went out, and any unpaid supplier invoices flagged on the account (see *mark an invoice as on a job account*).
- {{chip:blue|Costs}} — money **out**, told straight: four numbers first — **true margin at completion**, **spent so far**, **earned so far** (% done × the price) and **time left** — then the baseline strip and one chart (owners, controllers and master techs; see *read the cost and value timeline on a job*), then **Where the money went** (each source's share) above the parts accordions (supply house invoices, card charges, parts from tally, other job charges with **+ Add other charge**). The daily spend and the Cost Timeline sit behind **Show the timeline · daily spend**. Everything that used to sit at the bottom of Bill lives here now.
