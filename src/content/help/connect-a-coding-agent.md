...
1. Type which machine the key is for — that is its label, and how you will revoke it later — and press {{button:blue|Issue a key}}.
2. The key is shown **once**. Press {{button:blue|Copy shell line}} and paste the line into your shell profile (`~/.zshrc`). The key lives in your environment; it never goes in the repo or in a chat.
