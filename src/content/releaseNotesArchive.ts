...
/**
 * Frozen pre-fragments release notes (everything up to the fragments cutover).
 * DO NOT add entries here — new release notes are one file each in
 * src/content/releaseNotes/v2.NNNN.ts (see src/content/releaseNotes.ts).
 */
export const RELEASE_NOTES_ARCHIVE: ReleaseNote[] = [
  ...
  },
  {
    ...
    kind: 'fix',
    highlights: [
      'When assigning a clock session, search results now show the job or bid name on its own full-width line, with the status, dollars, and dates on a second line — instead of squeezing everything side by side and cutting the name down to "J891 · …".',
    ],
  },
  {
    version: 'v2.1581',
