...
type SupplyHouseInvoiceWithPO = SupplyHouseInvoice
type InvoiceJobAllocation = { job_id: string; pct: number }
type SupplyHouseInvoiceWithAllocations = SupplyHouseInvoiceWithPO & { job_allocations?: InvoiceJobAllocation[] }
...
export default function Materials() {
  ...
  const [invoicePurchaseOrderNumber, setInvoicePurchaseOrderNumber] = useState<string>('')
  ...
  async function loadSupplyHouseDetail(sh: SupplyHouse) {
    ...
    const invoices = (invRes.data as SupplyHouseInvoiceWithPO[]) ?? []
  ...
  // Invoice job search: debounced search when modal is open
  useEffect(() => {
    const t = setTimeout(() => {
  ...
  // Invoice job allocations: load job details for display
  useEffect(() => {
                                      ...
                                      <section>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                                          ...
                                          <button
                                            ...
                                            style={{ padding: '0.35rem 0.75rem', fontSize: '0.875rem', background: '#3b82f6', color: 'white', border: 'none', borderRadius: 4, cursor: creatingPOForSupplyHouse || !selectedServiceTypeId ? 'not-allowed' : 'pointer' }}
                                          >
                ...
                <form onSubmit={applyPayment}>
                  ...
                  <div style={{ marginBottom: '0.75rem' }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontWeight: 500 }}>
                      <input
                        type="checkbox"
                        checked={applyPaymentShowAll}
                        onChange={(e) => setApplyPaymentShowAll(e.target.checked)}
                      />
                      Show all invoices
                    </label>
                  </div>
