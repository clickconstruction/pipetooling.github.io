...
describe('words', () => {
  it('the sentence and the chip', () => {
    ...
    expect(formatOrderIncrement({ increment: 10, unit: 'box' })).toBe('box of 10')
    expect(formatOrderIncrement({ increment: 2.5, unit: 'bundle' })).toBe('bundle of 2.5')
...
describe('roundUpToIncrement', () => {
  it('105 ft at 20 ft sticks is 120 ft; an exact multiple stays; float dust does not add a stick; zero stays zero', () => {
