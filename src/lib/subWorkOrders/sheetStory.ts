...
export type SheetStoryFact = { k?: string; text: string; quote?: boolean; /** `paperwork`: the text is a door to the sub's Person Desk → Paperwork. */ link?: 'paperwork' }
...
export const SHEET_STORY_ACTION_LABEL: Record<SheetStoryAction, string> = {
  ...
  reoffer: 'Re-offer…',
  nudge: 'Nudge',
  to_walkthrough: 'Move to Pre-inspection',
  to_customer_pays: 'Passed → Trigger draw',
  back_to_work: 'Step back to Work',
  back_to_walkthrough: 'Step back to Pre-inspection',
...
export type SheetStoryRow = {
  ...
  sees: string | null
  /** Where `sees` lives on the portal — the sheet card or the offer card — so the office can be shown it there. */
  seesOn: 'sheet' | 'offer' | null
...
export const SUB_SEES: Record<'work' | 'walkthrough' | 'customer_pay' | 'queued' | 'offer', string> = {
  ...
  work: '“Finish the work, so we can call it in for inspection.” with ✓ My work here is done.',
  walkthrough: '“You told us the work\'s done. We\'ve called it in for inspection — a short punch list may come first.”',
  customer_pay: '“Passed inspection. Draw requested from the customer — their payment is the last thing between you and this money…”',
...
export function buildSheetStory(input: SheetStoryInput): SheetStoryRow[] {
  ...
  if (!crewPay) {
    // Drafted
    const drafted: SheetStoryRow = { key: 'drafted', label: 'Drafted', state: stateOf('drafted'), office: true, chip: null, facts: [], sees: null, seesOn: null, actions: [] }
    if (c.kind === 'none') {
    ...
    // Sent
    const sent: SheetStoryRow = { key: 'sent', label: 'Sent', state: stateOf('sent'), office: true, chip: null, facts: [], sees: null, seesOn: null, actions: [] }
    ...
    // Signed
    const signed: SheetStoryRow = { key: 'signed', label: 'Signed', state: stateOf('signed'), office: true, chip: null, facts: [], sees: null, seesOn: null, actions: [] }
    ...
    if (paperwork) {
      ...
      signed.facts.push({ k: 'Binds under', text: bits.join(' · '), link: 'paperwork' })
    }
    rows.push(signed)
  }
  ...
  // Pre-inspection
  const walk: SheetStoryRow = { key: 'inspection', label: SHEET_RAIL_STEP_LABEL.inspection, state: stateOf('inspection'), office: false, chip: null, facts: [], sees: null, seesOn: null, actions: [] }
  const toWalk = [...events].reverse().find((e) => e.to === 'walkthrough')
  if (toWalk) {
    walk.facts.push({ k: when(toWalk.occurred_at), text: toWalk.source === 'portal' ? `${subName} tapped My work here is done on the portal` : `moved to Pre-inspection${toWalk.actorName ? ` by ${toWalk.actorName}` : ''} (${sourceWord(toWalk.source)})` })
  ...
  // Post-inspection: Trigger draw
  const pays: SheetStoryRow = { key: 'customer_pays', label: SHEET_RAIL_STEP_LABEL.customer_pays, state: stateOf('customer_pays'), office: false, chip: null, facts: [], sees: null, seesOn: null, actions: [] }
  if (job) {
  ...
  // Paid
  const paid: SheetStoryRow = { key: 'paid', label: SHEET_RAIL_STEP_LABEL.paid, state: stateOf('paid'), office: false, chip: null, facts: [], sees: null, seesOn: null, actions: [] }
