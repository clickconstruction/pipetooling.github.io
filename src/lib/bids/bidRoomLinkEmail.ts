/**
 * App-side TWIN of `supabase/functions/_shared/bidRoomLinkEmail.ts` (v2.2732) — the letter a GC
 * receives with their bid-room link, buildable in the browser so Settings → Email templates &
 * testing → Outbound email catalog can open it as a preview. App code must not import `_shared`,
 * so the body is copied verbatim; `bidRoomLinkEmail.sharedParity.test.ts` fails the build the
 * moment the two drift by a byte. Edit the shared file first, then paste here.
 */
import type { BidRoomRevisionPayloadV1 as SharedBidRoomPayload } from './bidRoomPayload'

/** Same four replacements as `_shared/estimateEmailBrandImage.ts#escapeHtmlForEmail` (parity-tested). */
