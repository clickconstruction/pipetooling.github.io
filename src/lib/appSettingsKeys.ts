 ...
 */
export const APP_SETTINGS_KEY_LABOR_BURDEN_FACTOR_V1 = 'labor_burden_factor_v1' as const

export const DEFAULT_LABOR_BURDEN_FACTOR = 1.2

/** Parse the burden factor: missing, garbage, or outside 1–3 falls back to the default (a wage never shrinks under burden). */
export function parseLaborBurdenFactor(value: number | string | null | undefined): number {
  if (value === null || value === undefined || value === '') return DEFAULT_LABOR_BURDEN_FACTOR
  const n = typeof value === 'number' ? value : Number(String(value).trim())
