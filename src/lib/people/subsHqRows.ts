...
export type SubsHqSheetInput = Pick<LaborJob, 'id' | 'labor_rate' | 'items' | 'payments'> & {
  ...
  /** v2.2836: the sheet's job date (YYYY-MM-DD) — feeds the sub's "last worked". */
  jobDateYmd?: string | null
  /** v2.2836: when the sheet was paid (YYYY-MM-DD) — also counts as work. */
...
export type SubsHqCommitmentInput = {
  ...
  /** v2.2836: when the sub accepted the offer (YYYY-MM-DD) — counts as work. */
...
export type SubsHqRow = {
  ...
  /** v2.2836: newest attributed sheet job date / paid date / accepted work order (YYYY-MM-DD); null = never. */
