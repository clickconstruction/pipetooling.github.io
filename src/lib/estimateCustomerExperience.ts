/** Merge dev defaults (app_settings), per-estimate overrides, and template vars into customer-facing copy. Keep in sync with `supabase/functions/_shared/estimateCustomerExperience.ts`. */
...
export const ESTIMATE_APP_SETTING_LABELS: Record<string, string> = {
  ...
  estimate_accept_checkbox_label: 'Agreement checkbox label',
  estimate_accept_submit_label: 'Submit button label',
  estimate_accept_submitting_label: 'Submitting button label',
  estimate_thank_you_title: 'Thank you heading',
  estimate_thank_you_body: 'Thank you body',
  estimate_doc_title_fallback: 'Document title fallback (empty estimate title)',
  estimate_doc_valid_through_prefix: '“Expires on” line prefix (before date)',
  estimate_doc_line_items_heading: 'Line items heading',
  estimate_doc_terms_heading: 'Terms heading',
  estimate_doc_total_label: 'Total label (before amount)',
  estimate_accept_page_footer:
    'Acceptance page footer (below sign-off). Pre-filled with built-in copy when no value is saved; Save persists org-wide. Clear and Save leaves the row empty and runtime uses built-in again.',
}
