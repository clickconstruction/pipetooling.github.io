/**
 * Submittal rows (Submittals stage 2b): one row per fixture tag on the plan's
...
/**
 * Does the quote line's text carry the specified model number? A token that is
 * the model exactly says yes; a token that is only a near match (B74-CH against
 * B74C) says no, so the status kernel can flag it `near` instead of calling it
 * the specified unit; otherwise the joined text is searched for a model the
 * vendor split across tokens ("CT708 UVG").
 */
function labelContainsModel(label: string | null, model: string | null): boolean {
  ...
  const rawTokens = label.split(/\s+/)
  const tokens = rawTokens.map((t) => normalizeModel(t))
  if (tokens.some((t) => t === spec)) return true
  if (rawTokens.some((t) => sameUnitNear(t, model ?? ''))) return false
