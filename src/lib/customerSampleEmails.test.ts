...
describe('sample emails (What customers see)', () => {
  ...
  it('the job-contract emails (v2.3510) come from the senders\' builder over the sample job, with the signing link', () => {
    ...
    const paper = buildSampleEmail('job-contract-paper', ctx)
    expect(paper.subject).toBe('Please sign: Service agreement for 100 Sample St, Kyle, TX 78640 — Job #1042 (PDF attached)')
    expect(paper.text).toContain('sign and date the last page')
    expect(paper.html).toContain('https://x.test/contract/sign?t=sample')
