 ...
 *
 * Pure — consumed by the Bill tab, the Edit read-out, the customer drawer and
 * (ported) the customer-portal function. Shaped like `subs/subsTabRows.ts`.
 */
...
export function gcView(plan: StagePlan): GcView {
  ...
  shared.forEach((r, i) => {
    ...
    steps.push({ name: r.name, number, state, line, pct: state === 'now' && r.work === 'working' ? r.pct : null, askable: state === 'next', ask: state === 'next' ? r.ask : null })
  })
