...
export const CLOCK_SESSION_TODAY_STRIP_SELECT =
  'id, user_id, clocked_in_at, clocked_out_at, approved_at, rejected_at, revoked_at, notes, job_ledger_id, bid_id, users!clock_sessions_user_id_fkey(name), jobs_ledger!clock_sessions_job_ledger_id_fkey(hcp_number, job_name, job_address), bids!clock_sessions_bid_id_fkey(bid_number, project_name, address, customers(name))'

/** Month grid + day modal: own sessions with job/bid embeds (no GPS / approver joins). */
export const CLOCK_SESSION_CALENDAR_SELECT =
  'id, user_id, work_date, clocked_in_at, clocked_out_at, notes, job_ledger_id, bid_id, origin, rejected_at, revoked_at, jobs_ledger!clock_sessions_job_ledger_id_fkey(hcp_number, job_name, job_address), bids!clock_sessions_bid_id_fkey(bid_number, project_name, address, customers(name))'
