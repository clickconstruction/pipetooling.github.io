...
- **4 Major Systems** (+ significant subsystems):
  ...
  - **Estimates** (internal proposals; global **Quote #** `estimate_number`, staff **`/estimates/{quote#}`** (detail **`# n` + title** one heading line); **Line item catalog** modal with **Insert from catalog** / **Edit book** in one header row; **sent**: **Copy/Open customer link** under waiting copy (`EstimateCustomerAcceptLinkButtons`); **customer_accepted**: **Unlink job** (modal; clears link only), **Create job from estimate** / Jobs **Source estimate**; customer acceptance via public `/estimate/accept` token link (**Approve** modal: no **`accept_instructions`** line, centered submit; document **Total** right-aligned; inline **Customer acceptance** record shows disclosure + agreement checkbox); **Customer activity** timeline (**`estimate_customer_events`**: DB trigger on first **`sent` → `customer_accepted`**, **`record_estimate_public_link_view`** for link opens, **`client_ip`** when forwarded); Edge Functions + RLS; see `PROJECT_DOCUMENTATION.md`, **RECENT_FEATURES** v2.244)
...
| Recent changes and features | `RECENT_FEATURES.md` → Chronological updates (e.g. v2.284 Banking **Mercury/Stripe** URL + dev **Invoices/Data** grids; Jobs **Stages** thread-stats debounce/chunks; v2.281 Dashboard **Mix** copy day job % + **My Time** clock preview (**punch** locked); v2.257 linked **job_schedule_blocks** + Dispatch linked copy (`+` on cards); v2.249 salary split sync overlap + clock-in TZ date; v2.231 strip **My Time** from **Jobs worked today** duration; v2.229 salary indexed-slot split + overlap guard; v2.228 Salaried workday UI + continuous sync guard) |
...
| Dashboard **My Time** / **Edit time** (this-week-only, Form/Visual, merge + job override, `myTimeDayTimeline`; **Overlapping clock times** / **Multiple jobs/bids in this span**; **read-only punch** from **Clock** preview — **`clockTimesReadOnly`**, save on close) | `RECENT_FEATURES.md` → v2.281, v2.231, v2.193, v2.192, v2.179; `PROJECT_DOCUMENTATION.md` → Dashboard **My Time**; `src/components/DashboardMyTimeSection.tsx`; `src/components/DashboardMyTimeDayEditorModal.tsx`; `src/components/my-time-day-editor/`; `src/lib/myTimeDayTimeline.ts`; `src/lib/myTimeDaySavePlan.ts` |
...
| People Hours: Audit modal edit mode, shared clock split/create modal, **Highlight by job** | `RECENT_FEATURES.md` → v2.171; `PROJECT_DOCUMENTATION.md` → People §5 Hours; `PeopleHoursDayAuditModal.tsx`; `ClockSessionEditSplitModal.tsx` |
