...
| People **Contracts** (`/people` tab): **Contracts** tab **search** (person + document names; matching document lines), **templates** (**Manage templates** indented doc lists), **Assign template** modal (**search** + list), **Contract Book** (library **View** / **Edit**), **Add document** **Upload Signed** (saved **`status`** signed) vs **Request Signature**, **`person_contract_documents`**, public **`/contract/accept`** | `RECENT_FEATURES.md` → v2.355, v2.354, v2.353, v2.352, v2.351, v2.346; [`People.tsx`](src/pages/People.tsx); [`ContractBookModal.tsx`](src/components/contracts/ContractBookModal.tsx); [`ContractAccept.tsx`](src/pages/ContractAccept.tsx); [`EDGE_FUNCTIONS.md`](EDGE_FUNCTIONS.md) → **send-contract-for-signature** |
