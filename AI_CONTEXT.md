...
- **4 Major Systems** (+ significant subsystems):
  ...
  4. Checklist (recurring tasks, Today/History/**Review**/Manage/**Roadmap** tabs, `?tab=roadmap` + `roadmap=` — **`checklist_tech_tree_roadmaps`**, members, **`roadmap_id`** on groups, **canvas** icon controls + full-screen header **v2.408** / **v2.407** / **v2.380**)
  ...
  - **Prospects** (lead management, Convert tab, callbacks; **Team** activity in **`prospectTeamActivity`**) and **Quickfill** (billing workflow, **Stages: customer & job pictures** **v2.413**, **Banking sorting** snapshot, **Prospects** section — warmth + **Team** line chart (**`ProspectTeamActivityLineChart`**, **`recharts`**) + link — **`QuickfillProspectsSection`**, Crew Jobs / Bids with Realtime **`CrewJobsBlock`**) are major subsystems
