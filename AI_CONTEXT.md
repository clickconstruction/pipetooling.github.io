...
- **4 Major Systems** (+ significant subsystems):
  ...
  - **Estimates** (internal proposals; global **Quote #** `estimate_number`, staff **`/estimates/{quote#}`** (detail **`# n` + title** one heading line); **Line item catalog** modal with **Insert from catalog** / **Edit book** in one header row; **sent**: **Copy/Open customer link** under waiting copy (`EstimateCustomerAcceptLinkButtons`); **customer_accepted**: **Unlink job** (modal; clears link only), **Create job from estimate** / Jobs **Source estimate**; customer acceptance via public `/estimate/accept` token link (**Approve** modal: no **`accept_instructions`** line, centered submit; document **Total** right-aligned; inline **Customer acceptance** record shows disclosure + agreement checkbox); Edge Functions + RLS; see `PROJECT_DOCUMENTATION.md`, **RECENT_FEATURES** v2.239)
