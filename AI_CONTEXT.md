...
- **4 Major Systems** (+ significant subsystems):
  ...
  4. Checklist (recurring tasks, Today/History/**Review**/Manage tabs, push notifications; **Review**: **Outstanding by person** above dispatch/estimator inbox cards — **v2.380**)
  ...
  - **Prospects** (lead management, Convert tab, callbacks; **Team** activity in **`prospectTeamActivity`**) and **Quickfill** (billing workflow, **Banking sorting** snapshot, **Prospects** section — warmth + **Team** line chart (**`ProspectTeamActivityLineChart`**, **`recharts`**) + link — **`QuickfillProspectsSection`**, Crew Jobs / Bids with Realtime **`CrewJobsBlock`**) are major subsystems
...
- **`src/pages/Checklist.tsx`** - Recurring checklist (Today, History, **Review**, Manage); **Review** lists **Outstanding by person** then **`ChecklistReviewInboxes`** (v2.380)
- **`src/components/ChecklistTitleWithLinks.tsx`** - Renders checklist titles with [1], [2], etc. as clickable links
- **`src/pages/Jobs.tsx`** - Jobs (Reports, Stages, Billing, Team Labor, Sub Labor, Parts, **Job Summary**, Inspections, Teams Summary tabs); **Job Summary**: dedicated all-status [`fetchJobsLedgerWithDetailsForStages`](src/lib/fetchJobsLedgerWithDetailsForStages.ts), **HCP #** min filter ([`applyMinHcpFilter`](src/lib/jobSummaryHcpFilter.ts), default **500**, **RECENT_FEATURES** v2.395 / v2.396); **Sub Labor** New/Edit modal UX (**RECENT_FEATURES** v2.305): crew search filters, **Add Sub**, line-items + footer layout
...
- **`src/components/Layout.tsx`** - Nav; right cluster **Task Dispatch** / **Estimator Inbox** / **Task** / **Bid** share **`headerActionButtonBase`** height; **gear** menu **Banking** (v2.380)
