...
- **4 Major Systems** (+ significant subsystems):
  ...
  2. Bids (estimation system: Bid Board, Builder Review, Unsent/Working Kanban, Counts, Takeoff, Cost Estimate, Pricing, Cover Letter, Submission, RFI, Change Order, Lien Release; **trade-specific ledger prefixes** for bid # from **`service_types.ledger_bid_prefix`** with client **[`ledgerDisplayPrefixes.ts`](src/lib/ledgerDisplayPrefixes.ts)** + **[`LedgerDisplayPrefixContext`](src/contexts/LedgerDisplayPrefixContext.tsx)** — **RECENT_FEATURES** **v2.432**)
...
| People Hours: **`PeopleHoursDashboardClockStrip`** (**`DashboardTeamActiveClockStrip`**, **`dashboard_clock_strip_scope`**, **`peopleHoursClockStripSelectedDay`** day nav — **v2.453**) above Hours chrome; **v2.455** — **Pay** merged into **Hours** (section jump, **`#cost-matrix`**, **`?section=rejected`**); Audit modal edit mode, shared clock split/create modal, **Highlight by job**; manual grid blur → **My Time** proportional scale or draft (`peopleHoursProportionalScale.ts`, `peopleHoursManualDraftSession.ts`); merged job/bid search shows **trade** pills on jobs (**`RECENT_FEATURES`** **v2.433**) | `RECENT_FEATURES.md` → **v2.455**, **v2.453**, **v2.433**, v2.297, v2.291, v2.171; `PROJECT_DOCUMENTATION.md` → People §5 Hours; `ACCESS_CONTROL.md`; `src/components/people/PeopleHoursDashboardClockStrip.tsx`; `src/lib/dashboardClockStripScopeStorage.ts`; `src/lib/peopleHoursClockStripSelectedDay.ts`; `src/components/PeopleHoursDayAuditModal.tsx`; `src/components/ClockSessionEditSplitModal.tsx`; `src/components/DashboardMyTimeDayEditorModal.tsx`; `src/pages/People.tsx` |
