...
| Dashboard **Clock In** / **Update Focus** / **Review before clock out**: assigned jobs auto-load; optional **left a report on all my jobs** checkbox on clock-out review (**not** required; **v2.416**); **View today’s time** clock preview → **My Time** [v2.281]; **no assigned jobs** toast once per modal (v2.191); field borders / focus | `RECENT_FEATURES.md` → v2.416, v2.281, v2.182, v2.191; `PROJECT_DOCUMENTATION.md` → Dashboard **Clock In/Out**; `src/components/ClockInOutButton.tsx`; `src/contexts/ToastContext.tsx` |
