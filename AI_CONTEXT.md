...
- **4 Major Systems** (+ significant subsystems):
  ...
  - **Documents** (`/documents`) — ledgers for **Estimates**, **Bid proposals**, **Jobs** (tabs **`?tab=`**); **Docs** column + soft **+** to add/check Google links ([`Documents.tsx`](src/pages/Documents.tsx), [`DocumentsAddDriveLinkModal.tsx`](src/components/documents/DocumentsAddDriveLinkModal.tsx), [`checkGoogleDriveAttachmentUrl.ts`](src/lib/checkGoogleDriveAttachmentUrl.ts)); full-width search; compact header (**RECENT_FEATURES** **v2.314**; **`PROJECT_DOCUMENTATION.md`** → Documents page)
...
- **`src/pages/Bids.tsx`** (~14k lines) - Bids: Bid Board, Builder Review (PIA per customer), Counts, Takeoff, Cost Estimate, Pricing, Cover Letter, Submission, RFI, Change Order, Lien Release; Bid Board **Notes** row: tabs **All notes** (default), **Bid notes**, **Customer notes** — see `src/components/bidBoard/UnifiedBidCustomerNotes.tsx`; **Cover Letter** combined HTML: single **`<p>`**, **`pre-wrap`**, clipboard **HTML-only** for Google Docs paste (**v2.329**); **Submission** `?tab=submission-followup` pending deep link; note times via **`datetimeLocal.ts`** in **`BidNotesTable`** / **`UnifiedBidCustomerNotes`**; New/Edit bid modal uses **`SearchableSelect`** (`src/components/SearchableSelect.tsx`) for Estimator, Account Man, Service Type, Win/Loss; grid `bid-form-top-fields` (desktop vs mobile); **720px** modal width; **Project Address** then Distance + Plan Pages row
...
- **`src/pages/ScheduleDispatch.tsx`** - Week hub (**People** / **Jobs** / **Day** — **`hubTab`**; **Day** tab embeds **`QuickfillScheduleSection`**; URL **`day=`** column focus — [`scheduleDispatchColumnFocus.ts`](src/lib/scheduleDispatchColumnFocus.ts), v2.309) + job grid; **Add schedule block** modal with occupied timeline + draft moves ([`scheduleDispatchAddBlockTimeline.ts`](src/lib/scheduleDispatchAddBlockTimeline.ts), [`DispatchAddBlockTimeRange.tsx`](src/components/schedule/DispatchAddBlockTimeRange.tsx); **RECENT_FEATURES** v2.296, v2.310 session vs occupied strip sizing)
- **`src/pages/Documents.tsx`** - **Documents** ledgers (**Estimates** / **Bid proposals** / **Jobs** / **Upload**); **`documentsPageTab`**, **`DocumentsAddDriveLinkModal`**, shared ledger search + **Docs** icons (**RECENT_FEATURES** v2.314)
