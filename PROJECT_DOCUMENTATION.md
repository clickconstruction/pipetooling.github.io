...
- **Data hook**: [`useMapPageData.ts`](src/hooks/useMapPageData.ts) — loads address-bearing jobs/bids/estimates (RLS-visible rows), merges **`address_geocodes`**; clears **Loading…** immediately after cached coordinates apply, then invokes Edge **`geocode-address-batch`** in chunks (≤20 addresses) for misses with a **generation** guard so **Reload** does not race; optional **Resolving addresses…** line plus collapsible progress list; **`geocode-one`** remains for [**MapGeocodeReviewModal**](src/components/map/MapGeocodeReviewModal.tsx) **`refresh_google_only`** flows ([`invokeGeocodeOneRefreshGoogleOnly.ts`](src/lib/map/invokeGeocodeOneRefreshGoogleOnly.ts)); see [**EDGE_FUNCTIONS.md**](./EDGE_FUNCTIONS.md) (**`geocode-one`**, **`geocode-address-batch`**)
