...
- **Sorting `localStorage`**: Banking and Quickfill **Banking sorting** share **`banking_sorting_config_v1_<userId>`** ([`loadBankingSortingConfig`](src/lib/bankingSortingConfig.ts)). Jobs **Stages** → **Bank payments** → **Accounts Receivable Sorting** uses a **separate** key **`bank_payments_sorting_config_v1_<userId>`** (first load copies from the shared key; edits do not change Banking). **Kind** badge labels/colors for Bank Payments use **`bank_payments_kind_badges_v1`** (per-origin, not per-user; see [`bankPaymentsKindBadges.ts`](src/lib/bankPaymentsKindBadges.ts)).
