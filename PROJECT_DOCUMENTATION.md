...
- **Usage**: Settings → Team Hours Sharing (all managers see the value; **edit** visibility: dev only); extends `clock_sessions` SELECT/UPDATE and `approve_clock_sessions` / `revoke_clock_sessions` for team-lead paths.
...
- **Salary scheduling** (auto **`salary_schedule`** rows): Templates **`salary_work_schedule_templates`** (**`exclude_weekends`** default true) and optional **`salary_work_schedule_day_overrides`**; unpaid **`user_time_off`**, missing template, and excluded-weekend paths delete **non-final** `salary_schedule` rows for the day and then **close any remaining open** `clock_sessions` for that `work_date` at sync time. Edge **`sync-salary-sessions`** calls **`sync_salary_clock_sessions_for_day`** for the **America/Chicago** calendar date; Settings save triggers **`sync_salary_clock_sessions_for_user_day`**. Core logic: **`salary_sync_one_user_clock_sessions`** — **boundary model** (`20260404050204`): at each template block end, set **`clocked_out_at`** on **all** opens for that user/day to that block end (all origins; **`approved_at`** does not block); inside each block, insert/reopen canonical `salary_schedule` only when **no** session is open that day. **Continuous** days with pending **indexed** `salary_schedule` segments (after splitting the continuous row) still **skip** new NULL-index inserts (**`20270402100000`** behavior preserved). Splitting an **indexed** salary slot produces **`user_punch`** children with **`salary_segment_index` null**. **Operator / AI runbook**: [`SALARY_CLOCK_SESSIONS.md`](./SALARY_CLOCK_SESSIONS.md). **Settings UI**: [`SalaryWorkScheduleSettings.tsx`](src/components/SalaryWorkScheduleSettings.tsx), [`salaryScheduleEndTimeDisplay.ts`](src/lib/salaryScheduleEndTimeDisplay.ts). Dashboard: **On shift** / **Off shift** — [`ClockInOutButton.tsx`](src/components/ClockInOutButton.tsx). **`people_pay_config`** self-read for salaried Settings: policy **`Users can read own people pay config row`** (`20270331160000`).
...
- **Purpose**: Tracks which person-weeks have been marked as reviewed in the Pay tab. Supports a weekly "hours reviewed" workflow for dev, pay-approved masters, and assistants.
- **Key Fields**:
...
- **Usage**: Review Hours modal "Mark as reviewed" checkbox; Hours reviewed ledger on Pay tab
...
- **UI**: [`Estimates.tsx`](src/pages/Estimates.tsx) (list **Quote #** column; detail header: **`# {estimate_number}`** + title in one **`h1`** with muted **#** prefix; **`sent`** / **`customer_accepted`**: **Customer activity** timeline from **`estimate_customer_events`** — **“Customer opened quote link”** / **“Customer accepted estimate”** with optional **` · {client_ip}`** when stored, **`(with signature)`** on accept when applicable, then **—** and local datetime (refetch on **`window` `focus`** while **`sent`**); **`sent`**: **Copy customer link** / **Open customer link** under the waiting-for-customer line; shared [`EstimateCustomerAcceptLinkButtons.tsx`](src/components/estimates/EstimateCustomerAcceptLinkButtons.tsx) — when **`sent`**, those buttons are omitted from the top of **Customer experience** to avoid duplication; **`customer_accepted`**: **Job** section — **Create job from estimate** / link, **Unlink job** (modal confirm; clears **`job_ledger_id` only**); **Customer experience** collapsible with **Email**, **Acceptance page**, **Thank you** previews; **draft** optional **Customize customer copy** fields appear **under** the matching preview tab, from org defaults in **`app_settings`**; **Line item catalog** modal: single header row with **Insert from catalog** and **Edit book** between the title and **×**, close trailing with **`marginLeft: 'auto'`** when the user can manage the catalog), public [`EstimateAccept.tsx`](src/pages/EstimateAccept.tsx). Shared body + modal: [`EstimateAcceptBody.tsx`](src/components/estimates/EstimateAcceptBody.tsx) — **Approve** modal omits **`accept_instructions`**; primary submit control centered; when staff show an **accepted** record inline, **Customer acceptance** includes modal-matching signature disclosure plus a checked disabled **`accept_checkbox_label`** row before **Full name**. Shared customer-facing markup: [`EstimateCustomerDocument.tsx`](src/components/estimates/EstimateCustomerDocument.tsx) (line-items **Total** right-aligned), [`EstimateCustomerThankYou.tsx`](src/components/estimates/EstimateCustomerThankYou.tsx)—**Valid through** (prefix + date) renders only when **`valid_until`** is set; the document **`doc_*`** title fallback is used only when the estimate **title** is empty. Nav: Materials → **Estimates** → Bids (where applicable).
- **Customer copy**: Merge order: **`customer_experience_sent`** (frozen when estimate moves to **`sent`**) overrides live merges; else **`app_settings`** keys `estimate_*` (dev: [`Settings.tsx`](src/pages/Settings.tsx) **Estimate customer experience defaults**) plus optional **`customer_experience_overrides`** on the row. Draft **Customize customer copy** groups fields into **Email**, **Acceptance page**, and **Thank you** under each respective preview tab (with **Acceptance** covering both **`doc_*`** quote-document strings and **`accept_*`** accept-form strings); each textarea **shows** merged defaults (builtins plus **`app_settings`**) until staff edits, and **`customer_experience_overrides`** persists only changed keys. Templates support **`{{accept_url}}`**, **`{{title}}`**, **`{{estimate_number}}`** in email subject/body. Shared logic: [`src/lib/estimateCustomerExperience.ts`](src/lib/estimateCustomerExperience.ts) and [`supabase/functions/_shared/estimateCustomerExperience.ts`](supabase/functions/_shared/estimateCustomerExperience.ts) (keep in sync). Public GET JSON includes **`customer_experience`** (no email fields) for the accept page; **`already_accepted`** **409** includes **`customer_experience`** for thank-you. Legacy **`estimateCustomerEmail.ts`** files are unused by the app; Edge sends **`resolveEstimateCustomerExperience`** output.
...
### 5. People Roster
- **Page**: `People.tsx`
- **Tabs**: **Users** (default), **Pay History**, **Pay**, **Hours**, **Team Costs**, **Vehicles**, **Housing**, **Offsets**, **Review** (dev-only)
- **Features**:
  - **Users tab** lists roster groups in fixed order: Master Technicians, Assistants, Primaries, Estimators, Superintendents, Subcontractors, then Devs (Devs section dev-only).
  ...
  - **Pay History tab**: Single-person pay report generator (person + date range). **Generate Pay Reports** (bulk modal) sets the pay period (**Last week** / **Next week**, compact start/end dates); per-person status includes **Paid**, **Partial**, or **Report only** from **`pay_stub_payments`** vs **Net Pay** (gross minus **`pay_stub_deductions`** plus sum of **`pay_stub_additional_lines`**); summary **N of M paid** counts people **fully** paid for the period. **Draft Payroll** opens with the **prior** Sunday–Saturday week (**`en-CA`**) and merges **`people_crew_jobs`** / **`people_crew_bids`** into the review grid for that period. **Ledger** lists pay stubs with **Search** (person name); below the **Ledger** heading, a line shows how many stubs in the filtered list are not fully paid and the sum of **Balance**. Columns: Person (link → **Annual Pay to Date** grid: earned vs **allocated** pay by work day from `pay_stub_days`—not cash payout dates), Period, Hours, **Gross Pay**, **Less** (click dollar amount to open modal: manual charges or apply pending offsets that are not employee credits), **Additional** (click to edit quantity × rate lines), **Net Pay**, **Paid to date**, **Balance** (vs Net Pay), **Payment** (Unpaid / Partial / Paid, detail icon for installments and optional per-row delete, or legacy memo), **Record payment** (single **Amount paid**; confirm records up to remaining balance, optional flow to record excess as **employee credit**; DB trigger prevents total installments from exceeding **Net Pay**), Created, **Actions** (**Print**; **View** only in bulk modal). Devs: red trash **PayStubDeleteIcon** to delete a stub. Section header: generate control on the right.
  - **Pay Report** (pay stub document): Date | Hours | **Jobs / Bids** from `people_crew_jobs` / `people_crew_bids`; **Gross Pay**, optional itemized **Additional** (`pay_stub_additional_lines`), **Less** and **Net Pay**; pending offsets FYI; **Vehicles** then **Housing** (weekly amounts for units in possession during the period); **Physical payments** footer from `pay_stub_payments` (amount, date, memo, total).
...
- **Features**:
  ...
  - **Billing Tab** (HCP Jobs): Jobs ledger (HCP #, Job Name, Address, materials, team members, revenue); New Job, search; **Edit** and **Delete** per row, vertically centered in the row. Google Drive and Job Plans icons shown when links are filled; stacked vertically. New/Edit Job modals include Job Total / Bid ($) and Remaining ($) side by side; **Job Total / Bid** and **Payments received** amounts show **comma thousands** on blur (`Jobs.tsx`, `MoneyDecimalAmountInput`). **Billing** accordion in **Edit Job** ([`JobFormModal.tsx`](src/components/jobs/JobFormModal.tsx)):
    - **Ready to Bill**: draft invoice list with **View in Stages** and **Preview / Stripe bill…** (Bill Customer via [`BillCustomerModalContext`](src/contexts/BillCustomerModalContext.tsx)).
    - **Partial invoice** (below **Ready to Bill**, above **Outstanding billing**): gray card with centered **Make Invoice:** (amount field) and **Create invoice**; gray footer line *Break off an amount to send through Ready to Bill. Job stays in Working.* only (no section title; **Remaining (billable)** / **Use full remaining** not shown).
    - **Outstanding billing**: table **Date** (short month + day and **`(+n)`** calendar age from invoice creation), **Billed**, **Actions**—**Stages**, **Bill** (Stripe hosted invoice), and compact **StripeInvoiceSharePanel** copy/SMS/email icons in one row; optional full-width second row for **outside send Note** and **Stripe memo** (no gray panel; no top border above that row). **Label** column removed; layout uses fixed column widths.
    - **Payments received**: same section title / scroll wrapper pattern as Outstanding billing; columns **Date**, **Amount ($)**, **Memo**; **Record Payment** (right-aligned). **Stripe-linked** payment rows: default body background + **blue left inset** only (not the same gray as **thead**).
  - **Customer link for Ready to Bill billing** (`jobs_ledger.customer_id`): Required before **Invoice / Update** or **Ham mode** instant **billed** on **Stages**. If missing: toast and **Edit Job** opens with **Customer** expanded and red emphasis on **Link to customer** + **Create customer from job** (`billingCustomerHighlight`). **Dashboard** Ready to Bill shows a toast only. **Bill Customer** (**`SendRecordInvoiceModal`**) is opened via shared **[`BillCustomerModalContext`](src/contexts/BillCustomerModalContext.tsx)** / **`BillCustomerModalProvider`** in **`App.tsx`** from Jobs Stages, Dashboard Ready to Bill, and **Edit Job** billing (**Preview / Stripe bill…** on **Ready to Bill** rows). The Stripe tab shows a debounced **pre-submit preview** via Edge **`preview-stripe-invoice`** ([`StripeBillPreSubmitPreview.tsx`](src/components/jobs/StripeBillPreSubmitPreview.tsx), [`stripeInvoicePreview.ts`](src/lib/stripeInvoicePreview.ts)); share/copy helpers live in [`stripeInvoiceShareCopy.ts`](src/lib/stripeInvoiceShareCopy.ts) (**`StripeInvoiceSharePanel`**). **`SendRecordInvoiceModal`** and Edge **`create-stripe-invoice`** enforce the same customer rule. **`get_jobs_ledger_by_status`** includes **`customer_id`** for Dashboard job cards (`20260330065236`).
...
- **Features**:
  ...
  - **Assigned Jobs**: Job cards from `list_assigned_jobs_for_dashboard` (status working; user in jobs_ledger_team_members). RPC returns `last_report_at` (max reports.created_at for job) for subcontractor "time since last report" display. Each card shows HCP, name, address, View Reports, Send to Billing, Open X (time since created or since last report). Subcontractors also see Last report, Leave Report. Superintendents: View link removed. **In-progress stage banner**: When job has `in_progress_stage_name`, a full-width soft purple banner spans the bottom of the card; click navigates to Workflow at that step. On mobile, "Open X" displays on one line.
  - **Superintendent Jobs** (superintendent role): Collapsible section (expanded by default) for project-linked jobs from `list_superintendent_jobs_for_dashboard` that are not already in Assigned Jobs. Same card layout as Assigned Jobs (View Reports, Send to Billing, Open X; no View link). Superintendents can mark jobs Ready for Billing (see `update_job_status` and migration `20260624000000_allow_superintendent_send_to_billing`). In-progress stage banner at bottom links to workflow step.
...
- **Supply Houses section**: Summary at top with AP total (Supply Houses: $X); expandable rows per supply house; Add Supply House button. Per supply house: name, address, phone (with **Open website** when **`website_url`** is set), email, **Monthly payment date** (day 1-31, sets Due column); invoices (Invoice #, Date, Due Date, Amount, Link, Paid checkbox); purchase orders linked via `supply_house_id`. Due column uses `monthly_payment_day` from supply house (e.g. "15th"); unpaid invoices sum to outstanding. Tables: `supply_house_invoices`, `supply_houses`; `purchase_orders.supply_house_id`.
...
- Selecting a bid shows an inline panel with **Add row**, **Import**, and its fixture/count rows. **Import** opens a modal to paste tab- or comma-separated text (Fixture, Count, Plan Page per line) for bulk import. Table columns: **Reorder** (drag handle), **Count\***, **Fixture\***, **Group/Tag**, **Plan Page**, **Actions** (centered headers). **Drag-and-drop reordering**: Drag the grip icon to reorder rows; order persisted via `update_bids_count_rows_order` RPC.
...
- **Layout**: Two tab rows. **Product**: **Mercury** | **Stripe** (Stripe row visible to **dev** only; master/assistant URLs that set **`product=stripe`** clamp to Mercury **Sorting**). **Mercury** second row: **Ledger**, **Sorting** (assistants land on Sorting; full Mercury includes Configuration / sync where implemented). **Stripe** second row: **Invoices**, **Data** (dev only).
- **URL**: **`?product=mercury|stripe`** and **`?tab=…`**. Mercury tabs: **`ledger`**, **`sorting`**. Stripe tabs: **`invoices`**, **`data`**. **`?tab=sorting`** without **`product`** still opens Mercury **Sorting** (legacy bookmarks).
...
- `index.html` - Inline script (before the app bundle) restores the route after **Hard Reload** / **force reload**: those flows save the current URL to `sessionStorage` and load **`/?nocache=…`** so GitHub Pages serves the shell with a **200**; see [`src/lib/hardReload.ts`](src/lib/hardReload.ts) and [`TROUBLESHOOT_404.md`](TROUBLESHOOT_404.md)
