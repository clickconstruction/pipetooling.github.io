...
- **RPCs**: `approve_clock_sessions(p_session_ids UUID[])` merges hours into `people_hours` and marks sessions approved; for sessions with `job_ledger_id`, also creates/updates `people_crew_jobs` (percentages by hours); for sessions with `bid_id`, also creates/updates `people_crew_bids`. `revoke_clock_sessions(p_session_ids UUID[])` subtracts hours from `people_hours` and moves sessions back to Pending; for sessions with `job_ledger_id`, recomputes or removes `people_crew_jobs`; for sessions with `bid_id`, recomputes or removes `people_crew_bids`.
