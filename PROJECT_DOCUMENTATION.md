...
- **Features**:
  ...
  - **Pay History tab**: Single-person pay report generator (person + date range). **Generate Pay Reports** (bulk modal) sets the pay period (**Last week** / **Next week**, compact start/end dates); per-person status includes **Paid**, **Partial**, or **Report only** from **`pay_stub_payments`** vs stub **gross_pay**; summary **N of M paid** counts people **fully** paid for the period. **Ledger** lists pay stubs with **Search** (person name). Columns: Person (link → **Annual Pay to Date** grid: earned vs **allocated** pay by work day from `pay_stub_days`—not cash payout dates), Period, Hours, Gross Pay, **Paid to date**, **Balance**, **Payment** (Unpaid / Partial / Paid, detail icon for installments and optional per-row delete, or legacy memo), **Record payment** (amount + sent date + note; DB trigger prevents overpay), **Clear payments** (confirm: delete all installments and clear legacy `paid_at` / `paid_by` / `paid_note`), Created, **Actions** (**Print**; **View** only in bulk modal). Devs: red trash **PayStubDeleteIcon** to delete a stub. Section header: generate control on the right.
  - **Pay Report** (pay stub document): Date | Hours | **Jobs / Bids** from `people_crew_jobs` / `people_crew_bids`; optional **Physical payments** footer from `pay_stub_payments` (amount, date, memo, total).
