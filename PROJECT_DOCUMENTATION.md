...
- **Features**:
  ...
  - **Offsets Tab** (dev, pay-approved masters, assistants): Backcharges and damages per person. Offsets can be Pending (not yet applied) or Applied (linked to a pay stub via `pay_stub_id`). Pending offsets appear on printed pay reports for visibility. Dollar impact on a check is tracked in **Pay History** as **`pay_stub_deductions`** (**Less**); applying an offset from the Less modal links the offset and adds a deduction line. Apply/Unapply on the Offsets tab still links `person_offsets` to a stub (keep in sync with Less when using that flow).
  - **Pay History tab**: Single-person pay report generator (person + date range). **Generate Pay Reports** (bulk modal) sets the pay period (**Last week** / **Next week**, compact start/end dates); per-person status includes **Paid**, **Partial**, or **Report only** from **`pay_stub_payments`** vs **Net Pay** (gross minus **`pay_stub_deductions`** plus sum of **`pay_stub_additional_lines`**); summary **N of M paid** counts people **fully** paid for the period. **Ledger** lists pay stubs with **Search** (person name); below the **Ledger** heading, a line shows how many stubs in the filtered list are not fully paid and the sum of **Balance**. Columns: Person (link → **Annual Pay to Date** grid: earned vs **allocated** pay by work day from `pay_stub_days`—not cash payout dates), Period, Hours, **Gross Pay**, **Less** (click dollar amount to open modal: manual charges or apply pending offsets), **Additional** (click to edit quantity × rate lines), **Net Pay**, **Paid to date**, **Balance** (vs Net Pay), **Payment** (Unpaid / Partial / Paid, detail icon for installments and optional per-row delete, or legacy memo), **Record payment** (amount + sent date + note; DB trigger prevents total installments from exceeding **Net Pay**), Created, **Actions** (**Print**; **View** only in bulk modal). Devs: red trash **PayStubDeleteIcon** to delete a stub. Section header: generate control on the right.
...
- **Features**:
  ...
  - **Month grid — bottom chip column**: PTO links, scheduled **workday** link(s), **NCNS**, and optional **recorded** line sit in a bottom flex stack with **`alignItems: center`** so each row shrink-wraps and centers horizontally (top stack for workflow / bid / prospect unchanged unless product asks).
