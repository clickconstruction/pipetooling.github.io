...
- **Features**:
  ...
  - **Pay History tab**: Single-person pay report generator (person + date range). **Generate Pay Reports** (bulk modal) sets the pay period (**Last week** / **Next week**, compact start/end dates); shows per-person status, **View** / Mark paid actions, and a centered summary (**N of M paid**, estimated total) above the table. **Ledger** lists all generated pay stubs with **Search** (filters by person name, case-insensitive). Columns include Person (link opens **Annual Pay to Date** year grid: earned vs paid by day from pay config + `people_hours` + `pay_stub_days`), Period (short M/D–M/D), Hours, Gross Pay, Created, Paid (optional memo icon, Mark paid / Unmark), and **Actions**: **Print** only in the table row (HTML / print flow); **View** for a stub is available from the bulk modal, not the ledger row. Devs see a borderless red trash icon to delete a stub (confirm dialog); icon component `PayStubDeleteIcon`. **Generate Pay Reports** section control is on the right in the header row with the section title.
  - **Pay Report** (pay stub document): Generated pay stubs show Date | Hours | **Jobs / Bids** table with per-day job and bid assignments from `people_crew_jobs` and `people_crew_bids` (e.g. "Job 651 (Dudley Mason) 2.5 hrs, Bid 88 (Hagedorn) 1.5 hrs").
