...
- **Features**:
  ...
  - **Stages Tab**: **Working** shows jobs with "Ready for Billing" button; **Ready to Bill** and **Billed** show both **jobs** (status-based) and **invoices** in a **unified table** (jobs use blue buttons, invoices use green). **Paid in Full** shows jobs where payments_made ≥ revenue. Job actions: Ready for Billing, Mark as Billed, Mark Paid, Send back. Invoice actions: Mark as Billed, Mark Paid, Send back. Edit pencil opens Edit Job modal. Stages opens by default (not Reports) for non-primary users. **Checkbox confirmation modals** (when Ham mode OFF): Ready for Billing, Mark as Billed, Mark Paid, and Send back each require checkbox confirmation before proceeding.
