...
- **Key Fields**:
  ...
  - `percent_complete` (integer, nullable, **CHECK 0-100**) - **v2.559** Optional 0-100 progress estimate. NULL = "not tracked" (the default; the column is brand new so most rows will be NULL until users start filling it in). Editable from the **Forecast Specific gutter** (`%` column, edit gate `canAlignStages(myRole)`) and from the **Workflow expanded stage card** (`Complete: [ N ] %` row, edit gate `canManageStages || s.assigned_to_name === currentUserName`). Both surfaces parse input through shared `parsePercentCompleteInput.ts`, which maps **empty / non-numeric / explicit `0` / anything that clamps or rounds to 0 (e.g. negatives, `0.4`)** to `null` — typing `0` clears the cell on either surface (a 0% estimate is functionally identical to "not tracked"). Migration `20260519214147_add_percent_complete_to_project_workflow_steps.sql`.
...
- **`%` column** (**v2.559**): right-aligned cell inside the sticky left gutter on every row, with a `%` column header in the gutter header (this sub-tab only — All Stages keeps its empty gutter header). Backed by new `project_workflow_steps.percent_complete` (INT 0-100, nullable; migration `20260519214147_add_percent_complete_to_project_workflow_steps.sql`). Empty cell == `NULL` == "not tracked". Edit gate: **`dragEdit && canAlignStages(myRole)`** — the page's **Edit** toggle has to be on too (mirrors the `+` insert button on the same gutter row), so by default every role sees the same muted read-only `NN%` text. **Hide-when-empty**: when the currently-selected job has no `percent_complete` values anywhere AND the user isn't in Edit mode, the entire column (header + per-row cells) is omitted and `labelGutterWidth` shrinks from 300 back to the pre-v2.559 **260** so the stage name reclaims the freed gutter space. Driven by `showPercentColumn = dragEdit || resolvedBars.some(b => b.percentComplete != null)`, plumbed through `renderGutterLabel` as the new `showPercentCell` prop on `StageGutterLabel` (defaults to `true` so any future caller keeps the original contract). Once any stage has a value the column shows for every role (including read-only ones), and toggling Edit on a job that already has at least one value doesn't reflow the timeline. Uncontrolled `<input type="number" min={0} max={100}>` (no-spinner) re-keyed off the persisted value so a realtime refresh updates the defaultValue automatically; Enter blurs to commit; user input flows through shared [`parsePercentCompleteInput.ts`](src/lib/parsePercentCompleteInput.ts) (empty → null, **`0` → null** (typing `0` clears the field on both Workflow and Forecast Specific — a 0% estimate is functionally identical to "not tracked"), clamp 0-100, round fractionals). Commits via `withSupabaseRetry(supabase.from('project_workflow_steps').update({ percent_complete: next }).eq('id', stageId))` with a `formatErrorMessage(err)` toast on failure. No optimistic overlay needed — the uncontrolled input already shows the typed value locally and the parent's realtime channel reconciles within ~280ms. Same field is also editable from the Workflow page expanded stage card (see Workflow Management → Stage Cards below).
