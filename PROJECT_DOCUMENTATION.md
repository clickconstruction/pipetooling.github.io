...
- **Features**:
  ...
  - **Billing Tab** (HCP Jobs): Jobs ledger (HCP #, Job Name, Address, materials, team members, revenue); New Job, search; **Edit** and **Delete** per row, vertically centered in the row. Google Drive and Job Plans icons shown when links are filled; stacked vertically. New/Edit Job modals include Job Total / Bid ($) and Remaining ($) side by side; **Payments received ($)** table with Add Payment; **Create invoice** inline with Add Payment (Amount input, Create invoice button; lists open invoices with View in Stages).
