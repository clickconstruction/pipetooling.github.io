...
- **URL**: **`?product=mercury|stripe`** and **`?tab=…`**. Mercury tabs: **`ledger`**, **`sorting`** (User Sort), **`drag_sort`**. Stripe tabs: **`invoices`**, **`data`**. **`?tab=sorting`** without **`product`** still opens Mercury **User Sort** (legacy bookmarks).
