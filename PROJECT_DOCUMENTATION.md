...
- **Features**:
  ...
  - **Billing Tab** (HCP Jobs): Jobs ledger (HCP #, Job Name, Address, materials, team members, revenue); New Job, search; **Edit** and **Delete** per row, vertically centered in the row. Google Drive and Job Plans icons shown when links are filled; stacked vertically. New/Edit Job modals include Job Total ($) and Remaining ($) side by side; **Job Total** and **Payments received** amounts show **comma thousands** on blur (`Jobs.tsx`, `MoneyDecimalAmountInput`). **Billing** accordion in **Edit Job** ([`JobFormModal.tsx`](src/components/jobs/JobFormModal.tsx)):
