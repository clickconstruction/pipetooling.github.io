...
- **Ledger display prefixes (jobs/bids)**:
  ...
  - **Search RPCs** (same migration): `search_jobs_ledger` and `search_bids_for_clock` return **`service_type_id`** and treat typed queries as **legacy `J`/`B` + digits** or **configured prefix + remainder** when matching `hcp_number` / `bid_number`.
