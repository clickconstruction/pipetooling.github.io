...
- **Features**:
  ...
  - **Ready to Bill / Billed Awaiting Payment** (dev, master_technician, assistant): Lists mirror Jobs Stages (including **merged** job + **`is_primary_rtb_bundle`** invoice via **`readyToBillDashboardUnits`**); **Invoice / Update** opens [`SendRecordInvoiceModal`](src/components/jobs/SendRecordInvoiceModal.tsx) (job rows load **`jobs_ledger`** billing fields when the modal opens). No Ham-mode shortcut on the Dashboard (always modal).
