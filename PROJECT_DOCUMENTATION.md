...
- **Routing / tabs**: [`App.tsx`](src/App.tsx) route **`documents`**; primary tab from **`?tab=`** — `estimates`, `bid-proposals`, `jobs`, `upload` ([`documentsPageTab.ts`](src/lib/documentsPageTab.ts)); legacy `ledger=` query supported. Dashboard pins: [`pinnedTabs.ts`](src/lib/pinnedTabs.ts).
...
- **Jobs ledger**: **`jobs_ledger`** (recent rows); **Docs** opens **`google_drive_link`** (Job Files); **+** sets that field; title links **`/jobs?edit=`**; status labels via [`jobsLedgerStatusPipeline.ts`](src/lib/jobsLedgerStatusPipeline.ts).
