...
- **Page**: `Quickfill.tsx`
- **Route**: `/quickfill`
- **Features**: Sections (in order): Jobs Billing Reminder, **Billed Awaiting Payment** (count and total, HCP/Job/Assigned/Remaining table, link to Jobs Stages), Unpriced Fixtures (count + link to Jobs Parts), Cant Reach, Hours, **Crew Jobs**, Supply Houses. Unpriced fixtures visible to dev, master_technician, assistant. **Crew Jobs** (below Hours): same as Jobs Team Labor tab; date picker, Crew Jobs table (Name, Crew, Jobs), Team Job Labor table (HCP, Job, People, Man hours with breakdown modal; Job Cost column hidden in Quickfill). Visible to dev, pay-approved masters, assistants, cost-matrix-shared users.
