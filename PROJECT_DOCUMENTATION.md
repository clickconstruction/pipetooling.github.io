...
- **Features**:
  ...
  - **Parts Tab**: Shows jobs with tally parts, Billed Materials, or Invoices from Supply Houses. Includes jobs that have only Billed Materials (no tally parts) or only supply house invoice allocations (no tally parts, no materials). Tally parts from Job Parts Tally; search and "Show my jobs only" (hidden for subcontractors). Fixture-only entries (sent to office) have editable cost; jobs with unpriced fixtures highlighted in red. Parts total includes fixture cost. Columns: Parts from Tally, Billed Materials, Invoices from Supply Houses, Total Parts Cost.
