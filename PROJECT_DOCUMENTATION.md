...
### 5. People Roster
- **Page**: `People.tsx`
- **Tabs**: **Users** (default), **Payroll** (legacy URL **`?tab=pay_stubs`**), **Hours** (timesheet + cost matrix + pay tools — former **Pay** merged here; legacy **`?tab=pay`** / **`?tab=team_costs`** rewrite to **`hours`**), **Team Costs**, **Vehicles**, **Housing**, **Offsets**, **Contracts** (dev, pay-approved masters, assistants), **Review** (dev-only), **Feedback** (dev-only; URL **`?tab=feedback`**)
- **Features**:
