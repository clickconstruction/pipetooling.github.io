...
- **Developer docs ship with features too** (they drifted a full week once — 2026-07-12 catch-up, PR #234): every feature/fix PR adds its `docs/RECENT_FEATURES.md` entry (`## Latest Updates (v2.NNN)` above the previous newest, next version number, update the frontmatter `last_updated`); every migration gets a `docs/MIGRATIONS.md` entry; every new/changed edge function gets a `docs/EDGE_FUNCTIONS.md` section (+ TOC line); update `docs/AI_CONTEXT.md` / `PROJECT_DOCUMENTATION.md` / `GLOSSARY.md` / `ACCESS_CONTROL.md` when a PR renames a surface, adds a tab/section/modal, or changes role permissions.
