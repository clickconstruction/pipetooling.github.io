...
- **Release notes ship with features** (v2.944): every feature/fix PR adds an entry to `src/content/releaseNotes.ts` with the **same `v2.NNN`** as its `docs/RECENT_FEATURES.md` entry — short, user-readable (title + 1–4 highlight bullets, no file paths); it renders at Settings → Release notes for all roles. The drift test in `src/lib/releaseNotes.test.ts` fails CI until the newest versions match, and two further guards fail on the damage version races cause: the same `v2.NNN` documented **twice** in `RECENT_FEATURES.md`, and a release note with **no** `RECENT_FEATURES.md` entry. Both carry a frozen allowlist of pre-existing cases (v2.25/95/545 duplicated; v2.1037–v2.1040 + v2.1012 undocumented) — **shrink it, never extend it**; a new entry on either list means a PR lost its changelog entry.
