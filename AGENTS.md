...
| People Hours: Correct-day audit **Edit** (crew + clock + add session); **ClockSessionEditSplitModal**; **Highlight by job** on grid; **manual cell blur → My Time draft** (`peopleHoursManualDraftSession.ts`, draft **`INSERT`** in **`DashboardMyTimeDayEditorModal`**, grid **`max(people_hours, pending clock)`**, **`saveHours(0)`** after draft save, **NCNS** off for draft path) | `RECENT_FEATURES.md` → v2.291, v2.171; `PROJECT_DOCUMENTATION.md` → People, Quickfill; `src/components/PeopleHoursDayAuditModal.tsx`; `src/pages/People.tsx`; `src/lib/peopleHoursManualDraftSession.ts`; `src/components/DashboardMyTimeDayEditorModal.tsx` |
