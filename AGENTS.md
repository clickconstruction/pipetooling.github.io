...
1. **Never edit existing migrations** — append-only; create a new migration to change schema.
   ...
   - **Start every new migration with `SET lock_timeout = '3s';`** — DDL waiting on a lock freezes prod for everyone queued behind it; the timeout makes the push fail fast instead (retry later). CI-enforced for versions after 2026-07-29.
