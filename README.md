...
- **Bids Counts**: Fixture or Tie-in quick-select, number pad for count, Save / Save and Add; **Import** button for bulk paste (tab- or comma-separated: Fixture, Count, Plan Page per line); Project Name required; "Save and start Counts" in New Bid modal; Edit Bid button on Counts tab. Updating Counts refreshes Takeoff and Cost Estimate for the same bid; switching to Takeoff or Cost Estimate refetches so data stays current without a page refresh.
