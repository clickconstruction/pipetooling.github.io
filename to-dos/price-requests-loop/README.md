...
**The quote lands as a pasted link, not a dropped PDF.** Grace, reviewing the before/after mock-up: *"I would only like to have a link pasted for the PDF and no drop the quote PDF here."* Every "drop-zone" below reads as the paste-a-link box on a waiting row; no upload path, no storage bucket for quotes.
...
3. **PR 3 · the row carries the quote.** Fold in a one-line bug the mock-up exposed: `requestCell` calls `linkHostLabel(url ?? '')` on a hand-sent row with no link, so the meta line reads the literal word *link* (`new URL('')` throws and the helper falls back to it) — guard on `url`. Status column (waiting / late / quote in) from `needed_by` + `status`; drop-zone on waiting rows (upload → `quote_url`, status `quoted`); paste still works; **Call** on hand-sent rows.
