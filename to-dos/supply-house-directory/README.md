...
Status: **in progress — PR 1 shipped (v2.3166: Directory pane + `materialsTabsFor` kernel + shared house editor), PR 2 shipped (v2.3167: the estimator's tab + `can_manage_supply_house_directory()`, migration `20260908184533`), PR 3 shipped (v2.3168: the Price coverage modal is stats-only — quirk 16 closed), PR 4 shipped (v2.3170: reps are the only home for contacts; `contact_name` / `email` dropped from `supply_houses`), PR 5 shipped (v2.3172: `vendor_kind` with the hand seed; `is_insurer` derived by trigger), PR 6 shipped (v2.3173: `supply_house_service_types` + trade chips + the RFQ picker's bid-trade default) — all six PRs on `main` or in the stack; remaining chores: push the four migrations in order after each client deploys, then `npm run gen-types:linked`, then a later PR drops `is_insurer`** · asked, weighed and mocked up 2026-09-08; refined mock-up approved by the owner the same day (artifact *Supply House Directory*: coverage line, "Needs a rep" band, rep provenance, who-quoted-last, five-value Kind, trades served, hand-classified seed) · mock-up: [`mockup.html`](./mockup.html) beside this file (five screens; open it in a browser)
...
[`mockup-refined.html`](./mockup-refined.html) — the version the owner approved on 2026-09-08 and the six PRs built: the same five screens redrawn against the live roster (coverage line, "Needs a rep" band, rep provenance, who-quoted-last with outcome, five-value Kind, Trades served), plus the access model, the hand-classified vendor-kind seed, and the six decisions with the assumption each PR took.
