...
Status: **shipped, delete me** · all six PRs on main 2026-09-08 (v2.3166 Directory pane + tabs kernel · v2.3167 estimator's tab + `can_manage_supply_house_directory()` · v2.3168 Price coverage stats-only, quirk 16 closed · v2.3170 reps the only contact home · v2.3172 `vendor_kind` + hand seed · v2.3173 trades served) and the close-out 2026-09-10 (v2.3243 rep provenance — `created_by` now defaults to the caller · v2.3244 `is_insurer` dropped); all migrations pushed, types regenerated · mock-ups: [`mockup.html`](./mockup.html) (the 2026-09-08 draft) and [`mockup-refined.html`](./mockup-refined.html) (the approved design the PRs built) · the "Where it plugs in" table below is the pre-build snapshot and is kept as history
