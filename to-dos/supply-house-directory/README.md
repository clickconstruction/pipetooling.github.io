...
Status: **in progress — PR 1 shipped (v2.3166: Directory pane + `materialsTabsFor` kernel + shared house editor), PR 2 shipped (v2.3167: the estimator's tab + `can_manage_supply_house_directory()`, migration `20260908184533`), PR 3 shipped (v2.3168: the Price coverage modal is stats-only — quirk 16 closed), PR 4 shipped (v2.3170: reps are the only home for contacts; `contact_name` / `email` dropped from `supply_houses`), PRs 5–6 next** · asked, weighed and mocked up 2026-09-08; refined mock-up approved by the owner the same day (artifact *Supply House Directory*: coverage line, "Needs a rep" band, rep provenance, who-quoted-last, five-value Kind, trades served, hand-classified seed) · mock-up: [`mockup.html`](./mockup.html) beside this file (five screens; open it in a browser)
