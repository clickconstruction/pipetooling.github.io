...
Status: in progress · PR 1 (derivation fix) v2.2865 #2607 · PR 2 (rail kernel + `SheetRail`) v2.2866 #2608 · PR 3 (Work Orders rows become sheets, all eight mock-up changes — artifact `1e154fbb`) v2.2867 #2609 · live test passed 2026-09-05, follow-ups v2.2868 #2611 · next: PR 4 (Sub Labor on the same spine) and PR 5 (chips elsewhere) · designed 2026-09-05 · mock-up: [`work-orders-one-row-spine.html`](./work-orders-one-row-spine.html) (also published at https://claude.ai/code/artifact/a8ce5a7d-d47a-4e2f-905b-558e9d67e298)
...
> PRs 1–3 shipped 2026-09-05 (v2.2865–v2.2867). PR 3 folded in the eight changes from the "Work Orders Board Refresh" mock-up: handshake tiles, filters = rail groups (Declined/Expired are red row states), Link-to-a-job door, Next column with the button first + nudge after 3 days, amber panel dissolved into the first group + Signed collapsed, Sheet › on every row + WO number opens the record, no Subs-only toggle (crew pay never listed), phone cards. `buildWorkOrderBoard` (`src/lib/subWorkOrders/workOrderBoardRows.ts`) is the row kernel PR 4 should reuse for Sub Labor; `isRosterSub` is the Crew-pay rule.
...
- Live test 2026-09-05 (steps 2–5 with the Claude Test Sub on a throwaway job `ZZWO1` + sheet): all passed; see `docs/recent-features/v2.2868.md` for the table. Setup that worked: insert the job (`master_user_id`, `service_type_id`, `hcp_number`, `status: 'working'`) and the sheet (`assigned_to_name` alone syncs the assignee junction) through the page's own supabase client from `javascript_tool`; the portal token is `sub_portal_links.token`; `dispatch_requests` refuses deletes — close the signed-inbox item with a note instead. The portal's third step is **Walk-through** (not Inspection).
