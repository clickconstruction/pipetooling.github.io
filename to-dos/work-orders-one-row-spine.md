...
- Live test 2026-09-05 (steps 2–5 with the Claude Test Sub on a throwaway job `ZZWO1` + sheet): all passed; see `docs/recent-features/v2.2869.md` for the table. Setup that worked: insert the job (`master_user_id`, `service_type_id`, `hcp_number`, `status: 'working'`) and the sheet (`assigned_to_name` alone syncs the assignee junction) through the page's own supabase client from `javascript_tool`; the portal token is `sub_portal_links.token`; `dispatch_requests` refuses deletes — close the signed-inbox item with a note instead. The portal's third step is **Walk-through** (not Inspection).
