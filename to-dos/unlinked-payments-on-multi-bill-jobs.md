...
- **Job 102 · Samantha Coyle** — one bill $5,355, one unlinked $3,000 check. The invoice correctly reads *Balance due $2,355*. The demand header reads **$5,355.00 open**. The letter overstates by $3,000.
...
Whichever is chosen, **one kernel must answer for every reader** — the invoice payment history, `sumApplied` in the demand letter, and the lien claim amount — or these two surfaces will drift apart again.
...
- `src/lib/jobsDocuments/demandLetter.ts` → `sumApplied`, used by `buildDemandStatement` and `buildDemandLetterPrefill.outstanding` (the claim side).
- `src/components/jobs/LienInstrumentsModal.tsx` → `demandableInvoices` uses the same linked-only rule to decide which bills a letter covers.
