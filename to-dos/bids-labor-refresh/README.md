...
status: >
  ...
  before / after drawn 2026-09-18 (`before-after-pr6.html`, from a read of the books that day)
summary: >
...
next: >
  Fold and retire, or keep the parallel run? The drawing raises two calls inside the fold: the
  three Default-vs-Robot conflicts (Water Fountain, Gas drop, Hose bib — equal totals, stages
  swapped: Robot's split or Default's?) and whether an estimator's calibration Set goes live or
  waits for a leader (drawn as propose).
size: M
blocker: Your call.
...
3. **Extract the L2 autosave payload + one cost total — SHIPPED v2.3292.** `costEstimateAutosavePayload.ts` (quirk #4 closed: the footer says "Not saved — <box>") and `bidTotalCostBreakdown.ts` (`computeBidCostBreakdown`; Workbench, Pricing print/CSV, Labor page print, approval PDF read it; the documents now count team labor and the five direct-cost tables, and the approval PDF's margins amount counts travel). The Pricing grid's per-row labor routes through `laborRowHours` too.
4. **Rate and other direct costs** — **4a SHIPPED v2.3294**: the crew-rate card (`crewRate.ts`, `useBidCrewRate` off the Burn section's cached day ledger × the burden setting; "save it on the bid" persists it as `labor_rate`), the overhead-per-field-hour tile (lens A, shown not added), estimator time retired (the kernel reports `estimatorCost` / `teamLaborCost` but adds neither; the Old box became "Bid labor recorded"), the bottom line "Direct cost of this bid". **4b SHIPPED v2.3295**: `BidsDirectCostsSection` — one list with a kind chip over the five tables (handlers stay per table), the driving line on top in crew-day words. **Deferred**: driving *from* crew-days × trips/day (a formula change — today trips = hours ÷ hours per trip; decide with the owner whether 2 trips per crew-day replaces the hours-per-trip box).
5. **Calibration — SHIPPED v2.3307.** `laborBookCalibration.ts` (`bookMultiplier` with the 25 % / 8-day rule, `entryEvidence` by the book's own share, `calibratedEntryHours`), `useLaborBookCalibration` (bids on the book → `jobs_ledger.bid_id` → rows, `get_man_hours_by_job`, clock dates), the Book vs jobs tile, evidence chips, the Set / Keep drawer. Reads the links the Burn train creates — zero exist until the owner presses Link (Costs tab · Bid Board · Settings → Data), so the tile reads *no linked jobs yet* today. Entry history is not kept.
