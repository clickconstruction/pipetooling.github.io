...
Status: **in progress** · PR 1 shipped as v2.3276 (`claude/labor-new-view`, the New view beside Old) · designed 2026-09-11 · mock-up [`mockup.html`](./mockup.html) (published as the artifact *Hours That Learn*; v1 → "best we can do" → v2 → a second pass, both recorded in its §06).
...
2. **Data** — `labor_book_entries.unit` (`each` | `per_100ft`) + `kind` (`fixture` | `task`); `cost_estimate_labor_rows.kind` + `source` (`book` | `alias` | `typed` | `robot`) + `source_note` ("learned from B375 by Wendi"); `app_settings` key for the burden factor; one view `cost_estimate_direct_costs` over the five direct-cost tables (kind column). Then the queue's Task / Sub kinds and per-100-ft rows become real (today Task = `is_fixed`).
