...
## A · Money and reporting SQL (one migration, highest value)

| Function (latest definer) | What it does with the match | Change |
|---|---|---|
| `get_paid_job_email_payload(uuid)` — `20260803000000_payment_made_email.sql:76,152` | Paid-job email cost breakdown: `sub_labor_books` CTE joins sheets on `lower(trim(job_number)) = lower(trim(hcp_number))` | `plj.job_ledger_id = job.id` |
| `get_billed_aging_costs()` — `20260820120000_billed_aging_costs.sql:23,78` | Sub-labor cost per billed job (Billed tab aging) | same |
| `get_paid_profit_stats()` — `20260820140000_paid_profit_stats.sql:9,65` | Paid-job profit stats | same |
| `partner_job_cost_buckets(uuid)` — `20260820180000_partner_profit_share.sql:28,62` | Partner profit share cost buckets | same |
| `get_weekly_money_movement_payload(date)` — `20260807060000_weekly_money_payload_mercury_sign.sql:138` | Weekly money email: correlated `btrim(hcp_number) = btrim(job_number)` — **case-sensitive**, newest job | `lj.job_ledger_id` |
...
| Site | Flow | Change |
|---|---|---|
| `settle_step_commitment(uuid, boolean)` — `20260905050035_work_orders_on_jobs.sql:486-533` | Step-anchored settlement on a project with exactly one job: writes that job's `hcp_number`; a click-only job yields a numberless, unlinked sheet | write `job_ledger_id = jl.id` (the trigger then fills the number) |
