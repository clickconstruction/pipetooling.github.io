...
Status: A–E converted (v2.3059, v2.3060, v2.3065, v2.3068, v2.3071); what is left is the dated cleanup under "After all rows are gone" (validated 2026-09-14: `sub-portal` / `submit-sub-portal` still carry `job_number` reads as the one-release fallback). Census taken 2026-09-07 (v2.3055 added `people_labor_jobs.job_ledger_id`, back-filled it, and moved the Team board, the stage → activity trigger and the superintendent access helper to the link). Everything below still resolves a sheet's job by comparing `job_number` to `jobs_ledger.hcp_number` as text. Delete a row when its system reads the link.
