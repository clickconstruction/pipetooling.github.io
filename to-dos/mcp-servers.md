...
next: **Path 0 is confirmed** (2026-09-21 evening: the owner added `clicktooling_dev` on claude.ai with a request header and it connected) and the dev half shipped in v2.3686 (the Dev MCP keys card walks it). Path 0's twin half shipped v2.3721 (the fresh-key card, `TwinFreshKeyPanel`), and PR 6 shipped and deployed v2.3722. Next: the keyed live pass of PR 6's verify list (owner issues a dev key), then PR 6's brief is closed; after that PR 6 (writes; both calls taken), and PR 7 (Shape 1) only when someone needs to know WHO holds a twin seat. Owed live checks that need a twin key: `get_brief` with a `ptt_` key, an old bare key still answering, `twin-setup` redeem returning `ptt_`.
size: L
