...
next: Whoever picks this up — read *Picking this up* below first. PRs 3 and 5 are deployed and live-checked; one check is left — `get_brief` answers with a `ptt_` key. PR 6 has an owner decision on HR writes; PR 7 starts with a one-day spike.
size: L
