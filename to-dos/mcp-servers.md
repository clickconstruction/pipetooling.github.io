...
size: L
blocker: None open on the built PRs. PR 7 — a KV namespace on the Cloudflare account (the owner's `wrangler kv namespace create`). The owed live checks wait on a twin key and a dev key in the owner's hands.
...
| 6 · dev-mcp, writes — **shipped v2.3722**, deployed 2026-09-22 (keyed live check owed) | `plan_cost_batch` / `apply_cost_batch` / `revert_cost_batch` over the existing dev-gated RPCs, as the dev; and — **decided 2026-09-21** — `plan_hr_entry` / `apply_hr_entry` over a new dev-gated dry-run wrapper. Never DDL, deploys, sends or payments. **Brief below.** | function, one migration (the HR wrapper) |
