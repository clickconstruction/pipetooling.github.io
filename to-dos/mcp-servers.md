...
next: The PR 7 spike is DONE (2026-09-21, *The spike's answer* under PR 7). First, the owner's five-minute check — does *Add custom connector* on claude.ai show a **Request headers** section? Yes → Path 0 ships as one small PR and PR 7 waits for a real want. Then PR 7 as Shape 1 (three PRs, briefed), then PR 6. The owed live checks need a dev key the owner issues (the profile's key is revoked).
size: L
blocker: Path 0 — one look at the connector dialog (the request-headers beta is per organization). PR 7 — a KV namespace on the Cloudflare account (the owner's `wrangler kv namespace create`). The owed live checks wait on a fresh dev key on the owner's Mac.
...
- **Partly verified** (2026-09-20: `get_job` on JP1032 answered profit $1,180 with a live key — the expected figure; not yet held beside the Job window on screen): `get_job`'s profit side by side with the Job window's profit band (the agent's browser was signed out) — open JP1032 and confirm $1,180; and a *twin* key presented at `/dev` (no live twin key was at hand; the lookup only consults `dev_mcp_credentials`, so a refusal is expected).
