...
next: Whoever picks this up — read *Picking this up* below first. Order decided 2026-09-21: the PR 7 spike (one day, write the answer in), then PR 7, then PR 6 with both the cost pair and the HR pair. The owed live checks need a dev key the owner issues (the profile's key is revoked).
size: L
blocker: None. PR 7 — the spike's outcome. The owed live checks wait on a fresh dev key on the owner's Mac.
