...
| [`pipeline-on-a-map/`](./pipeline-on-a-map/README.md) | in progress · PR 1 v2.3396 (the card) · PR 2 v2.3397 (the rail + hover) · next As of · decisions taken as recommended | The Bid Board's map card at the top of Jobs → Pipeline (same canvases, office rings, rail with dollars to collect) plus an **As of** slider that rewinds every pin to the status it had on any day since Feb 22, 2026 (`job_status_events`); optional crew-on-the-day layer. Four client-only PRs; mock-up in the folder. |
