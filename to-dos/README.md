...
| [`accounts-receivable-refresh/`](./accounts-receivable-refresh/README.md) | in progress · PR 1 v2.3379 (header summary, ⋯ menu, row state chips) · PRs 2–4 the deposit header + allocation rows, matches list + guard card, footer + Apply & next | Option A of the AR modal refresh: same two rooms, better furniture — every behaviour kept, each PR lifts a piece out of the 2,497-line component. Mock-up in the folder. |
