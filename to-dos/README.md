...
Closed 2026-09-26 (file deleted; no mock-up — a test-harness change): `render-tests-settle-first` (#39) — PR 1 v2.3852 (`settle()` / `renderSettled()`, the rule in the harness header and `docs/AI_CONTEXT.md`, the three one-off fixes re-expressed) and PR 2 v2.3854 (the sweep: `scripts/render-smokes-settle.mjs` put `await settle()` at 376 places in 84 of the 263 render smokes, the 85 files with no async mount work left alone; two selectors tightened by hand where the settled thread doubled a name; `// first paint` opts a read out). The lint rule stays unbuilt, by decision.
