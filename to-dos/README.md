...
| [`pipeline-on-a-map/`](./pipeline-on-a-map/README.md) | **shipped, delete me** · v2.3396 (the card) · v2.3397 (the rail + hover) · v2.3398 (As of) · v2.3399 (crews on the day) — all on main 2026-09-14 | The Bid Board's map card at the top of Jobs → Pipeline (same canvases, office rings, rail with dollars to collect) plus an **As of** slider that rewinds every pin to the status it had on any day since Feb 22, 2026 (`job_status_events`); optional crew-on-the-day layer. Four client-only PRs; mock-up in the folder. |
