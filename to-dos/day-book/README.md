...
### PR 4 — estimators (migration 2) — **shipped v2.3727** for payroll viewers; **PR 4b shipped v2.3735 — via Bids, the owner's call**: Bids → Day book mounts the same tab under the Day book's gate (`canOpenDayBook`, dev · controller for now); when access widens to estimators this is their door, since `/people` stays closed to the role.
...
### PR 6 — Crew Day one-liner + email — **6a shipped v2.3728** (the Dashboard line, client only: the section reads `get_day_book_payload` for its day as the viewer, so no RPC change); **6b shipped v2.3733**: the body moved into `day_book_payload_for(p_uid, …)` with two doors — the app's `get_day_book_payload` and `get_day_book_payload_for_user(p_user_id, p_day)` (service_role only); the dispatcher reads the recipient's lines through it and `_shared/dayBookOneLiner.ts` writes them (a parity test pins it to the Dashboard's wording).
...
## Live pass, 2026-09-22 (after every migration applied — prod 645/645)

As the dev on a worktree dev server against prod rows; the dev-mcp key in that session was revoked, so nothing below is a row read.
