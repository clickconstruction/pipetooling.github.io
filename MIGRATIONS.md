...
- **Purpose**: Optional **0-100 progress estimate** on each workflow stage. Editable from two surfaces with a single source of truth: (1) **Forecast Specific gutter** (`/projects?tab=forecast`, Specific sub-tab) — right-aligned cell inside `StageGutterLabel` with a `%` column header in the sticky gutter header; (2) **Workflow expanded stage card** — new `Complete: [ N ] %` row directly under the existing Expected dates row. NULL = "not tracked"; clamped 0-100 at the DB layer so ad-hoc SQL writers can't bypass the UI's `parsePercentCompleteInput` helper. Forecast All Stages intentionally does NOT render the column.
