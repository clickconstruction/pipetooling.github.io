-- Extend split_own_clock_session_segments: allow work_date in current OR previous
-- America/Denver Sunday–Saturday week (aligns with dashboard this week + last week).
...
COMMENT ON FUNCTION public.split_own_clock_session_segments(uuid, jsonb) IS
  'Replace own clock session with N contiguous segments inside the original span; current or previous calendar week (America/Denver Sunday-Saturday). Approved sessions: people_hours rollback + crew sync, then delete; new rows pending.';
