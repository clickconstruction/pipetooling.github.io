-- Allow superintendents to mark jobs Ready for Billing when job is in a project they supervise.
-- Used by Dashboard Superintendent Jobs "Send to Billing" button.
...
END;
$$;
COMMENT ON FUNCTION public.update_job_status(UUID, TEXT) IS 'Updates job status. Forward: working->ready_to_bill (team/office/superintendent), ready_to_bill->billed, billed->paid (office). Revert: ready_to_bill->working, billed->ready_to_bill, paid->billed (office only).';
