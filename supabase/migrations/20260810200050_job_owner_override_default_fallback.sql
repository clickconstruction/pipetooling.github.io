...
-- create_job_from_estimate: honor the org-wide job_owner_override_default
-- (v2.1532). The client resolver (src/lib/resolveEffectiveJobMasterUserId.ts)
-- gained the same fallback; this keeps the SQL path (estimate acceptance)
-- consistent: project owner -> personal job_owner_override_<uid> ->
-- job_owner_override_default -> auth user. Both overloads re-created verbatim
-- from the baseline with only the override block extended. Idempotent
-- (CREATE OR REPLACE).
...
BEGIN
  ...
  ELSE
    ...
    IF v_override IS NULL OR trim(v_override) = '' THEN
      -- v2.1532: org-wide fallback — the master every user without their own
      -- override row creates jobs as (Settings -> Jobs & dispatch, dev only).
      SELECT s.value_text INTO v_override
      FROM public.app_settings s
...
--
-- Name: FUNCTION "create_job_from_estimate"("p_estimate_id" "uuid", "p_hcp_number" "text", "p_job_name" "text", "p_job_address" "text", "p_revenue" numeric, "p_customer_id" "uuid"); Type: COMMENT; Schema: public; Owner: postgres
--
...
--
-- Name: FUNCTION "create_job_from_estimate"("p_estimate_id" "uuid", "p_hcp_number" "text", "p_job_name" "text", "p_job_address" "text", "p_revenue" numeric, "p_customer_id" "uuid", "p_fixtures" "jsonb"); Type: COMMENT; Schema: public; Owner: postgres
--
