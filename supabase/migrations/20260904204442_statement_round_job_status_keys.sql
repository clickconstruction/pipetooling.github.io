...
-- Statement round RPC: the merged-row key also depends on JOB STATUS
-- (v2.2779, third cut). buildBilledStageRows bundles a job's single billed
-- line into a job-keyed row only for jobs whose status is `billed`; a
-- `working` job's billed break-off line stays an invoice row keyed by the
-- invoice id. 20260904202939 re-keyed every single-line job, which flipped
-- Loberg (a working job with one billed line) to "changed". Verified
-- read-only against every round GC before this push: zero unmatched rows.
...
group_rows AS (
  SELECT g.gc_id,
         ...
         CASE
           ...
           -- Only a job whose STATUS is billed bundles its single billed line
           -- into a merged job row; a working job's billed line (a break-off)
           -- stays an invoice row keyed by invoice id, however many it has.
