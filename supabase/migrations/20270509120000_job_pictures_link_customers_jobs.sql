-- Job Pictures: optional URL on customers (Edit Customer) and jobs_ledger (Edit Job); extend merge_customers.

ALTER TABLE public.customers
  ADD COLUMN IF NOT EXISTS job_pictures_link text NULL;

COMMENT ON COLUMN public.customers.job_pictures_link IS 'Optional URL for job pictures (folder or album; Edit Customer).';

ALTER TABLE public.jobs_ledger
  ADD COLUMN IF NOT EXISTS job_pictures_link text NULL;

COMMENT ON COLUMN public.jobs_ledger.job_pictures_link IS 'Optional URL for job pictures (folder or album; Edit Job).';
...
COMMENT ON FUNCTION public.merge_customers(uuid, uuid, jsonb) IS 'Atomically merge victim into survivor; p_field_choices keys name, address, contact_info, customer_type, date_met, master_user_id, google_drive_link, job_pictures_link with values survivor|victim.';
