...
-- gc_statement stream -> email schedule surfaces (v2.1428) — Phase 4 of the
-- gc_statement Report Subscriptions stream (docs/REPORT_SUBSCRIPTIONS.md).
--
-- Bodies below are the LIVE prod definitions dumped via pg_get_functiondef
-- on 2026-08-06 (per the v2.1400 never-rebuild-from-baseline rule), plus:
--   * get_my_email_schedule(): a gc_statement_oneoffs CTE + one_offs branch —
--     REQUESTER-scoped (outside recipients have no user row), detail
--     "{entity} -> {email}", pending rows plus sent-this-week rows (error-free
--     only, so 'skipped: nothing outstanding' weeks never show as sent);
--   * get_global_email_schedule(): a gc_statement_requests key (dev view).
-- Grants/behavior otherwise unchanged.
...
gc_statement_oneoffs AS (
  ...
  CROSS JOIN chicago c
  WHERE g.requested_by = (SELECT uid FROM me)
    AND (
      g.sent_at IS NULL
