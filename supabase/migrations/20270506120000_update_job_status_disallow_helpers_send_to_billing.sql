-- Helpers on the job team cannot move Working -> Ready to Bill (UI hides Send to Billing; deny via RPC).
...
END;
$$;
COMMENT ON FUNCTION public.update_job_status(UUID, TEXT) IS 'Updates job status. Forward: working->ready_to_bill (team/office/superintendent; helpers cannot use team path), ready_to_bill->billed, billed->paid (office). Revert: ready_to_bill->working (deletes RTB draft invoices), billed->ready_to_bill, paid->billed (dev/master/assistant/primary with job access).';
