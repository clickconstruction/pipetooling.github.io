...
-- Ready to Bill notifications v2 (v2.1844): per-recipient delivery channels.
-- The v1 model (uuid list + org-wide {email,push} channels) becomes a v2 key
-- 'ready_to_bill_notify_recipients_v2' holding a JSON array of
-- { id, email, push } objects — each person picks their own channels, and
-- being a recipient means having at least one channel on. This migration:
--   1. Converts v1 → v2 once (only when v2 doesn't exist yet), applying the
--      org-wide channels to every v1 id. Both-channels-off converts to [].
--   2. Rebuilds get_my_email_schedule() / get_global_email_schedule() to read
--      v2 (membership = an entry with ≥1 channel; the global payload carries
--      each person's email/push flags so Settings can show them).
-- The v1 keys stay in place as the edge function's fallback for the window
-- between client deploy and this push; nothing writes them anymore.

-- ── 1. One-time v1 → v2 conversion ───────────────────────────────────────────

INSERT INTO public.app_settings (key, value_text)
SELECT
  'ready_to_bill_notify_recipients_v2',
  COALESCE(
    (
      ...
      FROM (
        SELECT CASE WHEN value_text ~ '^\s*\[' THEN value_text::jsonb ELSE '[]'::jsonb END AS ids
        FROM public.app_settings WHERE key = 'ready_to_bill_notify_recipients_v1'
      ) v1
      CROSS JOIN LATERAL jsonb_array_elements_text(v1.ids) AS x(uid)
      CROSS JOIN (
        SELECT
          COALESCE((c.j ->> 'email') IS DISTINCT FROM 'false', true) AS email_on,
          COALESCE((c.j ->> 'push') IS DISTINCT FROM 'false', true) AS push_on
        FROM (
          SELECT CASE WHEN value_text ~ '^\s*\{' THEN value_text::jsonb ELSE '{}'::jsonb END AS j
          FROM public.app_settings WHERE key = 'ready_to_bill_notify_channels_v1'
        ) c
      ) ch
      WHERE ch.email_on OR ch.push_on
    ),
    '[]'::jsonb
  )::text
WHERE NOT EXISTS (
  SELECT 1 FROM public.app_settings WHERE key = 'ready_to_bill_notify_recipients_v2'
);

-- ── 2. Email-schedule surfaces read v2 ───────────────────────────────────────
-- Bodies are the LIVE definitions — last rebuilt by 20260819230000 (v2.1836),
-- reproduced verbatim with only the v2 changes: the RTB membership check moves
-- out of setting_list into a jsonb-object scan of the v2 key, and the global
-- RPC's ready_to_bill_recipients gains per-person email/push flags.
...
COMMENT ON FUNCTION public.get_my_email_schedule() IS
  'Everything the app is configured to email the calling user: weekly digests, one-off sends addressed to them, and every standing event-stream subscription (Paid in Full, Payment received, Estimate accepted, Ready to Bill v2 per-person channels). Self-scoped; EXECUTE for authenticated.';
...
COMMENT ON FUNCTION public.get_global_email_schedule() IS
  'Dev-only org-wide view of every email stream: recurring digests with recipients, the standing event lists (Paid in Full, Payment received, Ready to Bill with per-person email/push flags), and pending one-off sends. Returns NULL for non-devs.';
