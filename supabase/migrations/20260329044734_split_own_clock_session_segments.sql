...
-- Week gate uses America/Denver Sunday-start week (documented; may differ slightly from browser-local getDefaultWeekRange).
...
COMMENT ON FUNCTION public.split_own_clock_session_segments(uuid, jsonb) IS
  'Replace own clock session with N contiguous segments inside the original span; current week (America/Denver). Approved sessions: people_hours rollback + crew sync, then delete; new rows pending.';
