-- Company calendar unified on America/Chicago (salary RLS "today", template default, week gates, sync default date).

UPDATE public.salary_work_schedule_templates
SET timezone = 'America/Chicago'
WHERE timezone = 'America/Denver';
...
COMMENT ON TABLE public.salary_work_schedule_day_overrides IS
'Per-day overrides; NULL field means inherit template. Owners may write today (Chicago date) only; pay roles any date.';

DROP POLICY IF EXISTS "salary_override_insert" ON public.salary_work_schedule_day_overrides;
...
WITH CHECK (
  (
    user_id = auth.uid()
    AND work_date = (timezone('America/Chicago', now()))::date
  )
  OR public.salary_schedule_staff_or_self_target(user_id)
);

DROP POLICY IF EXISTS "salary_override_update" ON public.salary_work_schedule_day_overrides;
...
DROP POLICY IF EXISTS "salary_override_delete" ON public.salary_work_schedule_day_overrides;
...
-- Internal: sync one user for one calendar work_date (Chicago boundaries handled by caller p_work_date).
...
COMMENT ON FUNCTION public.sync_salary_clock_sessions_for_day(date) IS 'Service role: sync all salary templates for a calendar date (default Chicago today).';
...
-- Owner-only RPC: replace one clock session with N contiguous segments (split / repartition).
-- If the session was approved, rolls back people_hours and crew sync like revoke (no revoked_at — row is deleted).
-- Week gate uses America/Chicago Sunday-start week (documented; may differ slightly from browser-local getDefaultWeekRange).
...
-- Replace N contiguous same-job/bid clock_sessions (same user) with M segments in one transaction.
-- Used by Dashboard My Time when a "cluster" of rows is edited as one vertical bar.
...
-- Leader / pay-safe split & replace RPCs: same behavior as split_own_* / replace_own_*
-- but authorize session owner, team lead for member, or pay-approved master/assistant.
