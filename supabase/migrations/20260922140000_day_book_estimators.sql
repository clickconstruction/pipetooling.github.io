...
--    a best effort recorded, a house asked for prices (with the quotes that came back),
--    a verdict on a robot audit, a robot question answered, a human follow-up with a GC
--    (bids_submission_entries with a contact_method — the robot pipeline's log lines have
--    none). Counted fixtures and why-we-lost carry no actor and are not day lines.
-- 3. `bids` in the result for the labels, and `estimating` — the range strip for ONE picked
--    person: sent · after due date · decided · hit rate over the trailing 90 days by value
--    (with its denominator — under five it is not a rate) · lost with no reason · no
--    follow-up in 7 days · prices asked → in (median days) · robot delta (median) · bid hours;
--    each measure for this window and the equal window before it ("was"). Money: a payroll
--    viewer, or the person looking at their own bids (they see them on the Bid Board today).
--
