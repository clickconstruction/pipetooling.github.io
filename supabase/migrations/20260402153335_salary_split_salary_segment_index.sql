-- Salary split: assign salary_segment_index 1..N for continuous salary_schedule multi-row inserts;
-- reject splitting a single indexed segment; optional per-segment job/bid on split_*_segments RPCs.
...
BEGIN
  ...
  IF v_parent.origin = 'salary_schedule' AND v_parent.salary_segment_index IS NOT NULL THEN
  ...
  FOR v_i IN 0 .. (v_n - 1) LOOP
    ...
    VALUES (
      ...
      v_work,
      v_notes,
      COALESCE(v_seg_job, v_parent.job_ledger_id),
      COALESCE(v_seg_bid, v_parent.bid_id),
      v_parent.origin,
      CASE
        WHEN v_parent.origin = 'salary_schedule' AND v_n >= 2 AND v_parent.salary_segment_index IS NULL
          THEN (v_i + 1)::smallint
        ELSE v_parent.salary_segment_index
      END,
      NULL,
      NULL,
      NULL,
