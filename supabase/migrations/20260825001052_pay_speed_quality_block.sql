...
-- get_billed_customer_pay_speeds v6 — the data-health quality block
-- (v2.2259; v5 was 20260824215810). Adds one `quality` key so the Pay
-- speeds breakdown can show measurability where the person who cares is
-- already looking (Billing Truth Plan Phase 4):
--
--   quality: { payments12mo, measurable, unlinked, undatedInvoices, quarantined }
--
-- All counts share the medians' 12-month payment window except
-- undatedInvoices (an all-time backlog of billed/paid invoices with no bill
-- date at all). Everything else is unchanged from v5.
...
COMMENT ON FUNCTION public.get_billed_customer_pay_speeds() IS
  'Per-customer median bill→paid gap (12 months; bill clock = COALESCE(billed_at, estimated_bill_date); unverified HCP-import same-day pairs quarantined) + company/segment medians + customer types + receipts + data-health quality counts. NULL outside dev/master/assistant-like/primary.';
