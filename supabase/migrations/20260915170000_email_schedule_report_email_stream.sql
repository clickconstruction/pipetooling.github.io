...
-- Field report emails on My email schedule (v2.3472). Owner ask: "offer some
-- people every report submitted on every job" — that stream already exists
-- (report_email_subscriptions, v2.746, 2026-07-18: all authors or selected
-- authors, auto-sent by send-report-email the moment a report is filed), but
-- neither schedule RPC read it, so a subscriber could not see it on Settings.
--
--   1. get_my_email_schedule(): `report_emails` — every subscription addressed
--      to the caller (by user id, or an external address equal to the caller's
--      account email): enabled · auto_send · all_authors · author names.
--      The `weekly` digest rows also carry `crew_filter` so the standing row
--      can read "jobs yesterday · my team".
--   2. get_global_email_schedule(): `report_email_subscriptions` — the dev
--      Email streams card: recipient name (user or external address), label,
--      flags, author names.
--
-- Both bodies are the live definitions from 20260904201238 with only the
-- additions above. Self-scoped SECURITY DEFINER as before; no table change,
-- so no fence appliers.
...
COMMENT ON FUNCTION public.get_my_email_schedule() IS
  'Self-scoped: every email stream configured to reach auth.uid() — weekly digests (with activity scope + crew filter), pending one-offs across all scheduled streams, event streams, per-estimate subscriptions, and (v2.3472) the field-report email subscriptions addressed to the caller.';
COMMENT ON FUNCTION public.get_global_email_schedule() IS
  'Dev-only: every stream''s recipients and pending sends for the Settings → Email streams panel; NULL for non-devs. v2.3472 adds report_email_subscriptions.';
