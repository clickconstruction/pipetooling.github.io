-- Insert report via RPC - SECURITY DEFINER bypasses RLS
-- Used by estimators when direct insert hits RLS issues
-- Validates caller is estimator and inserts with created_by_user_id = auth.uid()
...
COMMENT ON FUNCTION public.insert_report(uuid, jsonb, uuid, uuid) IS
  'Inserts a report. SECURITY DEFINER bypasses RLS. Only estimators.';;
