-- migrate_job_ledger_costs_and_delete: add target revenue merge (source + target) before deleting source.
-- Replaces public.migrate_job_ledger_costs_and_delete; see 20270425120000 for prior definition.
...
COMMENT ON FUNCTION public.migrate_job_ledger_costs_and_delete(uuid, uuid) IS
  'Repoints labor/parts/materials-related rows from source jobs_ledger to target, merges Mercury and supply allocations and crew JSON, adds source revenue to target revenue, then deletes the source job. Source job must be working or ready_to_bill. Blocked when source has billing artifacts.';
