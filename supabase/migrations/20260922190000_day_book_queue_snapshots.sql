...
-- Day book, decision 6 (to-dos/day-book, v2.3736): history for deposits, contracts and bills.
--
-- The owner said yes to a queue snapshot. The counts the tab needs (deposits still to
-- match, jobs still without a contract, bills still to send) are computed by client
-- kernels for a signed-in dev or controller — the Dashboard's Needs You card — and a
-- cron has no account to run them as. So the snapshot is written BY that Dashboard:
-- once a day per viewer, record_day_book_queue upserts the card's figures for the day,
-- stamped with who looked. The Day book's queue reads them for past days beside the
-- approvals it reconstructs, so the Month grid can go amber on those rows and a past
-- Applied / Sent contracts / Billed line ends with what was still waiting. A day nobody
-- looked has no row and stays unknown — the grid stays plain there, honestly.

CREATE TABLE IF NOT EXISTS public.day_book_queue_snapshots (
  day date NOT NULL,
  kind text NOT NULL CHECK (kind IN ('deposits', 'contracts', 'billing')),
...
COMMENT ON TABLE public.day_book_queue_snapshots IS
  'What a dev or controller''s Dashboard counted as still waiting on a day (deposits to match, jobs without a contract, bills to send) — written by record_day_book_queue, read by the Day book (v2.3736). The last look of the day wins.';

ALTER TABLE public.day_book_queue_snapshots ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS day_book_queue_snapshots_select ON public.day_book_queue_snapshots;
CREATE POLICY day_book_queue_snapshots_select ON public.day_book_queue_snapshots
  FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM public.users u WHERE u.id = auth.uid() AND u.role IN ('dev', 'controller')));
REVOKE ALL ON public.day_book_queue_snapshots FROM anon;
GRANT SELECT ON public.day_book_queue_snapshots TO authenticated;
GRANT ALL ON public.day_book_queue_snapshots TO service_role;

-- The Dashboard's write: p_counts is {"deposits": n, "contracts": n, "billing": n} — any
-- subset; a non-integer or negative value is skipped. Devs and controllers only.
CREATE OR REPLACE FUNCTION public.record_day_book_queue(p_day date, p_counts jsonb)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF v_uid IS NULL THEN
    RAISE EXCEPTION 'record_day_book_queue: not authenticated';
  END IF;
  SELECT u.role INTO v_role FROM public.users u WHERE u.id = v_uid;
  IF v_role IS NULL OR v_role NOT IN ('dev', 'controller') THEN
    RAISE EXCEPTION 'record_day_book_queue: not authorized';
  END IF;
  IF p_day IS NULL OR p_counts IS NULL OR jsonb_typeof(p_counts) <> 'object' THEN
    RETURN 0;
  END IF;
  FOREACH v_kind IN ARRAY ARRAY['deposits', 'contracts', 'billing'] LOOP
    IF p_counts ? v_kind AND jsonb_typeof(p_counts -> v_kind) = 'number' THEN
      v_n := floor((p_counts ->> v_kind)::numeric)::integer;
      IF v_n >= 0 THEN
        INSERT INTO public.day_book_queue_snapshots (day, kind, n, taken_at, taken_by)
        VALUES (p_day, v_kind, v_n, now(), v_uid)
        ON CONFLICT (day, kind) DO UPDATE SET n = EXCLUDED.n, taken_at = EXCLUDED.taken_at, taken_by = EXCLUDED.taken_by;
...
REVOKE ALL ON FUNCTION public.record_day_book_queue(date, jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.record_day_book_queue(date, jsonb) FROM anon;
GRANT EXECUTE ON FUNCTION public.record_day_book_queue(date, jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION public.record_day_book_queue(date, jsonb) TO service_role;

-- The Day book's body (20260922180000_day_book_for_user.sql) with the snapshot rows in
-- its queue; the two doors (get_day_book_payload, get_day_book_payload_for_user) are unchanged.
