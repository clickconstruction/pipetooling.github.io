-- Subcontractor Dashboard: latest of thread note vs field report for "Last activity" clock.
...
AS $$
  SELECT
    ...
    (SELECT max(x.v) FROM (
      SELECT (SELECT max(n.created_at) FROM public.jobs_ledger_thread_notes n WHERE n.job_id = jl.id) AS v
      UNION ALL
...
COMMENT ON FUNCTION public.list_assigned_jobs_for_dashboard() IS
  'Jobs assigned to current user (team) with status working. last_job_activity_at = max(thread note, report); last_report_at preserved; project stage + job status.';
...
COMMENT ON FUNCTION public.list_ready_to_bill_assigned_jobs_for_dashboard() IS
  'Team-assigned jobs with status ready_to_bill. last_job_activity_at = max(thread note, report).';
