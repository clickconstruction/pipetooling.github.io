-- RPC to revoke approved clock sessions: subtract hours from people_hours and move session back to Pending
...
DECLARE
  ...
  v_new_hours numeric;
BEGIN
  -- Caller must have pay access (matches approve_clock_sessions)
  ...
  LOOP
    ...
    -- Subtract hours from people_hours; delete row if result <= 0
    ...
    -- Clear approved state so session returns to Pending
...
COMMENT ON FUNCTION public.revoke_clock_sessions(UUID[]) IS 'Revoke approved clock sessions: subtract hours from people_hours and move sessions back to Pending. Caller must have pay access.';
