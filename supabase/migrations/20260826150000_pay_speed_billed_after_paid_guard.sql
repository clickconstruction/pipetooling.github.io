...
-- The billed-after-paid guard (v2.2337, owner call after the 2026-08-26 data
-- repair): a bill date AFTER the bill's own payment is a fabrication —
-- retro-created paperwork stamped with its creation day — and the old
-- GREATEST(0, …) clamp turned each one into a fake same-day sample that
-- dragged the medians down (37 found across history). From now on such
-- pairs can never count:
--
-- 1) get_billed_customer_pay_speeds v11: samples require paid_on >= the
--    bill date, so medians/receipts never see a billed-after-paid pair.
-- 2) get_pay_speed_transactions v5: those rows bucket as 'unlinked'
--    (Missing info) with gapDays NULL; the client chips them
--    "billed after paid" and offers the inline date editor to fix them.
-- 3) get_undated_bill_worklist v2: the Quickfill "Missing bill dates"
--    station also lists bills whose date sits after a payment (payload
--    gains billedYmd so the row can say what's wrong), so assistants fix
--    them alongside the undated backlog.
...
samples AS (
  ...
  WHERE p.invoice_id IS NOT NULL
    ...
    -- The guard: a payment received before its bill's date is not a
    -- measurable pair — the date is wrong (or it's a pre-bill deposit);
    -- either way it teaches the medians nothing.
...
COMMENT ON FUNCTION public.get_billed_customer_pay_speeds() IS
  'Per-customer median bill→paid gap (12 months; bill clock = COALESCE(billed_at, estimated_bill_date); HCP same-day pairs quarantined; billed-after-paid pairs NEVER sampled - v11 guard) + company/segment medians + customer types + receipts + data-health quality counts. NULL outside dev/master/assistant-like/primary.';
...
'Every 12-month payment with job identity, invoiceId + status bucket (measurable/unlinked/quarantined/excluded; billed-after-paid buckets as unlinked - v5 guard) and the all-time undated-bills backlog - the Data health drill-down. NULL outside dev/master/assistant-like/primary.';
...
bills AS (
  SELECT
    ...
    COALESCE((i.billed_at AT TIME ZONE 'America/Chicago')::date, i.estimated_bill_date) AS billed_on,
    j.id AS job_id,
    j.job_name,
    j.job_address,
    j.customer_name,
  ...
  CROSS JOIN LATERAL (
    SELECT
      ...
      MIN(p.paid_on) AS earliest_paid_on
    FROM public.jobs_ledger_payments p
    WHERE p.invoice_id = i.id
  ) pay
  WHERE i.status IN ('billed', 'paid')
    AND (
      -- undated: no bill date at all
      (i.billed_at IS NULL AND i.estimated_bill_date IS NULL)
      -- v2 guard: dated, but the date sits AFTER a payment — provably wrong
      OR (
        COALESCE((i.billed_at AT TIME ZONE 'America/Chicago')::date, i.estimated_bill_date) IS NOT NULL
        AND pay.earliest_paid_on IS NOT NULL
        AND pay.earliest_paid_on < COALESCE((i.billed_at AT TIME ZONE 'America/Chicago')::date, i.estimated_bill_date)
      )
    )
    AND (
      EXISTS (
...
'Bills needing a date fix past the No Count Date: undated billed/paid bills PLUS bills whose date sits after a payment (billedYmd carries the wrong date - v2 guard), with clues (job identity, HCP, payment dates) - the Quickfill Missing bill dates station. NULL outside dev/master/assistant-like.';
