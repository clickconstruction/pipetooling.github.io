-- Restore boundary mass-close: shift end sets clocked_out_at for all applicable opens
-- (including approved and user_punch), half-open-safe split passes, and PTO/no-template/weekend
-- close remaining opens at p_now after deleting non-final salary_schedule rows.
...
COMMENT ON FUNCTION public.salary_sync_one_user_clock_sessions(uuid, date, timestamptz) IS 'Definer: half-open template slots [t_open,t_close); at each block end sets clocked_out_at on all applicable open sessions (approved does not block; rejected/revoked excluded) with clocked_in_at < boundary; work_date or clock-in date in template tz. PTO/no template/excluded weekend: delete non-final salary_schedule rows then close remaining opens at p_now. Continuous skips NULL-index INSERT when any indexed salary_schedule rows exist for the day (non-rejected/non-revoked).';
