...
GRANT EXECUTE ON FUNCTION public.leader_split_clock_session_segments(uuid, jsonb) TO authenticated;CREATE OR REPLACE FUNCTION public.leader_split_clock_session_cluster(
  p_session_ids uuid[],
  p_segments jsonb
)
RETURNS TABLE(inserted_ids uuid[], error_message text)
...
GRANT EXECUTE ON FUNCTION public.leader_split_clock_session_cluster(uuid[], jsonb) TO authenticated;CREATE OR REPLACE FUNCTION public.leader_replace_clock_session_cluster_mixed(
  p_session_ids uuid[],
  p_segments jsonb
)
RETURNS TABLE(inserted_ids uuid[], error_message text)
