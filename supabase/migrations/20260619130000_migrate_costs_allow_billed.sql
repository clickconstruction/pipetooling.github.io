-- Add an opt-in `p_allow_billed` flag to migrate_job_ledger_costs_and_delete.
--
-- Today the function refuses (billing guard) unless the source job is Working /
-- Ready-to-bill with no invoices/payments/collect-flow. That's correct for the
-- Combine flow, but it blocks the "reassign this job's costs to another job, then
-- delete" action we now want to offer from the Delete-job modal for BILLED jobs.
--
-- When p_allow_billed = true, the billing guard is skipped: costs, labor, crew,
-- reports, schedule and revenue still move to the target, and the source job's own
-- invoices/payments simply cascade-delete with it. Default false keeps the existing
-- Combine caller (2-arg) behavior identical. Everything else is byte-for-byte the
-- same as the prior definition. DROP first so there is a single 3-arg signature.

drop function if exists public.migrate_job_ledger_costs_and_delete(uuid, uuid);
