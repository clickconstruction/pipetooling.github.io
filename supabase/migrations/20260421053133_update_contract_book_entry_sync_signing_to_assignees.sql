-- After Contract Book save, push library body + canonical into person rows for template assignees (signer reads person_contract_documents).
...
BEGIN
  ...
  UPDATE public.person_contract_documents p
  SET
    signing_body_html = p_book_body_html,
    signing_body_format = p_book_body_format,
    canonical_document_url = v_canon,
    updated_at = NOW()
  WHERE p.document_name = v_trim_name
    AND EXISTS (
      SELECT 1
      FROM public.person_contract_assignments a
      WHERE a.template_id = v_template_id
...
COMMENT ON FUNCTION public.update_contract_book_entry(uuid, text, text, text, text[], text) IS
  'Updates a Contract Book row; renames matching person rows when document_name changes; syncs signing body + canonical to assignees. Pay-staff only. SECURITY DEFINER.';
