...
-- Day book access (to-dos/day-book, v2.3732): devs and controllers only.
--
-- The owner's decision 2 (2026-09-22): "No one should have access to day books except
-- for the devs and controllers at the moment." Same function as
-- 20260922160000_day_book_estimators.sql with the gate narrowed — masters, assistants and
-- estimators are refused server-side; the client hides the tab, the Crew Day line and the
-- Bids door for them as well. Money and the person pick stay on for the two roles.
--
...
BEGIN
  ...
  IF public.is_dev() OR v_role = 'controller' THEN
    v_money := true;
    v_pick := true;
  ELSE
