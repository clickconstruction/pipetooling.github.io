-- Allow migrate_delete when source job status is working OR ready_to_bill (not working only).
-- Replaces public.migrate_job_ledger_costs_and_delete; see 20270424120000 for original.
...
COMMENT ON FUNCTION public.migrate_job_ledger_costs_and_delete(uuid, uuid) IS
  'Repoints labor/parts/materials-related rows from source jobs_ledger to target, merges Mercury and supply allocations and crew JSON, then deletes the source job. Source job must be working or ready_to_bill. Blocked when source has billing artifacts.';
