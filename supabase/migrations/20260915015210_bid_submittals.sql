...
COMMENT ON TABLE public.bid_submittals IS
  'Submittals stage 2 (v2.3461): one revision of a bid''s submittal package; items in bid_submittal_items; files in the bid-submittals bucket under <bid_id>/<submittal_id>/.';
...
COMMENT ON TABLE public.bid_submittal_items IS
  'Submittals stage 2 (v2.3461): one row per fixture tag on a revision — specified vs submitted, status, reason, lead time, sheet pages; stage 4 adds the reviewer''s decision.';
