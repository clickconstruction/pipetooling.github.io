-- Stale tally staff list: optional include_all_unlinked skips Chicago calendar-day age filter.
-- Replaces single-arg overload with (min_age_days, include_all_unlinked) to avoid PostgREST ambiguity.

DROP FUNCTION IF EXISTS public.list_stale_unlinked_mercury_transactions_for_tally_staff(integer);
...
COMMENT ON FUNCTION public.list_stale_unlinked_mercury_transactions_for_tally_staff(integer, boolean) IS
  'Dev/master/assistant: unlinked Mercury rows for linked-card users in staff_can_view_user_for_tally_followup. When include_all_unlinked, no min-age filter (still job_tally_min_posted_ymd floor).';
