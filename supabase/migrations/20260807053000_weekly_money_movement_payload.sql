...
-- get_weekly_money_movement_payload (v2.1442) — Weekly Money Movement Phase 1
-- (docs/WEEKLY_MONEY_PLAN.md). ONE source of truth for the report: this RPC
-- feeds both the client modal (dev/controller) and, later, the weekly_money
-- email dispatcher (service role) — deliberately RPC-first so no client/SQL
-- hand-mirroring is ever needed.
--
-- Money-out conventions mirror the existing surfaces exactly (reconciliation
-- invariants in the plan doc):
--   * Team labor = people_crew_jobs assignments: day hours (salary 8/0 Mon–Fri,
--     else people_hours) × pct/100 × people_pay_config.hourly_wage — byte-math
--     parity with src/utils/teamLabor.ts (Crew P&L / Job Summary convention).
--   * Sub labor  = people_labor_jobs dated in-week (job_date), item cost per
--     lineLaborCost semantics + drive cost (miles × mileage + miles ×
--     time-per-mile × rate), linked to jobs_ledger by HCP number.
--   * Materials  = mercury allocations by Chicago posted date, supply-invoice
--     allocations (pct × invoice amount) by invoice_date, tally lines by
--     Chicago created_at (fixture_cost×qty when no part, else price_at_time×qty),
--     manual jobs_ledger_materials by Chicago created_at.
--   * Money in   = jobs_ledger_payments by paid_on.
--   * pct_start/pct_end from job_pct_events (Phase 0) as-of week bounds.
-- The configured Overhead office job's rows fold into the overhead bucket
-- (never a job row); crew BID labor also folds into overhead.
--
-- p_week_monday NULL = previous complete Central (Mon–Sun) week.
