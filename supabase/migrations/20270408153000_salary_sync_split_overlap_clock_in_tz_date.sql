-- Split-mode salary sync: overlap guard for canonical slots 1/2 required cs.work_date = p_work_date.
-- Split children use v_work from clock-in in a fixed zone; template tz may differ, so work_date can
-- disagree with p_work_date while the session still overlaps [t_start,t_end). That let sync INSERT a
-- second empty salary_schedule row for slot 1. Also match rows where clock-in falls on p_work_date in
-- the template timezone.
  ...
  ELSE
    IF NOT EXISTS (
      ...
      WHERE cs.user_id = p_user_id
        ...
        AND COALESCE(cs.clocked_out_at, p_now) > t_start
        AND (
  ...
  ELSE
    IF NOT EXISTS (
      ...
      WHERE cs.user_id = p_user_id
        ...
        AND COALESCE(cs.clocked_out_at, p_now) > t_start2
        AND (
          cs.work_date = p_work_date
          OR (cs.clocked_in_at AT TIME ZONE tz)::date = p_work_date
        )
...
COMMENT ON FUNCTION public.salary_sync_one_user_clock_sessions(uuid, date, timestamptz) IS 'Definer: create/close salary_schedule clock_sessions for one user/day; PTO and no template and excluded weekends delete non-final salary rows; weekend work via meaningful day override; continuous mode skips INSERT when pending indexed split segments already exist for the day; split mode skips canonical slot INSERT when any non-rejected/non-revoked session overlaps the template window [t_start,t_end) or [t_start2,t_end2), matching by work_date or clock-in civil date in template tz.';
