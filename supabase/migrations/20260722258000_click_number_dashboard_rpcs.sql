-- Dashboard job-list RPCs: return click_number as its own column.
-- The v2.9xx client sweep renders effectiveJobLedgerNumber(hcp_number,
-- click_number) at every display site; these three RPCs were the last ones
-- whose RETURNS TABLE lacked click_number (their hcp_number output is already
-- the server-baked COALESCE(hcp, click) from
-- 20260619160000_click_number_remaining_rpcs_2.sql — that stays untouched;
-- the raw click_number column is appended at the END of RETURNS TABLE so
-- existing name-based readers are unaffected).
...
-- the previous explicit grants. Idempotent as a unit; bodies otherwise
-- identical to their previous definitions
-- (20260721160000_assigned_jobs_customer_name.sql for
-- list_assigned_jobs_for_dashboard; 20260619160000 sections 1–2 for the
-- other two).

-- =====================================================================
-- 1. list_assigned_jobs_for_dashboard()
-- =====================================================================
...
CREATE OR REPLACE FUNCTION public.list_assigned_jobs_for_dashboard()
 RETURNS TABLE(id uuid, hcp_number text, job_name text, job_address text, google_drive_link text, job_plans_link text, job_pictures_link text, revenue numeric, master_user_id uuid, created_at timestamp with time zone, last_report_at timestamp with time zone, my_last_report_at timestamp with time zone, last_thread_note_at timestamp with time zone, last_clock_activity_at timestamp with time zone, last_schedule_activity_at timestamp with time zone, last_job_activity_at timestamp with time zone, project_id uuid, in_progress_stage_name text, in_progress_step_id uuid, status text, service_type_id uuid, service_type_name text, customer_name text, click_number text)
...
-- =====================================================================
-- 2. list_ready_to_bill_assigned_jobs_for_dashboard()
-- =====================================================================
...
CREATE OR REPLACE FUNCTION public.list_ready_to_bill_assigned_jobs_for_dashboard()
 RETURNS TABLE(id uuid, hcp_number text, job_name text, job_address text, google_drive_link text, job_plans_link text, job_pictures_link text, revenue numeric, master_user_id uuid, created_at timestamp with time zone, last_report_at timestamp with time zone, my_last_report_at timestamp with time zone, last_thread_note_at timestamp with time zone, last_clock_activity_at timestamp with time zone, last_schedule_activity_at timestamp with time zone, last_job_activity_at timestamp with time zone, status text, click_number text)
 LANGUAGE sql
...
-- =====================================================================
-- 3. list_superintendent_jobs_for_dashboard()
-- =====================================================================
...
CREATE OR REPLACE FUNCTION public.list_superintendent_jobs_for_dashboard()
 RETURNS TABLE(id uuid, hcp_number text, job_name text, job_address text, google_drive_link text, job_plans_link text, job_pictures_link text, revenue numeric, created_at timestamp with time zone, my_last_report_at timestamp with time zone, project_id uuid, in_progress_stage_name text, in_progress_step_id uuid, click_number text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
