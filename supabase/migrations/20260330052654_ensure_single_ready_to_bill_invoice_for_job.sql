-- Ensure at most one ready_to_bill jobs_ledger_invoices row for Stripe-from-job flows.
-- Unallocated = revenue - payments_made - SUM(amount) for invoices in ready_to_bill or billed.
...
BEGIN
  ...
  -- Visibility aligned with jobs_ledger_invoices UPDATE policy (broader than INSERT).
  IF NOT (
    EXISTS (
...
COMMENT ON FUNCTION public.ensure_single_ready_to_bill_invoice_for_job(uuid) IS
  'Creates or syncs a single ready_to_bill invoice from unallocated job balance for Stripe-from-job UX.';
