...
-- RUN_SUBS_PLAN Phase 2, PR 2.3: settle a step commitment into the Sub Labor
-- ledger. Settlement creates (or reuses) the people_labor_jobs sub sheet with
-- ONE direct-amount line = amount x (1 - retainage_pct/100), stamps the
-- project/step anchors (PR 0.3), links commitment.labor_job_id, and flips the
-- commitment to 'settled'. The Sub Labor tab stays the single AP ledger —
-- payments/backcharges are recorded there exactly as today.
--
-- p_dry_run uses the rollback-sentinel technique (merge_user_accounts /
-- migrate_job_ledger_costs_to_bid_and_delete precedent): the whole settlement
-- runs inside a nested block, the report is thrown as an exception detail,
-- the handler catches it — Postgres rolls the nested block's work back — and
-- the report is returned. The preview IS the real settlement, so the numbers
-- can never drift. No bare DELETEs (pg_safeupdate is loaded on PostgREST).
...
COMMENT ON FUNCTION public.settle_step_commitment(uuid, boolean) IS
  'Settles a step commitment: creates/reuses the anchored people_labor_jobs sub sheet with one direct-amount line = amount x (1 - retainage_pct/100), links labor_job_id, flips status to settled. p_dry_run rolls back and returns the same report (sentinel-exception technique).';
