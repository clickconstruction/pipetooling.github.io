-- Split mode: if segment B local start resolves to the same instant as segment A, slot-2 window
-- [t_start2,t_end2) was identical to slot-1, so the closed slot-1 row always half-overlaps slot-2’s
-- insert window and canonical segment 2 is never created. Remap: second block starts at t_end (A’s end), contiguous.
