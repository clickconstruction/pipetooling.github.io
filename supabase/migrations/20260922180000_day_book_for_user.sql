...
-- Day book PR 6b (to-dos/day-book, v2.3733): the Crew Day email's line.
--
-- The crew-day-email-dispatch function runs as the service role, which has no
-- auth.uid(), so it could not call get_day_book_payload for a recipient. The body
-- moves into day_book_payload_for(p_uid, …) — the same SQL as
-- 20260922170000_day_book_access_dev_controller.sql, with the viewer passed in and the
-- gate read off users.role (dev · controller) instead of is_dev(), which only knows
-- auth.uid(). Two thin doors call it:
--   get_day_book_payload(p_from, p_to, p_person)  — the app's, as auth.uid(); unchanged shape.
--   get_day_book_payload_for_user(p_user_id, p_day) — service_role only, the
--     get_crew_day_payload_for_user pattern: one day, as that recipient; a recipient
--     the gate refuses gets {'error':'forbidden'} and the email carries no line.
--
...
REVOKE ALL ON FUNCTION public.day_book_payload_for(uuid, date, date, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.day_book_payload_for(uuid, date, date, uuid) FROM anon;
REVOKE ALL ON FUNCTION public.day_book_payload_for(uuid, date, date, uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.day_book_payload_for(uuid, date, date, uuid) TO service_role;
...
SET search_path = public
AS $$
  SELECT public.day_book_payload_for(auth.uid(), p_from, p_to, p_person);
$$;
...
CREATE OR REPLACE FUNCTION public.get_day_book_payload_for_user(p_user_id uuid, p_day date)
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT public.day_book_payload_for(p_user_id, p_day, p_day, NULL);
$$;

REVOKE ALL ON FUNCTION public.get_day_book_payload_for_user(uuid, date) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_day_book_payload_for_user(uuid, date) FROM anon;
REVOKE ALL ON FUNCTION public.get_day_book_payload_for_user(uuid, date) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.get_day_book_payload_for_user(uuid, date) TO service_role;
