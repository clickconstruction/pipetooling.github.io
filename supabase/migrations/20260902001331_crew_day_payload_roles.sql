...
-- Crew Day payload: users carry their role (v2.2617). The round-two Crew Day
-- section folds OFFICE-ROLE people away by default for superintendent viewers
-- ("field crews first" — owner decision 2026-09-01); the client needs each
-- involved person's role to bucket them. Both payload functions are rebuilt
-- VERBATIM from their shipping migrations (20260901215024 /
-- 20260901220804) with one change each: the users array gains 'role'.
-- Additive key — older clients ignore it. Still no pay data.
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'pct_notes', COALESCE(
      ...
      '[]'::jsonb),
    'users', COALESCE(
      (SELECT jsonb_agg(jsonb_build_object('id', u.id, 'name', u.name, 'role', u.role))
       FROM public.users u
       WHERE u.id IN (
...
COMMENT ON FUNCTION public.get_crew_day_payload(date) IS
  'Dashboard Crew Day section payload (v2.2602; +user roles v2.2617): one company-calendar day of clock sessions, schedule blocks, field reports, and % complete thread notes, with name/label/role rows for the people and jobs involved. Office roles company-wide; superintendent scoped to jobs on their accessible projects or team-membership jobs (job-less rows dropped). Grouping/flags in src/lib/crewDay.ts.';

COMMENT ON FUNCTION public.get_crew_day_payload_for_user(uuid, date) IS
  'Crew Day email payload for crew-day-email-dispatch (v2.2603; +user roles v2.2617). Service-role only. get_crew_day_payload computed for the RECIPIENT: office roles company-wide; superintendent scoped via project_superintendents + jobs_ledger_team_members; other roles {error:forbidden}. Keep in sync with get_crew_day_payload.';
