...
-- weekly_money stream -> email schedule surfaces (v2.1449) — WEEKLY_MONEY_PLAN
-- Phase 5 / REPORT_SUBSCRIPTIONS checklist step 5. get_my_email_schedule()
-- gains a recipient-scoped weekly_money one-offs branch (recipients are
-- dev/controller users); get_global_email_schedule() gains
-- weekly_money_requests.
--
-- Bodies are the LIVE definitions (v2.1400 rule) — last rebuilt by this
-- train's own 20260807025314 (v2.1438), reproduced verbatim with only the
-- weekly_money additions; closers semicolon-terminated (v2.1428 lesson).
