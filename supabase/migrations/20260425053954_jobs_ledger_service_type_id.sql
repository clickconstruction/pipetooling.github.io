-- jobs_ledger: required service_type_id (trade). Backfill existing rows to Plumbing.
...
WHERE jl.service_type_id IS NULL
  AND st.name = 'Plumbing';

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM public.jobs_ledger WHERE service_type_id IS NULL) THEN
    RAISE EXCEPTION 'jobs_ledger.service_type_id backfill failed: Plumbing row missing or NULLs remain';
  END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_jobs_ledger_service_type_id
  ON public.jobs_ledger(service_type_id);

COMMENT ON COLUMN public.jobs_ledger.service_type_id IS 'Trade / service type for the job (Plumbing, Electrical, …). Independent of optional bid_id.';

-- create_job_from_estimate (6-arg, no fixtures): set service_type_id to Plumbing
...
-- create_job_from_estimate (7-arg, p_fixtures): set service_type_id to Plumbing
CREATE OR REPLACE FUNCTION public.create_job_from_estimate(
  p_estimate_id uuid,
  p_hcp_number text,
  p_job_name text DEFAULT NULL,
