...
COMMENT ON FUNCTION public.split_own_clock_session_cluster(uuid[], jsonb) IS
  'Replace N contiguous same-job/bid clock sessions (owner) with M segments; current or previous Denver week; approved rollback per removed row; new rows pending.';
