-- Include time (HH:MM:SS) in Job Tally PO names to avoid collisions on multiple saves same day
...
BEGIN
  ...
  -- 3. Build PO name: "Job Parts [HCP#] [YYYY-MM-DD HH:MM:SS]" (time avoids collisions on multiple saves)
  v_po_name := 'Job Parts ' || v_hcp || ' ' || to_char(now(), 'YYYY-MM-DD HH24:MI:SS');
...
COMMENT ON FUNCTION public.create_po_from_job_tally(uuid, jsonb) IS
  'Creates a Purchase Order from Job Tally entries. Called when user saves on Job Tally. PO name: Job Parts [HCP#] [date time]. SECURITY DEFINER so subs can trigger.';
