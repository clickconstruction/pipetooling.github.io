-- Person identity Phase C-2 (v2.1011; docs/PERSON_IDENTITY_PLAN.md): the paid
-- email payload's wage joins go person-first. Both labor sections previously
-- joined people_pay_config on lower(trim(person_name)) = lower(trim(users.name))
-- — a rename silently zeroed wages. Now: users → people.account_user_id →
-- people_pay_config.person_id, with the exact old name-join kept as fallback
-- for users lacking a linked person. Body otherwise identical to
-- 20260722262000_paid_email_last_payment.sql (v2.969 last_payment preserved).
-- CREATE OR REPLACE, same signature — grants unchanged (service_role-only).
