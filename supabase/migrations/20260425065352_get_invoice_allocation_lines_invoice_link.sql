-- Job Summary: per-invoice document link (Materials supply_house_invoices.link), same as Supply Houses "View"
...
COMMENT ON FUNCTION public.get_invoice_allocation_lines_for_jobs(uuid[]) IS
  'Per-invoice lines allocated to jobs for Job Summary Parts Cost. Includes supply_houses.website_url and supply_house_invoices.link (document URL). Visibility matches get_invoice_amounts_for_jobs.';
