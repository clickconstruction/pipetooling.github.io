-- Bank Payments (Jobs Stages): link jobs_ledger_payments to mercury_transactions; list + apply RPCs.

ALTER TABLE public.jobs_ledger_payments
  ADD COLUMN IF NOT EXISTS mercury_transaction_id UUID REFERENCES public.mercury_transactions (id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_jobs_ledger_payments_mercury_transaction_id
  ON public.jobs_ledger_payments (mercury_transaction_id)
  WHERE mercury_transaction_id IS NOT NULL;

COMMENT ON COLUMN public.jobs_ledger_payments.mercury_transaction_id IS
  'When set, this payment row was recorded from a Mercury bank transaction (Jobs Stages Bank Payments).';

-- Lowercased debit card UUID from raw JSON (align with app mercuryDebitCardIdFromRaw).
CREATE OR REPLACE FUNCTION public._mercury_raw_debit_card_id_lower(p_raw jsonb)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT nullif(
    lower(trim(coalesce(
      p_raw #>> '{details,debitCardInfo,id}',
      p_raw #>> '{debitCardInfo,id}',
      ''
    ))),
    ''
  );
...
BEGIN
  ...
  -- First pass: validate each allocation + seed running remainders in v_inv_rem / v_job_rem
  FOR v_elem IN SELECT value FROM jsonb_array_elements(p_allocations)
  LOOP
    v_amt := (v_elem->>'amount')::numeric;
  ...
  v_ref := nullif(trim(coalesce(v_mt.mercury_id, '')), '');

  -- Second pass: insert payments and update ledgers
  FOR v_elem IN SELECT value FROM jsonb_array_elements(p_allocations)
  LOOP
    v_amt := (v_elem->>'amount')::numeric;
    IF v_elem ? 'invoice_id' AND nullif(trim(v_elem->>'invoice_id'), '') IS NOT NULL THEN
...
COMMENT ON FUNCTION public.apply_mercury_bank_payment_allocations(uuid, date, text, text, jsonb) IS
  'Applies one Mercury bank transaction across multiple billed jobs/invoices (non-Stripe only). Dev/master/assistant/primary.';

REVOKE ALL ON FUNCTION public.apply_mercury_bank_payment_allocations(uuid, date, text, text, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.apply_mercury_bank_payment_allocations(uuid, date, text, text, jsonb) TO authenticated;
