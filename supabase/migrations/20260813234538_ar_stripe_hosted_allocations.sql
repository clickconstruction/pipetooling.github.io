...
-- AR: allocate bank deposits to Stripe-hosted billed lines, opt-in (v2.1614).
--
-- The customer-paid-by-check-instead-of-the-Stripe-link case had no one-step
-- path: apply_mercury_bank_payment_allocations hard-rejected Stripe-hosted
-- invoices (directly AND via linked recorded payments). Both guards now relax
-- ONLY when the new p_allow_stripe_hosted argument is true — the AR modal
-- sends it after the user checks an explicit "paid outside Stripe" gate that
-- also reminds them to void / mark the invoice out-of-band in Stripe so the
-- emailed link can't be paid twice. Default false keeps every existing caller
-- byte-identical (old deployed clients call with five named args and get the
-- default).
--
-- Signature change (added defaulted param) would otherwise CREATE a second
-- overload, so the old five-parameter function is dropped first. Body is the
-- LIVE definition from 20260811200048_b6_payments_guard_and_race_fix.sql with
-- exactly the two guard conditions edited. Grants restored to match baseline.

DROP FUNCTION IF EXISTS "public"."apply_mercury_bank_payment_allocations"("uuid", "date", "text", "text", "jsonb");

CREATE FUNCTION "public"."apply_mercury_bank_payment_allocations"("p_mercury_transaction_id" "uuid", "p_paid_on" "date", "p_payment_type" "text", "p_note" "text", "p_allocations" "jsonb", "p_allow_stripe_hosted" boolean DEFAULT false) RETURNS "jsonb"
...
GRANT ALL ON FUNCTION "public"."apply_mercury_bank_payment_allocations"("p_mercury_transaction_id" "uuid", "p_paid_on" "date", "p_payment_type" "text", "p_note" "text", "p_allocations" "jsonb", "p_allow_stripe_hosted" boolean) TO "anon";
GRANT ALL ON FUNCTION "public"."apply_mercury_bank_payment_allocations"("p_mercury_transaction_id" "uuid", "p_paid_on" "date", "p_payment_type" "text", "p_note" "text", "p_allocations" "jsonb", "p_allow_stripe_hosted" boolean) TO "authenticated";
GRANT ALL ON FUNCTION "public"."apply_mercury_bank_payment_allocations"("p_mercury_transaction_id" "uuid", "p_paid_on" "date", "p_payment_type" "text", "p_note" "text", "p_allocations" "jsonb", "p_allow_stripe_hosted" boolean) TO "service_role";

-- ---------------------------------------------------------------------------
-- list_unlinked_payments_for_bank_payments: include manually recorded
-- payments that were applied to Stripe-hosted invoices (e.g. a check recorded
-- in Edit Job against a Stripe bill), flagged via the new stripe_hosted
-- column so the AR modal can require the same out-of-band confirmation.
-- True Stripe webhook payments (note = 'Stripe') stay excluded. Return-shape
-- change forces DROP + CREATE; old clients ignore the extra column.
-- ---------------------------------------------------------------------------

DROP FUNCTION IF EXISTS "public"."list_unlinked_payments_for_bank_payments"();

CREATE FUNCTION "public"."list_unlinked_payments_for_bank_payments"()
RETURNS TABLE(
  "payment_id" "uuid",
  "job_id" "uuid",
  "amount" numeric,
...
COMMENT ON FUNCTION "public"."list_unlinked_payments_for_bank_payments"() IS
  'AR modal (v2.1191, v2.1614): recorded jobs_ledger_payments rows eligible to be linked to a Mercury deposit — unlinked, not Stripe-provenance (note <> Stripe), positive amount, paid_on within 180 Chicago days. Payments applied to Stripe-hosted invoices ARE listed since v2.1614, flagged stripe_hosted (the client requires an out-of-band confirmation and sends p_allow_stripe_hosted). Zero rows for roles outside dev/master/assistant/primary.';
