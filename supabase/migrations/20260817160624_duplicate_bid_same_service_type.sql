...
-- Same-trade bid duplication (v2.1765): the Copy Bid modal gains "Duplicate this
-- <trade> bid". duplicate_bid_to_service_type refused a target equal to the
-- source's service type (baseline guard) — the ONLY change here is removing
-- that guard and suffixing the project name with " (copy)" for same-type
-- copies. Body otherwise reproduced verbatim from the live definition
-- (baseline 20250101000000, never altered since).
