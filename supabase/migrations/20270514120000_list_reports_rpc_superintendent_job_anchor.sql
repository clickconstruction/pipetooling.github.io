-- JobReportsModal calls list_reports_with_job_info (full list, then filters by job).
-- Superintendent branch still required jl.project_id NOT NULL + can_access_project_row,
-- while reports table RLS uses superintendent_report_job_anchor_allowed (project OR team assignment).
-- Align RPC visibility so superintendents see job reports they may insert.
