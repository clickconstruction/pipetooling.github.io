...
BEGIN
  ...
  BEGIN
    ...
    v_handled := v_handled || 'customers.master_user_id';
    ...
    v_handled := v_handled || 'jobs_ledger.master_user_id';
    ...
    v_handled := v_handled || 'labels.master_user_id';
    ...
    IF EXISTS (SELECT 1 FROM public.people WHERE account_user_id = p_absorbed_user_id) THEN
      IF EXISTS (SELECT 1 FROM public.people WHERE account_user_id = p_survivor_user_id) THEN
        v_warnings := v_warnings ||
          'Both accounts are linked to roster people; the absorbed account''s roster link was left in place. Merge the roster entries manually if needed.';
      ELSE
        ...
        v_handled := v_handled || 'people.account_user_id';
      END IF;
    ELSE
      v_handled := v_handled || 'people.account_user_id';
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM public.estimates WHERE accept_notify_user_ids @> ARRAY[p_absorbed_user_id]) THEN
      v_leftovers := v_leftovers || 'estimates.accept_notify_user_ids';
    END IF;
