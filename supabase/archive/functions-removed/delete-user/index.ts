...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: userData, error: userError } = await supabase.from('users').select('role').eq('id', authUser.id).single();
    if (userError || !userData || userData.role !== 'dev') {
      return new Response(JSON.stringify({
        error: 'Forbidden - Only devs can delete users'
      }), {
        status: 403,
        headers: {
          ...corsHeaders,
    ...
    // Parse request body
    const { email, name, reassign_customers_to } = await req.json();
    if (!email && !name) {
      return new Response(JSON.stringify({
    ...
    if (email) {
      const { data: userByEmail, error: emailError } = await supabase.from('users').select('id, email, name').eq('email', email.trim()).maybeSingle();
      if (emailError) {
        return new Response(JSON.stringify({
          error: `Error finding user: ${emailError.message}`
        }), {
          status: 500,
          headers: {
            ...corsHeaders,
    ...
    if (!userToDelete && name) {
      const { data: userByName, error: nameError } = await supabase.from('users').select('id, email, name').eq('name', name.trim()).maybeSingle();
      if (nameError) {
        return new Response(JSON.stringify({
          error: `Error finding user: ${nameError.message}`
        }), {
          status: 500,
          headers: {
            ...corsHeaders,
    ...
    if (reassign_customers_to) {
      ...
      const { data: newMaster, error: newMasterError } = await supabase.from('users').select('id, role').eq('id', reassign_customers_to).single();
      ...
      const { count, error: countError } = await adminClient.from('customers').select('id', {
        count: 'exact',
        head: true
      }).eq('master_user_id', userToDelete.id);
      if (countError) {
        return new Response(JSON.stringify({
          error: `Failed to count customers: ${countError.message}`
        }), {
          status: 500,
          headers: {
            ...corsHeaders,
      ...
      if (customerCount > 0) {
        const { error: reassignError } = await adminClient.from('customers').update({
          master_user_id: reassign_customers_to
        }).eq('master_user_id', userToDelete.id);
        if (reassignError) {
          return new Response(JSON.stringify({
    ...
    const { error: deleteUserError } = await adminClient.from('users').delete().eq('id', userToDelete.id);
    if (deleteUserError) {
      return new Response(JSON.stringify({
    ...
    return new Response(JSON.stringify({
      success: true,
      message: reassign_customers_to ? `User ${userToDelete.email || userToDelete.name} deleted and ${customerCount} customer${customerCount !== 1 ? 's' : ''} reassigned` : `User ${userToDelete.email || userToDelete.name} deleted successfully`,
