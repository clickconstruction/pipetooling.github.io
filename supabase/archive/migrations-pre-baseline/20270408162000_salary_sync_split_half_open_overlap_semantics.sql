-- Half-open interval semantics for split-mode canonical slot overlap (see function header comment).
-- Supersedes logic from 20270408153000 with explicit t_open/t_close overlap predicates and docs.
--
-- Boundary test matrix (half-open template [t_open, t_close), session [s_in, s_out_eff),
-- s_out_eff = COALESCE(clocked_out_at, p_now)):
--   1) Slot1 closed [8,t_end), nothing else, p_now inside slot2 → canonical slot2 may INSERT (no overlap).
--   2) Open session from slot1 with s_out_eff past t_start2 → overlaps slot2 → canonical slot2 INSERT blocked.
--   3) Session with clocked_out_at = t_start2 → t_start2 < s_out_eff is false → does not block slot2.
--   4) Adjacent blocks: slot1 ends at t_end, slot2 starts at t_end; slot1 row closed with clocked_out_at = t_end
--      → t_start2 < clocked_out_at means t_end < t_end false → no false overlap at joint boundary.
--
-- If Mike-style issues persist: segment1 may be approved with clocked_out_at still null; sync only auto-closes
-- when approved_at IS NULL in the slot UPDATE branch — separate product/migration follow-up.
...
COMMENT ON FUNCTION public.salary_sync_one_user_clock_sessions(uuid, date, timestamptz) IS 'Definer: half-open template slots [t_open,t_close); split overlap = clock_in < t_close AND t_open < coalesce(out, now); create/close salary_schedule rows; PTO/no template/excluded weekends; continuous skips INSERT when pending indexed splits; work_date or clock-in date in template tz.';
