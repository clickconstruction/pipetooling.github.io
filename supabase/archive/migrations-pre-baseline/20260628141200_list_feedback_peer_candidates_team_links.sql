-- Include peer candidates from the master roster plus people rows matching linked team users
-- (assistants, superintendents, primaries) by email so non–roster-only subs appear when those
-- users have a people row (peer ratings still reference people.id).
...
COMMENT ON FUNCTION public.list_feedback_peer_candidates() IS
  'People for peer feedback: same master roster plus people rows matching assistants, superintendents, and primaries linked to that master (by user email). Capped at 5000 rows.';
