-- Tail migration: re-apply the consolidated salary_sync_one_user_clock_sessions body so it always
-- wins on a fresh `supabase db reset`.
--
-- Why this exists:
--   `20260515092032` repaired live drift (six 2027* salary migrations were recorded as applied but
--   the live function body was older). On the linked DB nothing else needs to happen — those 2027
--   files are recorded as applied and `db push` won't re-run them. But ASCII timestamp ordering
--   means a fresh `db reset` would run them after `20260515092032`, overwriting the consolidated
--   body with `20270516120000`'s snapshot (close, but missing the product change that lets sync
--   close approved-but-open salary rows at t_end). This file pins the right body last.
--
--   This file's body is identical to `20260515092032_salary_sync_close_at_t_end_including_approved_open.sql`.
--   Keep them in sync if either changes; better yet, write a new tail.
--
-- Behavior summary (matches 20260515092032):
--   - Half-open template slots [t_open, t_close); clocked_out_at stores the exclusive end.
--   - Continuous canonical NULL row + indexed (1..N) salary_schedule fragments close at t_end when
--     p_now >= t_end. Includes approved-but-open rows; rejected / revoked never modified.
--   - Continuous skips NULL-index INSERT when any non-rejected/non-revoked indexed salary_schedule
--     rows exist for the day.
--   - Split slot 1 / slot 2 close branches: same gate (rejected / revoked terminal; approval ok).
--   - Degenerate split (segment_b_start_local = segment_a_start_local): remap slot 2 to start at
--     slot 1 t_end so half-open overlap doesn't block slot 2 INSERT.
--   - Split overlap NOT EXISTS uses work_date OR clock-in date in template tz vs p_work_date.
