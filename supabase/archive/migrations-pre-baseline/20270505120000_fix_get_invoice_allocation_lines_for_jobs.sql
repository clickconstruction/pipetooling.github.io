-- Job Summary Parts Cost: 10-column return (supply_houses.website_url, supply_house_invoices.link) with
-- helpers/subcontractor team visibility via auth_uid_is_helpers_or_subcontractor(). Runs after
-- 20270504120001_helpers_subcontractor_parity_policies_rpc.sql (parity omits overlapping CREATE OR REPLACE).
...
COMMENT ON FUNCTION public.get_invoice_allocation_lines_for_jobs(uuid[]) IS
  'Per-invoice lines allocated to jobs for Job Summary Parts Cost. Includes supply_houses.website_url and supply_house_invoices.link (document URL). Visibility matches helpers/subcontractors on team and get_invoice_amounts_for_jobs.';
