-- Copy manifest: bids header + book version FKs + materials_model; bids_count_rows;
-- bid_count_row_custom_prices; bid_pricing_assignments; bids_takeoff_rough_part_lines;
-- bids_takeoff_template_mappings; cost_estimates (PO FKs omitted) + cost_estimate_labor_rows.
-- Omits: bids_submission_entries, outcome/sent/attestation/financial columns (defaults),
-- bid_number (trigger assigns on insert).
...
COMMENT ON FUNCTION public.duplicate_bid_to_service_type(uuid, uuid) IS
  'Creates a new bid copying counts, pricing, takeoff, and cost estimate from the source with a new service_type_id; resets status/financial/sent fields.';

GRANT EXECUTE ON FUNCTION public.duplicate_bid_to_service_type(uuid, uuid) TO authenticated;
