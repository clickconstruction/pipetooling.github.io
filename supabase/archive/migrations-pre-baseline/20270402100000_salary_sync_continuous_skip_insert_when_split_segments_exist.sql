-- Continuous salary: do not INSERT a duplicate salary_segment_index IS NULL row when the day
-- already has pending salary_schedule rows with non-null segment indices (post-split of auto row).
...
BEGIN
  ...
  IF v_mode = 'continuous' THEN
    ...
    IF FOUND THEN
      ...
      END IF;
    ELSE
      -- Post-split continuous day: pending rows may have salary_segment_index set; avoid a second synthetic row at t_start.
      IF NOT EXISTS (
        SELECT 1
        FROM public.clock_sessions
        WHERE user_id = p_user_id
