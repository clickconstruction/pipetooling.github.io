-- Contract Book: atomic save of library body + optional rename that cascades to person_contract_documents
-- for people assigned to the template (same scope as template assignment).
...
REVOKE ALL ON FUNCTION public.update_contract_book_entry(uuid, text, text, text, text[]) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.update_contract_book_entry(uuid, text, text, text, text[]) TO authenticated;

COMMENT ON FUNCTION public.update_contract_book_entry(uuid, text, text, text, text[]) IS
