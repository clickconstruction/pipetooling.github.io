-- Primary RTB bundle: merge Stripe/ensure line with parent job in Stages/Dashboard UI.

ALTER TABLE public.jobs_ledger_invoices
  ADD COLUMN IF NOT EXISTS is_primary_rtb_bundle boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.jobs_ledger_invoices.is_primary_rtb_bundle IS
  'When true, this ready_to_bill row is the ensure/Stripe full-balance line; Stages/Dashboard show one row with the job. Manual partials stay false.';

CREATE UNIQUE INDEX IF NOT EXISTS jobs_ledger_invoices_one_primary_rtb_bundle_per_job
  ON public.jobs_ledger_invoices (job_id)
  WHERE status = 'ready_to_bill' AND is_primary_rtb_bundle = true;

-- Ensure RPC: set flag on INSERT and UPDATE (sync) paths.
...
BEGIN
  ...
  IF v_rtb_count = 0 THEN
    ...
    v_est := NULL;
    IF j.estimated_completion_date IS NOT NULL AND trim(j.estimated_completion_date::text) <> '' THEN
      BEGIN
        v_est := j.estimated_completion_date::date;
...
COMMENT ON FUNCTION public.ensure_single_ready_to_bill_invoice_for_job(uuid) IS
  'Creates or syncs a single ready_to_bill invoice from unallocated job balance for Stripe-from-job UX. Sets is_primary_rtb_bundle for UI merge.';
