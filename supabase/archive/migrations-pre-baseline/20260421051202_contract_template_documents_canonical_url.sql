-- Contract Book: optional canonical Doc/PDF URL per library row (aligns with person_contract_documents.canonical_document_url).

ALTER TABLE public.contract_template_documents
  ADD COLUMN IF NOT EXISTS canonical_document_url TEXT;

COMMENT ON COLUMN public.contract_template_documents.canonical_document_url IS
  'Optional HTTPS URL to authoritative Doc/PDF for this library entry; copied into person flows and assign upsert when applicable.';

DROP FUNCTION IF EXISTS public.update_contract_book_entry(uuid, text, text, text, text[]);
...
REVOKE ALL ON FUNCTION public.update_contract_book_entry(uuid, text, text, text, text[], text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.update_contract_book_entry(uuid, text, text, text, text[], text) TO authenticated;

COMMENT ON FUNCTION public.update_contract_book_entry(uuid, text, text, text, text[], text) IS
  'Updates a Contract Book row; when document_name changes, renames matching person_contract_documents for assignees of that template. Pay-staff only. SECURITY DEFINER.';
