-- Skip salary_schedule materialization on PTO days; remove non-final salary sessions.
...
COMMENT ON FUNCTION public.salary_sync_one_user_clock_sessions(uuid, date, timestamptz) IS 'Definer: create/close salary_schedule clock_sessions for one user/day; PTO days delete non-final salary rows and skip.';
