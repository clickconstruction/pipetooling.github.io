-- Ready to Bill -> Working: cancel in-progress field collect payment flow (draft / pending_dispatch / approved_for_terminal).
...
DECLARE
  ...
  v_cancelled_collect_flow INTEGER := 0;
BEGIN
  ...
  IF p_to_status = 'working' AND v_current_status = 'ready_to_bill' THEN
    ...
    SET
      ...
      last_error = NULL
    WHERE job_id = p_job_id
      AND status IN ('draft', 'pending_dispatch', 'approved_for_terminal');
    GET DIAGNOSTICS v_cancelled_collect_flow = ROW_COUNT;
  END IF;
  ...
  RETURN jsonb_build_object(
    'ok', true,
    'deleted_ready_to_bill_invoices', v_deleted_rtb,
    'cancelled_collect_payment_flows', v_cancelled_collect_flow
  );
END;
$$;
COMMENT ON FUNCTION public.update_job_status(UUID, TEXT) IS 'Updates job status. Forward: working->ready_to_bill (team/office/superintendent), ready_to_bill->billed, billed->paid (office). Revert: ready_to_bill->working (deletes RTB draft invoices; cancels in-progress collect payment flow draft/pending_dispatch/approved_for_terminal), billed->ready_to_bill, paid->billed (dev/master/assistant/primary with job access).';
