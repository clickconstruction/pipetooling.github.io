/**
 * weekly-movement-email-dispatch — scheduled Weekly movement report sends
 * (v2.1437, weekly_movement Report Subscriptions stream).
 *
 * CRON-ONLY (X-Cron-Secret): scheduling/cancelling are direct RLS-gated
 * writes to weekly_movement_email_requests from the client. Per due row the
 * report covers the PREVIOUS complete Central week (payload RPC's NULL
 * default), rebuilt fresh at send time. Recipients are internal
 * office-capable users (the report names who moved what); a quiet week still
 * sends — "no moves" is information for this stream, unlike GC statements.
 *
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: payloadRaw, error: rpcErr } = await admin.rpc('get_weekly_movement_email_payload', {
      p_week_monday: null,
    })
    if (rpcErr) return jsonResponse({ error: `payload rpc: ${rpcErr.message}` }, 500)
    const payload = payloadRaw as WeeklyMovementPayload
    if (!payload || !Array.isArray(payload.sections)) return jsonResponse({ error: 'empty payload' }, 500)
    const weekLabel = weekLabelFromMonday(payload.week_monday)
    const wording = await resolveServerEmailWording('weekly_movement', { week: weekLabel }, weeklyMovementSubject(weekLabel))
      ...
      try {
        ...
        const resendResponse = await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            ...
            to: [recipientEmail],
            subject,
            html: (wording.introHtml ?? '') + renderWeeklyMovementHtml(payload, weekLabel, requester?.name ?? undefined),
            text: (wording.introText ? wording.introText + '\n\n' : '') + renderWeeklyMovementText(payload, weekLabel),
        ...
        if (row.repeat_weekly) {
          ...
          if (!existing || existing.length === 0) {
            await admin.from('weekly_movement_email_requests').insert({
    ...
    console.error('weekly-movement-email-dispatch', e)
