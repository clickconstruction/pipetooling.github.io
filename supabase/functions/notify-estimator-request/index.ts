...
serve(async (req) => {
  ...
  try {
    ...
    if (!estimator_request_id || typeof estimator_request_id !== 'string') {
      return new Response(JSON.stringify({ error: 'Missing estimator_request_id' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    const { data: members, error: memErr } = await adminClient.from('estimator_group_members').select('user_id')
    if (memErr) {
    ...
    if (recipientIds.length === 0) {
      return new Response(
        JSON.stringify({
          success: true,
          message: 'Estimator inbox group is empty; no notifications sent',
          push_sent: 0,
          recipients: 0,
        }),
        { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
    ...
    const { data: sender } = await adminClient
      .from('users')
      .select('name, email')
      .eq('id', estimatorRow.from_user_id)
      .maybeSingle()
    ...
    let refSuffix = ''
    const summ = estimatorRow.reference_summary?.trim()
    if (summ) {
      refSuffix = ` · ${summ}`
    ...
    if (estimatorRow.location_lat != null && estimatorRow.location_lng != null) {
      refSuffix += ' · Location attached'
    }

    const titlePart = truncateTitle(estimatorRow.title, 160)
    ...
    for (const recipient_user_id of recipientIds) {
      ...
      if (sentForRecipient > 0) {
        await adminClient.from('notification_history').insert({
          recipient_user_id,
          template_type: 'estimator_request',
          title: pushTitle,
    ...
    console.error('notify-estimator-request:', error)
