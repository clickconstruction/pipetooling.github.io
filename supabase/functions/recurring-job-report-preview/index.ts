...
type Body = {
  ...
  timezone?: string
  /** Civil today in schedule TZ (`YYYY-MM-DD`); required to resolve yesterday/today/week windows when `window` omitted. */
  anchor_date: string
...
serve(async (req) => {
  ...
  try {
    ...
    const tz = (body.timezone ?? APP_CALENDAR_TZ).trim() || APP_CALENDAR_TZ
    ...
    console.error('recurring-job-report-preview', e)
