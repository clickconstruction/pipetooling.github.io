...
// v2.3512: the email lives in _shared/rfqEmail.ts so Settings → What customers see renders the same one over the sample.
function buildEmail(args: Omit<RfqEmailInput, 'appOrigin'>): { subject: string; text: string; html: string } {
  return buildRfqEmail({ ...args, appOrigin: APP_ORIGIN })
}
