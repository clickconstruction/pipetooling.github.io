...
const TOOLS = [
  ...
  },
  {
    name: 'void_shadow',
    description:
      "Take a shadow OUT of the scoring loop (v2.3027): a category error (wrong division), a wrong reference, a contaminated run. Own shells only; a run that already scored stays scored (the owner judges gate eligibility instead). Stamps '[shadow VOID] <reason>' on the ledger. Use it instead of leaving a bad lock to auto-score.",
    inputSchema: {
      type: 'object',
      properties: {
...
// ---------------------------------------------------------------------------
// Discipline fence (v2.3027): twin-estimator-1 is a PLUMBING estimator. On
...
async function callTool(req: Request, name: string, args: Record<string, unknown>) {
  ...
  switch (name) {
    ...
    case 'get_shadow_queue': {
      ...
      // v2.3027: plumbing-only — the twin's discipline (see disciplineRefusal).
    ...
    case 'next_shadow': {
      ...
      // v2.3027: plumbing-only candidates — the 2026-09-07 b378 category error.
    ...
    case 'void_shadow': {
      // v2.3027: the door for a shadow that should never score — a category error, a
