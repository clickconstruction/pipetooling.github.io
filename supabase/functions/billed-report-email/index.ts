/**
 * billed-report-email — share the Billed Awaiting Payment report (v2.1315).
 *
 * Modes on POST JSON body (skeleton mirrors paid-job-email):
 *
 * - { mode: 'preview' } — caller JWT, staff role (dev/master_technician/
 *   assistant/controller); returns { html }. No DB writes, no send.
 * - { mode: 'test_send' } — same gate; sends the report via Resend to the
 ...
 *   in billed_report_email_requests (send_at = now), sends immediately,
 *   stamps sent_at. The email footer carries "Sent by {caller}".
 * - cron (no mode or { mode: 'dispatch' }) — X-Cron-Secret must equal
 *   CRON_SECRET. Drains billed_report_email_requests rows with
 ...
 * controller/primary) — the report carries AR dollars those roles already see
 * on the Stages board. There is no sterilized variant.
 *
...
import {
  ...
  renderBilledReportEmail,
  type BilledReportPayload,
} from './render.ts'
...
/** Who may RECEIVE the report (office-capable — mirrors PaidInFullEmailSettingsModal's isOfficeCapableRole). */
...
async function fetchPayload(admin: Admin): Promise<BilledReportPayload> {
  const { data, error } = await admin.rpc('get_billed_report_email_payload')
  if (error) throw new Error(`get_billed_report_email_payload: ${error.message}`)
  if (!data || typeof data !== 'object' || !(data as BilledReportPayload).totals) {
    throw new Error('Empty payload')
  }
  return data as BilledReportPayload
}
...
async function runDispatch(admin: Admin, resendApiKey: string): Promise<Response> {
  ...
  let payload: BilledReportPayload
  try {
  ...
  for (const row of rows) {
    try {
      ...
      const mail = await sendReport(resendApiKey, payload, recipientEmail, requester?.name ?? null)
      if (mail.success) {
        await admin
          .from('billed_report_email_requests')
...
serve(async (req) => {
  ...
  try {
    ...
    if (mode === 'preview' || mode === 'test_send' || mode === 'send_now') {
      ...
      if (mode === 'preview') {
        return jsonResponse({ html: renderBilledReportEmail(payload, gate.name ?? undefined) })
      }
      ...
      if (mode === 'test_send') {
        ...
        const mail = await sendReport(resendApiKey, payload, gate.email, gate.name, '[TEST] ')
      ...
      const mail = await sendReport(resendApiKey, payload, recipientEmail, gate.name)
      await admin
        .from('billed_report_email_requests')
        .update(
          mail.success
    ...
    console.error('billed-report-email', e)
    return jsonResponse({ error: e instanceof Error ? e.message : String(e) }, 500)
  }
})
