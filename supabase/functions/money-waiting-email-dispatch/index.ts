/**
 * money-waiting-email-dispatch — share the Pay speeds "Money waiting" list
 * (v2.2565). Skeleton mirrors payment-forecast-email-dispatch verbatim; only
 * the payload RPC, requests table, and renderer differ.
 *
 * Modes on POST JSON body:
 * - { mode: 'preview' } — caller JWT, staff role; returns { html }.
 * - { mode: 'test_send' } — same gate; [TEST]-prefixed send to the caller.
 * - { mode: 'send_now', recipient_user_id } — audit row + immediate send.
 * - cron (no mode) — X-Cron-Secret; drains money_waiting_email_requests,
 *   repeat_weekly re-enqueues +7d. An all-on-pace week still sends a
 *   one-liner (a silent skip reads as a broken subscription).
 *
...
/** Who may RECEIVE the report (office-capable — mirrors payment-forecast-email). */
...
async function fetchPayload(admin: Admin): Promise<MoneyWaitingEmailPayload> {
  const { data, error } = await admin.rpc('get_money_waiting_email_payload')
  if (error) throw new Error(`get_money_waiting_email_payload: ${error.message}`)
  if (!data || typeof data !== 'object' || !Array.isArray((data as MoneyWaitingEmailPayload).rows)) {
    throw new Error('Empty payload')
  }
  return data as MoneyWaitingEmailPayload
}
...
async function sendMoneyWaiting(
  resendApiKey: string,
  payload: MoneyWaitingEmailPayload,
...
async function runDispatch(admin: Admin, resendApiKey: string): Promise<Response> {
  const { data: pending, error: qErr } = await admin
    .from('money_waiting_email_requests')
  ...
  let payload: MoneyWaitingEmailPayload
  try {
  ...
  for (const row of rows) {
    try {
      ...
      const mail = await sendMoneyWaiting(resendApiKey, payload, recipientEmail, requester?.name ?? null)
      if (mail.success) {
        await admin
          .from('money_waiting_email_requests')
        ...
        if (row.repeat_weekly) {
          ...
          if (!existing || existing.length === 0) {
            await admin.from('money_waiting_email_requests').insert({
...
serve(async (req) => {
  ...
  try {
    ...
    if (mode === 'preview' || mode === 'test_send' || mode === 'send_now') {
      ...
      if (mode === 'preview') {
        return jsonResponse({ html: renderMoneyWaitingEmail(payload, gate.name ?? undefined) })
      }
      ...
      if (mode === 'test_send') {
        ...
        const mail = await sendMoneyWaiting(resendApiKey, payload, gate.email, gate.name, '[TEST] ')
      ...
      const mail = await sendMoneyWaiting(resendApiKey, payload, recipientEmail, gate.name)
      await admin
        .from('money_waiting_email_requests')
        .update(
          mail.success
    ...
    console.error('money-waiting-email-dispatch', e)
    return jsonResponse({ error: e instanceof Error ? e.message : String(e) }, 500)
  }
})
